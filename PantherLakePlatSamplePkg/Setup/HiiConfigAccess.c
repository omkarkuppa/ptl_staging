...
#endif
#if FixedPcdGetBool (PcdCnvSetupMenu) == 1
...
//InitPciBusStrings, InitCPUStrings, InitSaStrings, InitSBStrings, InitAcpiStrings, InitMeInfo, InitICCStrings, InitOverClockStrings, InitPlatformStrings, InitUSBStrings, InitBoardStrings,
    ...
    if (EfiStrStr (ConfigRequest, L"OFFSET") == NULL) {
      ...
      if (EfiStrStr (StringPtr, L"&") == NULL) {
        ...
        if (StrCmp (L"SaSetup", Name) == 0) {
...
#if FixedPcdGetBool(PcdOverclockEnable) == 1
        } else if (StrCmp (L"OcSetup", Name) == 0) {
          Length = sizeof (OC_SETUP);
#endif
          ...
          Length = sizeof (SETUP_DATA);
#if FixedPcdGetBool (PcdDptfFeatureEnable) == 1
        } else if (StrCmp (DPTF_CONFIG_VARIABLE_NAME, Name) == 0) {
          Length = sizeof (DPTF_CONFIG);
#endif
#if FixedPcdGetBool (PcdMipiCamFeatureEnable) == 1
        } else if (StrCmp (MIPICAM_CONFIG_VARIABLE_NAME, Name) == 0) {
          Length = sizeof (MIPICAM_CONFIG);
#endif
        }
