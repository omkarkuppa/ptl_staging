  ...
  )
{
  ...
  switch (OsDebugPort) {
    ...
    case 3:
      ...
      DebugPortTable->BaseAddress.RegisterBitOffset = 0;
      DebugPortTable->BaseAddress.Reserved = 0;
      DebugPortTable->BaseAddress.Address  = GetSerialIoBar (PciCfgBase);
      //
      // Update DBG2 Device Base Address
      //
...
{
  UINT16    Tco1Cnt;
  Tco1Cnt = IoRead16 (PcdGet16 (PcdTcoBaseAddress) + R_TCO_IO_TCTL1);
  if (Tco1Cnt & B_TCO_IO_TCTL1_NR_MSUS) {
    IoWrite16 (PcdGet16 (PcdTcoBaseAddress) + R_TCO_IO_TCTL1, Tco1Cnt & (UINT16)~B_TCO_IO_TCTL1_NR_MSUS);
    if (Tco1Cnt == IoRead16 (PcdGet16 (PcdTcoBaseAddress) + R_TCO_IO_TCTL1)) {
      return TRUE;
    }
    //
    // Restore Tco1Cnt
    //
    IoWrite16 (PcdGet16 (PcdTcoBaseAddress) + R_TCO_IO_TCTL1, Tco1Cnt);
  }
  //
  ...
  )
{
  ...
  *Version = EFI_ACPI_TABLE_VERSION_1_0B | EFI_ACPI_TABLE_VERSION_2_0 | EFI_ACPI_TABLE_VERSION_3_0;
  ...
  case EFI_ACPI_6_5_LOW_POWER_IDLE_TABLE_STRUCTURE_SIGNATURE:
    {
      ...
      if (LpitStateEntries != 0) {
        ...
        //
        Lpi_Counter.Data8 = mSystemConfiguration.LpitResidencyCounter;
        ...
        if (Lpi_Counter.Bits.Slp_S0) {
          ...
          //
          Address = (UINT64)PmcGetPwrmBase () + R_PMC_PWRM_SLP_S0_RESIDENCY;
        ...
        if (Lpi_Counter.Bits.Ps_On) {
          ...
          // Read PWRM Base Address to fill in Residency counter Address Space
          //
          Address = (UINT64)PmcGetPwrmBase () + R_PMC_PWRM_PS_ON_RESIDENCY;
  ...
  case EFI_ACPI_6_5_WATCHDOG_ACTION_TABLE_SIGNATURE:
    //
    ...
    if (!TcoNrReadOnly) {
      ...
      //
      IoAnd16 (PcdGet16 (PcdTcoBaseAddress) + R_TCO_IO_TCTL1, (UINT16)~B_TCO_IO_TCTL1_NR_MSUS);
    }
    ...
    *WatchdogStatus = 0;
    ...
    for (Index = 0; Index < ((EFI_ACPI_WATCHDOG_ACTION_1_0_TABLE *) Table)->NumberWatchdogInstructionEntries; Index ++) {
      if (WdatActionEntry[Index].WatchdogAction == EFI_ACPI_WDAT_1_0_WATCHDOG_ACTION_QUERY_WATCHDOG_STATUS) {
        WdatActionEntry[Index].RegisterRegion.AddressSpaceId = EFI_ACPI_6_5_SYSTEM_MEMORY;
