...
GLOBAL_REMOVE_IF_UNREFERENCED DEVICE_MANAGER_CALLBACK_DATA  gDeviceManagerPrivate = {
...
GLOBAL_REMOVE_IF_UNREFERENCED EFI_GUID mDeviceManagerGuid = DEVICE_MANAGER_FORMSET_GUID;
GLOBAL_REMOVE_IF_UNREFERENCED EFI_GUID mDriverHealthGuid = DRIVER_HEALTH_FORMSET_GUID;

//
// Which Mac Address string is select
// it will decide what menu need to show in the NETWORK_DEVICE_FORM_ID form.
//
GLOBAL_REMOVE_IF_UNREFERENCED EFI_STRING  mSelectedMacAddrString;

//
// Which form Id need to be show.
//
GLOBAL_REMOVE_IF_UNREFERENCED EFI_FORM_ID      mNextShowFormId = DEVICE_MANAGER_FORM_ID;

//
// The Mac Address show in the NETWORK_DEVICE_LIST_FORM_ID
//
GLOBAL_REMOVE_IF_UNREFERENCED MAC_ADDRESS_NODE_LIST  mMacDeviceList;

GLOBAL_REMOVE_IF_UNREFERENCED DEVICE_MANAGER_MENU_ITEM  mDeviceManagerMenuItemTable[] = {
...
GLOBAL_REMOVE_IF_UNREFERENCED HII_VENDOR_DEVICE_PATH  mDeviceManagerHiiVendorDevicePath = {
  {
    {
...
GLOBAL_REMOVE_IF_UNREFERENCED HII_VENDOR_DEVICE_PATH  mDriverHealthHiiVendorDevicePath = {
  {
    {
      HARDWARE_DEVICE_PATH,
        HW_VENDOR_DP,
  ...
  )
{
  ...
  gDeviceManagerPrivate.DriverHealthHandle = NULL;
  Status = gBS->InstallMultipleProtocolInterfaces (
  ...
  if (mNextShowFormId == NETWORK_DEVICE_FORM_ID) {
    ...
    if (NewStringTitle == NULL) goto EXIT;
  ...
  for (Index = 0; HiiHandles[Index] != NULL; Index++) {
    ...
    if (!ExtractDisplayedHiiFormFromHiiHandle (HiiHandles[Index], &gEfiHiiPlatformSetupFormsetGuid, &FormSetGuid, &FormSetTitle, &FormSetHelp)) {
      continue;
    }
  ...
  )
{
  ...
  StartOpCodeHandle       = NULL;
  EndOpCodeHandle         = NULL;
  StartOpCodeHandleRepair = NULL;
  EndOpCodeHandleRepair   = NULL;
  String                  = NULL;
  TmpString               = NULL;
  ...
  if (StartOpCodeHandleRepair == NULL) goto EXIT;

  EndOpCodeHandleRepair = HiiAllocateOpCodeHandle ();
  ASSERT (EndOpCodeHandleRepair != NULL);
  if (EndOpCodeHandleRepair == NULL) goto EXIT;
  ...
  while (!IsNull (&DriverHealthList, Link)) {
    ...
    if (EFI_ERROR (Status)) {
      ...
      DriverName       = UiDevicePathToStr (DriverDevicePath);
    }
    //
    // Add the Driver name & Controller name into FormSetTitle string
    //
    StrCatS (String, 0x200 / sizeof (CHAR16), DriverName);
    ...
    if (!EFI_ERROR (Status)) {
      ...
      //
      StrCatS (String, 0x200 / sizeof (CHAR16), L"    ");
      StrCatS (String, 0x200 / sizeof (CHAR16), ControllerName);
    }
    ...
    if ((DriverHealthInfo->MessageList != NULL) && (DriverHealthInfo->MessageList->StringId != 0)) {
       ...
       if (TmpString == NULL) goto EXIT;
       StrCatS (String, 0x200 / sizeof (CHAR16), TmpString);
       FreePool (TmpString);
       TmpString = NULL;
    } else {
      //
...
EXIT:
  ...
  if (StartOpCodeHandleRepair != NULL) {
    HiiFreeOpCodeHandle (StartOpCodeHandleRepair);
  }

  if (EndOpCodeHandleRepair != NULL) {
