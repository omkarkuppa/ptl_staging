  ...
  )
{
  ...
  for (Index = 0; Index < NoHandles; Index++) {
    ...
    if ((Acpi != NULL) && IsIsaSerialNode (Acpi)) {
      ...
      BufferLength = 10 - StrLen(NewMenuEntry->DisplayString);
      UnicodeValueToStringS(NewMenuEntry->DisplayString + StrLen(NewMenuEntry->DisplayString), BufferLength, 0, NewMenuEntry->OptionNumber, 0);
  ...
  if (TerminalMenu.MenuNumber > 0) {
    for (Index = 0; Index < TerminalMenu.MenuNumber; Index++) {
      ...
      if (NewTerminalContext->IsOutOfBand) {
        CallbackData->BmmFakeNvData.COMOutOfBandPort = (UINT16) Index;
        break;
      }
    }
