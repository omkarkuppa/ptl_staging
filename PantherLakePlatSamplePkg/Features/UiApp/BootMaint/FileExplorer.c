  ...
  )
{
  ...
  for (Index2 = 0; Index2 < sizeof (mShellArray) / sizeof (mShellArray[0]); Index2++) {
    ...
    if (!EFI_ERROR (Status)) {
      if (!IsUefiShellExist && (IS_STRING_IDENTICAL(mShellArray[Index2].ShellName, INTERNAL_UEFI_SHELL_NAME))) {
        //
  ...
  )
{
  ...
  *Progress = Configuration;
  ...
  //
  Result = HiiSetBrowserData (&mFileExplorerGuid, mFileExplorerStorageName, sizeof (FILE_EXPLORER_NV_DATA), (UINT8 *) FeData, NULL);
  if (!Result) {
    DEBUG ((DEBUG_INFO, "SetVariableInBrowser Failed. \n"));
  }
