  ...
  )
{
  ...
  for (Index = 0; HiiHandles[Index] != NULL; Index++) {
    if (!ExtractDisplayedHiiFormFromHiiHandle (HiiHandles[Index], &gEfiIfrFrontPageGuid, &FormSetGuid, &FormSetTitle, &FormSetHelp)) {
      continue;
    }
...
/**
  Call the menu add according the classguid of gEfiIfrFrontPageGuid.
  ...
  )
{
  ...
  if (CompareGuid (SetupClassGuid, &gEfiHiiPlatformSetupFormsetGuid)) {
    while (GuidCount > 0) {
      if (CompareGuid (&gEfiIfrBootMaintenanceGuid, ClassGuid) ||
          CompareGuid (&gEfiIfrFrontPageGuid, ClassGuid) ||
