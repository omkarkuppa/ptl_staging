...
export NONFITPAYLOADS_FD_NAME=NONFITPAYLOADS
...
function SetBuildEnv {
  ...
  BUILD_DIR="Build/NonFitPayloadsPkg/$TARGET"
...
function PayloadBuildClean {
  echo "Build Clean from NonFitPayloads Start"
...
function PayloadBuild {
  export NONFIT_PAYLOAD_BUILD_FLAGS="-p $WORKSPACE_PLATFORM/$PLATFORM_PACKAGE/PayloadManagement/NonFitPayloads/NonFitPayloadsPkg.dsc $EXT_BUILD_FLAGS"
  build $NONFIT_PAYLOAD_BUILD_FLAGS
  if [ $? -ne 0 ]; then
...
numargs=$#
  ...
  if [ "$1" = "IntegrateBuild" ]; then
     export INTEGRATE_PAYLOAD_BUILD=TRUE
  elif [ "$1" = "r" ]; then
...
if [ "$INTEGRATE_PAYLOAD_BUILD" = "TRUE" ]; then
  ...
  cp $WORKSPACE/$BUILD_DIR/FV/NONFITPAYLOADS.fd $WORKSPACE/RomImages/NONFITPAYLOADS.fd
fi
