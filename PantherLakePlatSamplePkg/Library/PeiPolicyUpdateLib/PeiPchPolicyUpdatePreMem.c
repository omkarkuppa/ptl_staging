  ...
  )
{
  ...
  COMPARE_AND_UPDATE_POLICY (PchGeneralPreMemConfig->Port80Route,  PchSetup->IchPort80Route);
  UPDATE_POLICY (PchGeneralPreMemConfig->GpioOverride, 0);
  COMPARE_AND_UPDATE_POLICY (PchGeneralPreMemConfig->IoeDebugEn,  PchSetup->IoeDebugEn);
  COMPARE_AND_UPDATE_POLICY (PchGeneralPreMemConfig->PmodeClkEn,  PchSetup->PmodeClkEn);
  COMPARE_AND_UPDATE_POLICY (PchGeneralPreMemConfig->PmcPrivacyConsent,  PchSetup->PmcPrivacyConsent);
  ...
  )
{
  ...
  COMPARE_AND_UPDATE_POLICY (DciPreMemConfig->DciEn,              PchSetup->DciEn             );
  COMPARE_AND_UPDATE_POLICY (DciPreMemConfig->DciClkEnable,       PchSetup->DciClkEnable      );
  COMPARE_AND_UPDATE_POLICY (DciPreMemConfig->DciDbcMode,         PchSetup->DciDbcMode        );
  COMPARE_AND_UPDATE_POLICY (DciPreMemConfig->KeepEarlyTrace,     PchSetup->KeepEarlyTrace    );
}
  ...
  )
{
  ...
  UPDATE_POLICY (SmbusPreMemConfig->Enable,         TRUE);
  UPDATE_POLICY (SmbusPreMemConfig->SmbAlertEnable, PcdGetBool (PcdSmbusAlertEnable));

  SmBusReservedTable = NULL;
  SmBusReservedNum   = 0;
  UPDATE_POLICY (SmbusPreMemConfig->SmbusIoBase, PcdGet16 (PcdSmbusBaseAddress));
  SmBusReservedTable = mSmbusSTPRsvdAddresses;
  SmBusReservedNum   = sizeof (mSmbusSTPRsvdAddresses);

  if (SmBusReservedTable != NULL) {
    UPDATE_POLICY (SmbusPreMemConfig->NumRsvdSmbusAddresses, SmBusReservedNum);
  ...
  )
{
  ...
  COMPARE_AND_UPDATE_POLICY (WatchDogPreMemConfig->DisableAndLock, (UINTN)(!PchSetup->WdtEnable));
}
  ...
  )
{
  ...
  GET_POLICY (PcieRpPreMemConfig->RpEnabledMask, RpEnabledMask);
  ...
  COMPARE_AND_UPDATE_POLICY (PcieRpPreMemConfig->RpEnabledMask, RpEnabledMask);
  ...
  ASSERT_EFI_ERROR (Status);
    ...
    //
    COMPARE_AND_UPDATE_POLICY (PcieRpPreMemConfig->SkipExtGfxScan,   SaSetup.SkipExtGfxScan);
    //
    // SA:GfxRemovePostPOEnd
    //
  UPDATE_POLICY (PcieRpPreMemConfig->PcieResizableBarSupport, PchSetup->PcieResizableBarSupport);
  ...
  for (RpIndex = 0; RpIndex < GetPchMaxPcieClockNum(); RpIndex++) {
    if (PchSetup->PchPcieClkReqSupport[RpIndex] == 0xFF) {
      UPDATE_POLICY (PcieRpPreMemConfig->PcieClock[RpIndex].ClkReq, (UINT8)PCH_PCIE_NO_SUCH_CLOCK);
    }
    if (PchSetup->PchPcieClockUsageOverride[RpIndex] == 1) {
      UPDATE_POLICY (PcieRpPreMemConfig->PcieClock[RpIndex].Usage, PchClockUsageUnspecified);
    } else if (PchSetup->PchPcieClockUsageOverride[RpIndex] == 2) {
      UPDATE_POLICY (PcieRpPreMemConfig->PcieClock[RpIndex].Usage, PchClockUsageNotUsed);
    }
  }
  ...
  )
{
  ...
  for (Index = 0; Index < PCH_MAX_HDA_SDI; Index++) {
    COMPARE_UPDATE_POLICY_ARRAY (HdaPreMemConfig->AudioLinkHda.SdiEnable[Index], PchSetup->PchHdAudioSdiLaneEnable[Index], Index);
  }
  COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->Enable,               PchSetup->PchHdAudio                   );
  COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->DspEnable,            PchSetup->PchHdAudioDsp                );
  COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->IDispLinkFrequency,   PchSetup->PchHdaIDisplayLinkFreq       );
  COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->IDispLinkTmode,       PchSetup->PchHdaIDisplayLinkTmode      );
  COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->IDispCodecDisconnect, PchSetup->PchHdaIDisplayCodecDisconnect);
  COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->DspUaaCompliance,     PchSetup->PchHdAudioDspUaaCompliance   );

  COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkHda.Enable,  PchSetup->PchHdAudioHdaLinkEnable);
  for (Index = 0; Index < GetPchHdaMaxDmicLinkNum (); Index++) {
    COMPARE_UPDATE_POLICY_ARRAY (HdaPreMemConfig->AudioLinkDmic[Index].Enable,          PchSetup->PchHdAudioDmicLinkEnable[Index],  Index);
    COMPARE_UPDATE_POLICY_ARRAY (HdaPreMemConfig->AudioLinkDmic[Index].DmicClockSelect, HdaDmicClockSelectClkA, Index);
  }

  for (Index = 0; Index < GetPchHdaMaxSndwMultilaneNum (); Index++) {
    COMPARE_UPDATE_POLICY_ARRAY (HdaPreMemConfig->SndwMultilane[Index].Enable,        PchSetup->PchHdAudioSndwMultilaneEnable[Index], Index);
  }
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkDmic[0].PinMux.ClkA, GPIOV2_PTL_PCD_MUXING__XXGPP_S_4__DMIC_CLK_A_0);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkDmic[0].PinMux.Data, GPIOV2_PTL_PCD_MUXING__XXGPP_S_5__DMIC_DATA_0);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkDmic[1].PinMux.ClkA, GPIOV2_PTL_PCD_MUXING__XXGPP_S_6__DMIC_CLK_A_1);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkDmic[1].PinMux.Data, GPIOV2_PTL_PCD_MUXING__XXGPP_S_7__DMIC_DATA_1);

  for (Index = 0; Index < GetPchHdaMaxSndwMultilaneNum (); Index++) {
    COMPARE_UPDATE_POLICY_ARRAY (HdaPreMemConfig->SndwMultilane[Index].PinMux.Clk,    0, Index);
    COMPARE_UPDATE_POLICY_ARRAY (HdaPreMemConfig->SndwMultilane[Index].PinMux.Data0,  0, Index);
    COMPARE_UPDATE_POLICY_ARRAY (HdaPreMemConfig->SndwMultilane[Index].PinMux.Data2,  0, Index);
    COMPARE_UPDATE_POLICY_ARRAY (HdaPreMemConfig->SndwMultilane[Index].PinMux.Data3,  0, Index);
  }
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->SndwMultilane[0].PinMux.Data1, GPIOV2_PTL_PCD_MUXING__XXGPP_S_6__SNDW2_DATA1);
  COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->SndwMultilane[1].PinMux.Data1, 0);
  COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->SndwMultilane[0].SndwInterface, 2);
  COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->SndwMultilane[1].SndwInterface, 3);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[0].PinMux.Sclk, GPIOV2_PTL_PCD_MUXING__XXGPP_D_10__I2S0_SCLK);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[0].PinMux.Sfmr, GPIOV2_PTL_PCD_MUXING__XXGPP_D_11__I2S0_SFRM);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[0].PinMux.Txd,  GPIOV2_PTL_PCD_MUXING__XXGPP_D_12__I2S0_TXD);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[0].PinMux.Rxd,  GPIOV2_PTL_PCD_MUXING__XXGPP_D_13__I2S0_RXD);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[1].PinMux.Sclk, GPIOV2_PTL_PCD_MUXING__XXGPP_S_2__I2S1_SCLK);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[1].PinMux.Sfmr, GPIOV2_PTL_PCD_MUXING__XXGPP_S_3__I2S1_SFRM);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[1].PinMux.Txd,  GPIOV2_PTL_PCD_MUXING__XXGPP_S_0__I2S1_TXD);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[1].PinMux.Rxd,  GPIOV2_PTL_PCD_MUXING__XXGPP_S_1__I2S1_RXD);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[2].PinMux.Sclk, GPIOV2_PTL_PCD_MUXING__XXGPP_S_4__I2S2_SCLK);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[2].PinMux.Sfmr, GPIOV2_PTL_PCD_MUXING__XXGPP_S_5__I2S2_SFRM);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[2].PinMux.Txd,  GPIOV2_PTL_PCD_MUXING__XXGPP_S_6__I2S2_TXD);
    COMPARE_AND_UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[2].PinMux.Rxd,  GPIOV2_PTL_PCD_MUXING__XXGPP_S_7__I2S2_RXD);

  for (Index = 0; Index < GetPchHdaMaxSspLinkNum (); Index++) {
    COMPARE_UPDATE_POLICY_ARRAY (HdaPreMemConfig->AudioLinkSsp[Index].Enable,  PchSetup->PchHdAudioSspLinkEnable[Index],  Index);
  }
  //
  ...
  if (CnviCrfModuleIsPresent()) {
    UPDATE_POLICY (HdaPreMemConfig->AudioLinkSsp[PchSetup->HdaDiscBtOffSspLink].Enable,  0);
  }
  for (Index = 0; Index < GetPchHdaMaxSndwLinkNum (); Index++) {
    COMPARE_UPDATE_POLICY_ARRAY (HdaPreMemConfig->AudioLinkSndw[Index].Enable, PchSetup->PchHdAudioSndwLinkEnable[Index], Index);
  }
  UPDATE_POLICY (HdaPreMemConfig->HdaDiscBtOffload.HdaDiscBtOffEnabled, PchSetup->HdaDiscBtOffEnabled);
  UPDATE_POLICY (HdaPreMemConfig->HdaDiscBtOffload.HdaDiscBtOffSspLink, PchSetup->HdaDiscBtOffSspLink);
  ...
  )
{
  ...
  COMPARE_AND_UPDATE_POLICY (IshPreMemConfig->Enable, PchSetup->PchIshEnable);
}
  ...
  )
{
  ...
  UPDATE_POLICY (CnviPreMemConfig->DdrRfim, SaSetup->CnviDdrRfim);
}
/**
  ...
  )
{
  ...
  COMPARE_AND_UPDATE_POLICY (SpiPreMemConfig->ExtendedBiosDecodeRangeEnable, PchSetup->ExtendedBiosDecodeRangeEnable);
}
  ...
  )
{
  ...
  if (SpiInfoHob != NULL) {
    ...
    UPDATE_POLICY (SpiInfoHobData->ExtendedBiosDecodeRangeEnable, FixedPcdGetBool(PcdExtendedBiosRegionSupport));
