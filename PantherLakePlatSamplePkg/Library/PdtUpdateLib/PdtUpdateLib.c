/** @file
  Library to support ISH PDT update
  ...
  )
{
  ...
  Status = ExtractPdtBinary (SensorBomId, FileBuffer, MultiPdtSize, &PdtFileSize, &PdtFileBuffer);
  DEBUG ((DEBUG_INFO, "Pdt Binary extracted Status = %r\n", Status));
  DEBUG ((DEBUG_INFO, "SensorBomId = 0x%x, PdtFileBuffer = 0x%x, Package Size = 0x%x Binary Size = 0x%x 1st 4 byte = %x\n", SensorBomId, (UINTN)PdtFileBuffer, MultiPdtSize, (UINTN)PdtFileSize, *((UINT32*)(UINTN)PdtFileBuffer)));
   if (EFI_ERROR (Status)) {
    return Status;
  }
