...
#include "SmmPlatformStatusCodeHandlerLib.h"

EFI_SMM_RSC_HANDLER_PROTOCOL  *mEfiRscHandlerProtocol       = NULL;
...
)
{
  ...
  if (PcdGetBool (PcdStatusCodeUseUsb3)) {
    ...
    mEfiRscHandlerProtocol->Register (Usb3StatusCodeReportWorker);
  }
  ...
  if (PcdGetBool (PcdStatusCodeUseSerialIoUart)) {
    ...
    mEfiRscHandlerProtocol->Register (SerialIoUartStatusCodeReportWorker);
  }
...
EFI_STATUS
EFIAPI
StatusCodeHandlerSmmConstructor (
