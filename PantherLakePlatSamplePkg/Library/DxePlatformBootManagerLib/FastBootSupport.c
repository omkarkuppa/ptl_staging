...
/**
  Check PS2 Keybaord ConIn variable present or not.
  @param    VarConIn       Is Ps2 keyboard connected
  @retval   TRUE           Variable present.
  @retval   FALSE          Variable doesn't present.
**/
BOOLEAN
CheckPs2keybaordConInVariable(
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "Fastboot OnSetupConnectAllConsoles Start\n"));
  ...
  //
  //  Update gPlatformConsole to ConIn here and
  //  ConInVariable would be connected later in UiEntry.
  //
  UpdatePlatformConsoleToConInVairable ();

  DEBUG ((DEBUG_INFO, "Fastboot OnSetupConnectAllConsoles End\n"));
}

/**
  Update the console variable without create another backup & modify variables.
  ...
  )
{
  ...
  ProcessVariable (&Variable, &VariableSize);
    ...
    for (Index = 0; Index < HandleCount; Index++) {
      ...
      if (!EFI_ERROR (Status)) {
        ...
        if ((!EFI_ERROR (Status)) &&
          ...
          if (EFI_ERROR (Status)) {
            DEBUG ((DEBUG_VERBOSE, "Failed to connect to USB handle\n"));
          }
        }
      }
    }
