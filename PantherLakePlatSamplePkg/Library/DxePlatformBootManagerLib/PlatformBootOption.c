  ...
  )
{
  ...
  VOID                               *Buffer;
  UINTN                              Size;
  EFI_HANDLE                         BootOptionHandler;

  BootOptionHandler = NULL;
  ...
  if (!CompareGuid (&gUefiShellFileGuid, FileGuid)) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if (!EFI_ERROR (Status)) {
        ...
        BootOptionHandler = LoadedImage->DeviceHandle;
      }
    }
    //
    // If the FileGuid is not found in current Firmware Volume we try to
    // find the boot option in other FVs
    //
    if (EFI_ERROR (Status)) {
      UINTN                         NumHandles;
      EFI_HANDLE                    *HandleBuffer;
      UINTN                         Index;
      EFI_FIRMWARE_VOLUME2_PROTOCOL *OtherFv;
      ...
      for (Index = 0; Index < NumHandles; ++Index) {
        ...
        //
        // Skip the current FV when running through
        //
        if (EFI_ERROR(Status) || OtherFv==Fv) {
          continue;
        }
        Buffer = NULL;
        Size = 0;
        Status = OtherFv->ReadSection(
          OtherFv,
          FileGuid,
          EFI_SECTION_PE32,
          0,
        ...
        if (!EFI_ERROR(Status)) {
          BootOptionHandler = HandleBuffer[Index];
          break;
        }
      }
    }

    if (BootOptionHandler == NULL) {
      return EFI_NOT_FOUND;
    }
    DevicePath = AppendDevicePathNode (
                   DevicePathFromHandle (BootOptionHandler),
  ...
  )
{
  ...
  if (GetDisplayBootMode () == NORMAL_BOOT && (Timeout != 0)) {
    //
    // Show progress
    //
  ...
  )
{
  ...
  EnterSetup = AsfIsEnterSetupEnabled ();
  ...
  )
{
  ...
  if ((BOOLEAN) (mSystemConfiguration.BootFirstToShell)) {
    ...
    //
    ChangeFirstBootDevice (INTERNAL_UEFI_SHELL_NAME);
  ...
  if (GetDisplayBootMode () == NORMAL_BOOT) {
    ...
    if (AmtIsRemoteAssistancePromptSupported ()) {
      Print (L"     Press [CTRL+ALT+F1] to enter MEBx Remote Assistance.\n");
    }
...
/**
  ...
  @param[in] Description  The description of boot option.
**/
VOID
ChangeFirstBootDevice (
  CHAR16                          *Description
  )
{
  ...
  //
  if(StrCmp (BootOption[FirstBootdevice].Description, Description) == 0) {
    return ;
  }
  //
  ...
  for(IndexI=1;IndexI<BootOptionCount;IndexI++) {
    ...
    //
    if(StrCmp (BootOption[IndexI].Description, Description) == 0) {
      //
      // Swap the Boot Order
      //
