/** @file
  ...
  A driver or application uses the I2C Controller protocol to
  manage the Controller mode of an I2C controller.  Note that
...
PERIPHERAL|              V          MANAGEMENT       | ACPI
  ...
  2.  A list of logic blocks contained in one or more I2C devices:
      ...
      b.  Logic block Peripheral address
  ...
  2.  The order for the blocks of logic that get referenced
      by the entries in the Peripheral address array.
  ...
  which consumes two Peripheral addresses, one for each logic block.  The
  ...
  single I2C Peripheral address using the relative value of zero (0).
  However if the third party I2C driver writer chooses to write a
  single driver which consumes multiple Peripheral addresses then the
  third party I2C driver writer needs to convey the order of the
  I2C Peripheral address entries in the Peripheral address array to the
  ...
  The platform hardware designer picks the actual Peripheral addresses
  ...
  then places the Peripheral addresses into the Peripheral address array in the
  ...
  specifying the index value as the relative Peripheral address.  The
  relative value always starts at zero (0) and its maximum value is
  the number of entries in Peripheral address array minus one.

  The Peripheral address is specified as a 32-bit integer to allow room
  for future Peripheral address expansion.  Only the port driver knows
  the maximum Peripheral address value.  All other drivers and
  applications must look for the EFI_NOT_FOUND status for the
  indication that the maximum Peripheral address was exceeded.
  ...
  and then converts the relative value to an actual I2C Peripheral address.
  ...
  I2C devices are addressed as the tuple: BusConfiguration:DeviceAddress.
  ...
  controller uses the Peripheral address to access the requested I2C device.
  ...
  number required for the I2C device, the Peripheral address array
  and the device path.
...
**/
typedef struct _EFI_I2C_MASTER_PROTOCOL   EFI_I2C_MASTER_PROTOCOL;  ///<  I2C Controller protocol
...
/**
  ...
  The operation is performed by selecting the I2C device with its Peripheral
  address and then sending all write data to the I2C device.  If read data
  is requested, a restart is sent followed by the Peripheral address and then
  ...
  @param[in] This           Address of an EFI_I2C_MASTER_PROTOCOL
                            structure
  @param[in] DeviceAddress   Address of the device on the I2C bus.
  @param[in] Event          Event to set for asynchronous operations,
                            NULL for synchronous operations
  @param[in] RequestPacket  Address of an EFI_I2C_MASTER_REQUEST_PACKET
  ...
  @retval EFI_DEVICE_ERROR      There was an I2C error (NACK) during the operation.
                                This could indicate the Peripheral device is not present.
  @retval EFI_INVALID_PARAMETER RequestPacket is NULL
  @retval EFI_INVALID_PARAMETER TPL is too high
  @retval EFI_NOT_FOUND         DeviceAddress exceeds maximum address
  ...
  @retval EFI_NO_RESPONSE       The I2C device is not responding to the
                                Peripheral address.  EFI_DEVICE_ERROR may also be
...
(EFIAPI *EFI_I2C_MASTER_START_REQUEST) (
  ...
  IN CONST EFI_I2C_MASTER_REQUEST_PACKET *RequestPacket,
  OUT EFI_STATUS *I2cStatus OPTIONAL
  );

///
/// I2C Controller mode protocol
///
/// This protocol manipulates the I2C host controller to perform transactions as a
/// Controller on the I2C bus using the current state of any switches or multiplexers
/// in the I2C bus.
///
struct _EFI_I2C_MASTER_PROTOCOL {
  ...
  ///
  /// Start an I2C transaction in Controller mode on the host controller
  ///
