  ...
  )
{
  ...
  //
  if (CpuAgent->DiscoveryTbl.Header.DW0.CrashType != CRASH_TYPE_PTL_DESKTOP) {
  ...
  //
  CpuAgent->CapStsPtr = (CAPABILITY_STATUS_PHASE_2 *) &(CpuAgent->DiscoveryTbl);
  if (CpuAgent->CapStsPtr->ClDis == 1) {
    DEBUG ((DEBUG_ERROR, " %a - error: Crashlog disabled bit is set.\n", __FUNCTION__));
    return EFI_UNSUPPORTED;
  }
...
{
  PUNIT_CONTROL_INTERFACE            *Control;
  UINT32                             ControlData32;

  Control = (PUNIT_CONTROL_INTERFACE *)(VOID *) &ControlData32;
  ControlData32 = 0;
  ...
  if (CpuAgent->ControlConfig.Enable == 0) {
    ControlData32 = 0;
    Control->SetDis = 1;
    AccessMmioDword (&ControlData32, CpuAgent->DiscoveryTblAddr + 5 * sizeof (UINT32), 1, WRITE);
    DEBUG ((DEBUG_INFO, "Cpu Crashlog - Set disable bit to command register (0x%08x)", ControlData32));
  } else {

    //
    // Check if storage-off
    //
    if (CpuAgent->ControlConfig.StorageOff == 1 && CpuAgent->CapStsPtr->StorageOff == 1) {
      ControlData32 = 0;
      Control->SetStorageOff = 1;
      AccessMmioDword (&ControlData32, CpuAgent->DiscoveryTblAddr + 5 * sizeof (UINT32), 1, WRITE);
      DEBUG ((DEBUG_INFO, "Cpu Crashlog - Set storage off bit to command register (0x%08x)", ControlData32));
      if (!IsSetStorageOff ()) {
        DEBUG ((DEBUG_INFO, " storage off status bit was not asserted ! \n"));
      }
    }

    //
    // Check if re-arm trigger
    //
    if (CpuAgent->ControlConfig.ReArm == 1) {
      ControlData32 = 0;
      Control->SetReArm = 1;
      AccessMmioDword (&ControlData32, CpuAgent->DiscoveryTblAddr + 5 * sizeof (UINT32), 1, WRITE);
      DEBUG ((DEBUG_INFO, "Cpu Crashlog - Set re-arm bit to command register (0x%08x)", ControlData32));
    }
  }
  ...
  )
{
  ...
  CpuAgent->ControlConfig.StorageOff    = Variable.CpuStorageOff;
  ...
  if (CtlVariableHob != NULL) {
    CtlVariableHob->CpuClear = 0;
    CtlVariableHob->CpuRearm = 1;
    CtlVariableHob->CpuStorageOff = (CpuAgent->CapStsPtr->StorageOff == 1) ? 1 : 0;
  }
