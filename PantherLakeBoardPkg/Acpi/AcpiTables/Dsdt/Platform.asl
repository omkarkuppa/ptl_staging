...
External (\_SB.DTTE, IntObj) // EnableDptf
...
Method (W2SP, 1, Serialized)
{
  Store (0, Local0)
  // Wait for the serial port to be ready.
  While (LNotEqual (And(LSR, 0x20), 0x20)) {
    Stall (10)
    Increment (Local0)
    If (LGreater (Local0, 100))
    {
      Return ()
    }
  }
...
Method (SDBG, 1, Serialized)
{
  ...
  // Local4=buffer, Local5=size, Local6=iterator
  ToHexString (Arg0, Local4)   // convert argument to Hexadecimal String in case it isn't a string already. If it is, nothing happens.
  Store (Sizeof (Local4), Local5)

  Store (0, Local6)
  While (LLess (Local6, Local5)) {
    Mid (Local4, Local6, 1, Local7) // Store () doesn't work. Mid () does. Not sure what's the difference
    //Stall (100)
    W2SP (Local7)
    Increment (Local6)
  }
  ...
  If (CondRefOf (\_SB.DTTE)) {
    If (LEqual (\_SB.DTTE,1)) {
      If (CondRefOf (\_SB.IETM)) {
        ...
        If (LAnd (CondRefOf (\_SB.PC00.LPCB.H_EC), LEqual (\_SB.CHGE,1))){
