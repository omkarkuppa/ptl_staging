...
External (\_SB.PC00.XHC.DUAM, MethodObj)  //DUAM - Device User Absent Mode for XHCI controller
...
Mutex (HLDI, 0) // EC Hold indicator: 0- No one accessing the EC Power State 1- Someone else is accessing the EC Power State
...
Method (P_CS,0,Serialized)
{
    ...
    If (LEqual (OSYS,2012)) {    // ONLY for Win8 OS
      If (CondRefOf (\_SB.PC00.XHC.DUAM)) { // Notify USB port- RVP
        \_SB.PC00.XHC.DUAM ()
      }
    }
...
Scope (\)
{
  Method (NHPG,0,Serialized)
  {
    ...
    Store (1,\_SB.PC00.TRP0.HPSX) // clear the hot plug SCI enable bit
    Store (1,\_SB.PC00.TRP1.HPSX) // clear the hot plug SCI enable bit
    Store (1,\_SB.PC00.TRP2.HPSX) // clear the hot plug SCI enable bit
    Store (1,\_SB.PC00.TRP3.HPSX) // clear the hot plug SCI enable bit
  }
  ...
  Method (NPME,0,Serialized)
  {
    ...
    Store (0,\_SB.PC00.TRP0.PMEX) // clear the hot plug SCI enable bit
    Store (0,\_SB.PC00.TRP1.PMEX) // clear the hot plug SCI enable bit
    Store (0,\_SB.PC00.TRP2.PMEX) // clear the hot plug SCI enable bit
    Store (0,\_SB.PC00.TRP3.PMEX) // clear the hot plug SCI enable bit
    ...
    Store (1,\_SB.PC00.TRP0.PMSX) // clear the hot plug SCI enable bit
    Store (1,\_SB.PC00.TRP1.PMSX) // clear the hot plug SCI enable bit
    Store (1,\_SB.PC00.TRP2.PMSX) // clear the hot plug SCI enable bit
    Store (1,\_SB.PC00.TRP3.PMSX) // clear the hot plug SCI enable bit
  }
  //
  ...
  Method (_OSC,4,Serialized)
  {
    ...
    If (LEqual (Arg0,ToUUID ("0811B06E-4A27-44F9-8D60-3CBBC22E7B48")))
    {
      ...
      If (LEqual (Arg1,One))
      {
        ...
        {
          Store (0x04, OSCO)
          If (LNotEqual (And (HGDT,0x0F),4)) // Check Hybrid graphics is not enabled in bios setup?
          {
