...
if [ "$FSP_RESET" = "TRUE" ]; then
    FitGen -D $WORKSPACE/$BUILD_DIR/FV/CLIENTBIOS.fd $WORKSPACE/$BUILD_DIR/FV/CLIENTBIOS.fd -F 0x40 -NA -L $SLOT_SIZE $MICROCODE_ARRAY_FFS_GUID -I $BIOS_INFO_GUID $STARTUP_AC_PARA -O 0x0C RESERVE 0xA00 -O 0x0B RESERVE 0x400 -P 0xA 0x70 0x71 0x1 0x4 0x2A -T $FIXED_FIT_LOCATION
else
