...
#include "TestCaseInstallBtPpagVer2.h"
...
/**
  Unit-Test Function for InstallBtPpag.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallBtPpagVer2 (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS          Status;
  BT_PPAG_CONFIG_SET  TestBtPpagVar;
  BT_PPAG_CONFIG_SET  *GetBtPpagVar;

  ZeroMem (&TestBtPpagVar, sizeof (BT_PPAG_CONFIG_SET));
  TestBtPpagVar.Header.Revision                 = 99;
  TestBtPpagVar.AntGainMode.BitModeRev1.PPAG_EU = 9;
  ...
  UT_ASSERT_EQUAL (TestBtPpagVar.Header.Revision, 99);
  if (TestBtPpagVar.Header.Revision != 99) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }

  UT_ASSERT_EQUAL (TestBtPpagVar.AntGainMode.BitModeRev1.PPAG_EU, 1);
  if (TestBtPpagVar.AntGainMode.BitModeRev1.PPAG_EU != 1) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }
