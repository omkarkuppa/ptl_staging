...
#include "TestCaseInstallMpccVer2.h"
...
/**
  Unit-Test Function for InstallMpcc.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallMpccVer2 (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS            Status;
  WLAN_MPCC_CONFIG_SET  TestMpccVar;
  WLAN_MPCC_CONFIG_SET  *GetMpccVar;

  ZeroMem (&TestMpccVar, sizeof (WLAN_MPCC_CONFIG_SET));
  TestMpccVar.M21aAssembled              = 9;
  TestMpccVar.WifiPlatCurrentConsumption = 9;

  GetMpccVar = AllocateZeroPool (sizeof (WLAN_MPCC_CONFIG_SET));

  //
  // Mock run time service: GetVariable for InstallMpcc ()
  //
  ...
  UT_ASSERT_EQUAL (TestMpccVar.M21aAssembled, 9);
  if (TestMpccVar.M21aAssembled != 9) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }

  UT_ASSERT_EQUAL (TestMpccVar.WifiPlatCurrentConsumption, 9);
  if (TestMpccVar.WifiPlatCurrentConsumption != 9) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }
