...
#include "TestCaseInstallBrdsRev2.h"
...
/**
  Unit-Test Function for InstallBrdsRev2.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallBrdsRev2 (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS            Status;
  CNV_UEFI_VAR_BRDS_R2  TestBrdsRev2Var;
  CNV_UEFI_VAR_BRDS_R2  *GetBrdsRev2Var;

  ZeroMem (&TestBrdsRev2Var, sizeof (CNV_UEFI_VAR_BRDS_R2));
  TestBrdsRev2Var.Header.Revision = 99;

  GetBrdsRev2Var = AllocateZeroPool (sizeof (CNV_UEFI_VAR_BRDS_R2));

  //
  // Mock run time service: GetVariable for InstallBrdsRev2 ()
  //
  ...
  will_return (MockGetVariable, GetBrdsRev2Var);
  ...
  Status = UpdateVariables (
             sizeof (CNV_UEFI_VAR_BRDS_R2),
             CNV_UEFI_BRDS_VAR_NAME,
             &gUefiIntelCnvBtVariablesGuid,
             (VOID *)&TestBrdsRev2Var,
             FALSE,
             FALSE
             );

  UT_ASSERT_EQUAL (TestBrdsRev2Var.Header.Revision, 99);

  FreePool (GetBrdsRev2Var);
  FreePool (gRT);
