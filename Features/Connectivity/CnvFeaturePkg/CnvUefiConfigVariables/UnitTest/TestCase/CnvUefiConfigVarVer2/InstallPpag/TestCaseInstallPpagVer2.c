...
#include "TestCaseInstallPpagVer2.h"
...
/**
  Unit-Test Function for InstallPpag.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallPpagVer2 (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS         Status;
  CNV_UEFI_VAR_PPAG  TestPpagVar;
  CNV_UEFI_VAR_PPAG  *GetPpagVar;

  ZeroMem (&TestPpagVar, sizeof (CNV_UEFI_VAR_PPAG));
  TestPpagVar.Header.Revision              = 99;
  TestPpagVar.WifiAntGainMode.PpagModesU32 = 9;
  TestPpagVar.AntennaGainChainA[0]         = 0x99;
  TestPpagVar.AntennaGainChainB[10]        = 0x99;

  GetPpagVar = AllocateZeroPool (sizeof (CNV_UEFI_VAR_PPAG));

  //
  // Mock run time service: GetVariable for InstallPpag ()
  //
  ...
  UT_ASSERT_EQUAL (TestPpagVar.Header.Revision, 99);
  if (TestPpagVar.Header.Revision != 99) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }

  UT_ASSERT_EQUAL (TestPpagVar.WifiAntGainMode.PpagModesU32, 9);
  if (TestPpagVar.WifiAntGainMode.PpagModesU32 != 9) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }

  UT_ASSERT_EQUAL (TestPpagVar.AntennaGainChainA[0], 0x99);
  if (TestPpagVar.AntennaGainChainA[0] != 0x99) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }

  UT_ASSERT_EQUAL (TestPpagVar.AntennaGainChainB[10], 0x99);
  if (TestPpagVar.AntennaGainChainB[10] != 0x99) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }
