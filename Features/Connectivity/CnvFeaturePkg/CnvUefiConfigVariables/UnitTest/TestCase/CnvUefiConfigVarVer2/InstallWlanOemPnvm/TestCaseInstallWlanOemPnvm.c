...
#include "TestCaseInstallWlanOemPnvm.h"
...
/**
  Unit-Test Function for InstallWlanOemPnvm.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallWlanOemPnvm (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS     Status;
  WLAN_OEM_PNVM  TestWlanOemPnvmVar;
  WLAN_OEM_PNVM  *GetWlanOemPnvmVar;

  ZeroMem (&TestWlanOemPnvmVar, sizeof (WLAN_OEM_PNVM));
  TestWlanOemPnvmVar.Header.Revision = 9;

  GetWlanOemPnvmVar = AllocateZeroPool (sizeof (WLAN_OEM_PNVM));

  //
  // Mock run time service: GetVariable for InstallWlanOemPnvm ()
  //
  ...
  Status = UpdateVariables (
             ...
             (VOID *)&TestWlanOemPnvmVar,
             FALSE,
             FALSE
             );

  UT_ASSERT_EQUAL (TestWlanOemPnvmVar.Header.Revision, 9);
