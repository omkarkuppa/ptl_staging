...
#include "TestCaseInstallBbfbVer2.h"
...
/**
  Unit-Test Function for InstallBbfb.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallBbfbVer2 (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS                   Status;
  ICNV_BT_BIQUAD_BYPASS_REV_1  TestBbfbVar;
  ICNV_BT_BIQUAD_BYPASS_REV_1  *GetBbfbVar;

  ZeroMem (&TestBbfbVar, sizeof (ICNV_BT_BIQUAD_BYPASS_REV_1));
  TestBbfbVar.Header.Revision = 99;
  ...
  UT_ASSERT_EQUAL (TestBbfbVar.Header.Revision, 99);
  if (TestBbfbVar.Header.Revision != 99) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }
