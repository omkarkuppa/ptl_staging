...
#include "TestCaseInstallSplcVer2.h"
...
/**
  Unit-Test Function for InstallSplc.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallSplcVer2 (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS         Status;
  CNV_UEFI_VAR_SPLC  TestSplcVar;
  CNV_UEFI_VAR_SPLC  *GetSplcVar;

  ZeroMem (&TestSplcVar, sizeof (CNV_UEFI_VAR_SPLC));
  TestSplcVar.DefaultPowerLimit = 0x12345678;

  GetSplcVar = AllocateZeroPool (sizeof (CNV_UEFI_VAR_SPLC));

  //
  // Mock run time service: GetVariable for InstallSplc ()
  //
  ...
  UT_ASSERT_EQUAL (TestSplcVar.DefaultPowerLimit, 0x12345678);
