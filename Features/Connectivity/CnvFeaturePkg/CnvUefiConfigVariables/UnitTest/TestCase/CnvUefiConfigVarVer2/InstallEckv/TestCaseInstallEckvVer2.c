...
#include "TestCaseInstallEckvVer2.h"
...
/**
  Unit-Test Function for InstallEckv.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallEckvVer2 (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS         Status;
  CNV_UEFI_VAR_ECKV  TestEckvVar;
  CNV_UEFI_VAR_ECKV  *GetEckvVar;

  ZeroMem (&TestEckvVar, sizeof (CNV_UEFI_VAR_ECKV));
  TestEckvVar.Header.Revision       = 99;
  TestEckvVar.CnvExternal32kHzClock = 9;
  ...
  UT_ASSERT_EQUAL (TestEckvVar.Header.Revision, 99);
  if (TestEckvVar.Header.Revision != 99) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }

  UT_ASSERT_EQUAL (TestEckvVar.CnvExternal32kHzClock, 9);
  if (TestEckvVar.CnvExternal32kHzClock != 9) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }
