...
#include "TestCaseInstallEwrdVer2.h"
...
/**
  Unit-Test Function for InstallEwrd.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallEwrdVer2 (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS         Status;
  CNV_UEFI_VAR_EWRD  TestEwrdVar;
  CNV_UEFI_VAR_EWRD  *GetEwrdVar;

  ZeroMem (&TestEwrdVar, sizeof (CNV_UEFI_VAR_EWRD));
  TestEwrdVar.Header.Revision = 99;
  ...
  UT_ASSERT_EQUAL (TestEwrdVar.Header.Revision, 99);
  if (TestEwrdVar.Header.Revision != 99) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }
