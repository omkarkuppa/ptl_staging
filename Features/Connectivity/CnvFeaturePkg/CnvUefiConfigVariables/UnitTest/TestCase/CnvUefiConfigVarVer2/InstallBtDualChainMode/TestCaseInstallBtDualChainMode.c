...
#include "TestCaseInstallBtDualChainMode.h"
...
/**
  Install BtDualChainMode for BT
...
/**
  Unit-Test Function for InstallBtDualChainMode.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallBtDualChainMode (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS                     Status;
  ICNV_BT_DUAL_CHAIN_MODE_REV_1  TestBtDualChainModeVar;
  ICNV_BT_DUAL_CHAIN_MODE_REV_1  *GetBtDualChainModeVar;

  ZeroMem (&TestBtDualChainModeVar, sizeof (ICNV_BT_DUAL_CHAIN_MODE_REV_1));
  TestBtDualChainModeVar.Header.Revision = 99;

  GetBtDualChainModeVar = AllocateZeroPool (sizeof (ICNV_BT_DUAL_CHAIN_MODE_REV_1));

  //
  // Mock run time service: GetVariable for InstallBtDualChainMode ()
  //
  ...
  will_return (MockGetVariable, GetBtDualChainModeVar);
  ...
  Status = UpdateVariables (
             ...
             (VOID *)&TestBtDualChainModeVar,
             FALSE,
             FALSE
             );

  UT_ASSERT_EQUAL (TestBtDualChainModeVar.Header.Revision, 99);

  FreePool (GetBtDualChainModeVar);
  FreePool (gRT);
