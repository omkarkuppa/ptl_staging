...
#include "TestCaseInstallWlanGeneralCfg.h"
...
/**
  Install WlanGeneralCfg for WiFi
...
/**
  Unit-Test Function for InstallWlanGeneralCfg.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallWlanGeneralCfg (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS               Status;
  WLAN_GENERAL_CONFIG_SET  TestWlanGeneralCfgVar;
  WLAN_GENERAL_CONFIG_SET  *GetWlanGeneralCfgVar;

  ZeroMem (&TestWlanGeneralCfgVar, sizeof (WLAN_GENERAL_CONFIG_SET));
  TestWlanGeneralCfgVar.Header.Revision = 9;

  GetWlanGeneralCfgVar = AllocateZeroPool (sizeof (WLAN_GENERAL_CONFIG_SET));

  //
  // Mock run time service: GetVariable for InstallWlanGeneralCfg ()
  //
  ...
  Status = UpdateVariables (
             ...
             (VOID *)&TestWlanGeneralCfgVar,
             FALSE,
             FALSE
             );

  UT_ASSERT_EQUAL (TestWlanGeneralCfgVar.Header.Revision, 9);
