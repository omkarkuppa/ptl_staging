...
#include "TestCaseInstallWcscVer2.h"
...
/**
  Unit-Test Function for InstallWcsc.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallWcscVer2 (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS         Status;
  CNV_UEFI_VAR_WCSC  TestWcscVar;
  CNV_UEFI_VAR_WCSC  *GetWcscVar;

  ZeroMem (&TestWcscVar, sizeof (CNV_UEFI_VAR_WCSC));
  TestWcscVar.Header.Revision = 99;

  GetWcscVar = AllocateZeroPool (sizeof (CNV_UEFI_VAR_WCSC));

  //
  // Mock run time service: GetVariable for InstallWcsc ()
  //
  ...
  UT_ASSERT_EQUAL (TestWcscVar.Header.Revision, 99);
  if (TestWcscVar.Header.Revision != 99) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }
