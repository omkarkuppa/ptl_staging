...
#include "TestCaseInstallWpfcVer2.h"
...
/**
  Unit-Test Function for InstallWpfc.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallWpfcVer2 (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS         Status;
  UEFI_CNV_VAR_WPFC  TestWpfcVar;
  UEFI_CNV_VAR_WPFC  *GetWpfcVar;

  ZeroMem (&TestWpfcVar, sizeof (UEFI_CNV_VAR_WPFC));
  TestWpfcVar.Header.Revision = 9;

  GetWpfcVar = AllocateZeroPool (sizeof (UEFI_CNV_VAR_WPFC));

  //
  // Mock run time service: GetVariable for InstallWpfc ()
  //
  ...
  UT_ASSERT_EQUAL (TestWpfcVar.Header.Revision, 9);
