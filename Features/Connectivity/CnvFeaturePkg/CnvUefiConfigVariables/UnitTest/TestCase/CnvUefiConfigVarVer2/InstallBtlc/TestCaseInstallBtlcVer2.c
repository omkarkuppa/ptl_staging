...
#include "TestCaseInstallBtlcVer2.h"
...
/**
  Unit-Test Function for InstallBtlc.
...
UNIT_TEST_STATUS
EFIAPI
UnitTestInstallBtlcVer2 (
  IN UNIT_TEST_CONTEXT  Context
  )
{
  EFI_STATUS         Status;
  CNV_UEFI_VAR_BTLC  TestBtlcVar;
  CNV_UEFI_VAR_BTLC  *GetBtlcVar;

  ZeroMem (&TestBtlcVar, sizeof (CNV_UEFI_VAR_BTLC));
  TestBtlcVar.Header.Revision = 99;
  ...
  UT_ASSERT_EQUAL (TestBtlcVar.Header.Revision, 99);
  if (TestBtlcVar.Header.Revision != 99) {
    return UNIT_TEST_ERROR_TEST_FAILED;
  }
