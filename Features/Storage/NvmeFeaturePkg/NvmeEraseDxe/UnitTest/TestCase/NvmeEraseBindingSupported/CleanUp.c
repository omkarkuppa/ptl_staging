/**@file
    Unit test case CleanUp of NvmeEraseBindingSupported.
  ...
  )
{
  ...
  if (Parameter->OpenBlockIoMock.BlockIO != NULL) {
    if (Parameter->OpenBlockIoMock.BlockIO->Media != NULL) {
      FreePool (Parameter->OpenBlockIoMock.BlockIO->Media);
    }
    FreePool (Parameter->OpenBlockIoMock.BlockIO);
  }
