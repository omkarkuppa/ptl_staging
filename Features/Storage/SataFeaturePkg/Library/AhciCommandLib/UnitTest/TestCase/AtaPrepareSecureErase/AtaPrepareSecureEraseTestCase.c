/**@file
    Unit test cases of AtaPrepareSecureErase.
...
#include "AtaPrepareSecureEraseMock.h"
  ...
  )
{
  ...
  Status = AtaPrepareSecureErase (AtaDevice, 1, 0xffff);
  UT_ASSERT_EQUAL (Status, EFI_SUCCESS);

  //
  // Mock Idenfity device data
  // refer to ATA spec, Device in frozen mode
  //
  IdentifyData = IdentifyDataConstructMock ();
  IdentifyData->security_status |= BIT3;
  Status = AtaPrepareSecureErase (AtaDevice, 1, 0xffff);
