/**@file
  Definition of SATA erase block protocol
...
#ifndef __SATA_ERASE_BLOCK_PROTOCOL_H__
#define __SATA_ERASE_BLOCK_PROTOCOL_H__

#define SATA_ERASE_BLOCK_PROTOCOL_GUID \
  { \
    0xd1beccbb, 0x3928, 0x4e33, { 0xaa, 0x47, 0x32, 0x10, 0xd1, 0x4c, 0x91, 0x11 } \
  }

typedef struct _SATA_ERASE_BLOCK_PROTOCOL SATA_ERASE_BLOCK_PROTOCOL;

#define SATA_ERASE_BLOCK_PROTOCOL_REVISION ((2<<16) | (60))

///
/// SATA_ERASE_BLOCK_TOKEN
///
typedef struct {
  //
...
#define SATA_MANUFACURER_MAX_LEN 20
#define SATA_SN_MAX_LEN          20
#define SATA_MN_MAX_LEN          40
#define SATA_SANITIZE_COUNT      4
...
typedef struct {
  UINT32                  VidDid;
  CHAR8                   Manufacturer[SATA_MANUFACURER_MAX_LEN + 1];
  CHAR8                   ModelName[SATA_MN_MAX_LEN + 1];
  CHAR8                   SerialNumber[SATA_SN_MAX_LEN + 1];
  SATA_ERASE_TYPE         Method;
  EFI_STATUS              Result;
} SATA_ERASE_INFO;
...
typedef
EFI_STATUS
(EFIAPI *SATA_ERASE_BLOCK_PROGRESS) (
IN UINTN Completion
);
...
/**
  ...
  @param[in]NVME_ERASE_BLOCK_PROGRESS This        Indicates a pointer to the calling context.
...
///
struct _SATA_ERASE_BLOCK_PROTOCOL {
  //
  ...
  SATA_BLOCK_ERASE            EraseBlocks;
  GET_SATA_ERASE_INFO         GetEraseInfo;
  GET_SATA_ESTIMATE_TIME      GetEstimateTime;
};

extern EFI_GUID gSataEraseBlockProtocolGuid;
