## @file
#  FMP device instance for Extended BIOS firmware.
#
#  @copyright
#  INTEL CONFIDENTIAL
...
#
EXT_BIOS_DEVICE_NAME      : str  = 'ExtendedBios'
EXT_BIOS_DEVICE_NAME_UPPER: str  = EXT_BIOS_DEVICE_NAME.upper ()
EXT_BIOS_FIRMWARE_CONFIG  : dict = gFmpFirmwareInfo[EXT_BIOS_DEVICE_NAME]
...
#
# Region flash order of different Extended BIOS image type.
#
EXT_BIOS_UPDATE_FLASH_ORDER: dict = {
    BIOS_NORMAL_TYPE    : [
        BIOS_REGION_IBBR,
        BIOS_REGION_IBB,
        BIOS_REGION_OBB,
...
EXT_BIOS_BUILD_OP_VER_1: int = 1

#
# Generic-defined.
#
if EXT_BIOS_FIRMWARE_CONFIG is not None:
    CAP_FILE_PREFIX : str  = EXT_BIOS_FIRMWARE_CONFIG[CAP_FILE_PREFIX_KEY]
    CAP_FILE_POSTFIX: str  = EXT_BIOS_FIRMWARE_CONFIG[CAP_FILE_POSTFIX_KEY]
    GUID_DATA       : dict = EXT_BIOS_FIRMWARE_CONFIG[FMP_GUID_SECTION_KEY]
    BUILD_OP_VER    : int  = EXT_BIOS_FIRMWARE_CONFIG[FW_VERSION_KEY]

#
# Default FMP instance should define as the Null one.
#
class ExtendedBios (FmpDeviceNull):
    def __init__ (self, *Args, **KwArgs):
        DEBUG (DEBUG_WARN, f'Extended BIOS FMP instance is not implemented.')

#
# Override with different build operation.
#
if (BUILD_OP_VER == EXT_BIOS_BUILD_OP_VER_1):
    BGSL_TEMPLATE_KEY : str = 'BgslTemplateFile'

    EXT_BIOS_FLASH_MAP_DATA    : dict = \
        EXT_BIOS_FIRMWARE_CONFIG[FLASH_MAP_OFFSET_SECTION]
    EXT_BIOS_BGSL_TEMPLATE_PATH: str  = \
        JoinPath (
          TEMPLATE_DIR_PATH,
          EXT_BIOS_FIRMWARE_CONFIG[BGSL_TEMPLATE_KEY],
          )

    EXT_BIOS_FDF_FV_NAME: str = 'CapsulePayloadExtBios'
    #
    ...
    class ExtendedBios (FmpDevice):
        def __init__ (self, Params: Dict[str, Any]) -> None:
            """ Class for the FMP device instance. (Extended BIOS ver.1)
            ...
            #
            # Get the Extended BIOS image configuration.
            #
            ...
            #
            # Get the version information for Extended BIOS.
            #
        ...
        def BiosSvn (self) -> str:
            """ Return the Extended BIOS SVN value. (HEX with 0x prefix)
            ...
            Returns:
                str:
                    Value of Extended BIOS SVN HEX value with 0x prefix.
            """
            return self.__BiosSvn

        @property
        def BiosType (self) -> str:
            """ Return the type of the Extended BIOS image.
            ...
            Returns:
                str:
                    Type of Extended BIOS image should be defined in
        ...
        def BuildType (self) -> str:
            """ Return the build type of this Extended BIOS image.
            ...
            Returns:
                str:
                    Build type of this Extended BIOS image, it should be
                    defined in one of BUILD_TARGET_TYPE_LIST.
            """
        ...
        def BiosCapVer (self) -> BiosCapsuleVersion:
            ...
            Returns:
                BiosCapsuleVersion:
                    The Extended BIOS version converted into capsule version.
            """
        ...
        def __PreCheck (self) -> None:
            ...
            Raises:
                ...
                ValueError:
                    ...
                    (2) Caller input the Extended BIOS firmware as ignore.
            ...
            if self.__BiosPath:
                DEBUG (DEBUG_TRACE, f'Assigned the Extended BIOS FW image.')
                if IsFwImageIgnored (self.__BiosPath):
                    raise ValueError (
                            f'Extended BIOS FW can not ignore when building Extended BIOS capsule.'
                            )
        ...
        def __PostCheck (self) -> None:
            ...
            Note:
                (1) Check the Extended BIOS SVN and image version relationship.
            ...
            if int (self.__BiosSvn) < int (BiosVer):
                DEBUG (
                  DEBUG_WARN,
                  f'Extended BIOS SVN {self.__BiosSvn} is lower than '
                  f'Extended BIOS version {BiosVer}.'
                  )
            else:
                DEBUG (
                  DEBUG_INFO,
                  f'Extended BIOS SVN is {self.__BiosSvn}, '
                  f'Extended BIOS major version is {BiosVer}.'
                  )
        ...
        def __GetImageBuffer (self) -> ByteBuffer:
            ...
            if self.__BiosPath:
                DEBUG (
                  DEBUG_TRACE,
                  f'User assign the Extended BIOS path: {self.__BiosPath}'
                  )
                CopyFile (
                  self.__BiosPath,
                  BIOS_IMG_FILE_PATH
        ...
        def __GetBiosType (self, IsAutoDetect: bool) -> str:
            """ Identify the type of this Extended BIOS image.

            Args:
                IsAutoDetect (bool):
                    If let program logic to decide the Extended BIOS type.
            ...
            #
            # Check the Extended BIOS ID signature located in IBBr.
            #
            if IsAutoDetect:
        ...
        def __GetCapsulePayload (self) -> ByteBuffer:
            ...
            if self.__IsBgupBuild:
                BiosBgupGenerator = \
                    _BiosBgupGenerator (
                      ...
                      BgslTemplatePath = EXT_BIOS_BGSL_TEMPLATE_PATH,
            ...
            ConfigIniGenerator = \
                _BiosCfgIniGenerator (
                  FlashMapDict     = self.__FlashMapInfo,
                  BiosUpdateOrder  = EXT_BIOS_UPDATE_FLASH_ORDER[self.__BiosType],
                  BiosImgInfoDict  = BiosImgInfoDict,
                  BiosBgupInfoDict = BiosBgupInfoDict,
                  )
            ConfigIniGenerator.Save (EXT_BIOS_CFG_FILE_PATH)

            #
            # 4. Generate the FDF file
            #
            FdfGenerator = CapsulePayloadFdfGeneratorApi (EXT_BIOS_FDF_FV_NAME, self.__BlockSize)
            FdfGenerator.AddRawFile (uuid.UUID (EXT_BIOS_CFG_FILE_GUID), EXT_BIOS_CFG_FILE_NAME, None)
            ...
            #
            Edk2FvBuilder = Edk2FdGeneratorApi (EXT_BIOS_FDF_FV_NAME)
        ...
        def GetDeviceName (self) -> Tuple[int, str]:
            ...
            """
            return FMP_SUCCESS, EXT_BIOS_DEVICE_NAME
