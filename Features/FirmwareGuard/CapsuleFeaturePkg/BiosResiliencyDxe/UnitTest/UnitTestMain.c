/**@file
    Unit test main of the Bios Resiliency Dxe driver.
...
//
#include "TestCase/BiosResiliencyDxeTestCase.h"

#define UNIT_TEST_NAME     "BiosResiliencyDxe Unit Test"
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestBiosResiliencyDxe;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestBiosResiliencyDxe,     // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestBiosResiliencyDxe,                // SuiteHandle        : Unit test suite to add test to
