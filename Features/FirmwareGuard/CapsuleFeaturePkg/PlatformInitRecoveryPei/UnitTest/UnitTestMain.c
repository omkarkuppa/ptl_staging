/**@file
    Unit test main of the Platform Init Recovery Pei driver.
...
//
#include "TestCase/PlatformInitRecoveryPeiTestCase.h"

#define UNIT_TEST_NAME     "PlatformInitRecoveryPei Unit Test"
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestPlatformInitRecoveryPei;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestPlatformInitRecoveryPei,     // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestPlatformInitRecoveryPei,          // SuiteHandle        : Unit test suite to add test to
