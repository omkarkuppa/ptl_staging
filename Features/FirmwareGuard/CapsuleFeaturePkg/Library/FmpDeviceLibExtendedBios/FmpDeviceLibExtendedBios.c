/** @file
  FmpDeviceLib instance to support Extended Bios update
  ...
  )
{
  ...
  *ImageUpdatable = IMAGE_UPDATABLE_VALID;
  ...
  //
  if (!IsImageForExtendedBiosUpdate ((VOID *) Image, ImageSize)) {
    *ImageUpdatable = IMAGE_UPDATABLE_INVALID;
  }
  ...
  )
{
  ...
  UINT8                               *ExtendedBiosImage;
  UINTN                               ExtendedBiosImageSize;
  UINT8                               *ExtendedBiosObbImage;
  UINTN                               ExtendedBiosObbImageSize;
  UINT8                               *NonFitPayloadImage;
  UINTN                               NonFitPayloadImageSize;


  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (Extended BIOS) - Start\n"));
  ...
  ExtendedBiosObbImage     = NULL;
  ExtendedBiosImageSize    = 0;
  ExtendedBiosObbImageSize = 0;
  ...
  if (GetBiosResiliencyType () == SUPPORT_BIOS_RESILIENCY_RECOVERY) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "NO ESP, Extended BIOS update is not supported! \n"));
      *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
      return EFI_UNSUPPORTED;
    }
  }
  ...
  if (!IsPreviousUpdateUnfinished (&PreviousProgress)) {
    if (ExtractExtendedBiosFromImage ((VOID *) Image, ImageSize, (VOID **) &ExtendedBiosImage, &ExtendedBiosImageSize)) {
      SaveExtendedBiosToStorage ((VOID *) ExtendedBiosImage, ExtendedBiosImageSize);
      ExtendedBiosObbImage     = ExtendedBiosImage
  ...
  DEBUG ((DEBUG_INFO, "UpdateExtendedBios - Start\n"));
  Status = PerformExtendedBiosUpdateByConfigData (
             (VOID *) Image,
             ImageSize,
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "[%a]: Extended Bios Update Fail!\n", __FUNCTION__));
  }
  ...
  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (Extended BIOS) - %r\n", Status));
...
{
  DEBUG ((DEBUG_INFO, "FmpDeviceLib(Extended Bios): FmpDeviceLock() for system FLASH\n"));
