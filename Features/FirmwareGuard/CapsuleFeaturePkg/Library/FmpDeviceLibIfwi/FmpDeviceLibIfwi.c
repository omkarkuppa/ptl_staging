/** @file
  FmpDeviceLib instance to support IFWI update.
...
#define IFWI_UPDATE_START_PERCENTAGE 5
#define IFWI_UPDATE_END_PERCENTAGE   95

BOOLEAN  mIsBiosGuardEnabled = FALSE;

/**
  Returns the base address and size, in bytes, of the firmware device.
  This function is a internal function to get the region base address and size
  via the Spi Protocol. If the device base address and size can not be determined
  from the firmware device, then 0 on both must be returned.

  @param[out] BaseAddress  Pointer the get the firmware device region base address.
  ...
  )
{
  ...
  *Size        = 0;
  ...
  Status = SpiProtocol->GetRegionAddress (SpiProtocol, FlashRegionAll, BaseAddress, Size);
  if (EFI_ERROR (Status)) {
    *BaseAddress = 0;
    *Size        = 0;
  }
...
/**
  Check if the input Image contains expected files used for IFWI update.

  @param[in] Image      Pointer to FMP payload image.
                        (FMP image header is stripped off)
  @param[in] ImageSize  The size of Image in bytes.

  @retval TRUE   The image contains expected files used for IFWI update.
  @retval FALSE  The image does not contain expected files used for
                 IFWI update.

**/
BOOLEAN
IsImageForIfwiUpdate (
  IN VOID   *Image,
  IN UINTN  ImageSize
  )
{
  ...
  //
  // Get the flash device region.
  //
  Status = GetIfwiFlashRegion (&FlashRegionBaseAddress, &FlashRegionSize);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Failed to get the IFWI flash region (%r)\n", Status));
    goto CleanUp;
  }

  //
  // Separate the IFWI image, BGUP image, and config file.
  //
  Status = ExtractUpdateImageByConfigFile (
             ...
             &IfwiImage,
             &IfwiImageSize,
             mIsBiosGuardEnabled ? &BgupImage : NULL,
             mIsBiosGuardEnabled ? &BgupImageSize : NULL
             );
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Failed to ExtractUpdateImageByConfigFile (%r)\n", Status));
    goto CleanUp;
  }

  //
  // Check the image size and flash device size.
  //
  if (IfwiImageSize != FlashRegionSize) {
    DEBUG ((
      DEBUG_ERROR,
      "Image size (0x%x) is not matched to flash device size (0x%x)\n",
      IfwiImageSize,
      FlashRegionSize
      ));
    goto CleanUp;
  ...
  //
  // All conditions check pass, set image is supported.
  //
  IsSupported = TRUE;
  ...
  )
{
  ...
  *ImageUpdatable = IMAGE_UPDATABLE_VALID;
  ...
  //
  // Make sure the image has the information needed for updating IFWI.
  //
  if (!IsImageForIfwiUpdate ((VOID *) Image, ImageSize)) {
    *ImageUpdatable = IMAGE_UPDATABLE_INVALID;
  }
  ...
  )
{
  ...
  UINTN                 CurrUpdateSize;
  FLASH_ACCESS_REQUEST  FlashReq;
  UINTN                 BytesWritten;
  EFI_STATUS            ResetRegisterStatus;

  Status              = EFI_SUCCESS;
  DisplayLength       = IFWI_UPDATE_END_PERCENTAGE - IFWI_UPDATE_START_PERCENTAGE;
  ConfigData          = NULL;
  IfwiImage           = NULL;
  IfwiImageSize       = 0;
  BgupImage           = NULL;
  ...
  CurrUpdateSize      = 0;
  BytesWritten        = 0;
  ResetRegisterStatus = EFI_SUCCESS;

  DEBUG ((DEBUG_INFO, "%a (IFWI) - Start\n", __FUNCTION__));

  //
  // Check the progress callback available to use
  //
  if (Progress == NULL) {
    DEBUG ((DEBUG_ERROR, "%a - Invalid progress callback\n", __FUNCTION__));
    *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
    return EFI_INVALID_PARAMETER;
  }
  ...
  //
  Status = Progress (IFWI_UPDATE_START_PERCENTAGE);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a - Progress Callback failed with Status %r.\n", __FUNCTION__, Status));
  }

  //
  // Extract the config file, IFWI image file, and BGUP file
  //
  Status = ExtractUpdateImageByConfigFile (
             (VOID *) Image,
             ImageSize,
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a - Failed to ExtractUpdateImageByConfigFile (%r)\n", __FUNCTION__, Status));
    return Status;
  }

  //
  // Prepare the data for progress bar
  //
  ...
  //
  // Halt the ME before program the CSME full region directly.
  //
  Status = HaltMeBeforeFlash ();
  if (EFI_ERROR (Status)) {
    goto CleanUp;
  }

  //
  // Hook the Runtime Service
  //
  gRT->SetVariable = HookSetVariableFunction;
  gRT->GetVariable = HookGetVariableFunction;

  //
  // Perform the Full IFWI update
  //
  for (Index = 0; Index < ConfigHeader.NumOfUpdates; Index++) {
    DEBUG ((
      DEBUG_INFO,
      "%a (IFWI) (stage %d): BaseAddress - 0x%lx ImageOffset - 0x%x Length - 0x%x\n",
      __FUNCTION__,
      Index,
    ...
    CurrUpdateSize = ConfigData[Index].Length;

    //
    // Get flash access request
    //
    ZeroMem (&FlashReq, sizeof (FlashReq));
    GetIfwiUpdateRequest (ConfigData[Index], IfwiImage, BgupImage, &FlashReq);
    ...
    Status = FlashUpdate (
               &FlashReq,
               Progress,
               IFWI_UPDATE_START_PERCENTAGE + ((BytesWritten * DisplayLength) / TotalUpdateSize),
               IFWI_UPDATE_START_PERCENTAGE + (((BytesWritten + CurrUpdateSize) * DisplayLength) / TotalUpdateSize)
               );

    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "UpdateIfwiFirmware failed in stage %d (%r)\n", Index, Status));
      break;
    }

    BytesWritten += CurrUpdateSize;
  }
  ...
  if (EFI_ERROR (Status)) {
    *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
    ...
    if (ResetRegisterStatus == EFI_SUCCESS) {
      //
      // Register reset filter to change reset type to EC reset.
      //
      ResetRegisterStatus = CapsuleUpdateRegisterResetFilter (CAPSULE_UPDATE_RESET_TYPE_EC_RESET);
      DEBUG ((DEBUG_INFO, "RegisterResetFilter (EC reset) - %r\n", ResetRegisterStatus));
    }
  }

  Progress (100);

  DEBUG ((DEBUG_INFO, "%a (IFWI) - End (%r)\n", __FUNCTION__, Status));
...
/**
  Constructor for the FmpDeviceLibIfwi library.
...
EFI_STATUS
EFIAPI
FmpDeviceLibIfwiConstructor (
  IN EFI_HANDLE        ImageHandle,
  IN EFI_SYSTEM_TABLE  *SystemTable
  )
{
  //
  // Get the BiosGuard status.
  //
  mIsBiosGuardEnabled = IsBiosGuardEnabled ();
