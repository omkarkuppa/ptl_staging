/** @file
  FmpDeviceLib instance to support Fsp update
  ...
  )
{
  if (GetBiosResiliencyType () == NOT_SUPPORT_RECOVERY) {
    DEBUG ((DEBUG_INFO, "%a: Resiliency does not support. No need to install FMP at this moment.\n", __func__));
  ...
  )
{
  ...
  *ImageUpdatable = IMAGE_UPDATABLE_VALID;
  ...
  if (IsVerifiedBoot () && !IsBiosSupportFspSigned ()) {
    DEBUG ((DEBUG_ERROR, "CheckImage - FBM check fails.\n"));
    *ImageUpdatable = IMAGE_UPDATABLE_INVALID;
    return EFI_UNSUPPORTED;
  }
  //
  // Make sure the image has the information needed for updating Fsp
  //
  if (!IsImageForFspUpdate ((VOID *) Image, ImageSize)) {
    *ImageUpdatable = IMAGE_UPDATABLE_INVALID;
  }
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (Fsp) - Start\n"));
  ...
  if (GetBiosResiliencyType () != NOT_SUPPORT_RECOVERY) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "NO ESP, Fsp update is not supported! \n"));
      *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
      return EFI_UNSUPPORTED;
    }
  }
  ...
  DEBUG ((DEBUG_INFO, "UpdateFsp - End (%r)\n", Status));
  if (EFI_ERROR (Status)) {
  ...
  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (Fsp) - %r\n", Status));
...
{
  DEBUG ((DEBUG_INFO, "FmpDeviceLib(Fsp): FmpDeviceLock() for system FLASH\n"));
