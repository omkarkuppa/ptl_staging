/** @file
  FmpDeviceLib instance to support EC update
  ...
  )
{
  ...
  *Size = 0;
  ...
  Status = SpiProtocol->GetRegionAddress (SpiProtocol, FlashRegionEC, BaseAddress, Size);
  if (EFI_ERROR (Status)) {
    *BaseAddress = 0;
    *Size = 0;
  }
...
{
  return PlatformFmpGetEcVersionString (VersionString);
}
...
{
  return PlatformFmpGetEcVersion (Version);
}
  ...
  )
{
  ...
  *ImageUpdatable = IMAGE_UPDATABLE_VALID;
  ...
  //
  Status = GetEcFlashRegion (&RegionBaseAddress, &RegionSize);
  if (EFI_ERROR (Status)) {
    *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
    return EFI_UNSUPPORTED;
  }
  ...
  //
  if ((ImageSize != RegionSize) || (ImageSize < PlatformGetEcRecoveryImageOffset ())) {
    DEBUG ((DEBUG_ERROR, "CheckImage - EC Image size (0x%x) is unexpected.\n", ImageSize));
    *ImageUpdatable = IMAGE_UPDATABLE_INVALID;
  }
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (EC) - Start\n"));
  ...
  if (!IsPreviousUpdateUnfinished (&PreviousProgress)) {
    ...
    ASSERT (PreviousProgress.Component == UpdatingEc);
  }
  ...
  //
  // The check for ImageSize is done in FmpDeviceCheckImage(),
  // so ImageSize here must be larger than or equal to EC recovery image offset.
  //
  if ((PreviousProgress.Component == NotStarted) || (PreviousProgress.Progress == EcMainImage)) {
    //
    ...
    AccessReq.Data = (UINT8 *) Image;
    AccessReq.DataSize = EcRecoveryImageOffset;
    Status = UpdateEcFirmware (&AccessReq, Progress, 5, 45);
  ...
  AccessReq.Data = (UINT8 *) ((UINTN) Image + EcRecoveryImageOffset);
  AccessReq.DataSize = (UINTN) (ImageSize - EcRecoveryImageOffset);
  Status = UpdateEcFirmware (&AccessReq, Progress, 45, 95);
...
CleanUp:
  ...
  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (EC) - %r\n", Status));
  if (EFI_ERROR (Status)) {
    *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
  }
  return Status;
...
{
  DEBUG ((DEBUG_INFO, "FmpDeviceLib(EC): FmpDeviceLock() for system FLASH\n"));
  return EFI_UNSUPPORTED;
}
