/** @file
  PlatformDisplayUpdateProgressLib instance to support displaying update progress
...
//
// Set to TRUE to avoid repeatedly locating gEfiGraphicsOutputProtocolGuid for enhancing performance.
//
BOOLEAN mCheckGopFlag = FALSE;

//
// Gop protocol notification event;
//
EFI_EVENT  CheckGopProtocolNotifyEvent = NULL;

/**
  Function uses alternative means to indicate progress in case GOP cannot be located.
  ...
  )
{
  ...
  PostCode (0xF000 | (UINT8)Completion);
}

/**
  OnGopInstalled installed notification event handler.
  ...
  )
{
  ...
  //
  // If gEfiGraphicsOutputProtocolGuid is found, get its instance and reset flag.
  //
  if (Interface != NULL) {
    DEBUG ((DEBUG_INFO, "OnGopInstalled:  gEfiGraphicsOutputProtocolGuid has been found.\n"));
    mGop = (EFI_GRAPHICS_OUTPUT_PROTOCOL *)Interface;
    mCheckGopFlag = FALSE;
  }
}

/**
  Function registers gEfiGraphicsOutputProtocolGuid notify event.
  ...
  )
{
  ...
  if (CheckGopProtocolNotifyEvent == NULL) {
    CheckGopProtocolNotifyEvent = EfiCreateProtocolNotifyEvent (
                                    &gEfiGraphicsOutputProtocolGuid,
                                    TPL_CALLBACK,
                                    OnGopInstalled,
                                    NULL,
  ...
  )
{
  ...
  if ((BootLogo == NULL) || (EFI_ERROR (Status))) {
    DEBUG ((DEBUG_ERROR, "Failed to locate gEdkiiBootLogo2ProtocolGuid.  No Progress bar support. Status = %r \n", Status));
    return;
  }
  ...
  )
{
  ...
  //
  if ((mGop == NULL) && mCheckGopFlag) {
    //
    // Call alternative handle function and let the update continue.
    //
    AltDisplayUpdateProgress (Completion);
    return EFI_SUCCESS;
  }
