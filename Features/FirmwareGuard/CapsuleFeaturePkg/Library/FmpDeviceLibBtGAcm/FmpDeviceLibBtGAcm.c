/** @file
  FmpDeviceLib instance to support BtG ACM update
  ...
  )
{
  ...
  UnicodeSPrint (
    Version,
    sizeof (Version),
    L"BootGuard S-ACM v%d.%d.%d",
    (UINT8) (TxtInfoHob->Data.AcmMajorVersion),
    (UINT8) (TxtInfoHob->Data.AcmMinorVersion),
    (UINT8) (TxtInfoHob->Data.AcmRevision)
    );
  ...
  )
{
  ...
  TxtInfoHob = (TXT_INFO_HOB *)GetFirstGuidHob (&gTxtInfoHobGuid);
  if (TxtInfoHob != NULL) {
    *Version = (UINT8) (TxtInfoHob->Data.AcmMajorVersion) << 16 |
               (UINT8) (TxtInfoHob->Data.AcmMinorVersion) <<  8 |
               (UINT8) (TxtInfoHob->Data.AcmRevision);
  } else {
    return EFI_UNSUPPORTED;
  }
  ...
  )
{
  ...
  *ImageUpdatable = IMAGE_UPDATABLE_VALID;
  ...
  //
  // Make sure the image has the inforamtion needed for updating BtGACM
  //
  if (!IsImageForBtGAcmUpdate ((VOID *) Image, ImageSize)) {
    *ImageUpdatable = IMAGE_UPDATABLE_INVALID;
  }
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (BtGAcm) - Start\n"));
  ...
  if (GetBiosResiliencyType () != NOT_SUPPORT_RECOVERY) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "NO ESP, BtGAcm update is not supported! \n"));
      *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
      return EFI_UNSUPPORTED;
    }
  }
  ...
  if (!IsPreviousUpdateUnfinished (&PreviousProgress)) {
    ...
    ASSERT (PreviousProgress.Component == UpdatingBtGAcm);
  }

  DEBUG ((DEBUG_INFO, "UpdateBtGACM - Start\n"));
  SetUpdateProgress (UpdatingBtGAcm, 0);
  Status = PerformBtGAcmUpdateByConfigData (
             (VOID *) Image,
             ImageSize,
  ...
  DEBUG ((DEBUG_INFO, "UpdateBtGACM - End (%r)\n", Status));
  if (EFI_ERROR (Status)) {
  ...
  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (BtGACM) - %r\n", Status));
  if (EFI_ERROR (Status)) {
    *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
  }
...
{
  DEBUG ((DEBUG_INFO, "FmpDeviceLib(BtGACM): FmpDeviceLock() for system FLASH\n"));
