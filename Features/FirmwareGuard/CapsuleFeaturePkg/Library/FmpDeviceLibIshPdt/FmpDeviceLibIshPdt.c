...
/**
  Check if current ME mode and status is capable of supporting ISH PDT update.

  @retval EFI_SUCCESS      ME mode and statue is capable of supporting ISH PDT update.
  @retval other            ME mode and statue is unable to support ISH PDT update.

**/
EFI_STATUS
CheckUpdateInterfaceReadiness (
  VOID
  )
{
  EFI_STATUS                  Status;
  ...
  )
{
  ...
  // So return EFI_UNSUPPORTED here.
  //
  return EFI_UNSUPPORTED;
}
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (IshPdt) - Start\n"));
  ...
  Status = CheckUpdateInterfaceReadiness ();
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Abort the operation since current interface does not support. %r.\n", Status));
    *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
    return EFI_ABORTED;
  }
  ...
  Status = UpdateIshPdt (&AccessReq, Progress, 5, 95);
  DEBUG ((DEBUG_INFO, "Update ISH PDT - End (%r)\n", Status));

  Progress (100);

  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (IshPdt) - %r\n", Status));
  if (EFI_ERROR (Status)) {
    *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
  }
