  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (BIOS) - Start\n"));
  ...
  if (GetBiosResiliencyType () == SUPPORT_BIOS_RESILIENCY_RECOVERY) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "NO ESP, BIOS update is not supported! \n"));
      *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
      return EFI_UNSUPPORTED;
    }
  }
  ...
  if (!IsPreviousUpdateUnfinished (&PreviousProgress)) {
    if (ExtractBiosObbFromImage ((VOID *) Image, ImageSize, &BiosObbImage, &BiosObbImageSize)) {
      ...
      DEBUG ((DEBUG_ERROR, "Fail to find new BIOS Obb image\n"));
      ASSERT (FALSE);
    }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "[%a]: Bios Update Fail!\n", __FUNCTION__));
  }
  ...
  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (BIOS) - %r\n", Status));
