  ...
  )
{
  if (GetBiosResiliencyType () == NOT_SUPPORT_RECOVERY) {
    DEBUG ((DEBUG_INFO, "%a: Resiliency is not supported. No need to install FMP at this moment.\n", __func__));
    return EFI_NOT_READY;
  }

  //
  // FMP instance does not use Driver Binding Protocol.
  //
...
{
  UINT32   *UcodePatchAddressPtr;
  BOOLEAN  IsGetUcodePatchRegion;

  UcodePatchAddressPtr  = NULL;
  IsGetUcodePatchRegion = FALSE;

  if (Size == NULL) {
    return EFI_INVALID_PARAMETER;
  }

  IsGetUcodePatchRegion = GetMicrocodeRegion ((VOID**) &UcodePatchAddressPtr, Size);
  if (!IsGetUcodePatchRegion) {
    return EFI_UNSUPPORTED;
  }
...
{
  //
  // NOTE: For uCode solution. Always set lowest version to 0
  // Real update version match will be controlled by Policy PCD
  //
  *LowestSupportedVersion = mMicrocodeFmpPrivate->VersionInfo->LowestSupportedVersion;
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  //
  // Total Memory Size is safety to set to Version FFS content size .
  //
  FmpVersionStr = mMicrocodeFmpPrivate->VersionInfo->VersionString;
  VerionLenSize = StrnLenS(FmpVersionStr, mMicrocodeFmpPrivate->VersionInfoSize);

  DEBUG((DEBUG_INFO, "FmpDeviceGetVersionString VersionStr %s\n", FmpVersionStr));
  DEBUG((DEBUG_INFO, "                          StrLen %x\n", VerionLenSize));

  *VersionString = (CHAR16 *)AllocateZeroPool ((VerionLenSize + 1) * sizeof(CHAR16));

  if (*VersionString == NULL) {
    DEBUG((DEBUG_ERROR, "Version String Allocation Error.\n"));
    return EFI_OUT_OF_RESOURCES;
  }

  StrnCpyS(*VersionString, sizeof (VersionString) / sizeof(CHAR16), FmpVersionStr, VerionLenSize);
  ...
  )
{
  ...
  // Successful read
  //
  *Version = mMicrocodeFmpPrivate->VersionInfo->Version;
  ...
  )
{
  ...
  MicrocodeFmpPrivate = mMicrocodeFmpPrivate;

  if (*ImageSize < MicrocodeFmpPrivate->MicrocodePatchRegionSize) {
    *ImageSize = MicrocodeFmpPrivate->MicrocodePatchRegionSize;
    return EFI_BUFFER_TOO_SMALL;
  }

  //
  // We report the entire uCode Range as one image
  //
  *ImageSize = MicrocodeFmpPrivate->MicrocodePatchRegionSize;
  CopyMem (Image, MicrocodeFmpPrivate->MicrocodePatchAddress, MicrocodeFmpPrivate->MicrocodePatchRegionSize);
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (uCode) - Start\n"));
  ...
  if (GetBiosResiliencyType () != NOT_SUPPORT_RECOVERY) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "NO ESP, ucode update is not supported! \n"));
      *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
      return EFI_UNSUPPORTED;
    }
  }
  ...
  *AbortReason     = NULL;
  ...
  if (!IsPreviousUpdateUnfinished (&PreviousProgress)) {
    //
    // Last uCode Update is good. This is the next uCode update requst.
    //  1) Save capsule image to recovery from interrupted update
    ...
    //
    // Seamless recovery from interrupted Update case.
    // First recovery corrupted region then start update
    //
    ASSERT (PreviousProgress.Component == UpdatinguCode);
  }

  SetUpdateProgress (UpdatinguCode, 0);
  Status = MicrocodeWrite(
             MicrocodeFmpPrivate,
             (VOID *)Image,
             ImageSize,
             Progress,
  ...
  if (!EFI_ERROR(Status)) {
    ...
    DEBUG ((DEBUG_INFO, "[%a]: Microcode Update Fail!\n", __FUNCTION__));
  }

  PlatformFmpUcodeUpdatePostHook (Status);
  ClearUpdateProgress ();
  DeleteBackupFiles ();
  if (GetBiosResiliencyType () != NOT_SUPPORT_RECOVERY) {
    if (Status == EFI_SUCCESS) {
      ...
      //
      SetUpdateProgress (UpdatingResiliency, 0);
    }
  ...
  DEBUG ((DEBUG_INFO, "FmpDeviceSetImageWithStatus (uCode) - %r\n", Status));
...
{
  DEBUG ((DEBUG_INFO, "FmpDeviceLib(uCode): FmpDeviceLock() for system FLASH\n"));
  ...
  )
{
  ...
  //
  // If Progress 1 Update failure. We use MMIO read to get uCode info from Backup
  // At this time, Backup contains LastKnownGood which is exactly the same as Primary before update
  // So all the info here we get is correct even if primary content corrupts
  //
  ...
  Status = InitializeMicrocodePrivateData(mMicrocodeFmpPrivate);
  if (EFI_ERROR(Status)) {
