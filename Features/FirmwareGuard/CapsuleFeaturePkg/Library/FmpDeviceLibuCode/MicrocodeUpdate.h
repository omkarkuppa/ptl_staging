...
#include <Guid/MicrocodeVersionFfs.h>
...
//  EFI_HANDLE                           Handle;
  ...
  INTEL_MICROCODE_VERSION_FFS_DATA     *VersionInfo;
  UINTN                                VersionInfoSize;
};

typedef struct _MICROCODE_FMP_PRIVATE_DATA  MICROCODE_FMP_PRIVATE_DATA;

//
// Records each entry inside uCode Payload
// Entries are described in XDR format. Not there are 4 indexes.
//
typedef enum {
  uCodeFullRangeIndex,
  uCodeFullRangeBgupIndex,
  uCodeVersionIndex,
  uCodeArrayIndex,
  uCodeArrayMax,
} UCODE_ENTRY_INDEX;
