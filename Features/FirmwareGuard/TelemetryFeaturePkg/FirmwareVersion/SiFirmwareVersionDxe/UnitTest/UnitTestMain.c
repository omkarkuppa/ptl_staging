/**@file
    Unit test case of the Si Firmware Version Dxe driver.
  @copyright
  INTEL CONFIDENTIAL
...
//
#include "TestCase/SiFirmwareVersionDxeTestCase.h"

#define UNIT_TEST_NAME     "SiFirmwareVersionDxe Unit Test"
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestSiFirmwareVersionDxe;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestSiFirmwareVersionDxe,  // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestSiFirmwareVersionDxe,             // SuiteHandle        : Unit test suite to add test to
