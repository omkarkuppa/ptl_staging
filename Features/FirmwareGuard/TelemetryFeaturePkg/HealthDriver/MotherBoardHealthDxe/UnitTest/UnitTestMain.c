...
//
#include "TestCase/MotherBoardHealthDxeTestCase.h"

#define UNIT_TEST_NAME     "MotherBoardHealthDxe Unit Test"
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestMotherBoardHealthDxe;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestMotherBoardHealthDxe,  // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestMotherBoardHealthDxe,             // SuiteHandle        : Unit test suite to add test to
