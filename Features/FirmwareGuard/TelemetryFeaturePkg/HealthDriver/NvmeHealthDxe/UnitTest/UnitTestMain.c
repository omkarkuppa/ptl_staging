/**@file
    Unit test case of the Nvme Health Dxe driver.
  @copyright
  INTEL CONFIDENTIAL
...
//
#include "TestCase/NvmeHealthDxeTestCase.h"

#define UNIT_TEST_NAME     "NvmeHealthDxe Unit Test"
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestNvmeHealthDxe;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestNvmeHealthDxe,  // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestNvmeHealthDxe,                    // SuiteHandle        : Unit test suite to add test to
