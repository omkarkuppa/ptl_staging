/**@file
    Unit test case of the Csme Health Dxe driver.
  @copyright
  INTEL CONFIDENTIAL
...
//
#include "TestCase/CsmeHealthDxeTestCase.h"

#define UNIT_TEST_NAME     "CsmeHealthDxe Unit Test"
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestCsmeHealthDxe;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestCsmeHealthDxe,  // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestCsmeHealthDxe,                    // SuiteHandle        : Unit test suite to add test to
