/**@file
    Unit test case of the MRC Health Dxe driver.
  @copyright
  INTEL CONFIDENTIAL
...
//
#include "TestCase/MrcHealthDxeTestCase.h"

#define UNIT_TEST_NAME     "MrcHealthDxe Unit Test"
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestMrcHealthDxe;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestMrcHealthDxe,   // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestMrcHealthDxe,                     // SuiteHandle        : Unit test suite to add test to
