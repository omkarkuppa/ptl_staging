/**@file
    Unit test main of the XmlCliPei driver.
...
//
#include "TestCase/XmlCliCommonPeiTestCase.h"

#define UNIT_TEST_NAME     "XmlCliCommonPei Unit Test"
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestXmlCliCommonPei;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestXmlCliCommonPei,  // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestXmlCliCommonPei,                  // SuiteHandle        : Unit test suite to add test to
