/**@file
    Unit test main of the XmlCliDxe driver.
...
//
#include "TestCase/XmlCliCommonDxeTestCase.h"

#define UNIT_TEST_NAME     "XmlCliCommonDxe Unit Test"
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestXmlCliCommonDxe;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestXmlCliCommonDxe,  // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestXmlCliCommonDxe,                  // SuiteHandle        : Unit test suite to add test to
