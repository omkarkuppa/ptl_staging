/** @file
  ACPI methods of dTBT PCIE port for PCIE tunneling of CIO port 1
...
If (LAnd (CondRefOf (DTBT_US_PORT.DTID), CondRefOf (\DTRT))) {
  ...
  Method (_S0W, 0, Serialized) {
    If (LEqual (\DTRT, 0)) {
      ADBG (Concatenate("[dTBT] DS PCIE 1 _S0W returns 3 - dTBT ", ToDecimalString (DTBT_US_PORT.DTID)))
      Return (3)
    } Else {
      ADBG (Concatenate("[dTBT] DS PCIE 1 _S0W returns 4 - dTBT ", ToDecimalString (DTBT_US_PORT.DTID)))
      Return (4)
    }
  }

  Method (_PS0,0,Serialized) {
    ADBG (Concatenate ("[dTBT] DS PCIE 1 _PS0 - dTBT ", ToDecimalString (DTBT_US_PORT.DTID)))
  }

  Method (_PS3,0,Serialized) {
    ADBG (Concatenate ("[dTBT] DS PCIE 1 _PS3 - dTBT ", ToDecimalString (DTBT_US_PORT.DTID)))
  }
  ...
  If (LLessEqual (DTBT_US_PORT.DTID, MAX_TBT_DTID)) {
    Method (_DSD, 0) {
      ADBG (Concatenate ("[dTBT] DS PCIE 1 _DSD - dTBT ", ToDecimalString (DTBT_US_PORT.DTID)))
  ...
  ADBG ("[dTBT] DTBT_US_PORT.DTID or DTRT NVS not present in DS PCIE 1")
}
