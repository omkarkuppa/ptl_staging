/** @file
  FmpDeviceLib instance to support Discrete Thunderbolt Firmware update
  ...
  )
{
  ...
  //
  // @ToDo: Enable GetVersion after ensuring manipulation of multiple Discrete TBT firmwares is working
  ...
  )
{
  ...
  for (Index = 0; Index < DiscreteTbtPayloadHeader->PayloadCount; Index++, DiscreteTbtPayloadItem++) {
        ...
        *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_TBT_RETIMER_ERROR_USB2HC_PROTOCOL_NOT_FOUND;
        ...
        if (DeviceHandleBuffer != NULL) {
          for (HandleIndex = 0; HandleIndex < DeviceHandleCount; HandleIndex++) {
            ...
            Status = gBS->HandleProtocol (DeviceHandleBuffer[HandleIndex], &gEfiPciIoProtocolGuid, (VOID**) &PciIopProtocol);
            if (EFI_ERROR (Status)) {
              *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_TBT_RETIMER_ERROR_DEVICE_HANDLE_NOT_FOUND;
              gBS->FreePool (DeviceHandleBuffer);
              return Status;
            }
            PciIopProtocol->GetLocation (
                              PciIopProtocol,
                              &SegmentNumber,
                              &BusNumber,
                              &DeviceNumber,
    ...
    Status = UpdateDiscreteTbtNvmFirmware (
               DiscreteTbtDevice,
               (UINT8 *) Image + DiscreteTbtPayloadItem->ImageOffset,
               DiscreteTbtPayloadItem->ImageSize,
               Progress,
               6 + (90 / DiscreteTbtPayloadHeader->PayloadCount) * Index,
               6 + (90 / DiscreteTbtPayloadHeader->PayloadCount) * (Index + 1)
               );

    //
    // Destroy Discrete TBT device when we are done with it.
    //
