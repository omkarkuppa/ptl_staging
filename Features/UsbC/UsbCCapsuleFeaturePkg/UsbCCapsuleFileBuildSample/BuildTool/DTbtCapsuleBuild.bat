@REM @file
@REM Discrete Thunderbolt Controller Capsule build script.
@REM
@REM  @copyright
...
@REM Usage:
@REM DTbtCapsuleBuild.bat [Work Space Path] [Discrete Thunderbolt version]
@REM   Work Space Path               : The work space should be have edk2 module.
@REM   Discrete Thunderbolt version  : Set Discrete Thunderbolt version.
@REM
...
@if "%2" == "" (
  @echo ERROR!! Discrete Thunderbolt version did not assigned via argument.
  goto :WrongParametersStop
)
...
@if not defined DTBT_PAYLOAD_CONFIG_FILE (
...
@REM
@if not exist %DTBT_PAYLOAD_CONFIG_FILE% (
  @echo.
  @echo ERROR!! Discrete Thunderbolt payload INI file does not exist.
  @goto CapsuleNotBuilt
)

@del %DTBT_CAPSULE_PAYLOAD% 1>NUL 2>&1
@%PYTHON_COMMAND%  %USBC_CAPSULE_GEN_TOOLS_PATH%\GenerateCapsule\GenDiscreteTbtPayloadHeader.py %DTBT_PAYLOAD_CONFIG_FILE%
@if errorlevel 1 (
  @echo.
  @echo ERROR!! Failed to generate Discrete Thunderbolt Capsule payload from %DTBT_PAYLOAD_CONFIG_FILE%.
  @goto CapsuleNotBuilt
)

@if not exist %DTBT_CAPSULE_PAYLOAD% (
  @echo.
  @echo ERROR!! Discrete Thunderbolt binary does not exist.
  @goto CapsuleNotBuilt
)

@REM ===================================================
@REM =        Build DTBT Controller Capsule            =
@REM ===================================================
@REM Generate Discrete Thunderbolt FMP Capsule
@cd %USBC_CAPSULE_GEN_TOOLS_PATH%\GenerateCapsule
@call GenCapsuleDiscreteTbt.bat ^
        %DTBT_CAPSULE_PAYLOAD% ^
        %DTBT_VERSION_DEC% ^
...
@if errorlevel 1 (
  @echo.
  @echo !! Error when generating DTBT Controller capsule
  goto CapsuleNotBuilt
)

@REM Write to build result
set USBC_CAPSULE_OUTPUT_NAME=%CAP_PLATFORM_PREFIX%_DTbt_%DTBT_VERSION_DEC%.cap
if exist %USBC_CAPSULE_OUTPUT_PATH%\%USBC_CAPSULE_OUTPUT_NAME% (
  @echo Discrete Thunderbolt Capsule %USBC_CAPSULE_OUTPUT_NAME% is generated in >> %USBC_CAPSULE_BUILD_RESULT%
