@REM @file
@REM Discrete Thunderbolt Retimer Capsule build script.
@REM
@REM  @copyright
...
@REM Usage:
@REM DTbtRetimerCapsuleBuild.bat [Work Space Path] [DTBT Retimer Version]
@REM   Work Space Path       : The work space should be have edk2 module.
@REM   DTBT Retimer version  : Set Discrete Thunderbolt Retimer version.
@REM
...
set DTBT_RETIMER=DTbtRetimer
@if not defined DTBT_RETIMER_PAYLOAD_CONFIG_FILE (
...
@REM
@if not exist %DTBT_RETIMER_PAYLOAD_CONFIG_FILE% (
  @echo.
  @echo ERROR!! DTBT Retimer payload config file does not exist.
  @goto CapsuleNotBuilt
)

@del %DTBT_RETIMER_CAPSULE_PAYLOAD% 1>NUL 2>&1
@%PYTHON_COMMAND% %USBC_CAPSULE_GEN_TOOLS_PATH%\GenerateCapsule\GenRetimerPayloadHeader.py %DTBT_RETIMER_PAYLOAD_CONFIG_FILE%
@if errorlevel 1 (
  @echo.
  @echo ERROR!! Failed to generate DTBT Retimer Capsule payload from %DTBT_RETIMER_PAYLOAD_CONFIG_FILE%.
  @goto CapsuleNotBuilt
)

@if not exist %DTBT_RETIMER_CAPSULE_PAYLOAD% (
  @echo.
  @echo ERROR!! DTBT Retimer binary does not exist.
  @goto CapsuleNotBuilt
)

@REM ===================================================
@REM =            Build DTBT Retimer Capsule           =
@REM ===================================================
@REM Generate Retimer FMP Capsule
@cd %USBC_CAPSULE_GEN_TOOLS_PATH%\GenerateCapsule
@call GenCapsuleTbtRetimer.bat ^
        %DTBT_RETIMER_CAPSULE_PAYLOAD% ^
        %RETIMER_VERSION_DEC% ^
...
@if errorlevel 1 (
  @echo.
  @echo !! Error when generating DTBT Retimer capsule
  goto CapsuleNotBuilt
)

@REM Write to build result
set USBC_CAPSULE_OUTPUT_NAME=%CAP_PLATFORM_PREFIX%_DTbtRetimer_%RETIMER_VERSION_DEC%.cap
