...
#include "NvmExpressBlockIoCrypto.h"
...
#define CRYPTO_MEDIA_16B_ALIGN      0x10                // 16 byte alignment
#define CRYPTO_IV_INPUT_MAX_VALUE   (BIT44 - 1)         // Max value value stored in 44 bits.
...
struct _NVME_CONTROLLER_PRIVATE_DATA {
  ...
  EFI_NVM_EXPRESS_PASS_THRU_PROTOCOL    CryptoPassthru;
...
struct _NVME_DEVICE_PRIVATE_DATA {
  ...
  EFI_BLOCK_IO_MEDIA                       CryptoMedia;
...
#define NVME_DEVICE_PRIVATE_DATA_FROM_BLOCK_IO_CRYPTO(a)  \
  CR (a,                                            \
      NVME_DEVICE_PRIVATE_DATA,                     \
      BlockIoCrypto,                                \
      NVME_DEVICE_PRIVATE_DATA_SIGNATURE            \
