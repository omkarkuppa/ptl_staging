  ...
  )
{
  ...
  *PrpListNo = (UINTN)DivU64x64Remainder ((UINT64)Pages, (UINT64)PrpEntryNo - 1, &Remainder);
  ...
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] PrpEntryNo: 0x%x\n", PrpEntryNo));
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] PrpListNo:  0x%x\n", *PrpListNo));
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] Remainder:  0x%x\n", Remainder));
  ...
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru]  PrpListHost [0x%016llx]: 0x%016llx\n", PrpListHost, *PrpListHost));
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru]  PrpListPhyAddr [0x%016llx]\n", PrpListPhyAddr));

  DEBUG ((DEBUG_VERBOSE, "NvmeCreatePrpList: Filling all PRP lists except last one...\n"));
  for (PrpListIndex = 0; PrpListIndex < *PrpListNo - 1; ++PrpListIndex) {
    DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru]  PrpListIndex [0x%x]\n", PrpListIndex));
    PrpListBase = *(UINT64 *)PrpListHost + PrpListIndex * EFI_PAGE_SIZE;
    DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru]  PrpListBase: 0x%016llx\n", PrpListBase));
    ...
    for (PrpEntryIndex = 0; PrpEntryIndex < PrpEntryNo; ++PrpEntryIndex) {
      if (PrpEntryIndex != PrpEntryNo - 1) {
        ...
        *((UINT64 *)(UINTN)PrpListBase + PrpEntryIndex) = PhysicalAddr;
        DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru]  PrpEntryIndex [0x%x]: 0x%016llx\n", PrpEntryIndex, PhysicalAddr));
        ...
        *((UINT64 *)(UINTN)PrpListBase + PrpEntryIndex) = PrpListPhyAddr + (PrpListIndex + 1) * EFI_PAGE_SIZE;
        DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru]  New PrpList [0x%x]: 0x%016llx\n", PrpEntryIndex, PrpListPhyAddr + (PrpListIndex + 1) * EFI_PAGE_SIZE));
      }
    }
  }
  ...
  // Fill last PRP list.
  //
  DEBUG ((DEBUG_VERBOSE, "NvmeCreatePrpList: Filling last PRP list...\n"));
  PrpListBase = *(UINT64 *)PrpListHost + PrpListIndex * EFI_PAGE_SIZE;
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru]  PrpListBase: 0x%016llx\n", PrpListBase));
  for (PrpEntryIndex = 0; PrpEntryIndex < Remainder; ++PrpEntryIndex) {
    DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru]  PrpEntryIndex [0x%x] @ 0x%016llx: 0x%016llx\n", PrpEntryIndex, ((UINT64 *)(UINTN)PrpListBase + PrpEntryIndex), PhysicalAddr));
    *((UINT64 *)(UINTN)PrpListBase + PrpEntryIndex) = PhysicalAddr;
    PhysicalAddr                                   += EFI_PAGE_SIZE;
  }

  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] Returning PrpListPhyAddr  [0x%x]\n", PrpListPhyAddr));
  ...
  )
{
  ...
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] IoAlign   [0x%x]\n", IoAlign));
      ...
      ((Sq->Opc == NVME_ADMIN_CRIOCQ_CMD) || (Sq->Opc == NVME_ADMIN_CRIOSQ_CMD)))
  {
    ...
    if ((Sq->Opc & BIT0) != 0) {
      DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] EfiPciIoOperationBusMasterRead\n"));
      Flag = EfiPciIoOperationBusMasterRead;
    } else {
      DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] EfiPciIoOperationBusMasterWrite\n"));
      Flag = EfiPciIoOperationBusMasterWrite;
    }

    if ((Packet->TransferLength != 0) && (Packet->TransferBuffer != NULL)) {
      DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] Packet->TransferBuffer[0]    [0x%x]\n", Packet->TransferBuffer));
      DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] Packet->TransferLength[0]    [0x%x]\n", Packet->TransferLength));
  ...
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] Prp[0]:    0x%016llx\n", Sq->Prp[0]));
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] PrpOffset: 0x%x\n", Offset));
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] PrpBytes:  0x%x\n", Bytes));
  ...
  if ((Offset + Bytes) > (EFI_PAGE_SIZE * 2)) {
    DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] (Offset + Bytes) > (EFI_PAGE_SIZE * 2)\n"));
    //
    // Create PrpList for remaining data buffer.
    //
    PhyAddr = (Sq->Prp[0] + EFI_PAGE_SIZE) & ~(EFI_PAGE_SIZE - 1);
    DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] EFI_SIZE_TO_PAGES (Offset + Bytes) - 1: 0x%x\n", EFI_SIZE_TO_PAGES (Offset + Bytes) - 1));
    DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] PhyAddr    [0x%x]\n", PhyAddr));
    ...
    DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] (Offset + Bytes) > EFI_PAGE_SIZE\n"));
  }

  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] PrpListHost   [0x%016llx]\n", PrpListHost));
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] Prp[1]        [0x%016llx]\n", Sq->Prp[1]));
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] PrpListNo:    0x%x\n", PrpListNo));
...
/**
  ...
  Unlike the NvmExpressPassThru, the NvmExpressCryptoPassThru will submit the PRPs through the TSE protocol
  SubmitTseTableEntries () function in order to write the respective entries into the TSE DRAM table prior to
  sending the NVM Express Command Packet to an NVM Express controller.
  ...
  @retval EFI_UNSUPPORTED            The command described by the NVM Express Command Packet is not supported by the NVM Express
                                     controller. The NVM Express Command Packet was not sent so no additional status information
                                     is available.
  @retval EFI_UNSUPPORTED            The TSE protocol was not installed for this controller.
  @retval EFI_TIMEOUT                A timeout occurred while waiting for the NVM Express Command Packet to execute.

**/
EFI_STATUS
EFIAPI
NvmExpressCryptoPassThru (
  IN     EFI_NVM_EXPRESS_PASS_THRU_PROTOCOL        *This,
  IN     UINT32                                    NamespaceId,
  IN OUT EFI_NVM_EXPRESS_PASS_THRU_COMMAND_PACKET  *Packet,
  IN     EFI_EVENT                                 Event OPTIONAL
  )
{
  ...
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassThru] %a()\n", __FUNCTION__));
  ...
  //
  // BlockIoCrypto only supports IO operations. Admin packets are not supported.
  //
  ...
  //
  // Buffer alignment check for TransferBuffer
  //
  IoAlign = This->Mode->IoAlign;
  DEBUG ((DEBUG_VERBOSE, "[TseNvmExpressPassthru] IoAlign   [0x%x]\n", IoAlign));
  if ((IoAlign > 0) && (((UINTN)Packet->TransferBuffer & (IoAlign - 1)) != 0)) {
  ...
  if (Event == NULL) {
    ...
    //
    // Async Crypto IO is not supported
    //
    return EFI_UNSUPPORTED;
  }
  ...
  if ((Offset + Bytes) > (EFI_PAGE_SIZE * 2)) {
    ...
    DEBUG ((DEBUG_VERBOSE, "b) [TseNvmExpressPassthru] (Offset + Bytes) > EFI_PAGE_SIZE\n"));
    Sq->Prp[1] = (Sq->Prp[0] + EFI_PAGE_SIZE) & ~(EFI_PAGE_SIZE - 1);
  }
  ...
  //
  // Process PRP through TSE protocol if Prp[0] is not empty
  //
  if (Sq->Prp[0] != 0) {
    //
    // Reconstruct 64-bit LBA from NVMe packet
    //
    Lba = (Packet->NvmeCmd->Cdw10 | LShiftU64 (Packet->NvmeCmd->Cdw11, 32));
    Status = Private->TseProtocol->SubmitTseTableEntries (Sq->Prp, Lba, Bytes);

    //
    // Abort NVME command if TSE operation failed.
    //
    if (EFI_ERROR (Status)) {
      goto EXIT;
    }
