  ...
  )
{
  ...
  UINT8                      Mn[41];
  VOID                       *DummyInterface;
  UINTN                      PciControllerSegment;
  UINTN                      PciControllerBusNumber;
  UINTN                      PciControllerDeviceNumber;
  UINTN                      PciControllerFunctionNumber;

  TSE_PROTOCOL               *TseProtocol;
  BOOLEAN                    TseEnabledAndSupported;
  ...
  TseProtocol             = NULL;
  TseEnabledAndSupported  = FALSE;

  PciControllerSegment = 0;
  PciControllerBusNumber = 0;
  PciControllerDeviceNumber = 0;
  PciControllerFunctionNumber = 0;
  ...
  if (NamespaceData->Ncap == 0) {
    ...
    DEBUG ((DEBUG_INFO, "[TseNvmExpress] Media.IoAlign: 0x%x\n", Device->Media.IoAlign));

    Status = gBS->LocateProtocol (&gTseProtocolGuid, NULL, (VOID **) &TseProtocol);

    //
    // Check if TSE Protocol has been installed
    ...
    if (TseEnabledAndSupported == TRUE) {
      //
      // Clone Media structure contents into CryptoMedia.
      //
      CopyMem (&(Device->CryptoMedia), &(Device->Media), sizeof(EFI_BLOCK_IO_MEDIA));
      Device->CryptoMedia.IoAlign = CRYPTO_MEDIA_16B_ALIGN;
      DEBUG ((DEBUG_INFO, "[TseNvmExpress] CryptoMedia.IoAlign: 0x%x\n", Device->CryptoMedia.IoAlign));

      //
      // Store TSE protocol handle into controller's private data.
      //
      Private->TseProtocol = TseProtocol;

      //
      // Program NVMe controller's BDF into IOC
      //
      if (Device->Controller->PciIo != NULL) {
        Device->Controller->PciIo->GetLocation (
                                     Device->Controller->PciIo,
                                     &PciControllerSegment,
                                     &PciControllerBusNumber,
                                     &PciControllerDeviceNumber,
                                     &PciControllerFunctionNumber
                                     );

        DEBUG ((
          DEBUG_INFO,
          "Registering controller with B:0x%x, D:0x%x, F:0x%x\n",
          PciControllerBusNumber,
          PciControllerDeviceNumber,
          PciControllerFunctionNumber
          ));

        Status = TseProtocol->SetBDF (
                                (UINT8) PciControllerBusNumber,
                                (UINT8) PciControllerDeviceNumber,
                                (UINT8) PciControllerFunctionNumber
                                );

        if ((Status == EFI_SUCCESS) || (Status == EFI_ALREADY_STARTED)) {
          //
          // Create BlockIoCrypto Protocol instance
          //
          Device->BlockIoCrypto.Media            = &Device->Media;
          Device->BlockIoCrypto.Reset            = NvmeBlockIoCryptoReset;
          Device->BlockIoCrypto.GetCapabilities  = NvmeBlockIoCryptoGetCapabilities;
          Device->BlockIoCrypto.SetConfiguration = NvmeBlockIoCryptoSetConfiguration;
          Device->BlockIoCrypto.GetConfiguration = NvmeBlockIoCryptoGetConfiguration;
          Device->BlockIoCrypto.ReadExtended     = NvmeBlockIoCryptoReadBlocksEx;
          Device->BlockIoCrypto.WriteExtended    = NvmeBlockIoCryptoWriteBlocksEx;
          Device->BlockIoCrypto.FlushBlocks      = NvmeBlockIoCryptoFlushBlocks;
    ...
    if (!TseEnabledAndSupported) {
      DEBUG ((DEBUG_INFO, "TSE protocol not found. Skipping BlockIoCrypto Protocol installation...\n"));
      ...
      DEBUG ((DEBUG_INFO, "TSE protocol found. Installing BlockIoCrypto Protocol...\n"));
      Status = gBS->InstallMultipleProtocolInterfaces (
                      &Device->DeviceHandle,
                      &gEfiDevicePathProtocolGuid,
                      Device->DevicePath,
  ...
  )
{
  ...
  // If TSE Protocol was installed, it also needs to be uninstalled.
  //
  Status = gBS->LocateProtocol (&gTseProtocolGuid, NULL, (VOID **) &TseProtocol);
  ...
  )
{
  ...
  if (Status != EFI_ALREADY_STARTED) {
    ...
    Private->CryptoPassthru.Mode             = &Private->PassThruMode;
    Private->CryptoPassthru.PassThru         = NvmExpressCryptoPassThru;
    Private->CryptoPassthru.GetNextNamespace = NvmExpressGetNextNamespace;
    Private->CryptoPassthru.BuildDevicePath  = NvmExpressBuildDevicePath;
    Private->CryptoPassthru.GetNamespace     = NvmExpressGetNamespace;
