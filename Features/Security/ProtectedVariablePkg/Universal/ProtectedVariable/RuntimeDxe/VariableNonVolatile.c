  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "%a: %d NumVars = 0x%X\n",
              __FUNCTION__, __LINE__, NumVars));

  for (CurrVar = 0; CurrVar < NumVars; CurrVar++) {
    VarInfo.Buffer     = NULL;
    DEBUG ((DEBUG_INFO, "%a: %d VarList = 0x%x\n",
          __FUNCTION__, __LINE__, VarList[CurrVar]));
    VarInfo.StoreIndex = VarList[CurrVar];
    if (VarInfo.StoreIndex == VAR_INDEX_INVALID) {
      DEBUG ((DEBUG_INFO, "%a: %d Finished processing all valid store indexes. Name = %s, StorIndex = %d.\n",
              __FUNCTION__, __LINE__, VarInfo.Header.VariableName, VarInfo.StoreIndex));
      break;
    }

    DEBUG ((DEBUG_INFO, "%a: %d VariableName = %s\n",
          __FUNCTION__, __LINE__, VarInfo.Header.VariableName));
  ...
  // Search variable in the order of StoreIndex
  //
  DEBUG ((DEBUG_INFO, "%a: %d CurrentVar = %d, NumVar = %d.\n",
              __FUNCTION__, __LINE__, CurrVar, NumVars));
  ZeroMem (&VarInfo, sizeof (VarInfo));
  for ( ; CurrVar < NumVars; CurrVar++) {
    VarInfo.Buffer     = NULL;
    VarInfo.StoreIndex = VarList[CurrVar];
    Status             = ProtectedVariableLibFind (&VarInfo);
    DEBUG ((DEBUG_INFO, "%a: %d ProtectedVariableLibFindNextEx() Status = %r\n",
