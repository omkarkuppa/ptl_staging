  ...
  )
{
  ...
  //DEBUG ((DEBUG_INFO, "%a: %d CacheVariable: 0x%x, CacheVariable->StartPtr: 0x%x, CacheVariable->EndPtr: 0x%x\n",
  //        __FUNCTION__, __LINE__, CacheVariable, CacheVariable->StartPtr, CacheVariable->EndPtr));
  ...
  if ((CacheVariable->CurrPtr == NULL) || CacheVariable->Volatile) {
    ...
    //DEBUG ((DEBUG_INFO, "%a: %d VariableName: %s, CacheVariable->CurrPtr: 0x%x, CacheVariable->Volatile: 0x%x\n",
    //      __FUNCTION__, __LINE__, VariableName, CacheVariable->CurrPtr, CacheVariable->Volatile));
  } else {
    //
    ...
    //DEBUG ((DEBUG_INFO, "%a: %d VariableName: %s, Variable: 0x%x, VendorGuid: %g\n",
    //      __FUNCTION__, __LINE__, VariableName, Variable, VendorGuid));
    ASSERT_EFI_ERROR (Status);
  }
  ...
  //
  // Is there enough space to store the variable?
  //
  ...
  if (Status == EFI_OUT_OF_RESOURCES) {
    ...
    //
    // there are memory holes therefore,
    // trigger reclaim
    //
    Status = Reclaim (
               VarStoreBase,
    ...
    //
    // If this an existing variable, find the variable and update 'CacheVariable'.
    // Then rebase 'Variable' using 'CacheVariable'.
    //
    if (CacheVariable->CurrPtr != NULL) {
      ...
      if ((CacheVariable->CurrPtr == NULL) || EFI_ERROR (Status)) {
        DEBUG ((DEBUG_INFO, "%a: %d Error - could not find variable after reclaim.\n",
          __FUNCTION__, __LINE__));
        goto Done;
      }

      //
      // Need to rebase the Variable
      //
      Status = RebaseVariablePtr (
                CacheVariable,
                Variable,
                (VARIABLE_STORE_HEADER *)VarStoreBase,
                VariableName,
                VendorGuid,
                TRUE
                );
  ...
  //
  // The 'LastVariableOffset' is updated above in call to 'WriteVariable'
  // for both volatile and non-volatile variables.
  // For non-volatile variables, need to also update the
  // global common variable sizes below.
  //
  if (!VolatileFlag) {
    if ((Attributes & EFI_VARIABLE_HARDWARE_ERROR_RECORD) != 0) {
