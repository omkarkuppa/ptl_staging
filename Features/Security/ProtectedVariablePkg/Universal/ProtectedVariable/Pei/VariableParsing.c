  ...
  )
{
  ...
  if (VariableInfo->Flags.Auth) {
    ...
    VariableInfo->State                 = AuthVariableHeader->State;
    ...
    VariableInfo->State                 = VariableHeader->State;
  ...
  if (  (VariableInfo->Header.VariableName != NULL)
     && (VariableInfo->NameSize >= NameSize))
  {
    GetVariableNameOrData (
      StoreInfo,
      (UINT8 *)GetVariableNamePtr (VariablePtr, VariableInfo->Flags.Auth),
  ...
  )
{
  ...
  //
  // Note that variable might be in inconsecutive space. Always get a copy
