...
/**
  ...
  Derive 256-bit HMAC key from given variable root key.
  The function requires the caller to allocate a 256-bit buffer and size
...
{
  EFI_STATUS   Status;
  UINT8        SaltBuffer[METADATA_HMAC_SIZE];

  //
  // Generate Salt
  //
  Status = ProtectedVariableGenerate256Salt (
             (UINT8 *) PcdGetPtr (PcdMetaDataHmac256KeySeed),
             PcdGetSize (PcdMetaDataHmac256KeySeed),
             SaltBuffer,
             sizeof(SaltBuffer)
             );
  ...
  DEBUG ((DEBUG_INFO, "%a(): Salt Value: ", __FUNCTION__));
  for (UINT8 Index = 0; Index < sizeof(SaltBuffer); Index++) {
    DEBUG ((DEBUG_INFO, "%02x", SaltBuffer[Index]));
  }
  DEBUG ((DEBUG_INFO, "\n"));
  ...
  )
{
  ...
  BufferSize[1] = (UINT32)VarInfo->NameSize;
  ...
  if (!EFI_ERROR (Status)) {
    if (CompareMem (VAR_DIG_VALUE (VarDig), NewDigest, VarDig->DigestSize) != 0) {
      if (Global->Flags.RecoveryMode) {
        //
        // In recovery mode, this means the variable is changed (via patch table)
        // but the digest has not been updated. Update variable digest with new value
        // and recalculate the Meta Data HMAC value.
        //
        CopyMem (VAR_DIG_VALUE (VarDig), NewDigest, VarDig->DigestSize);
  ...
  )
{
  ...
  Status = ProtectedVariableRequestMonotonicCounter1 (&Counter);
  if (EFI_ERROR (Status)) {
  ...
  if (Global->Unprotected[IndexHmacAdded] != VAR_INDEX_INVALID) {
    ...
    CurrHmacVarInfo.NameSize            = HmacVarDig->NameSize;
    ...
    if (CurrHmacVarInfo.StoreIndex != VAR_INDEX_INVALID) {
       ...
       && (VarInfo->StoreIndex != VAR_INDEX_INVALID))
    {
      if (VarInfo->StoreIndex == VAR_DIG_PTR (Global->Unprotected[Index])->StoreIndex) {
        break;
                 ...
                 ) && (VarInfo->State == mUnprotectedVariables[Index].State))
    {
      break;
    }
  }
...
/**
  Determines Variable is to be encrypted on not based
  upon a pre-existing variable list.

  @param[in]  VariableName      Name of Variable to be checked for encryption.

  @retval TRUE      Variable is to be encrypted.
  @return FALSE     Variable is not to encrypted.

**/
BOOLEAN
IsVariableToBeEncrypted (
   IN CHAR16 *VariableName
  )
{
  UINTN    Index;
  CHAR16   *PcdStrBuffer;
  UINTN    PcdStrBufferSize;
  UINTN    PcdStrBufferCount;

  PcdStrBuffer      = (CHAR16 *) PcdGetPtr (PcdPlatformVariableName);
  PcdStrBufferSize  = PcdGetSize (PcdPlatformVariableName);
  PcdStrBufferCount = PcdGet32 (PcdPlatformVariableCount);

  //
  // Encrypt all variables if there is no pre-existing
  // variables to be encrypted list
  //
  if ((PcdStrBufferCount == 0) || (PcdStrBufferSize == 0)) {
    return TRUE;
  }

  //
  // Otherwise find a match within the pre-existing variable list
  //
  for(Index = 0; Index < PcdStrBufferCount; Index++) {
    //
    // check to see if variable is to be encrypted
    //
    if (StrnCmp (PcdStrBuffer, VariableName, StrLen (PcdStrBuffer)) == 0) {
      DEBUG ((DEBUG_INFO, "%a(): %s is to be encrypted\n", __FUNCTION__, VariableName));
      return TRUE;
    }

    //
    // advance to next variable in the list
    //
    PcdStrBuffer = PcdStrBuffer + StrLen(PcdStrBuffer) + 1;
  }

  DEBUG ((DEBUG_INFO, "%a(): %s is NOT to be encrypted\n", __FUNCTION__, VariableName));
  return FALSE;
}
  ...
  )
{
  ...
  if (IsVariableToBeEncrypted(VariableName) == FALSE) {
  ...
  )
{
  ...
  // Sync RPMC1 & RPMC2.
  //
  Status = ProtectedVariableRequestMonotonicCounter0 (&Counter0);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    return Status;
  }

  Status = ProtectedVariableRequestMonotonicCounter1 (&Counter1);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    return Status;
  }
  ...
  while (Counter0 < Counter1) {
    Status = ProtectedVariableIncrementMonotonicCounter0 ();
    if (EFI_ERROR (Status)) {
      ASSERT_EFI_ERROR (Status);
      return Status;
    }
  ...
  while (Counter1 < Counter0) {
    Status = ProtectedVariableIncrementMonotonicCounter1 ();
    if (EFI_ERROR (Status)) {
      ASSERT_EFI_ERROR (Status);
      return Status;
    }
  ...
  if ((VarInfo->PlainData == NULL) && (VarInfo->CipherData != NULL)) {
    ...
    switch (ContextIn->VariableServiceUser) {
      case FromPeiModule:
        ...
        Status = DecryptProtectedVariable (VarInfo);
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    //
    // Global is not found therefore
    // integrity check has not occured.
    // Let's discover variable store presence
    //
    Status = DiscoverVariableStore ();
    if(EFI_ERROR(Status)) {
      return EFI_NOT_FOUND;
    }

    //
    // perform integrity check since
    // Hob Variable Store is available
    //
    Status = PerformVariableIntegrityCheck ();

    //
    // Global should be created now so re-init
    // in order to process original GetVariable
    // request
    //
    Status = GetProtectedVariableGlobal (&Global);
    if (EFI_ERROR (Status)) {
  ...
  // will use this struct field internally.
  //
  ...
  //
  // Copy plain data to output data buffer
  //
  ...
  )
{
  ...
  if (VarInfo.Flags.Auth == TRUE) {
    ...
    VarInfo.NameSize            = AuthVariable->NameSize;
    ...
    VarInfo.NameSize            = Variable->NameSize;
  ...
  Buffer              = GET_BUFR (GET_ADRS (Buffer) + VarInfo.NameSize);
  Buffer              = GET_BUFR (GET_ADRS (Buffer) + GET_PAD_SIZE (VarInfo.NameSize));
  ...
  )
{
  ...
  //
  // integrity check had completed
  // proceed with get variable call
  //
  ContextIn = GET_CNTX (Global);
  ...
  VarInfo.NameSize            = StrSize (VariableName);
  ...
  if (VarDig->CacheIndex != VAR_INDEX_INVALID) {
    ...
    VarInfo.NameSize            = VarDig->NameSize;
  ...
  )
{
  ...
  VarInfo->NameSize            = Found->NameSize;
