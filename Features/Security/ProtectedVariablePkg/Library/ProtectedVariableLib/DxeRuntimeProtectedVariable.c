...
/**
  Lock "MetaDataHmacVar" to read-only for SetVariable() operation.
  ...
  )
{
  ...
  //
  // Make "MetaDataHmacVar" variable read-only for SetVariable() operation.
  //
  Status = gBS->LocateProtocol (&gEdkiiVariablePolicyProtocolGuid, NULL, (VOID**)&VariablePolicy);
  DEBUG ((DEBUG_INFO, "%a(): Locate Variable Policy protocol %g - %r\n", __FUNCTION__, gEdkiiVariablePolicyProtocolGuid, Status));
  if (!EFI_ERROR (Status)) {
    ...
    if(!EFI_ERROR(Status)){
      DEBUG ((DEBUG_INFO, "%a(): Lock %s variable (%r)\n", __FUNCTION__, METADATA_HMAC_VARIABLE_NAME, Status));
...
{
  VOID    *VariablePolicyProtocolRegistration;
  ...
  //
  // Register the event when Variable Policy Protocol is ready
  //
  VariablePolicyProtocolRegistration = NULL;
  EfiCreateProtocolNotifyEvent (
    &gEdkiiVariablePolicyProtocolGuid,
    TPL_CALLBACK,
    VariablePolicyProtocolReady,
    NULL,
    &VariablePolicyProtocolRegistration
    );
