...
/**
  Detect variable store presence

  @retval EFI_SUCCESS             The variable store is present.
  @retval EFI_NOT_FOUND           The variable store is not present.
  @retval EFI_OUT_OF_RESOURCES    Unable to allocate memory.

**/
EFI_STATUS
EFIAPI
DiscoverVariableStore (
  VOID
  )
{
  EFI_STATUS                     Status;
  ...
  //
  // Perform a read of first variable to detect
  // HOB or NV Variable Store availability
  //
  ...
  )
{
  ...
  //
  // Counter0 must be either equal to Counter1 or just one step ahead of Counter1.
  //
  if ((Counter0 > Counter1) && ((Counter0 - Counter1) > 1)) {
  ...
  while (VariableDig != NULL) {
    DEBUG ((DEBUG_INFO, "%a: %d VarDig = 0x%X, DigName = %s, StoreIndex = 0x%lX, CacheIndex = 0x%lX, DataSize = 0x%X, DigestSize = 0x%X",
            __FUNCTION__, __LINE__, VariableDig, VAR_DIG_NAME (VariableDig), VariableDig->StoreIndex, VariableDig->CacheIndex, VariableDig->DataSize, VariableDig->DigestSize));
    PrintVariableData ((UINT8 *) VAR_DIG_VALUE (VariableDig), VariableDig->DigestSize);
    //
  ...
  // need two HMAC values to check against: one for Counter0, one for Counter1.
  //
  ...
  {
    //
    // Might need to check Counter0. There must be something wrong in last boot.
    //
    Hmac1 = HmacSha256New ();
    ...
    if (  !HmacSha256Update (Hmac1, &Counter0, sizeof (Counter0))
  ...
  //
  // Always check Counter1.
  //
  if (  !HmacSha256Update (Hmac2, &Counter1, sizeof (Counter1))
  ...
  )
{
  ...
  while (TRUE) {
    if (VarDig != NULL) {
      ...
      VarInfo.NameSize            = (UINTN)DigestBuffer + (UINTN)*DigestBufferSize
    ...
    if (VarDig != NULL) {
      ...
      if (VarIndex >= UnprotectedVarIndexMax) {
        ...
        if (IsVariableToBeEncrypted(VAR_DIG_NAME (VarDig)) == FALSE) {
          VarDig->Flags.Encrypted = FALSE;
        }
        ...
        DEBUG ((DEBUG_INFO, "%a: %d VariableName: %s, VarDig->Flags.Protected = 0x%X, VarDig->Flags.Encrypted = 0x%X\n",
          __FUNCTION__, __LINE__, VAR_DIG_NAME (VarDig), VarDig->Flags.Protected, VarDig->Flags.Encrypted));
  ...
  )
{
  ...
  //
  Status      = ProtectedVariableGet256Key (Global->RootKey, sizeof (Global->RootKey));
  if (EFI_ERROR (Status)) {
    ASSERT (FALSE);
    Status = EFI_COMPROMISED_DATA;
  }

  DEBUG ((DEBUG_INFO, "%a(): Root Key Value: ", __FUNCTION__));
  for (Index = 0; Index < sizeof (Global->RootKey); Index++) {
    DEBUG ((DEBUG_INFO, "%02x", Global->RootKey[Index]));
  }
  DEBUG ((DEBUG_INFO, "\n"));

  //
  // Derive the MetaDataHmac256Key from root key
  //
  if (!GenerateMetaDataHmac256Key (
         Global->RootKey,
         ...
         ))
  {
    ASSERT (FALSE);
    Status = EFI_COMPROMISED_DATA;
    DEBUG ((DEBUG_INFO, "%a(): EFI_COMPROMISED_DATA ", __FUNCTION__));
  }

  DEBUG ((DEBUG_INFO, "%a(): MetaDataHmac Key Value: ", __FUNCTION__));
  for (Index = 0; Index < sizeof (Global->MetaDataHmacKey); Index++) {
    DEBUG ((DEBUG_INFO, "%02x", Global->MetaDataHmacKey[Index]));
  }
  DEBUG ((DEBUG_INFO, "\n"));
      ...
      || (Global->Unprotected[IndexHmacInDel] != VAR_INDEX_INVALID)))
  {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if (!EFI_ERROR (Status)) {
        //
        // MetaDataHmacVar is always calculated against Counter1+1. Updating
        // RPMCs to match it.
        //
        (VOID)ProtectedVariableIncrementMonotonicCounter0 ();
        (VOID)ProtectedVariableIncrementMonotonicCounter1 ();
      }
    }
  ...
  DEBUG ((DEBUG_ERROR, "%a: %d Variable Integrity check Status = %r\n", __FUNCTION__, __LINE__, Status));
  if (EFI_ERROR (Status)) {
    //
  ...
  //
  for (Index = 0; Index < IndexErrorFlag; ++Index) {
    if (  (Global->Unprotected[Index] != VAR_INDEX_INVALID)
