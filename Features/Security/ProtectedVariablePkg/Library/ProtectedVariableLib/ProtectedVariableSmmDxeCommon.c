  ...
  )
{
  ...
  if (NewVarInfo->Buffer != NULL) {
    ...
    if (NewVarDig->Flags.Encrypted) {
      ...
      Status                         = EncryptProtectedVariable (NewVarInfo);
      if (!EFI_ERROR (Status)) {
        //
  ...
  if (NewVarDig->CacheIndex != 0) {
    ...
    if ((CachedVarInfo.Header.DataSize != 0) && (NewVarInfo->CipherDataSize > CachedVarInfo.Header.DataSize)) {
      ...
      VarSize += NewVarInfo->NameSize + GET_PAD_SIZE (NewVarInfo->NameSize);
      ...
      if (Buffer != NULL) {
        VarSize  =  VARIABLE_HEADER_SIZE (NewVarDig->Flags.Auth);
        VarSize += CachedVarInfo.NameSize + GET_PAD_SIZE (CachedVarInfo.NameSize);
  ...
  )
{
  ...
  if (Global->Flags.RecoveryMode) {
    ...
    VarInfo.NameSize            = METADATA_HMAC_VARIABLE_NAME_SIZE;
  ...
  )
{
  ...
  } else if (UnprotectedVarIndex >= IndexErrorFlag) {
    //
  ...
  )
{
  ...
  NewVarSize += VarInfo.NameSize + GET_PAD_SIZE (VarInfo.NameSize);
