/** @file
  PEI Replay Protected Monotonic Counter (RPMC) Library.
  ...
  )
{
  ...
  ///
  Status = PeiHeciRequestMonotonicCounter (CounterIndex, CounterValue);
  if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  ///
  Status = PeiHeciIncrementMonotonicCounter (CounterIndex);
  if (EFI_ERROR (Status)) {
...
/**
  This constructor performs initialization for Pei Rpmc Lib.

  @retval EFI_SUCCESS      The constructor completed successfully.
**/
EFI_STATUS
EFIAPI
PeiRpmcLibConstructor (
  ...
  )
{
  ...
  ///
  Status = RequestMonotonicCounter (RPMC_COUNTER_0, &CounterValue);
  if (EFI_ERROR (Status)) {
    PcdSetBoolS (PcdRpmcSupported, FALSE);
    DEBUG ((DEBUG_INFO, "[RPMC] GetRpmcStatus failed status = %r.  RPMC is not supported\n", Status));
  } else {
    Status = RequestMonotonicCounter (RPMC_COUNTER_1, &CounterValue);
    if (EFI_ERROR (Status)) {
      PcdSetBoolS (PcdRpmcSupported, FALSE);
      DEBUG ((DEBUG_INFO, "[RPMC] GetRpmcStatus failed status = %r.  RPMC is not supported\n", Status));
    } else {
      PcdSetBoolS (PcdRpmcSupported, TRUE);
      DEBUG ((DEBUG_INFO, "[RPMC] GetRpmcStatus status = %r.  RPMC is supported\n", Status));
    }
  }
