/** @file
  Intel ACPI Reference Code for Mipi Camera
...
#define HDMI_DETECT         0x14
#define READY_STATUS        0x13
#define GPIOEXCSHAR         0
#define GPIOINTTRIGGER      0
#define GPIOINTLEVEL        0
...
0x1000
 ...
 External (\_SB.PC0X.FLMC, MethodObj)
...
Method (PINJ, 3, Serialized) { // Create GPIO resource template buffer
  ...
  Method (GPAD, 3, Serialized) {
    // Arg0 - GPIO Pin Community Number
    // Arg1 - GPIO Pin Group Number
    // Arg2 - GPIO Pin Pad Number
    // Refer GPIOV2_PAD_ID
    Store (0, Local0)
    Store (0, Local1)
    Store (0, Local2)
  ...
  Method (GPPI, 4, Serialized) { // Create information about GPIO pin used by sensor driver in _DSM method
    ...
    // GPIO DSM Offset : bit31-28:Active Value. 27-24:Initial Value. 23-8:PinNo. 0-7:Function
    ShiftLeft (Arg3, 28, Local3)
    ShiftLeft (Arg2, 24, Local2)
    ShiftLeft (Arg1, 8, Local1)
    Or (Local1, Arg0, Local0)
    Or (Local0, Local2, Local0)
    Or (Local0, Local3, Local0)
  ...
  Method (HCID, 1, Serialized) { // return fixed or dynamic _HID or _CID
    ...
    Switch (ToInteger (Local0)) {
      case (0) { Return ("OVTI01AF") } // OVTI01AF
      case (1) { Return ("OVTI13B1") } // OV13B
      case (2) { Return ("OVTI00AB") } // OG0VA1B
      case (3) { Return ("SONY688A") } // IMX688
      case (4) { Return ("OVTI08F4") } // OV08X40
      case (5) { Return ("INTC10B1") } // LONTIUM
      case (6) { Return ("MAX96724") } // OV08X40
      default { Return (GRID (Arg0)) }
    }
  }

  Method (GIDF, 1, Serialized) { // Generate ID (_HID or _CID) for user defined flash models
    // Arg0 - Link Number
    Name (BUF, Buffer (9) {})
    Switch (ToInteger (Arg0)) {
      case (0) {
      Store (F0H0, Index (BUF,0))
      Store (F0H1, Index (BUF,1))
      Store (F0H2, Index (BUF,2))
      Store (F0H3, Index (BUF,3))
      Store (F0H4, Index (BUF,4))
      Store (F0H5, Index (BUF,5))
      Store (F0H6, Index (BUF,6))
      Store (F0H7, Index (BUF,7))
      Store (F0H8, Index (BUF,8))
      }
      case (1) {
      Store (F1H0, Index (BUF,0))
      Store (F1H1, Index (BUF,1))
      Store (F1H2, Index (BUF,2))
      Store (F1H3, Index (BUF,3))
      Store (F1H4, Index (BUF,4))
      Store (F1H5, Index (BUF,5))
      Store (F1H6, Index (BUF,6))
      Store (F1H7, Index (BUF,7))
      Store (F1H8, Index (BUF,8))
      }
      case (2) {
      Store (F2H0, Index (BUF,0))
      Store (F2H1, Index (BUF,1))
      Store (F2H2, Index (BUF,2))
      Store (F2H3, Index (BUF,3))
      Store (F2H4, Index (BUF,4))
      Store (F2H5, Index (BUF,5))
      Store (F2H6, Index (BUF,6))
      Store (F2H7, Index (BUF,7))
      Store (F2H8, Index (BUF,8))
      }
      case (3) {
      Store (F3H0, Index (BUF,0))
      Store (F3H1, Index (BUF,1))
      Store (F3H2, Index (BUF,2))
      Store (F3H3, Index (BUF,3))
      Store (F3H4, Index (BUF,4))
      Store (F3H5, Index (BUF,5))
      Store (F3H6, Index (BUF,6))
      Store (F3H7, Index (BUF,7))
      Store (F3H8, Index (BUF,8))
      }
      case (4) {
      Store (F4H0, Index (BUF,0))
      Store (F4H1, Index (BUF,1))
      Store (F4H2, Index (BUF,2))
      Store (F4H3, Index (BUF,3))
      Store (F4H4, Index (BUF,4))
      Store (F4H5, Index (BUF,5))
      Store (F4H6, Index (BUF,6))
      Store (F4H7, Index (BUF,7))
      Store (F4H8, Index (BUF,8))
      }
      default {
      Store (F5H0, Index (BUF,0))
      Store (F5H1, Index (BUF,1))
      Store (F5H2, Index (BUF,2))
      Store (F5H3, Index (BUF,3))
      Store (F5H4, Index (BUF,4))
      Store (F5H5, Index (BUF,5))
      Store (F5H6, Index (BUF,6))
      Store (F5H7, Index (BUF,7))
      Store (F5H8, Index (BUF,8))
      }
    }
    Return (ToString (BUF))
  }
    ...
    If (LEqual (Arg0, 0)) { // If Control Logic Device0 is to be used
      If (LEqual (C0TP, 1)) { // If Type of control logic is Discrete
        Switch (ToInteger (L0BS)) { // I2C bus
          case (0) { Return (Package () {\_SB.DSC0, \_SB.PC00.I2C0}) }
          case (1) { Return (Package () {\_SB.DSC0, \_SB.PC00.I2C1}) }
          case (2) { Return (Package () {\_SB.DSC0, \_SB.PC00.I2C2}) }
          case (3) { Return (Package () {\_SB.DSC0, \_SB.PC00.I2C3}) }
          case (4) { Return (Package () {\_SB.DSC0, \_SB.PC00.I2C4}) }
          case (5) { Return (Package () {\_SB.DSC0, \_SB.PC00.I2C5}) }
          default { Return (Package () {\_SB.DSC0}) }
        }
      }
      If(LEqual (C0TP, 5)) { Return (Package () {\_SB.LTH0}) }
      If (LAnd(LGreater (C0TP, 1), LNotEqual(C0TP, 5))) { Return (Package () {\_SB.CLP0}) } // when Control Logic Type is of type PMIC add dependency on CLP0
    }
    If (LEqual (Arg0, 1)) { // If Control Logic Device1 is to be used
      If (LEqual (C1TP, 1)) { // If Type of control logic is Discrete
        Switch (ToInteger (L1BS)) { // I2C bus
          case (0) { Return (Package () {\_SB.DSC1, \_SB.PC00.I2C0}) }
          case (1) { Return (Package () {\_SB.DSC1, \_SB.PC00.I2C1}) }
          case (2) { Return (Package () {\_SB.DSC1, \_SB.PC00.I2C2}) }
          case (3) { Return (Package () {\_SB.DSC1, \_SB.PC00.I2C3}) }
          case (4) { Return (Package () {\_SB.DSC1, \_SB.PC00.I2C4}) }
          case (5) { Return (Package () {\_SB.DSC1, \_SB.PC00.I2C5}) }
          default { Return (Package () {\_SB.DSC1}) }
        }
      }
      If(LEqual (C1TP, 5)) { Return (Package () {\_SB.LTH1}) }
      If (LAnd(LGreater (C1TP, 1), LNotEqual(C1TP, 5))) { Return (Package () {\_SB.CLP1}) } // when Control Logic Type is of type PMIC add dependency on CLP1
    }
    If (LEqual (Arg0, 2)) { // If Control Logic Device2 is to be used
      If (LEqual (C2TP, 1)) { // If Type of control logic is Discrete
        Switch (ToInteger (L2BS)) { // I2C bus
          case (0) { Return (Package () {\_SB.DSC2, \_SB.PC00.I2C0}) }
          case (1) { Return (Package () {\_SB.DSC2, \_SB.PC00.I2C1}) }
          case (2) { Return (Package () {\_SB.DSC2, \_SB.PC00.I2C2}) }
          case (3) { Return (Package () {\_SB.DSC2, \_SB.PC00.I2C3}) }
          case (4) { Return (Package () {\_SB.DSC2, \_SB.PC00.I2C4}) }
          case (5) { Return (Package () {\_SB.DSC2, \_SB.PC00.I2C5}) }
          default { Return (Package () {\_SB.DSC2}) }
        }
      }
      If(LEqual (C2TP, 5)) { Return (Package () {\_SB.LTH2}) }
      If (LAnd(LGreater (C2TP, 1), LNotEqual(C2TP, 5))) { Return (Package () {\_SB.CLP2}) } // when Control Logic Type is of type PMIC add dependency on CLP2
    }
    If (LEqual (Arg0, 3)) { // If Control Logic Device3 is to be used
      If (LEqual (C3TP, 1)) { // If Type of control logic is Discrete
        Switch (ToInteger (L3BS)) { // I2C bus
          case (0) { Return (Package () {\_SB.DSC3, \_SB.PC00.I2C0}) }
          case (1) { Return (Package () {\_SB.DSC3, \_SB.PC00.I2C1}) }
          case (2) { Return (Package () {\_SB.DSC3, \_SB.PC00.I2C2}) }
          case (3) { Return (Package () {\_SB.DSC3, \_SB.PC00.I2C3}) }
          case (4) { Return (Package () {\_SB.DSC3, \_SB.PC00.I2C4}) }
          case (5) { Return (Package () {\_SB.DSC3, \_SB.PC00.I2C5}) }
          default { Return (Package () {\_SB.DSC3}) }
        }
      }
      If(LEqual (C3TP, 5)) { Return (Package () {\_SB.LTH3}) }
      If (LAnd(LGreater (C3TP, 1), LNotEqual(C3TP, 5))) { Return (Package () {\_SB.CLP3}) } // when Control Logic Type is of type PMIC add dependency on CLP2
    }
    If (LEqual (Arg0, 4)) { // If Control Logic Device4 is to be used
      If (LEqual (C4TP, 1)) { // If Type of control logic is Discrete
        Switch (ToInteger (L4BS)) { // I2C bus
          case (0) { Return (Package () {\_SB.DSC4, \_SB.PC00.I2C0}) }
          case (1) { Return (Package () {\_SB.DSC4, \_SB.PC00.I2C1}) }
          case (2) { Return (Package () {\_SB.DSC4, \_SB.PC00.I2C2}) }
          case (3) { Return (Package () {\_SB.DSC4, \_SB.PC00.I2C3}) }
          case (4) { Return (Package () {\_SB.DSC4, \_SB.PC00.I2C4}) }
          case (5) { Return (Package () {\_SB.DSC4, \_SB.PC00.I2C5}) }
          default { Return (Package () {\_SB.DSC4}) }
        }
      }
      If(LEqual (C4TP, 5)) { Return (Package () {\_SB.LTH4}) }
      If (LAnd(LGreater (C4TP, 1), LNotEqual(C4TP, 5))) { Return (Package () {\_SB.CLP4}) } // when Control Logic Type is of type PMIC add dependency on CLP2
    }
    If (LEqual (Arg0, 5)) { // If Control Logic Device5 is to be used
      If (LEqual (C5TP, 1)) { // If Type of control logic is Discrete
        Switch (ToInteger (L5BS)) { // I2C bus
          case (0) { Return (Package () {\_SB.DSC5, \_SB.PC00.I2C0}) }
          case (1) { Return (Package () {\_SB.DSC5, \_SB.PC00.I2C1}) }
          case (2) { Return (Package () {\_SB.DSC5, \_SB.PC00.I2C2}) }
          case (3) { Return (Package () {\_SB.DSC5, \_SB.PC00.I2C3}) }
          case (4) { Return (Package () {\_SB.DSC5, \_SB.PC00.I2C4}) }
          case (5) { Return (Package () {\_SB.DSC5, \_SB.PC00.I2C5}) }
          default { Return (Package () {\_SB.DSC5}) }
        }
      }
      If(LEqual (C5TP, 5)) { Return (Package () {\_SB.LTH5}) }
      If (LAnd(LGreater (C5TP, 1), LNotEqual(C5TP, 5))) { Return (Package () {\_SB.CLP5}) } // when Control Logic Type is of type PMIC add dependency on CLP2
    }
  ...
  Method (FDEP, 1, NotSerialized) {
    Switch (ToInteger (Arg0)) {
      case (0) { Return (Package () {\_SB.PC00.I2C0}) }
      case (1) { Return (Package () {\_SB.PC00.I2C1}) }
      case (2) { Return (Package () {\_SB.PC00.I2C2}) }
      case (3) { Return (Package () {\_SB.PC00.I2C3}) }
      case (4) { Return (Package () {\_SB.PC00.I2C4}) }
      case (5) { Return (Package () {\_SB.PC00.I2C5}) }
      default  { Return (Package () {\_SB.PC00}) }
    }
  }
  ...
  Device (DSC0) { // Control Logic Device 0 when Discrete is selected
    ...
    Method (CLDB) {
      Name (PAR, Buffer () {
        ...
        0x00,                 // Platform Id         (Offset 5)
        0x00,                 // Emulation Config    (Offset 6)
      ...
      Store (PLID, Index (PAR, 5))  // Platform Id
      Store (EMUS, Index (PAR, 6))  // Emulation Config 00:Realboard 01:Emulation
    ...
    Method (_DSM, 4, NotSerialized) { // _DSM: Device-Specific Method
      If (LEqual (Arg0, ToUUID ("1EA54AB2-CD84-48CC-9DD4-7F594EC3B015"))) { // DsmInterfaceVersion
        Return (1)
      }
      If (LEqual (Arg0, ToUUID ("79234640-9E10-4FEA-A5C1-B5AA8B19756F"))) { // DsmGPIO
        // Function 0, Functions 1-7 supported for revision 1
        If (LEqual (Arg2, 0)) { Return (Buffer () {0x3F}) }
        // Function 1 (GPIO count)
        If (LEqual (Arg2, 1)) { Return (C0GP) }
        // Function 2
        If (LEqual (Arg2, 2)) { Return (GPPI (C0F0, GNUM (GPAD (C0C0, C0G0, C0P0)), C0I0, C0A0)) }
        // Function 3
        If (LEqual (Arg2, 3)) { Return (GPPI (C0F1, GNUM (GPAD (C0C1, C0G1, C0P1)), C0I1, C0A1)) }
        // Function 4
        If (LEqual (Arg2, 4)) { Return (GPPI (C0F2, GNUM (GPAD (C0C2, C0G2, C0P2)), C0I2, C0A2)) }
        // Function 5
        If (LEqual (Arg2, 5)) { Return (GPPI (C0F3, GNUM (GPAD (C0C3, C0G3, C0P3)), C0I3, C0A3)) }
        // Function 6
        If (LEqual (Arg2, 6)) { Return (GPPI (C0F4, GNUM (GPAD (C0C4, C0G4, C0P4)), C0I4, C0A4)) }
        // Function 7
        If (LEqual (Arg2, 7)) { Return (GPPI (C0F5, GNUM (GPAD (C0C5, C0G5, C0P5)), C0I5, C0A5)) }
        Return (Buffer () {0x00})
      }
      If (LNotEqual (PCHS, PCHX)) { // no IPU support on PCH_S
  ...
  Device (DSC1) { // Control Logic Device 1 when Discrete is selected
    ...
    Method (_DSM, 4, NotSerialized) { // _DSM: Device-Specific Method
      ...
      If (LEqual (Arg0, ToUUID ("79234640-9E10-4FEA-A5C1-B5AA8B19756F"))) { // DsmGPIO
        ...
        // Function 2
        If (LEqual (Arg2, 2)) { Return (GPPI (C1F0, GNUM (GPAD (C1C0, C1G0, C1P0)), C1I0, C1A0)) }
        // Function 3
        If (LEqual (Arg2, 3)) { Return (GPPI (C1F1, GNUM (GPAD (C1C1, C1G1, C1P1)), C1I1, C1A1)) }
        // Function 4
        If (LEqual (Arg2, 4)) { Return (GPPI (C1F2, GNUM (GPAD (C1C2, C1G2, C1P2)), C1I2, C1A2)) }
        // Function 5
        If (LEqual (Arg2, 5)) { Return (GPPI (C1F3, GNUM (GPAD (C1C3, C1G3, C1P3)), C1I3, C1A3)) }
        // Function 6
        If (LEqual (Arg2, 6)) { Return (GPPI (C1F4, GNUM (GPAD (C1C4, C1G4, C1P4)), C1I4, C1A4)) }
        // Function 7
        If (LEqual (Arg2, 7)) { Return (GPPI (C1F5, GNUM (GPAD (C1C5, C1G5, C1P5)), C1I5, C1A5)) }
      }
      If (LNotEqual (PCHS, PCHX)) { // no IPU support on PCH_S
        Include ("IpuImgClkDsm.asl")
      }
  ...
  Device (DSC2) { // Control Logic Device 2 when Discrete is selected
    ...
    Method (_DSM, 4, NotSerialized) { // _DSM: Device-Specific Method
      ...
      If (LEqual (Arg0, ToUUID ("79234640-9E10-4FEA-A5C1-B5AA8B19756F"))) { // DsmGPIO
        ...
        // Function 2
        If (LEqual (Arg2, 2)) { Return (GPPI (C2F0, GNUM (GPAD (C2C0, C2G0, C2P0)), C2I0, C2A0)) }
        // Function 3
        If (LEqual (Arg2, 3)) { Return (GPPI (C2F1, GNUM (GPAD (C2C1, C2G1, C2P1)), C2I1, C2A1)) }
        // Function 4
        If (LEqual (Arg2, 4)) { Return (GPPI (C2F2, GNUM (GPAD (C2C2, C2G2, C2P2)), C2I2, C2A2)) }
        // Function 5
        If (LEqual (Arg2, 5)) { Return (GPPI (C2F3, GNUM (GPAD (C2C3, C2G3, C2P3)), C2I3, C2A3)) }
        // Function 6
        If (LEqual (Arg2, 6)) { Return (GPPI (C2F4, GNUM (GPAD (C2C4, C2G4, C2P4)), C2I4, C2A4)) }
        // Function 7
        If (LEqual (Arg2, 7)) { Return (GPPI (C2F5, GNUM (GPAD (C2C5, C2G5, C2P5)), C2I5, C2A5)) }
      }
      If (LNotEqual (PCHS, PCHX)) { // no IPU support on PCH_S
        Include ("IpuImgClkDsm.asl")
      }
  ...
  Device (DSC3) { // Control Logic Device 3 when Discrete is selected
    ...
    Method (_DSM, 4, NotSerialized) { // _DSM: Device-Specific Method
      ...
      If (LEqual (Arg0, ToUUID ("79234640-9E10-4FEA-A5C1-B5AA8B19756F"))) { // DsmGPIO
        ...
        // Function 2
        If (LEqual (Arg2, 2)) { Return (GPPI (C3F0, GNUM (GPAD (C3C0, C3G0, C3P0)), C3I0, C3A0)) }
        // Function 3
        If (LEqual (Arg2, 3)) { Return (GPPI (C3F1, GNUM (GPAD (C3C1, C3G1, C3P1)), C3I1, C3A1)) }
        // Function 4
        If (LEqual (Arg2, 4)) { Return (GPPI (C3F2, GNUM (GPAD (C3C2, C3G2, C3P2)), C3I2, C3A2)) }
        // Function 5
        If (LEqual (Arg2, 5)) { Return (GPPI (C3F3, GNUM (GPAD (C3C3, C3G3, C3P3)), C3I3, C3A3)) }
        // Function 6
        If (LEqual (Arg2, 6)) { Return (GPPI (C3F4, GNUM (GPAD (C3C4, C3G4, C3P4)), C3I4, C3A4)) }
        // Function 7
        If (LEqual (Arg2, 7)) { Return (GPPI (C3F5, GNUM (GPAD (C3C5, C3G5, C3P5)), C3I5, C3A5)) }
      }
      If (LNotEqual (PCHS, PCHX)) { // no IPU support on PCH_S
        Include ("IpuImgClkDsm.asl")
      }
  ...
  Device (DSC4) { // Control Logic Device 4 when Discrete is selected
    ...
    Method (_DSM, 4, NotSerialized) { // _DSM: Device-Specific Method
      ...
      If (LEqual (Arg0, ToUUID ("79234640-9E10-4FEA-A5C1-B5AA8B19756F"))) { // DsmGPIO
        ...
        // Function 2
        If (LEqual (Arg2, 2)) { Return (GPPI (C4F0, GNUM (GPAD (C4C0, C4G0, C4P0)), C4I0, C4A0)) }
        // Function 3
        If (LEqual (Arg2, 3)) { Return (GPPI (C4F1, GNUM (GPAD (C4C1, C4G1, C4P1)), C4I1, C4A1)) }
        // Function 4
        If (LEqual (Arg2, 4)) { Return (GPPI (C4F2, GNUM (GPAD (C4C2, C4G2, C4P2)), C4I2, C4A2)) }
        // Function 5
        If (LEqual (Arg2, 5)) { Return (GPPI (C4F3, GNUM (GPAD (C4C3, C4G3, C4P3)), C4I3, C4A3)) }
        // Function 6
        If (LEqual (Arg2, 6)) { Return (GPPI (C4F4, GNUM (GPAD (C4C4, C4G4, C4P4)), C4I4, C4A4)) }
        // Function 7
        If (LEqual (Arg2, 7)) { Return (GPPI (C4F5, GNUM (GPAD (C4C5, C4G5, C4P5)), C4I5, C4A5)) }
      }
      If (LNotEqual (PCHS, PCHX)) { // no IPU support on PCH_S
        Include ("IpuImgClkDsm.asl")
      }
  ...
  Device (DSC5) { // Control Logic Device 5 when Discrete is selected
    ...
    Method (_DSM, 4, NotSerialized) { // _DSM: Device-Specific Method
      ...
      If (LEqual (Arg0, ToUUID ("79234640-9E10-4FEA-A5C1-B5AA8B19756F"))) { // DsmGPIO
        ...
        // Function 2
        If (LEqual (Arg2, 2)) { Return (GPPI (C5F0, GNUM (GPAD (C5C0, C5G0, C5P0)), C5I0, C5A0)) }
        // Function 3
        If (LEqual (Arg2, 3)) { Return (GPPI (C5F1, GNUM (GPAD (C5C1, C5G1, C5P1)), C5I1, C5A1)) }
        // Function 4
        If (LEqual (Arg2, 4)) { Return (GPPI (C5F2, GNUM (GPAD (C5C2, C5G2, C5P2)), C5I2, C5A2)) }
        // Function 5
        If (LEqual (Arg2, 5)) { Return (GPPI (C5F3, GNUM (GPAD (C5C3, C5G3, C5P3)), C5I3, C5A3)) }
        // Function 6
        If (LEqual (Arg2, 6)) { Return (GPPI (C5F4, GNUM (GPAD (C5C4, C5G4, C5P4)), C5I4, C5A4)) }
        // Function 7
        If (LEqual (Arg2, 7)) { Return (GPPI (C5F5, GNUM (GPAD (C5C5, C5G5, C5P5)), C5I5, C5A5)) }
      }
      If (LNotEqual (PCHS, PCHX)) { // no IPU support on PCH_S
        Include ("IpuImgClkDsm.asl")
      }
  ...
  Device (CLP0) { // Control Logic Device 0 when PMIC is selected
    ...
    Method (_CRS, 0x0, NotSerialized) {
      ...
      Return (\_SB.PC0X.IICB (C0IA, C0IB))
    }
    Method (_STA, 0, NotSerialized) {  // _STA: Status
      If (CL00) {
  ...
  Device (CLP1) { // Control Logic Device 1 when PMIC is selected
    ...
    Method (_CRS, 0x0, NotSerialized) {
      // Create I2C Bus resource template using as parameters data provided in the BIOS setup
      Return (\_SB.PC0X.IICB (C1IA, C1IB))
    }
    Method (_STA, 0, NotSerialized) {  // _STA: Status
      If (CL01) {
  ...
  Device (CLP2) { // Control Logic Device 2 when PMIC is selected
    ...
    Method (_CRS, 0x0, NotSerialized) {
      // Create I2C Bus resource template using as parameters data provided in the BIOS setup
      Return (\_SB.PC0X.IICB (C2IA, C2IB))
    }
    Method (_STA, 0, NotSerialized) {  // _STA: Status
      If (CL02) {
  ...
  Device (CLP3) { // Control Logic Device 3 when PMIC is selected
    ...
    Method (_CRS, 0x0, NotSerialized) {
      // Create I2C Bus resource template using as parameters data provided in the BIOS setup
      Return (\_SB.PC0X.IICB (C3IA, C3IB))
    }
    Method (_STA, 0, NotSerialized) {  // _STA: Status
      If (CL03) {
  ...
  Device (CLP4) { // Control Logic Device 4 when PMIC is selected
    ...
    Method (_CRS, 0x0, NotSerialized) {
      // Create I2C Bus resource template using as parameters data provided in the BIOS setup
      Return (\_SB.PC0X.IICB (C4IA, C4IB))
    }
    Method (_STA, 0, NotSerialized) {  // _STA: Status
      If (CL04) {
  ...
  Device (CLP5) { // Control Logic Device 5 when PMIC is selected
    ...
    Method (_CRS, 0x0, NotSerialized) {
      // Create I2C Bus resource template using as parameters data provided in the BIOS setup
      Return (\_SB.PC0X.IICB (C5IA, C5IB))
    }
    Method (_STA, 0, NotSerialized) {  // _STA: Status
      If (CL05) {
  ...
  Device (LTH0) { // Control Logic Device 0 when Discrete is selected
    Name (_HID, "INT3472")
    Name (_CID, "INT3472")
    Name (_DDN, "PMIC-CRDG")
    Name (_UID, 20)
      ...
      Method (_CRS, 0x0, NotSerialized) {
         If (LGreater (C0GP, 0)) {
          Switch(ToInteger(C0F0)) {
            case (0x13)  { Store (PINJ (C0P0, C0C0, C0G0), Local0) } /* Ready Status 0x13 */
            case (0x14)  { Store (PINM (C0P0, C0C0, C0G0), Local0) } /* HDMI detect 0x14 */
            Default      { Store (PINR (C0P0, C0C0, C0G0), Local0) }
          }
        }
        If (LGreater (C0GP, 1)) {
        ...
        If (LGreater (C0GP, 4)) {
          Switch(ToInteger(C0F4)) {
            case (0x13)  { Store (PINJ (C0P4, C0C4, C0G4), Local1) } /* Ready Status 0x13 */
            case (0x14)  { Store (PINM (C0P4, C0C4, C0G4), Local1) } /* HDMI detect 0x14 */
            Default      { Store (PINR(C0P4, C0C4, C0G4), Local1) }
          }
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        If (LGreater (C0GP, 5)) {
          Switch(ToInteger(C0F5)) {
            case (0x13)  { Store (PINJ (C0P5, C0C5, C0G5), Local1) } /* Ready Status 0x13 */
            case (0x14)  { Store (PINM (C0P5, C0C5, C0G5), Local1) } /* HDMI detect 0x14 */
            Default      { Store (PINR(C0P5, C0C5, C0G5), Local1) }
          }
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
  ...
  Device (LTH1) { // Control Logic Device 1 when Discrete is selected
    Name (_HID, "INT3472")
    Name (_CID, "INT3472")
    Name (_DDN, "PMIC-CRDG")
    Name (_UID, 21)
    ...
    If (LNotEqual (C1GP,0)) {
      Method (_CRS, 0x0, NotSerialized) {
         If (LGreater (C1GP, 0)) {
          ...
          case (0x13)  { Store (PINJ (C1P0, C1C0, C1G0), Local0) } /* Ready Status 0x13 */
          case (0x14)  { Store (PINM (C1P0, C1C0, C1G0), Local0) } /* HDMI detect 0x14 */
          Default      { Store (PINR (C1P0, C1C0, C1G0), Local0) }
          }
        }
        If (LGreater (C1GP, 1)) {
        ...
        If (LGreater (C1GP, 4)) {
          Switch(ToInteger(C1F4)) {
          case (0x13)  { Store (PINJ (C1P4, C1C4, C1G4), Local1) } /* Ready Status 0x13 */
          case (0x14)  { Store (PINM (C1P4, C1C4, C1G4), Local1) } /* HDMI detect 0x14 */
          Default      { Store (PINR(C1P4, C1C4, C1G4), Local1) }
          }
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        If (LGreater (C1GP, 5)) {
          Switch(ToInteger(C1F5)) {
          case (0x13)  { Store (PINJ (C1P5, C1C5, C1G5), Local1) } /* Ready Status 0x13 */
          case (0x14)  { Store (PINM (C1P5, C1C5, C1G5), Local1) } /* HDMI detect 0x14 */
          Default      { Store (PINR(C1P5, C1C5, C1G5), Local1) }
          }
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
  ...
  Device (LTH2) { // Control Logic Device 2 when Discrete is selected
    Name (_HID, "INT3472")
    Name (_CID, "INT3472")
    Name (_DDN, "PMIC-CRDG")
    Name (_UID, 22)
    ...
    If (LNotEqual (C2GP,0)) {
      Method (_CRS, 0x0, NotSerialized) {
        If (LGreater (C2GP, 0)) {
          ...
          case (0x13)  { Store (PINJ (C2P0, C2C0, C2G0), Local0) } /* Ready Status 0x13 */
          case (0x14)  { Store (PINM (C2P0, C2C0, C2G0), Local0) } /* HDMI detect 0x14 */
          Default      { Store (PINR (C2P0, C2C0, C2G0), Local0) }
          }
        }
        If (LGreater (C2GP, 1)) {
        ...
        If (LGreater (C2GP, 4)) {
          Switch(ToInteger(C2F4)) {
          case (0x13)  { Store (PINJ (C2P4, C2C4, C2G4), Local1) } /* Ready Status 0x13 */
          case (0x14)  { Store (PINM (C2P4, C2C4, C2G4), Local1) } /* HDMI detect 0x14 */
          Default      { Store (PINR(C2P4, C2C4, C2G4), Local1) }
          }
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        If (LGreater (C2GP, 5)) {
          Switch(ToInteger(C2F5)) {
          case (0x13)  { Store (PINJ (C2P5, C2C5, C2G5), Local1) } /* Ready Status 0x13 */
          case (0x14)  { Store (PINM (C2P5, C2C5, C2G5), Local1) } /* HDMI detect 0x14 */
          Default      { Store (PINR(C2P5, C2C5, C2G5), Local1) }
          }
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
  ...
  Device (LTH3) { // Control Logic Device 3 when Discrete is selected
    Name (_HID, "INT3472")
    Name (_CID, "INT3472")
    Name (_DDN, "PMIC-CRDG")
    Name (_UID, 23)
    ...
    If (LNotEqual (C3GP,0)) {
      Method (_CRS, 0x0, NotSerialized) {
       If (LGreater (C3GP, 0)) {
        ...
        case (0x13)  { Store (PINJ (C3P0, C3C0, C3G0), Local0) } /* Ready Status 0x13 */
        case (0x14)  { Store (PINM (C3P0, C3C0, C3G0), Local0) } /* HDMI detect 0x14 */
        Default      { Store (PINR (C3P0, C3C0, C3G0), Local0) }
        }
      }
      If (LGreater (C3GP, 1)) {
      ...
      If (LGreater (C3GP, 4)) {
        Switch(ToInteger(C3F4)) {
        case (0x13)  { Store (PINJ (C3P4, C3C4, C3G4), Local1) } /* Ready Status 0x13 */
        case (0x14)  { Store (PINM (C3P4, C3C4, C3G4), Local1) } /* HDMI detect 0x14 */
        Default      { Store (PINR(C3P4, C3C4, C3G4), Local1) }
        }
        ConcatenateResTemplate (Local0, Local1, Local2)
        Store (Local2, Local0)
      }
      If (LGreater (C3GP, 5)) {
        Switch(ToInteger(C3F5)) {
        case (0x13)  { Store (PINJ (C3P5, C3C5, C3G5), Local1) } /* Ready Status 0x13 */
        case (0x14)  { Store (PINM (C3P5, C3C5, C3G5), Local1) } /* HDMI detect 0x14 */
        Default      { Store (PINR(C3P5, C3C5, C3G5), Local1) }
        }
        ConcatenateResTemplate (Local0, Local1, Local2)
        Store (Local2, Local0)
      }
  ...
  Device (LTH4) { // Control Logic Device 4 when Discrete is selected
    Name (_HID, "INT3472")
    Name (_CID, "INT3472")
    Name (_DDN, "PMIC-CRDG")
    Name (_UID, 24)
    ...
    If (LNotEqual (C4GP,0)) {
      ...
      case (0x13)  { Store (PINJ (C4P0, C4C0, C4G0), Local0) } /* Ready Status 0x13 */
      case (0x14)  { Store (PINM (C4P0, C4C0, C4G0), Local0) } /* HDMI detect 0x14 */
      Default      { Store (PINR (C4P0, C4C0, C4G0), Local0) }
      }
      }
      If (LGreater (C4GP, 1)) {
      ...
      If (LGreater (C4GP, 4)) {
      Switch(ToInteger(C4F4)) {
      case (0x13)  { Store (PINJ (C4P4, C4C4, C4G4), Local1) } /* Ready Status 0x13 */
      case (0x14)  { Store (PINM (C4P4, C4C4, C4G4), Local1) } /* HDMI detect 0x14 */
      Default      { Store (PINR(C4P4, C4C4, C4G4), Local1) }
      }
      ConcatenateResTemplate (Local0, Local1, Local2)
      Store (Local2, Local0)
      }
      If (LGreater (C4GP, 5)) {
      Switch(ToInteger(C4F5)) {
      case (0x13)  { Store (PINJ (C4P5, C4C5, C4G5), Local1) } /* Ready Status 0x13 */
      case (0x14)  { Store (PINM (C4P5, C4C5, C4G5), Local1) } /* HDMI detect 0x14 */
      Default      { Store (PINR(C4P5, C4C5, C4G5), Local1) }
      }
      ConcatenateResTemplate (Local0, Local1, Local2)
      Store (Local2, Local0)
      }
  ...
  Device (LTH5) { // Control Logic Device 5 when Discrete is selected
    Name (_HID, "INT3472")
    Name (_CID, "INT3472")
    Name (_DDN, "PMIC-CRDG")
    Name (_UID, 25)
    ...
    If (LNotEqual (C5GP,0)) {
     ...
     case (0x13)  { Store (PINJ (C5P0, C5C0, C5G0), Local0) } /* Ready Status 0x13 */
     case (0x14)  { Store (PINM (C5P0, C5C0, C5G0), Local0) } /* HDMI detect 0x14 */
     Default      { Store (PINR (C5P0, C5C0, C5G0), Local0) }
     }
     }
     If (LGreater (C5GP, 1)) {
     ...
     If (LGreater (C5GP, 4)) {
     Switch(ToInteger(C5F4)) {
     case (0x13)  { Store (PINJ (C5P4, C5C4, C5G4), Local1) } /* Ready Status 0x13 */
     case (0x14)  { Store (PINM (C5P4, C5C4, C5G4), Local1) } /* HDMI detect 0x14 */
     Default      { Store (PINR(C5P4, C5C4, C5G4), Local1) }
     }
     ConcatenateResTemplate (Local0, Local1, Local2)
     Store (Local2, Local0)
     }
     If (LGreater (C5GP, 5)) {
     Switch(ToInteger(C5F5)) {
     case (0x13)  { Store (PINJ (C5P5, C5C5, C5G5), Local1) } /* Ready Status 0x13 */
     case (0x14)  { Store (PINM (C5P5, C5C5, C5G5), Local1) } /* HDMI detect 0x14 */
     Default      { Store (PINR(C5P5, C5C5, C5G5), Local1) }
     }
     ConcatenateResTemplate (Local0, Local1, Local2)
     Store (Local2, Local0)
     }
  ...
  Device (LNK0) {       // definition for Camera sensor module using Link0
    ...
    Method (_DEP, 0, NotSerialized) {  // _DEP: Operation Region Dependencies
      If (L0EN) { // If LNK0 device is to be enabled
        If (LNotEqual (\_SB.PC0X.LNKC (_UID), 0)) {
          Return (\_SB.PC0X.LNKD (_UID))
        }
        Return (CDEP (L0CL))
      }
      Else { Return (Package () {\_SB.PC0X}) }
    }
    ...
    Method (SSDB)
    {
      Name (PAR, Buffer () {
        ...
        0x00,                 // Platform Id                    (offset 80)
        0x00,                 // Emulation Config               (offset 81)
        ...
        0x00,0x00,0x00,0x00,  // Reserved                       (offset 102)
      ...
      Store (PLID, Index (PAR, 80)) // Platform Id
      Store (EMUS, Index (PAR, 81)) // Emulation Config 00:Realboard 01:Emulation
    ...
    Method (_DSM, 4, NotSerialized) {  // _DSM: Device-Specific Method
      ...
      If (LEqual (Arg0,ToUUID ("5815C5C8-C47D-477B-9A8D-76173176414B"))) { // DsmI2c - new implementation
        ...
        If (LEqual (Arg2, 1)) {
          ...
          CreateDWordField (DSMB, HDMI_DETECT, DEV4)
      ...
      If (LNotEqual (\_SB.PC0X.DSMM (_UID), 0xFFFFFFFF)) { // If LNK0 is selected for LCH
        If (LEqual (Arg0,ToUUID ("02F55F0C-2E63-4F05-84F3-BF1980F9AF79"))) { /*DsmLch*/
          ...
          // Function 1 - Returns LCH Config - (0:Disabled/1:USB Bridge)
          If (LEqual (Arg2, 1)) { Return (\_SB.PC0X.DSMM (_UID)) }
        }
      }
      Return (Buffer () {0})
    }
  ...
  Device (LNK1) {       // definition for Camera sensor module using Link1
    ...
    Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
    {
      ...
      If (LEqual (L1DI, 0)) {
        ...
        // Create I2C Bus resource template using as parameters data provided in the BIOS setup
        If (LGreater (L1DI, 0)) {
          Store (\_SB.PC0X.IICB (L1A0, L1BS), Local0)
        }
        // Add next resource buffer if 2 or more I2C devices were enabled
        If (LGreater (L1DI, 1)) {
          Store (\_SB.PC0X.IICB (L1A1, L1BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 3 or more I2C devices were enabled
        If (LGreater (L1DI, 2)) {
          Store (\_SB.PC0X.IICB (L1A2, L1BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 4 or more I2C devices were enabled
        If (LGreater (L1DI, 3)) {
          Store (\_SB.PC0X.IICB (L1A3, L1BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 5 or more I2C devices were enabled
        If (LGreater (L1DI, 4)) {
          Store (\_SB.PC0X.IICB (L1A4, L1BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 6 or more I2C devices were enabled
        If (LGreater (L1DI, 5)) {
          Store (\_SB.PC0X.IICB (L1A5, L1BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 7 or more I2C devices were enabled
        If (LGreater (L1DI, 6)) {
          Store (\_SB.PC0X.IICB (L1A6, L1BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 8 or more I2C devices were enabled
        If (LGreater (L1DI, 7)) {
          Store (\_SB.PC0X.IICB (L1A7, L1BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 9 or more I2C devices were enabled
        If (LGreater (L1DI, 8)) {
          Store (\_SB.PC0X.IICB (L1A8, L1BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 10 or more I2C devices were enabled
        If (LGreater (L1DI, 9)) {
          Store (\_SB.PC0X.IICB (L1A9, L1BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 11 or more I2C devices were enabled
        If (LGreater (L1DI, 10)) {
          Store (\_SB.PC0X.IICB (L1AA, L1BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 12 or more I2C devices were enabled
        If (LGreater (L1DI, 11)) {
          Store (\_SB.PC0X.IICB (L1AB, L1BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        Return (Local0)
    ...
    Method (_DSM, 4, NotSerialized) {  // _DSM: Device-Specific Method
      ...
      If (LNotEqual (\_SB.PC0X.DSMM (_UID), 0xFFFFFFFF)) { // If LNK1 is selected for LCH
        If (LEqual (Arg0,ToUUID ("02F55F0C-2E63-4F05-84F3-BF1980F9AF79"))) { /*DsmLch*/
          // Function 0 - returns bitmask of supported function.
          // BIT0 - indicates if any functions are supported
          // BIT1 - indicates Function 1 is Supported
  ...
  Device (LNK2) {       // definition for Camera sensor module using Link2
    ...
    Method (_DEP, 0, NotSerialized) {  // _DEP: Operation Region Dependencies
      If (L2EN) { // If LNK2 device is to be enabled
        If (LNotEqual (\_SB.PC0X.LNKC (_UID), 0)) {
          Return (\_SB.PC0X.LNKD (_UID))
        }
        Return (CDEP (L2CL))
    ...
    Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
    {
      ...
      If (LEqual (L2DI, 0)) {
        ...
        // Create I2C Bus resource template using as parameters data provided in the BIOS setup
        If (LGreater (L2DI, 0)) {
          Store (\_SB.PC0X.IICB (L2A0, L2BS), Local0)
        }
        // Add next resource buffer if 2 or more I2C devices were enabled
        If (LGreater (L2DI, 1)) {
          Store (\_SB.PC0X.IICB (L2A1, L2BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 3 or more I2C devices were enabled
        If (LGreater (L2DI, 2)) {
          Store (\_SB.PC0X.IICB (L2A2, L2BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 4 or more I2C devices were enabled
        If (LGreater (L2DI, 3)) {
          Store (\_SB.PC0X.IICB (L2A3, L2BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 5 or more I2C devices were enabled
        If (LGreater (L2DI, 4)) {
          Store (\_SB.PC0X.IICB (L2A4, L2BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 6 or more I2C devices were enabled
        If (LGreater (L2DI, 5)) {
          Store (\_SB.PC0X.IICB (L2A5, L2BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 7 or more I2C devices were enabled
        If (LGreater (L2DI, 6)) {
          Store (\_SB.PC0X.IICB (L2A6, L2BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 8 or more I2C devices were enabled
        If (LGreater (L2DI, 7)) {
          Store (\_SB.PC0X.IICB (L2A7, L2BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 9 or more I2C devices were enabled
        If (LGreater (L2DI, 8)) {
          Store (\_SB.PC0X.IICB (L2A8, L2BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 10 or more I2C devices were enabled
        If (LGreater (L2DI, 9)) {
          Store (\_SB.PC0X.IICB (L2A9, L2BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 11 or more I2C devices were enabled
        If (LGreater (L2DI, 10)) {
          Store (\_SB.PC0X.IICB (L2AA, L2BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 12 or more I2C devices were enabled
        If (LGreater (L2DI, 11)) {
          Store (\_SB.PC0X.IICB (L2AB, L2BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        Return (Local0)
    ...
    Method (_DSM, 4, NotSerialized) {  // _DSM: Device-Specific Method
      ...
      If (LNotEqual (\_SB.PC0X.DSMM (_UID), 0xFFFFFFFF)) { // If LNK2 is selected for LCH
        If (LEqual (Arg0,ToUUID ("02F55F0C-2E63-4F05-84F3-BF1980F9AF79"))) { /*DsmLch*/
          // Function 0 - returns bitmask of supported function.
          // BIT0 - indicates if any functions are supported
          // BIT1 - indicates Function 1 is Supported
  ...
  Device (LNK3) {       // definition for Camera sensor module using Link3
    ...
    Method (_DEP, 0, NotSerialized) {    // _DEP: Operation Region Dependencies
      If (L3EN) { // If LNK3 device is to be enabled
        If (LNotEqual (\_SB.PC0X.LNKC (_UID), 0)) {
          Return (\_SB.PC0X.LNKD (_UID))
        }
        Return (CDEP (L3CL))
    ...
    Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
    {
      ...
      If (LEqual (L3DI, 0)) {
        ...
        // Create I2C Bus resource template using as parameters data provided in the BIOS setup
        If (LGreater (L3DI, 0)) {
          Store (\_SB.PC0X.IICB (L3A0, L3BS), Local0)
        }
        // Add next resource buffer if 2 or more I2C devices were enabled
        If (LGreater (L3DI, 1)) {
          Store (\_SB.PC0X.IICB (L3A1, L3BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 3 or more I2C devices were enabled
        If (LGreater (L3DI, 2)) {
          Store (\_SB.PC0X.IICB (L3A2, L3BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 4 or more I2C devices were enabled
        If (LGreater (L3DI, 3)) {
          Store (\_SB.PC0X.IICB (L3A3, L3BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 5 or more I2C devices were enabled
        If (LGreater (L3DI, 4)) {
          Store (\_SB.PC0X.IICB (L3A4, L3BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 6 or more I2C devices were enabled
        If (LGreater (L3DI, 5)) {
          Store (\_SB.PC0X.IICB (L3A5, L3BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 7 or more I2C devices were enabled
        If (LGreater (L3DI, 6)) {
          Store (\_SB.PC0X.IICB (L3A6, L3BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 8 or more I2C devices were enabled
        If (LGreater (L3DI, 7)) {
          Store (\_SB.PC0X.IICB (L3A7, L3BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 9 or more I2C devices were enabled
        If (LGreater (L3DI, 8)) {
          Store (\_SB.PC0X.IICB (L3A8, L3BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 10 or more I2C devices were enabled
        If (LGreater (L3DI, 9)) {
          Store (\_SB.PC0X.IICB (L3A9, L3BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 11 or more I2C devices were enabled
        If (LGreater (L3DI, 10)) {
          Store (\_SB.PC0X.IICB (L3AA, L3BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 12 or more I2C devices were enabled
        If (LGreater (L3DI, 11)) {
          Store (\_SB.PC0X.IICB (L3AB, L3BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        Return (Local0)
    ...
    Method (SSDB)
    {
      ...
      Store (L3PC, Index (PAR, 100)) // Phy Configuration
      Store (L3LA, Index (PAR, 101)) // Lane Configuration
      Return (PAR)
    }
    ...
    Method (_DSM, 4, NotSerialized) {  // _DSM: Device-Specific Method
      ...
      If (LNotEqual (\_SB.PC0X.DSMM (_UID), 0xFFFFFFFF)) { // If LNK3 is selected for LCH
        If (LEqual (Arg0,ToUUID ("02F55F0C-2E63-4F05-84F3-BF1980F9AF79"))) { /*DsmLch*/
          // Function 0 - returns bitmask of supported function.
          // BIT0 - indicates if any functions are supported
          // BIT1 - indicates Function 1 is Supported
  ...
  Device (LNK4) {       // definition for Camera sensor module using Link4
    ...
    Method (_DEP, 0, NotSerialized) {    // _DEP: Operation Region Dependencies
      If (L4EN) {
        ...
        Return (CDEP (L4CL)) } // If LNK4 device is to be enabled
      Else { Return (Package () {\_SB.PC0X}) }
    }
    ...
    Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
    {
      ...
      If (LEqual (L4DI, 0)) {
        ...
        // Create I2C Bus resource template using as parameters data provided in the BIOS setup
        If (LGreater (L4DI, 0)) {
          Store (\_SB.PC0X.IICB (L4A0, L4BS), Local0)
        }
        // Add next resource buffer if 2 or more I2C devices were enabled
        If (LGreater (L4DI, 1)) {
          Store (\_SB.PC0X.IICB (L4A1, L4BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 3 or more I2C devices were enabled
        If (LGreater (L4DI, 2)) {
          Store (\_SB.PC0X.IICB (L4A2, L4BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 4 or more I2C devices were enabled
        If (LGreater (L4DI, 3)) {
          Store (\_SB.PC0X.IICB (L4A3, L4BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 5 or more I2C devices were enabled
        If (LGreater (L4DI, 4)) {
          Store (\_SB.PC0X.IICB (L4A4, L4BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 6 or more I2C devices were enabled
        If (LGreater (L4DI, 5)) {
          Store (\_SB.PC0X.IICB (L4A5, L4BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 7 or more I2C devices were enabled
        If (LGreater (L4DI, 6)) {
          Store (\_SB.PC0X.IICB (L4A6, L4BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 8 or more I2C devices were enabled
        If (LGreater (L4DI, 7)) {
          Store (\_SB.PC0X.IICB (L4A7, L4BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 9 or more I2C devices were enabled
        If (LGreater (L4DI, 8)) {
          Store (\_SB.PC0X.IICB (L4A8, L4BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 10 or more I2C devices were enabled
        If (LGreater (L4DI, 9)) {
          Store (\_SB.PC0X.IICB (L4A9, L4BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 11 or more I2C devices were enabled
        If (LGreater (L4DI, 10)) {
          Store (\_SB.PC0X.IICB (L4AA, L4BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 12 or more I2C devices were enabled
        If (LGreater (L4DI, 11)) {
          Store (\_SB.PC0X.IICB (L4AB, L4BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        Return (Local0)
    ...
    Method (SSDB)
    {
      ...
      Store (L4PC, Index (PAR, 100)) // Phy Configuration
      Store (L4LA, Index (PAR, 101)) // Lane Configuration
      Return (PAR)
    }
    ...
    Method (_DSM, 4, NotSerialized) {  // _DSM: Device-Specific Method
      ...
      If (LNotEqual (\_SB.PC0X.DSMM (_UID), 0xFFFFFFFF)) { // If LNK4 is selected for LCH
        If (LEqual (Arg0,ToUUID ("02F55F0C-2E63-4F05-84F3-BF1980F9AF79"))) { /*DsmLch*/
          // Function 0 - returns bitmask of supported function.
          // BIT0 - indicates if any functions are supported
          // BIT1 - indicates Function 1 is Supported
  ...
  Device (LNK5) {       // definition for Camera sensor module using Link5
    ...
    Method (_DEP, 0, NotSerialized) {  // _DEP: Operation Region Dependencies
      If (L5EN) {
        ...
        Return (CDEP (L5CL)) } // If LNK5 device is to be enabled
      Else { Return (Package () {\_SB.PC0X}) }
    }
    ...
    Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
    {
      ...
      If (LEqual (L5DI, 0)) {
        ...
        // Create I2C Bus resource template using as parameters data provided in the BIOS setup
        If (LGreater (L5DI, 0)) {
          Store (\_SB.PC0X.IICB (L5A0, L5BS), Local0)
        }
        // Add next resource buffer if 2 or more I2C devices were enabled
        If (LGreater (L5DI, 1)) {
          Store (\_SB.PC0X.IICB (L5A1, L5BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 3 or more I2C devices were enabled
        If (LGreater (L5DI, 2)) {
          Store (\_SB.PC0X.IICB (L5A2, L5BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 4 or more I2C devices were enabled
        If (LGreater (L5DI, 3)) {
          Store (\_SB.PC0X.IICB (L5A3, L5BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 5 or more I2C devices were enabled
        If (LGreater (L5DI, 4)) {
          Store (\_SB.PC0X.IICB (L5A4, L5BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 6 or more I2C devices were enabled
        If (LGreater (L5DI, 5)) {
          Store (\_SB.PC0X.IICB (L5A5, L5BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 7 or more I2C devices were enabled
        If (LGreater (L5DI, 6)) {
          Store (\_SB.PC0X.IICB (L5A6, L5BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 8 or more I2C devices were enabled
        If (LGreater (L5DI, 7)) {
          Store (\_SB.PC0X.IICB (L5A7, L5BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 9 or more I2C devices were enabled
        If (LGreater (L5DI, 8)) {
          Store (\_SB.PC0X.IICB (L5A8, L5BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 10 or more I2C devices were enabled
        If (LGreater (L5DI, 9)) {
          Store (\_SB.PC0X.IICB (L5A9, L5BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 11 or more I2C devices were enabled
        If (LGreater (L5DI, 10)) {
          Store (\_SB.PC0X.IICB (L5AA, L5BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        // Add next resource buffer if 12 or more I2C devices were enabled
        If (LGreater (L5DI, 11)) {
          Store (\_SB.PC0X.IICB (L5AB, L5BS), Local1)
          ConcatenateResTemplate (Local0, Local1, Local2)
          Store (Local2, Local0)
        }
        Return (Local0)
    ...
    Method (SSDB)
    {
      ...
      Store (L5PC, Index (PAR, 100)) // Phy Configuration
      Store (L5LA, Index (PAR, 101)) // Lane Configuration
      Return (PAR)
    }
    ...
    Method (_DSM, 4, NotSerialized) {  // _DSM: Device-Specific Method
      ...
      If (LNotEqual (\_SB.PC0X.DSMM (_UID), 0xFFFFFFFF)) { // If LNK5 is selected for LCH
        If (LEqual (Arg0,ToUUID ("02F55F0C-2E63-4F05-84F3-BF1980F9AF79"))) { /*DsmLch*/
          // Function 0 - returns bitmask of supported function.
          // BIT0 - indicates if any functions are supported
          // BIT1 - indicates Function 1 is Supported
  ...
  Device (FLM0) { // Definition for Camera Flash module using Link0
    ...
    Method (_DEP, 0, NotSerialized) {  // _DEP: Operation Region Dependencies
      If (L0EN) { // If LNK0 device is to be enabled
        If (LNotEqual (\_SB.PC0X.FLMC (_UID), 0)) {
          Return (\_SB.PC0X.FLMD (_UID))
        }
        Return (CDEP (L0CL))
      }
      Else {
        Return (Package () {\_SB.LNK0})
      }
    }
    Method (_HID, 0, NotSerialized) { // _HID: Hardware ID
      Return (FHCI (0))
    ...
    Method (_DSM, 4, NotSerialized) {  // _DSM: Device-Specific Method
      ...
      If ((Arg0 == ToUUID ("79234640-9e10-4fea-a5c1-b5aa8b19756f"))) { //DSDT_CAM_GPIO
        If ((Arg2 == One)) {
          If (LEqual (F0GT, 0x1)) { // If GPIO support enable give one GPIO PIN
            Return (0x01)
          }
          Return (0x00)             // If GPIO support disable give zero GPIO PIN
        }
        ...
        If ((Arg2 == 0x02)) {
          ...
          Return (GPPI (MIPICAM_GPIO_FUNCTION_FLASH_DRIVER_EN, GNUM (GPAD (F0CP, F0GG, F0GP)), F0IV, F0AV))
        }
      }
      ...
      If ((Arg0 == ToUUID ("DC49B5AA-E51D-4926-9B86-A6A908D8ACF3"))) {
        Return (EMUS)
      }
      Return (Buffer () {0})
    }
  ...
  Device (FLM1) { // Definition for Camera Flash module using Link1
    ...
    Method (_DSM, 4, NotSerialized) {  // _DSM: Device-Specific Method
      ...
      If ((Arg0 == ToUUID ("79234640-9e10-4fea-a5c1-b5aa8b19756f"))) { //DSDT_CAM_GPIO
        If ((Arg2 == One)) {
          If (LEqual (F1GT, 0x1)) { // If GPIO support enable give one GPIO PIN
            Return (0x01)
          }
          Return (0x00) // If GPIO support disable give zero GPIO PIN
        }
        ...
        If ((Arg2 == 0x02)) {
          //PinNo is not used by driver other than priting for log purpose, LKF needs 16 bit PinNo but due to existing strcuture compatibility making it as 8 bit
          Return (GPPI (MIPICAM_GPIO_FUNCTION_FLASH_DRIVER_EN, GNUM (GPAD (F1CP, F1GG, F1GP)), F1IV, F1AV))
        }
      }
      If ((Arg0 == ToUUID ("6665D1CE-20A3-40D0-99A1-E488DB5AF908"))) { //Flash Device ID
        // Function 0: Functions 1 is supported for revision 1
        If (LEqual (Arg2, 0)) { Return (Buffer() {0x3}) }
        // Function 1 (Returns Flash Id)
  ...
  Device (FLM2) { // Definition for Camera Flash module using Link2
    ...
    Method (_DSM, 4, NotSerialized) {  // _DSM: Device-Specific Method
      ...
      If ((Arg0 == ToUUID ("79234640-9e10-4fea-a5c1-b5aa8b19756f"))) { //DSDT_CAM_GPIO
        If ((Arg2 == One)) {
          If (LEqual (F2GT, 0x1)) { // If GPIO support enable give one GPIO PIN
            Return (0x01)
          }
          Return (0x00)             // If GPIO support disable give zero GPIO PIN
        }
        ...
        If ((Arg2 == 0x02)) {
          //PinNo is not used by driver other than priting for log purpose, LKF needs 16 bit PinNo but due to existing strcuture compatibility making it as 8 bit
          Return (GPPI (MIPICAM_GPIO_FUNCTION_FLASH_DRIVER_EN, GNUM (GPAD (F2CP, F2GG, F2GP)), F2IV, F2AV))
        }
      }
      If ((Arg0 == ToUUID ("6665D1CE-20A3-40D0-99A1-E488DB5AF908"))) { //Flash Device ID
        // Function 0: Functions 1 is supported for revision 0
      ...
      If ((Arg0 == ToUUID ("DC49B5AA-E51D-4926-9B86-A6A908D8ACF3"))) { //Flash Emulation Status
        Return (EMUS)
      }
      Return (Buffer () {0})
    }
  ...
  Device (FLM3) { // Definition for Camera Flash module using Link3
    ...
    Method (_DSM, 4, NotSerialized) {  // _DSM: Device-Specific Method
      ...
      If ((Arg0 == ToUUID ("79234640-9e10-4fea-a5c1-b5aa8b19756f"))) { //DSDT_CAM_GPIO
        If ((Arg2 == One)) {
          If (LEqual (F3GT, 0x1)) { // If GPIO support enable give one GPIO PIN
            Return (0x01)
          }
          Return (0x00)             // If GPIO support disable give zero GPIO PIN
        }
        ...
        If ((Arg2 == 0x02)) {
        //PinNo is not used by driver other than priting for log purpose, LKF needs 16 bit PinNo but due to existing strcuture compatibility making it as 8 bit
          Return (GPPI (MIPICAM_GPIO_FUNCTION_FLASH_DRIVER_EN, GNUM (GPAD (F3CP, F3GG, F3GP)), F3IV, F3AV))
        }
      }
      If ((Arg0 == ToUUID ("6665D1CE-20A3-40D0-99A1-E488DB5AF908"))) { //Flash Device ID
        // Function 0: Functions 1 is supported for revision 0
  ...
  Device (FLM4) { // Definition for Camera Flash module using Link4
    ...
    Name (_UID, 4)  // _UID: Unique ID - Link Index
    Method (_DEP, 0, NotSerialized) {  // _DEP: Operation Region Dependencies
      If (L4EN) { // If LNK4 device is to be enabled
        If (LNotEqual (\_SB.PC0X.FLMC (_UID), 0)) {
          Return (\_SB.PC0X.FLMD (_UID))
        }
        Return (CDEP (L4CL))
    ...
    Method (_DSM, 4, NotSerialized) {  // _DSM: Device-Specific Method
      ...
      If ((Arg0 == ToUUID ("79234640-9e10-4fea-a5c1-b5aa8b19756f"))) { //DSDT_CAM_GPIO
        If ((Arg2 == One)) {
          If (LEqual (F4GT, 0x1)) { // If GPIO support enable give one GPIO PIN
            Return (0x01)
          }
          Return (0x00)             // If GPIO support disable give zero GPIO PIN
        }
        ...
        If ((Arg2 == 0x02)) {
        //PinNo is not used by driver other than priting for log purpose, LKF needs 16 bit PinNo but due to existing strcuture compatibility making it as 8 bit
          Return (GPPI (MIPICAM_GPIO_FUNCTION_FLASH_DRIVER_EN, GNUM (GPAD (F4CP, F4GG, F4GP)), F4IV, F4AV))
        }
      }
      If ((Arg0 == ToUUID ("6665D1CE-20A3-40D0-99A1-E488DB5AF908"))) { //Flash Device ID
        // Function 0: Functions 1 is supported for revision 0
  ...
  Device (FLM5) { // Definition for Camera Flash module using Link5
    ...
    Name (_UID, 5)  // _UID: Unique ID - Link Index
    Method (_DEP, 0, NotSerialized) {  // _DEP: Operation Region Dependencies
      If (L5EN) { // If LNK5 device is to be enabled
        If (LNotEqual (\_SB.PC0X.FLMC (_UID), 0)) {
          Return (\_SB.PC0X.FLMD (_UID))
        }
        Return (CDEP (L5CL))
    ...
    Method (_DSM, 4, NotSerialized) {  // _DSM: Device-Specific Method
      ...
      If ((Arg0 == ToUUID ("79234640-9e10-4fea-a5c1-b5aa8b19756f"))) { //DSDT_CAM_GPIO
        If ((Arg2 == One)) {
          If (LEqual (F5GT, 0x1)) { // If GPIO support enable give one GPIO PIN
            Return (0x01)
          }
          Return (0x00)             // If GPIO support disable give zero GPIO PIN
        }
        ...
        If ((Arg2 == 0x02)) {
        //PinNo is not used by driver other than priting for log purpose, LKF needs 16 bit PinNo but due to existing strcuture compatibility making it as 8 bit
          Return (GPPI (MIPICAM_GPIO_FUNCTION_FLASH_DRIVER_EN, GNUM (GPAD (F5CP, F5GG, F5GP)), F5IV, F5AV))
        }
      }
      If ((Arg0 == ToUUID ("6665D1CE-20A3-40D0-99A1-E488DB5AF908"))) { //Flash Device ID
        // Function 0: Functions 1 is supported for revision 0
