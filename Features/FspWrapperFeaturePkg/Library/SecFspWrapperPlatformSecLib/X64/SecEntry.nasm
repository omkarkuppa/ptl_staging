...
extern   ASM_PFX(PcdGet32 (PcdFspWrapperPcdBootGuardCnbSize))
...
CallSecFspInit:
  ...
  ;  +                             +
  ;  |   . . . . . . . . . . . .   |
  ;  +=============================+  <-- BIOS usage Data Cache Base (PcdTemporaryRamBase)
  ;  |         CNB buffer          |
  ;  +#############################+  <-- Original Data Cache Base = CNB Base
  ;
  ;
  mov     rsp, rdx
  push    rdx
  ...
  rdmsr
  and     eax, 01h
  jz      NoBootGuardNemSetup
  mov     rax, ASM_PFX(PcdGet32 (PcdFspWrapperPcdBootGuardCnbSize))
  mov     rax, [rax]
  pop     rcx
  add     rcx, rax
  pop     rdx
  jmp     BootGuardNemSetupDone
NoBootGuardNemSetup:
  pop     rcx
  pop     rdx
BootGuardNemSetupDone:
