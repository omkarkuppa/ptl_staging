/**@file
  Unit test main of the check Fsp Error Info Hob driver.
...
//
#include "TestCase/CheckFspErrorInfoHobTestCase.h"

#define UNIT_TEST_NAME     "CheckFspErrorInfoHob Unit Test"
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestCheckFspErrorInfoHob;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestCheckFspErrorInfoHob,  // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestCheckFspErrorInfoHob,             // SuiteHandle        : Unit test suite to add test to
