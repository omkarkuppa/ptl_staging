/**@file
  Unit test main of the Fsp Wrapper Peim driver.
...
//
#include "TestCase/FspWrapperPeimTestCase.h"

#define UNIT_TEST_NAME     "FspWrapperPeim Unit Test"
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestFspWrapperPeim;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestFspWrapperPeim,  // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestFspWrapperPeim,                    // SuiteHandle        : Unit test suite to add test to
