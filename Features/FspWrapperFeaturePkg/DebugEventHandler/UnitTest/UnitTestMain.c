...
//
#include "TestCase/DebugEventHandlerTestCase.h"

#define UNIT_TEST_NAME     "DebugEventHandler Unit Test"
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestDebugEventHandler;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestDebugEventHandler,     // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestDebugEventHandler,                // SuiteHandle        : Unit test suite to add test to
