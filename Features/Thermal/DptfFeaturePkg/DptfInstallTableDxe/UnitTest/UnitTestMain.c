/**@file
    Unit test main of the Dptf Install Table Dxe driver.
...
#define UNIT_TEST_NAME     "DptfInstallTableDxe Unit Test"
...
UNIT_TEST_CASE_STRUCT mUnitTestCasesStruct [] = {
  { "Test-InitializeDptf",            "Test Initialize Dptf",     UnitTestInitializeDptf,        PreInitializeDptf,                     CleanUpInitializeDptf,               NULL,           0,            0},
  { "Test-LoadAcpiTables",            "Test Load AcpiTables",     UnitTestLoadAcpiTables,        PreLoadAcpiTables,                     CleanUpLoadAcpiTables,               NULL,           0,            0},
};
/**
  ...
  )
{
  ...
  UNIT_TEST_SUITE_HANDLE        UnitTestDptfInstallTableDxe;
  UINTN                         Index;
  ...
  Status = CreateUnitTestSuite (
             &UnitTestDptfInstallTableDxe,  // SuiteHandle  : Unit test suite to create
  ...
  for (Index = 0; Index < ARRAY_SIZE (mUnitTestCasesStruct); Index++) {
    Status = AddTestCase (
               UnitTestDptfInstallTableDxe,              // SuiteHandle        : Unit test suite to add test to
