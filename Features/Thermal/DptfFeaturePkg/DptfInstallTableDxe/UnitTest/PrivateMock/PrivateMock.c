  ...
  )
{
  ...
  CopyMem (mock_ptr_type (VOID*), Data, DataSize);
  return mock_type (EFI_STATUS);
}
  ...
  )
{
  ...
  *FileSize = ftell (FilePointer);
  ...
  AcpiTableFile  = malloc (*FileSize);
  if (AcpiTableFile == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  fseek (FilePointer, 0L, SEEK_SET);
  fread (AcpiTableFile, *FileSize, 1, FilePointer);

  for (UINTN Index = 0; Index < *FileSize; Index++) {
    AcpiTableFile[Index] &= 0xFF;
  }

  *FileData = AcpiTableFile;
  fclose (FilePointer);
  ...
  )
{
  ...
  check_expected_ptr (This);
  check_expected_ptr (AcpiTableBuffer);
  check_expected (AcpiTableBufferSize);
  check_expected_ptr (TableKey);
  
  ReturnTable = AllocateZeroPool (AcpiTableBufferSize);
  CopyMem (ReturnTable, AcpiTableBuffer, AcpiTableBufferSize);
  *mock_ptr_type (VOID**) = ReturnTable;
