  ...
  )
{
  ...
  if ((UINTN *) (NhltTable + NhltTableSize) < (UINTN *) (Endpoint)) {
    return NULL;
  }
  ...
  )
{
  ...
  Temp = ((UINTN)(UINT8*) (&(Endpoint->EndpointConfig)))
                                      + sizeof (Endpoint->EndpointConfig.CapabilitiesSize);
  FormatsConfig = (FORMATS_CONFIG*) (Temp + Endpoint->EndpointConfig.CapabilitiesSize);
...
GetNhltEndpointDeviceInfo (
  IN CONST ENDPOINT_DESCRIPTOR  *Endpoint,
  IN CONST UINT32               DeviceInfoIndex
  )
{
  DEVICES_INFO  *DevicesInfo;
  ...
  )
{
  ...
  Endpoint = GetNhltEndpoint (NhltTable, (NhltTable->EndpointCount), NhltTable->Header.Length);
  ...
  if ((UINTN *) (NhltTable + NhltTableSize) < (UINTN *) OedConfig) {
    return NULL;
  }
  ...
  )
{
  ...
  if(Endpoint == NULL) {
    DEBUG ((DEBUG_INFO, "Endpoint is NULL \n"));
    return;
  }
  ...
  )
{
  ...
  if (OedConfig == NULL) {
    DEBUG((DEBUG_INFO, "OedConfig Table is null and cannot be printed. \n"));
  }
  else {
  ...
  )
{
  ...
  if (NhltTable == NULL)
  {
    DEBUG ((DEBUG_INFO, "Nhlt Table is null and cannot be printed. \n"));
  } else {
    DEBUG ((DEBUG_INFO, "\n"));
    ...
    for (Index = 0; Index < NhltTable->EndpointCount; Index++) {
      NhltEndpointDump (GetNhltEndpoint (NhltTable, Index, NhltTable->Header.Length));
    }

    NhltOedConfigDump (GetNhltOedConfig (NhltTable, NhltTable->Header.Length));
    DEBUG ((DEBUG_INFO, "----------------------------------------------------------------------\n"));
  }
