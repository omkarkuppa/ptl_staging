  ...
  )
{
  ...
  UINT8 *SioAcpiTableFile;
  CHAR8 *AbsoluteFileName;
  INTN  Result;
  UINTN FileLength;

  AbsoluteFileName = AllocateZeroPool (AsciiStrSize (__FILE__) + AsciiStrSize (FileName));
  if (AbsoluteFileName == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  CopyMem (AbsoluteFileName, __FILE__, AsciiStrSize (__FILE__));
  for (UINTN Index = 0; AbsoluteFileName[Index] != '\0'; Index++) {
    if (AbsoluteFileName[Index] == '\\') {
      AbsoluteFileName[Index] = '/';
    }
  }
  CopyMem (AsciiStrStr (AbsoluteFileName, "Intel/Features"), FileName, AsciiStrSize (FileName));

  FilePointer = fopen (AbsoluteFileName, "rb");
  if (FilePointer == NULL) {
    FreePool (AbsoluteFileName);
  ...
  SioAcpiTableFile  = AllocateZeroPool (FileLength);
  if (SioAcpiTableFile == NULL) {
    fclose (FilePointer);
    return EFI_OUT_OF_RESOURCES;
  }

  fseek (FilePointer, 0L, SEEK_SET);
  FileLength = fread (SioAcpiTableFile, sizeof (UINT8), FileLength, FilePointer);

  for (UINTN Index = 0; Index < FileLength; Index++) {
    SioAcpiTableFile[Index] &= 0xFF;
  }

  *FileData = SioAcpiTableFile;
  *FileSize = FileLength;
  fclose (FilePointer);
...
/**
  ...
  @param[in]      NameGuid              Pointer to an EFI_GUID, which is the file
                                        name. All firmware file names are EFI_GUIDs.
                                        A single firmware volume must not have two
                                        valid files with the same file name
                                        EFI_GUID.

  @param[in,out]  Buffer                Pointer to a pointer to a buffer in which the
                                        file contents are returned, not including the
                                        file header.

  @param[in,out]  BufferSize            Pointer to a caller-allocated UINTN. It
                                        indicates the size of the memory
                                        represented by Buffer.

  @param[out]     FoundType             Pointer to a caller-allocated EFI_FV_FILETYPE.

  @param[out]     FileAttributes        Pointer to a  caller-allocated
                                        EFI_FV_FILE_ATTRIBUTES.

  @param[out]     AuthenticationStatus  Pointer to a caller-allocated
                                        UINT32 in which the
...
/**
  ...
  @param[in] NameGuid                 Pointer to an EFI_GUID, which indicates the
                                      file name from which the requested section
                                      will be read.

  @param[in] SectionType              Indicates the section type to return.
                                      SectionType in conjunction with
                                      SectionInstance indicates which section to
                                      return.

  @param[in] SectionInstance          Indicates which instance of sections
...
/**
  ...
  @param[in] AcpiTableBuffer      A pointer to a buffer containing the
                                  ACPI table to be installed.

  @param[in] AcpiTableBufferSize  Specifies the size, in bytes, of
...
{
  VOID *UnitTestSioAcpiTable;

  check_expected (This);
  check_expected (AcpiTableBuffer);
  check_expected (AcpiTableBufferSize);

  UnitTestSioAcpiTable = AllocatePool (AcpiTableBufferSize);
  ZeroMem (UnitTestSioAcpiTable, AcpiTableBufferSize);
  CopyMem (UnitTestSioAcpiTable, AcpiTableBuffer, AcpiTableBufferSize);

  *mock_ptr_type (VOID**) = UnitTestSioAcpiTable;
