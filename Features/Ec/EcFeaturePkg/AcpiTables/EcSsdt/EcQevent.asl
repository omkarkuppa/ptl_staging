...
External (\_SB.UBTC.MI00, IntObj)
External (\_SB.UBTC.MI01, IntObj)
External (\_SB.UBTC.MI02, IntObj)
External (\_SB.UBTC.MI03, IntObj)
External (\_SB.UBTC.MI04, IntObj)
External (\_SB.UBTC.MI05, IntObj)
External (\_SB.UBTC.MI06, IntObj)
External (\_SB.UBTC.MI07, IntObj)
External (\_SB.UBTC.MI08, IntObj)
External (\_SB.UBTC.MI09, IntObj)
External (\_SB.UBTC.MI0A, IntObj)
External (\_SB.UBTC.MI0B, IntObj)
External (\_SB.UBTC.MI0C, IntObj)
External (\_SB.UBTC.MI0D, IntObj)
External (\_SB.UBTC.MI0E, IntObj)
External (\_SB.UBTC.MI0F, IntObj)
External (\_SB.UBTC.MI10, IntObj)
External (\_SB.UBTC.MI11, IntObj)
External (\_SB.UBTC.MI12, IntObj)
External (\_SB.UBTC.MI13, IntObj)
...
External (\_SB.DPTF.DP30, MethodObj)
External (\_SB.DPTF.DP31, MethodObj)
External (\_SB.DPTF.DP32, MethodObj)
External (\_SB.DPTF.DP33, MethodObj)
External (\_SB.DPTF.DP34, MethodObj)
External (\_SB.DPTF.DP35, MethodObj)
External (\_SB.DPTF.DP37, MethodObj)
External (\_SB.DPTF.DP38, MethodObj)
External (\_SB.DPTF.DP39, MethodObj)
External (\_SB.DPTF.DP3A, MethodObj)
External (\_SB.DPTF.DTTV, MethodObj)
External (\_SB.DPTF.DTTI, MethodObj)

External (\UPIS, MethodObj)
...
{
  ADBG ("Q32, Real Battery Capacity Change Event!")
  // Perform needed ACPI Notifications.
  If (CondRefOf (\_SB.DPTF)) { // Check if DPTF Opregion is present or disabled by BIOS Setup menu
    \_SB.DPTF.DP32 ()
  }
...
Method (_Q33)    // Real Battery Insertion/Removal Event.
{
  ...
  ADBG ("_Q33 Battery Change Event!")
...
{
  ADBG ("Q34, PMAX changed by 250mw!")
  If (CondRefOf (\_SB.DPTF)) { // Check if DPTF Opregion is present or disabled by BIOS Setup menu
    \_SB.DPTF.DP34 ()
  }
}
...
{
  ADBG ("Q34, PMAX changed by 100mw!")
  If (CondRefOf (\_SB.DPTF)) { // Check if DPTF Opregion is present or disabled by BIOS Setup menu
    \_SB.DPTF.DP35 ()
  }
}
...
{
  ADBG ("Q38, RBHF value is changed over a threshold, 5mOhm.")
  If (CondRefOf (\_SB.DPTF)) { // Check if DPTF Opregion is present or disabled by BIOS Setup menu
    \_SB.DPTF.DP38 ()
  }
}
...
{
  ADBG ("Q39, VBNL value is changed over a threshold, for example, 50mV.")
  If (CondRefOf (\_SB.DPTF)) { // Check if DPTF Opregion is present or disabled by BIOS Setup menu
    \_SB.DPTF.DP39 ()
  }
}
...
{
  ADBG ("Q3A, CMPP value value is changed over a threshold, for example,  100mA.")
  If (CondRefOf (\_SB.DPTF)) { // Check if DPTF Opregion is present or disabled by BIOS Setup menu
    \_SB.DPTF.DP3A ()
  }
}
...
{
  ADBG ("Q44, Virtual Dock Presence Changed!")
  If (\DSTS)
  {
...
Method (UCEV, 0, Serialized)    // Any event on USBTypeC.
{
  ADBG ("EC USBC Event")
  Store ( Timer (), Local0 )
  Store ( MI00, \_SB.UBTC.MI00 )
  Store ( MI01, \_SB.UBTC.MI01 )
  Store ( MI02, \_SB.UBTC.MI02 )
  Store ( MI03, \_SB.UBTC.MI03 )
  Store ( MI04, \_SB.UBTC.MI04 )
  Store ( MI05, \_SB.UBTC.MI05 )
  Store ( MI06, \_SB.UBTC.MI06 )
  Store ( MI07, \_SB.UBTC.MI07 )
  Store ( MI08, \_SB.UBTC.MI08 )
  Store ( MI09, \_SB.UBTC.MI09 )
  Store ( MI0A, \_SB.UBTC.MI0A )
  Store ( MI0B, \_SB.UBTC.MI0B )
  Store ( MI0C, \_SB.UBTC.MI0C )
  Store ( MI0D, \_SB.UBTC.MI0D )
...
{
  ADBG ("Q56, Hot-Key Event (CAS+XXX)!")
  //Return if Scan Matrix Keyboard Support disabled
  If (LEqual (\SCMK, 0))
  {
    Return
  ...
  ElseIf (LEqual (Local0, 0x1F)) // cycle _PUR
  {
    ...
    If (CondRefOf (\_SB.PC00.TCPU.SPUR)) {
...
//
  ...
  {
    ADBG ("CAS+N")
    // Notify Intel(R) Dynamic Tuning Technology for end to end testing of a fail safe mechanism.
    If (CondRefOf (\_SB.DPTF)) { // Check if DPTF Opregion is present or disabled by BIOS Setup menu
      \_SB.DPTF.DTTV ()
    }
  }
  ...
  {
    ADBG ("CAS+M")
    If (CondRefOf (\_SB.PC00.TCPU.MAXT)) {
...
{
  ADBG ("QD5, 10s EC PB press")
  \_SB.PWPR ()
}

Method (_QD6)    // 10 second power button de-press.
{
  ADBG ("QD6, 10s EC PB release")
  \_SB.PWRR ()
}

Method (_Q80)    // Volume Up
{
  ADBG ("Q80, Volume Up Press!")
  If (LEqual (VBVP, 1))
  {
    ADBG ("Volume Up")
    // Volume Up.
...
{
  ADBG ("Q82, Platform Type Event - Tablet Mode!")
...
{
  ADBG ("Q83, Platform Type Event - Laptop Mode!")
  If (LEqual (SMSS, 1)) // SDS, PantherMtn, GrizzlyMtn or StarBrook
  {
    // Rotation Lock button
    // If VirtualButton driver loaded
...
{
  ADBG ("Q85, Windows Home Button presee!")
  If (LEqual (VBHB, 1))
  {
    // Windows Home Button
    // If VirtualButton driver loaded
...
{
  ADBG ("Q8A, Windows Home Button Release!")
  If (LEqual (VBHB, 1))
  {
...
{
  ADBG ("Q8B, Rotation Lock Release!")
  If (LEqual (VBRL, 1)) {
...
Method (_Q53)
{
  ADBG ("Q53, Virtual Battery Switch Event!")
...
Method (_QF1)
{
  ...
  ADBG ("QF1, Thermal Sensor Threshold!")
