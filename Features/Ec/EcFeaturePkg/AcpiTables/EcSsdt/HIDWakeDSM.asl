  ...
  If (LEqual (Arg0, ToUUID ("1730E71D-E5DD-4A34-BE57-4D76B6A2FE37")))
  {
    ...
    If (LEqual (Arg2, One))
    {
      Switch (ToInteger (DeRefOf (Index (Arg3, 0))))
      {
        ...
        {
          If (CondRefOf (\_SB.PARENT_OF_LPCB.LPCB.H_EC.UPBT))
          {
            \_SB.PARENT_OF_LPCB.LPCB.H_EC.UPBT (0x80, One) // Arg0:Bit7 - Dock Indicator change request, Arg1:One - Set the target bit.
          }
        }
        Case (3) // Send Undock Event
        {
          If (CondRefOf (\_SB.PARENT_OF_LPCB.LPCB.H_EC.UPBT))
          {
            \_SB.PARENT_OF_LPCB.LPCB.H_EC.UPBT (0x80, Zero) // Arg0:Bit7 - Dock Indicator change request, Arg1:Zero - Clear the target bit.
          }
        }
      }
      Return (0)
