...
Method (BTNL, 0, Serialized) // HID Button Enable/Disable Method
{
  ...
  \_SB.PARENT_OF_LPCB.LPCB.H_EC.CBTS (BTS1) //Enable/Disable SCIs from buttons
}
...
{
  If (LEqual (\AEAB, 1)) {
    Store (\_SB.PARENT_OF_LPCB.LPCB.H_EC.RBED (), BTS1) //Button Enable/Disable field in ACPI name space
  }
  Return (BTS1)
}
