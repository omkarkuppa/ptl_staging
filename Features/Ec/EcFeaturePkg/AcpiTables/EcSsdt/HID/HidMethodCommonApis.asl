...
External (\_SB.CBTS, MethodObj)
External (\_SB.RBED, MethodObj)
...
Method (BTNL, 0, Serialized) // HID Button Enable/Disable Method
{
  ...
  If (CondRefOf (\_SB.CBTS)) {
    \_SB.CBTS (BTS1)    // Enable/Disable SCIs from buttons
  }
}
...
{
  If (LEqual (\AEAB, 1)) {
    If (CondRefOf (\_SB.RBED)) {
      Store (\_SB.RBED (), BTS1)    //Button Enable/Disable field in ACPI name space
    }
  }
  Return (BTS1)
}
  ...
  If (LEqual (Arg0, ToUUID ("EEEC56B3-4442-408F-A792-4EDD4D758054")))
  {
    If (LEqual (1, ToInteger (Arg1)))        // Revision 1.
    {
      Switch (ToInteger (Arg2))            // Switch to Function Index.
      {
        ...
        Case (8)
        {
          Return (\_SB.PARENT_OF_LPCB.LPCB.H_EC.VGBS ())
        }
        //
