  ...
  )
{
  ...
  *FileSize = ftell (FilePointer);
  ...
  EcAcpiTableFile  = malloc (*FileSize);
  if (EcAcpiTableFile == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  fseek (FilePointer, 0L, SEEK_SET);
  fread (EcAcpiTableFile, *FileSize, 1, FilePointer);

  for (UINTN Index = 0; Index < *FileSize; Index++) {
    EcAcpiTableFile[Index] &= 0xFF;
  }

  *FileData = EcAcpiTableFile;
  fclose (FilePointer);
