/** @file
  Clean-up functions of the UpdateMiscSmbiosCallback function.
...
@par Specification
**/
#include "CleanUpUpdateMiscSmbiosCallback.h"
...
/**
  Clean up all variables used in testing UpdateMiscSmbiosCallback function.
...
EFI_STATUS
EFIAPI
CleanUpdateMiscSmbiosCallbackEnvironment(
  )
{
  UINTN Index;
  FreePool (gBS);
...
UNIT_TEST_STATUS
EFIAPI
CleanUpUpdateMiscSmbiosCallback (
  ...
  )
{
  ...
  Status = CleanUpdateMiscSmbiosCallbackEnvironment ();
  if (EFI_ERROR (Status)) {
