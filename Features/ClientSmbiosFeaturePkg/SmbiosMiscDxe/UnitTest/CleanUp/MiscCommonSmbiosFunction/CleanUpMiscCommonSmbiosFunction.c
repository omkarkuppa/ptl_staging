/** @file
  Clean-up functions of the MiscCommonSmbiosFunction function.
...
@par Specification
**/
#include "CleanUpMiscCommonSmbiosFunction.h"
#include <Library/DebugLib.h>
#include <Library/MemoryAllocationLib.h>
#include <PrivateMock.h>

/**
  Clean up all variables used in testing MiscCommonSmbiosFunction function.
...
EFI_STATUS
EFIAPI
CleanMiscCommonSmbiosFunctionEnvironment(
  )
{
  UINTN Index;
  FreePool (gSmbiosProtocol);
  for (Index = 0; Index < gSmbiosTableIndex; Index++) {
    if (gSmbiosTable[Index] == NULL) {
      break;
    }
...
UNIT_TEST_STATUS
EFIAPI
CleanUpMiscCommonSmbiosFunction (
  ...
  )
{
  ...
  Status = CleanMiscCommonSmbiosFunctionEnvironment ();
