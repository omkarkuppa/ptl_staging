  ...
  )
{
  ...
  //
  // Check for gVerifyTable content.
  //
  if (gVerifyTable != NULL) {
    DEBUG ((DEBUG_ERROR, "gVerifyTable is not NULL!!\n"));
    return EFI_OUT_OF_RESOURCES;
  }

  gVerifyTable = AllocateZeroPool (AcpiTableBufferSize);
  if (gVerifyTable == NULL) {
    DEBUG ((DEBUG_ERROR, "Can't locate memory for gVerifyTable!!\n"));
    return EFI_OUT_OF_RESOURCES;
  }
  //
  ...
  CopyMem (gVerifyTable, AcpiTableBuffer, AcpiTableBufferSize);
  ...
  )
{
  ...
  Status = MockPmaxAcpiTableFile ((UINT8**) &Table, &PmaxBufferSize);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Can't mock Pmax ACPI table!!\n"));
    return Status;
  }
  //
  // Mock boot service
  ...
  )
{
  ...
  Status = SetupInstallTableEnvironment ();
  UT_ASSERT_NOT_EFI_ERROR (Status);

  //
  // Running platform function
  //
  Status = InstallTable (Table);
