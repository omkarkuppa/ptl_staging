  ...
  )
{
  ...
  DataSize = HidKbDev->OutLedReportSize / 8;
  ...
  while (!IsNull (Link, Cur)) {
    ...
    if (ReportItem->UsagePage == BT_HID_LED_USAGE_PAGE) {
      ...
      Max = (ReportItem->UsageMax < (NUM_LED_KEYS - 1)) ? ReportItem->UsageMax : (NUM_LED_KEYS - 1);
      for (Index = ReportItem->UsageMin; Index <= Max; Index++) {
        if (ByteIndex >= DataSize) {
          DEBUG((DEBUG_ERROR, "Blekb: Index %u >= allocated memory %u\r\n", ByteIndex, DataSize));
          break;
        }
