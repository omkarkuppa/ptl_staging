  ...
  )
{
  ...
  if (!(HidInfo.HidType & HidTypeMouse)) {
    DEBUG ((EFI_D_ERROR, "HidMouseDriverBindingSupported: Not a Mouse\n"));
#else
  if (HidInfo.HidType != HidTypeMouse) {
#endif
...
#ifdef UEFI_COMBO_DEVICE_SUPPORTED
  Status = HidProtocol->Start (HidProtocol, NULL, NULL, OnMouseInterruptComplete, BtMouseDevice, HidInfo);
#else
