...
/* Restricting to number of devices display*/
UINT8  NO_OF_DEVICES_SEARCH_APP;
#define MAX_DEV_SEARCH_APP  30
...
read_attr (
  ...
  UINT16                                     end_handle,
  uuid_t                                     *uuid,
  EFI_BLUETOOTH_ATTRIBUTE_CALLBACK_FUNCTION  callback,
  VOID                                       *context
  );
...
static uuid_t *
uefi_uuid_to_ile_uuid (
  uuid_t              *ile_uuid,
  EFI_BLUETOOTH_UUID  *uefi_uuid
  )
{
...
static EFI_BLUETOOTH_UUID *
ile_uuid_to_uefi_uuid (
  EFI_BLUETOOTH_UUID  *uefi_uuid,
  uuid_t              *ile_uuid
  )
{
...
static EFI_BLUETOOTH_GATT_PRIMARY_SERVICE_INFO *
get_owner_prim_srv (
  UINT8   conn_id,
  UINT16  att_handle
  )
...
{
  /* Check for No of devices not more than MAX_DEV_SEARCH_APP,
     So that can aviod many advertisements, then scan hang won't happen.
     Also this macro is configurable */
  if (NO_OF_DEVICES_SEARCH_APP >= MAX_DEV_SEARCH_APP) {
    iLE_stop_scan (0);
    return;
  }

  NO_OF_DEVICES_SEARCH_APP++;       /* Increment for every device found*/
  ...
  )
{
  ...
  /* Set NO_OF_DEVICES_SEARCH_APP to 0 for every scan completes */
  NO_OF_DEVICES_SEARCH_APP = 0;
  ...
  )
{
  ...
  if (data && (data_len > 0)) {
    ...
    if (!hid_info->data ) {
      loge ("AllocateZeroPool failed of size:%d bytes", data_len);
      return;
    }
 ...
 */
static EFI_BLUETOOTH_GATT_CHARACTERISTIC_INFO *
find_charateristic_from_uuid (
  UINT8   conn_id,
  UINT16  uuid16,
  UINTN   start_index,
  UINT16  prev_handle,
 ...
 */
static EFI_BLUETOOTH_GATT_CHARACTERISTIC_DESCRIPTOR_INFO *
find_descriptor_from_uuid (
  UINT8   conn_id,
  UINT16  uuid16,
  UINTN   start_index
  )
  ...
  )
{
  ...
  if (char_val != NULL) {
    ...
    store_hid_info_to_list (
                            conn_id,
                            char_val->CharacteristicValueHandle,
                            BluetoothGattTypeCharacteristic,
                            BluetoothGattTypeHidReport,
                            char_val->CharacteristicProperties,
                            NULL,
                            0
                            );
    ...
    if (desc_val != NULL) {
      ...
      store_hid_info_to_list (
                              conn_id,
                              desc_val->Header.AttributeHandle,
  ...
  )
{
  ...
  InitializeListHead (&uefi_app_data.remote_device_list[conn_id].hid_info);
  if (uefi_app_data.remote_device_list[conn_id].BtLeSbDev != NULL) {
  ...
  )
{
  ...
  if ((opcode == ATT_OPCODE_PREP_WRITE_RESP) && prep_wr_value) {
    ...
    if (iLE_write_attribute_value (
                                   new_trans_id,
                                   conn_id,
                                   ATT_OPCODE_PREP_WRITE_REQ,
                                   att_id,
                                   prep_wr_value->offset,
                                   out_buffer
                                   ) == STATUS_SUCCESS)
    {
  ...
  )
{
  ...
  switch (key_type) {
    ...
    case PEER_IRK:
    {
      ...
      data_size = 0;
      data_type = EfiBluetoothSmpPeerAddressList;
...
void
uefi_app_register_application_cb (
  UINT8    app_handle,
  BOOLEAN  result
  )
{
...
stack_scan (
  UINT32                                  Timeout,
  EFI_BLUETOOTH_LE_CONFIG_SCAN_PARAMETER  *ScanParameter
  )
{
  BOOLEAN     active         = TRUE;
  BOOLEAN     limited        = FALSE;
  BOOLEAN     use_rejectlist = FALSE;
  ...
  if (ScanParameter) {
    ...
    DEBUG ((EFI_D_INFO, "scan parameters updated through API\n"));
    active         = ScanParameter->ScanType ? FALSE : TRUE;
    limited        = ScanParameter->AdvertisementFlagFilter & 1 ? TRUE : FALSE;
    use_rejectlist = ScanParameter->ScanningFilterPolicy & 1 ? TRUE : FALSE;
    scan_interval  = ScanParameter->ScanInterval;
    scan_window    = ScanParameter->ScanWindow;
  } else {
    DEBUG ((EFI_D_INFO, "scan parameters set to default\n"));

    /* if ScanParameter is NULL from upper API's, then the values (scan_interval, scan_window)
       are configured here.*/
...
stack_connect (
  BLUETOOTH_LE_ADDRESS                       *bd_addr,
  EFI_BLUETOOTH_LE_CONFIG_CONNECT_PARAMETER  *conn_params,
  ...
  switch (write_type) {
    ...
    case reliable_write:
      ...
      if ((data_size > (mtu = iLE_get_att_mtu (new_trans_id, conn_id)) - 3) || (write_type == reliable_write)) {
        ...
        if (uefi_app_data.remote_device_list[conn_id].prep_wr_value == NULL) {
          return EFI_OUT_OF_RESOURCES;
        }
...
EFI_STATUS
stack_register_notification_indication (
  UINT8                                      conn_id,
  UINT16                                     Handle,
...
EFI_STATUS
stack_get_local_data (
  EFI_BLUETOOTH_CONFIG_DATA_TYPE  data_type,
  UINTN                           *data_size,
  VOID                            *data
  )
{
...
stack_smp_auth (
  BLUETOOTH_LE_ADDRESS                  *bd_addr,
  EFI_BLUETOOTH_LE_SMP_EVENT_DATA_TYPE  data_type,
  UINTN                                 length,
  VOID                                  *data
  )
{
  ...
  switch (data_type) {
    case EfiBluetoothSmpAuthorizationRequestEvent:
      status = iLE_send_authorization (
                                       new_trans_id,
                                       bd_addr->Address,
                                       bd_addr->Type,
                                       *((UINT8 *)data),
                                       uefi_app_data.authreq,
                                       uefi_app_data.enckeysize
                                       );
      break;
