  ...
  )
{
  ...
  for (idx = 0; idx < mem_pool_header[pool_id].num_of_elem; idx++) {
    if (mem_pool_header[pool_id].usage_bitmap[idx] != 0xFF) {
      /* This one has a free space */
      ...
      for (offset = 0; offset < max_bit_offset; offset++) {
        if (((~mem_pool_header[pool_id].usage_bitmap[idx]) & (1 << offset)) == (1 << offset)) {
          logd (
                "Got a free space. bit offset: %d mem_pool_header[%d].usage_bitmap[idx]:%x",
                offset,
                pool_id,
                mem_pool_header[pool_id].usage_bitmap[idx]
                );
  ...
  )
{
  ...
  for (pool_id = 0; pool_id < MEM_BUF_POOL_MAX_ID; pool_id++) {
    logd (
          "mem_pool_header[%d].base_ptr:%p, mem_pool_header[%d].end_ptr:%p mem_pool_header[%d].elem_len:%u",
          pool_id,
          mem_pool_header[pool_id].base_ptr,
          pool_id,
          mem_pool_header[pool_id].end_ptr,
          pool_id,
          mem_pool_header[pool_id].elem_len
          );
  }
}
