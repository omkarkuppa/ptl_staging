  ...
  )
{
  ...
  for (idx = 0; idx < MAX_NUM_CONNECTED_DEVICE; idx++) {
    if (g_local_adapter.connected_device[idx]) {
      if (memcmp (
                  g_local_adapter.connected_device[idx]->id_addr.bdaddr,
                  bdaddr,
                  BD_ADDR_LEN
                  ) == 0)
      {
  ...
  )
{
  ...
  if (g_local_adapter.gap_data.autoconn_dev[slot] == NULL) {
    return;
  }
  ...
  )
{
  ...
  if (idx != MAX_NUM_CONNECTED_DEVICE) {
    ...
    gap_remove_from_rejectlist (g_local_adapter.gap_data.autoconn_dev[idx]->idaddr_type, addr);
