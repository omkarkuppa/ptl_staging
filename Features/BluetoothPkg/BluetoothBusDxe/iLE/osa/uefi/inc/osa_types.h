...
typedef void (*osa_timeout_func_t)(
  osa_timer_arg_t  arg
  );
...
int
osa_semaphore_create (
  osa_semaphore_t  *sem,
  unsigned int     value
  );

int
osa_semaphore_delete (
  osa_semaphore_t  *sem
  );
...
osa_create_timer (
  osa_timer_t           *tid,
  osa_timer_static_t    *stid,
  osa_timer_isr_list_t  *list,
  int                   list_count,
...
void *
osa_create_connected_device (
  UINT16  conn_handle
  );

void
osa_gap_setup_connection (
  UINT8  idx,
  UINT8  *remote_bdaddr,
  UINT8  addr_type
  );
...
void
osa_delete_connected_device (
  UINT8  conn_handle
  );

INT8
osa_gap_handle_command_status (
  UINT8  status
  );
