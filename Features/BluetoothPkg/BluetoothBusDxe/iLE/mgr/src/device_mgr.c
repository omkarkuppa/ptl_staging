  ...
  )
{
  ...
  if (!LL_PRIVACY_IS_SUPPORTED)
 #endif
  if (hci_read_bd_addr () != STATUS_SUCCESS) {
    gap_cleanup_complete_cb ();
  }
}

void
le_read_max_data_len_cmpl (
  UINT16  opcode,
  UINT8   status,
  UINT8   *evt,
  UINT8   len
  ...
  )
{
  ...
  if (g_local_adapter.ble_local_features[HCI_LE_FEATURE_DATA_PKT_LEN_EXTN_OFFSET] & HCI_LE_FEATURE_DATA_PKT_LEN_EXTN_MASK) {
    if (hci_ble_read_max_data_len () != STATUS_SUCCESS) {
      gap_cleanup_complete_cb ();
    }
 ...
 #if (CONTROLLER_PRIVACY)
  if (LL_PRIVACY_IS_SUPPORTED) {
    if (hci_ble_read_resolving_list_size () != STATUS_SUCCESS) {
      gap_cleanup_complete_cb ();
    }
  ...
  )
{
  ...
  if (hci_ble_read_local_feat () != STATUS_SUCCESS) {
    gap_cleanup_complete_cb ();
  }
}

void
le_read_white_list_size_cmpl (
  UINT8   *evt,
  UINT16  len
  )
{
  STREAM_TO_UINT8 (g_local_adapter.rejectlist_size, evt);
  if (hci_ble_read_buffer_size () != STATUS_SUCCESS) {
    gap_cleanup_complete_cb ();
  }
}
...
void
le_read_resolving_list_size_cmpl (
  UINT8   *evt,
  UINT16  len
  )
{
 ...
 #endif
  if (hci_ble_set_rpa_tout (0x0384) != STATUS_SUCCESS) {
    // 15min
    gap_cleanup_complete_cb ();
  }
}
  ...
  )
{
  ...
  if (g_local_adapter.buffer_size == 0) {
    ...
    if (hci_read_buffer_size () != STATUS_SUCCESS) {
      gap_cleanup_complete_cb ();
    }
  } else {
    STREAM_TO_UINT8 (g_local_adapter.num_of_buffer, evt);
  ...
  )
{
  ...
  if (hci_ble_read_white_list_size () != STATUS_SUCCESS) {
    gap_cleanup_complete_cb ();
  }
}
  ...
  )
{
  ...
  if (hci_ble_set_event_mask (mask) != STATUS_SUCCESS) {
    gap_cleanup_complete_cb ();
  }
}
  ...
  )
{
  ...
  if (hci_ble_set_event_mask_page_2 (mask) != STATUS_SUCCESS) {
    gap_cleanup_complete_cb ();
  }
  ...
  )
{
  ...
  logd (" send hci_ble_set_event_mask");
  if (hci_set_event_mask (event_mask) != STATUS_SUCCESS) {
    gap_cleanup_complete_cb ();
  }
}

void
read_local_feat_cmpl (
  UINT8   *evt,
  UINT16  len
  )
{
  ...
  )
{
  ...
  if (!status) {
    ...
    STREAM_TO_BDADDR (g_local_adapter.bdaddr, evt);
    g_local_adapter.rejectlist_size = bt_controller_info.le_rejectlist_size;
    ...
    if (g_local_adapter.local_features[HCI_LOCAL_FEATURE_MASK_CONTROLLER_LE_SUPPORTED_OFFSET] & HCI_LOCAL_FEATURE_MASK_CONTROLLER_LE_SUPPORTED_MASK) {
      ble_controller_event_mask_t  stmask = {
        { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff },
        {
...
{
  if (hci_manuf_mode (TRUE) != STATUS_SUCCESS) {
    gap_cleanup_complete_cb ();
  }
}
...
{
  if (hci_write_bd_data () != STATUS_SUCCESS) {
    gap_cleanup_complete_cb ();
  }
}
...
{
  if (hci_manuf_mode (FALSE) != STATUS_SUCCESS) {
    gap_cleanup_complete_cb ();
  }
}
