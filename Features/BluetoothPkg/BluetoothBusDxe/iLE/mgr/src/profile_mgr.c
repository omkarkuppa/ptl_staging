...
static INT8
pm_enqueue_req_in_pending_queue (
  UINT8                 conn_id,
  interface_req_elem_t  *incoming_req
  );

static void
pm_dequeue_and_cleanup_from_pending_queue (
  UINT8  conn_id
  );
  ...
  )
{
  ...
  for (idx = 0; idx < MAX_NUMBER_OF_PM_REQ; idx++) {
    if (g_local_adapter.pm_transac_list[idx] != NULL ) {
      logd (
            "app_handle:%d connected_device_idx:%d pm_operation:%d"
            ,
            g_local_adapter.pm_transac_list[idx]->app_handle,
            g_local_adapter.pm_transac_list[idx]->connected_device_idx,
...
static UINT8
pm_find_first_free_transac_slot (
  )
{
  UINT8  idx = 0;
      ...
      if ((req_status != STATUS_SUCCESS) && (req_status != STATUS_ERR_BUSY)) {
        ...
        if (app_data && app_data->app_gatt_cb && app_data->app_gatt_cb->iLE_on_attribute_read_cb) {
          app_data->app_gatt_cb->iLE_on_attribute_read_cb (
                                                           param->trans_id,
                                                           param->conn_id,
                                                           param->read_param.att_opcode,
                                                           ATT_ERR_LOCAL_ERROR,
                                                           param->read_param.attr_read_param.read_attr.att_id,
                                                           NULL
                                                           );
        }
  ...
  )
{
  ...
  if ((ret != STATUS_SUCCESS) && (ret != STATUS_OPERATION_FINISHED_LOCAL_SUCCESS)) {
    ...
    pm_finish_transac (
                       pm_find_transac_id (
                                           API_REQ_OPCODE_CONNECT,
                                           get_connected_device_idx_by_bdaddr (remote_bd_addr)
                                           ),
                       get_connected_device_idx_by_bdaddr (remote_bd_addr)
                       );
  }
  ...
  )
{
  ...
  if (enable) {
    ...
    if ((get_idx_from_autoconn_list (remote_bd_addr) == MAX_NUM_CONNECTED_DEVICE)) {
      ...
      logd ("Device not present in accept list");
      REVERSE_ARRAY_TO_STREAM (rev_bdaddr, remote_bd_addr, BD_ADDR_LEN);
      gap_add_to_rejectlist (addr_type, bdaddr);
    }
  ...
  )
{
  ...
  switch (read_param->att_opcode) {
    ...
    case ATT_OPCODE_READ_BLOB_REQ:
      ret = gatt_read_attribute_value (
                                       conn_id,
                                       read_param->att_opcode,
                                       read_param->attr_read_param.read_attr.att_id,
                                       read_param->attr_read_param.read_attr.offset
                                       );
      break;
    case ATT_OPCODE_READ_BY_TYPE_REQ:
      ret = gatt_read_characteristic_value_using_uuid (
                                                       conn_id,
  ...
  )
{
  ...
  if (result == STATUS_SUCCESS) {
    ...
    if (auto_idx != MAX_NUM_CONNECTED_DEVICE) {
      ...
      gap_add_to_rejectlist (g_local_adapter.gap_data.autoconn_dev[auto_idx]->idaddr_type, rev_bdaddr);
    }
  ...
  )
{
  ...
  logi ("");
  if (reason) {
    /* Non Zero = success with reason code, 0 = failed with no reason*/
  ...
  if (transac_id == MAX_NUMBER_OF_PM_REQ) {
    ...
    if (aidx != MAX_NUM_CONNECTED_DEVICE) {
      // TODO: This is redundant. Else part already covers this.
      UINT8  app_handle;
  ...
  #if (TEST_CODE == TRUE && LE_PING_ENABLE == TRUE)
void
pm_read_authpayload_timeout_cb (
  UINT8   status,
  UINT8   conn_id,
  ...
  )
{
  ...
  if ((app_handle < MAX_NUMBER_OF_APP) && (status == HCI_EVENT_STATUS_SUCCESS)) {
    if (g_local_adapter.app_list[app_handle] &&
        g_local_adapter.app_list[app_handle]->app_device_control_cb &&
...
void
pm_att_mtu_changed_cb (
  UINT8   conn_id,
  UINT16  updated_mtu_size
  )
{
...
#if (GATT_CLIENT_SUPPORTED == TRUE)
void
pm_primary_service_discovered_cb (
  UINT8    conn_id,
  UINT16   start_handle,
  UINT16   end_handle,
  uuid_t   *uuid,
...
void
pm_included_service_discovered_cb (
  UINT8    conn_id,
  UINT16   inc_handle,
  UINT16   start_handle,
  UINT16   end_handle,
...
void
pm_characteristic_discovered_cb (
  UINT8    conn_id,
  UINT8    property,
  UINT16   att_handle,
  UINT16   value_handle,
...
void
pm_on_characteristic_notification_cb (
  UINT8     conn_id,
  UINT16    att_id,
  buffer_t  *buffer,
