...
static void
create_add_bonded_dev_to_resolving_req (
  ...
  #if (HOST_PRIVACY)
static UINT8
find_free_slot_in_resolving_list (
  );
...
static INT8
smpm_enqueue_req_in_pending_queue (
  ...
  switch (key_type) {
    ...
    case PEER_LTK:
    {
      ...
      if (is_valid) {
        ...
        if (hci_ble_start_encryption (device->acl_handle, enc_info->rand, enc_info->ediv, enc_info->ltk) != STATUS_SUCCESS) {
    ...
    case LOCAL_LTK:
    {
      ...
      if (is_valid) {
        enc_info_t  *enc_info = key_value;
        if (hci_ble_ltk_req_reply (device->acl_handle, enc_info->ltk) != STATUS_SUCCESS) {
          g_local_adapter.smp_data.smp_cur_cb.status = HCI_EVENT_STATUS_HARDWARE_FAILURE;
          smp_clean_up ();
        }
      } else {
  ...
  )
{
    ...
    if (g_local_adapter.app_list[idx]) {
      if (g_local_adapter.app_list[idx]->app_device_control_cb) {
        if (g_local_adapter.app_list[idx]->app_device_control_cb->iLE_bonding_completed_cb) {
          g_local_adapter.app_list[idx]->app_device_control_cb->iLE_bonding_completed_cb (
                                                                                          trans_id,
                                                                                          g_local_adapter.connected_device[conn_id]->bdaddr,
                                                                                          g_local_adapter.connected_device[conn_id]->bdaddr_type,
                                                                                          conn_id,
                                                                                          status,
                                                                                          g_local_adapter.connected_device[conn_id]->id_addr.bdaddr,
                                                                                          g_local_adapter.connected_device[conn_id]->id_addr.type
                                                                                          );
        }
      }
    }
  ...
  if (g_local_adapter.smp_mgr_data.smp_bonding_complete_cb) {
    g_local_adapter.smp_mgr_data.smp_bonding_complete_cb (
                                                          trans_id,
                                                          g_local_adapter.connected_device[conn_id]->bdaddr,
                                                          g_local_adapter.connected_device[conn_id]->bdaddr_type,
                                                          conn_id,
  ...
  if (g_local_adapter.smp_mgr_data.smp_data_siging_complete_cb != NULL) {
    ...
    if (status == STATUS_SUCCESS) {
      ...
      if (g_local_adapter.smp_mgr_data.storage_mgr.storage_cb && g_local_adapter.smp_mgr_data.storage_mgr.storage_cb->key_update_cb) {
        ...
        g_local_adapter.smp_mgr_data.storage_mgr.storage_cb->key_update_cb (g_local_adapter.smp_mgr_data.storage_mgr.trans_id, device->id_addr.bdaddr, device->id_addr.type, LOCAL_COUNTER, 4, sc);
      }
  ...
  )
{
 ...
 #if (PRIVACY_FEATURE == TRUE)
    if (cur_key->keys_distributed & SMP_IRK_RECEIVED) {
      g_local_adapter.smp_mgr_data.storage_mgr.storage_cb->bonding_info_cb (
                                                                            g_local_adapter.smp_mgr_data.storage_mgr.trans_id,
                                                                            device->bdaddr,
                                                                            device->bdaddr_type,
                                                                            cur_key->id_bdaddr,
                                                                            cur_key->id_addr_type,
                                                                            cur_key
                                                                            );
    } else
 #endif
