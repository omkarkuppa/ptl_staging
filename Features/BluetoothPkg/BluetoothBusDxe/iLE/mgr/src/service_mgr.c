...
static INT8
sm_enqueue_req_in_pending_queue (
...
static void
sm_dequeue_and_cleanup_from_pending_queue (
  UINT8  conn_id
  )
{
...
{
  if (next_free_handle) {
    /* This should be 0. Otherwise the app is adding another service */
 ...
 * */
static uuid_t *
get_last_added_service_uuid (
  UINT16  last_added_handle
  )
{
  ...
  )
{
  ...
  for (idx = 0; idx < MAX_NUM_ATTR; idx++) {
    ...
    if (g_local_adapter.attribute_list[idx].attr_type != ATTR_TYPE_INVALID) {
      ...
      if (g_local_adapter.attribute_list[idx].attr_type == ATTR_TYPE_PRIMARY_SERVICE) {
        logd (
              "(%u) : handle:%u(0x%04x) %s end_handle:%u(%04x) uuid:%s",
              idx,
              g_local_adapter.attribute_list[idx].attr_handle,
              g_local_adapter.attribute_list[idx].attr_handle,
        ...
        logd (
              "(%u) : handle:%u(0x%04x) %s inc_srv_start_handle:%u(%04x) inc_srv_end_handle:%u(%04x) uuid:%s",
              idx,
              g_local_adapter.attribute_list[idx].attr_handle,
              g_local_adapter.attribute_list[idx].attr_handle,
              attribute_type_string[g_local_adapter.attribute_list[idx].attr_type],
  ...
  )
{
  ...
  if (start_idx == MAX_NUM_ATTR) {
    /* Arribute Handle not found */
  ...
  )
{
  ...
  switch (req->req_opcode) {
    case API_REQ_OPCODE_ADD_SERVICE:
    {
      ...
      status = (UINT8)add_service (
                                   g_local_adapter.next_free_handle,
                                   param->trans_id,
                                   &param->uuid,
                                   param->service_type,
                                   param->num_of_incl_srv,
                                   param->num_of_char,
                                   param->num_of_desc,
                                   &service_handle
                                   );
    ...
    case API_REQ_OPCODE_ADD_INCLUDED_SERVICE:
    {
      ...
      status = (UINT8)add_include_service (
                                           g_local_adapter.next_free_handle,
                                           param->trans_id,
                                           &param->uuid,
                                           param->service_type,
