...
void
set_local_adapter_entry (
  local_adapter_storage_t  *local_adapter_storage
  )
{
  logd ("");
  if (!storage_file.file_ptr) {
...
void
set_remote_device_entry (
  remote_device_storage_t  *device_entry
  )
{
  logd ("");
  if (!storage_file.file_ptr) {
...
void
delete_one_remote_device_entry (
  UINT8  *bdaddr
  )
{
  if (!storage_file.file_ptr) {
...
void
delete_all_remote_device_entry (
  )
{
  if (!storage_file.file_ptr) {
    return;
...
void
get_stored_irk_info (
  resolving_list_entry_t  *list_of_irk,
  UINT16                  count
  )
{
