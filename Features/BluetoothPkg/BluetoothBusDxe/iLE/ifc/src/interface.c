...
osa_thread_ret_t
stack_main_thread_handler (
  osa_thread_arg_t  arg
  );
 ...
 * @param use_rejectlist - Set if acceptlist is to be used for filtering.
 * @param background - Set if low duty cycle advertising is to be done.
 * @return Error code.
 */
INT8
iLE_start_discoverable (
  trans_id_t  trans_id,
  UINT32      timeout_msec,
  UINT8       type,
  UINT8       flags,
  ...
  )
{
  ...
  param->use_rejectlist = use_rejectlist;
...
/** @brief Requests core stack to add a device to acceptlist
 *
 * Function: iLE_add_to_rejectlist
 *                       This API allows the application to say to core stack that it wants
 *                       to add a device to the acceptlist for future operations
 *
 * @param app_handle - Handle of the application.
 ...
 */
INT8
iLE_add_to_rejectlist (
  trans_id_t  trans_id,
  UINT8       addr_type,
  UINT8       *bdaddr
  )
{
  dm_rejectlist_op_param_t  *param;
  interface_req_elem_t      *req;
  ...
  req = get_interface_req_buffer (API_REQ_OPCODE_ADD_TO_REJECTLIST, 0, sizeof (dm_rejectlist_op_param_t));
  if (req == NULL) {
    return STATUS_ERR_INSUFFICIENT_RESOURCES;
  }

  param = (dm_rejectlist_op_param_t *)req->parameter;
...
/** @brief Requests core stack to remove a device from acceptlist
 *
 * Function: iLE_remove_from_rejectlist
 *                       This API allows the application to say to core stack that it wants
 *                       to remove a device from the acceptlist for future operations
 *
 * @param app_handle - Handle of the application.
 * @param addr_type - Address type of the device (public/random).
 * @param bdaddr - Bluetooth MAC address of the remote device.
  ...
  )
{
  ...
  req = get_interface_req_buffer (API_REQ_OPCODE_REMOVE_FROM_REJECTLIST, 0, sizeof (dm_rejectlist_op_param_t));
  if (req == NULL) {
    return STATUS_ERR_INSUFFICIENT_RESOURCES;
  }
...
/** @brief Requests core stack to clear the acceptlist
 *
 * Function: iLE_clear_rejectlist
 *                       This API allows the application to say to core stack that it wants
 *                       to clear the acceptlist and remove all device entries
 *
 * @param app_handle - Handle of the application.
 * @return Error code.
 */
INT8
iLE_clear_rejectlist (
  trans_id_t  trans_id
  )
{
  interface_req_elem_t  *req;
  ...
  req = get_interface_req_buffer (API_REQ_OPCODE_CLEAR_REJECTLIST, 0, 0);
  if (req == NULL) {
    return STATUS_ERR_INSUFFICIENT_RESOURCES;
  }
 ...
 */
BOOLEAN
iLE_is_controller_privacy_supported (
  trans_id_t  trans_id
  )
{
  if (g_local_adapter.adapter_state != ADAPTER_STATE_ENABLED) {
 ...
 */
INT8
iLE_test_read_authenticated_payload_timeout (
  trans_id_t  trans_id,
  UINT8       conn_id
  )
{
 ...
 */
INT8
iLE_test_write_authenticated_payload_timeout (
  trans_id_t  trans_id,
  UINT8       conn_id,
 ...
 */
UINT16
iLE_l2cap_coc_get_max_mtu_size (
  trans_id_t  trans_id
  )
{
  logi ("");
    ...
    if (req) {
      ...
      switch (req->req_opcode) {
        ...
        case API_REQ_OPCODE_DEREGISTER_APPLICATION:
        {
          ...
          if ((g_local_adapter.adapter_state == ADAPTER_STATE_DISABLED) || (!g_local_adapter.app_list[param->app_handle])) {
            break;
          }
