...
gap_stop_advertise (
    ...
    #if (PRIVACY_FEATURE)
static BOOLEAN
gap_direct_adv_addr_needs_resolution (
  UINT32   timeout_msec,
  UINT8    type,
  UINT8    flags,
  UINT8    *remote_bd_addr,
...
static INT8
gap_remove_from_resolvinglist (
  UINT8  peer_addr_type,
  UINT8  *peer_bdaddr
  );

static INT8
gap_clear_resolvinglist (
  );

#endif /* CONTROLLER_PRIVACY */

static INT8
gap_clear_rejectlist (
  );
...
static void
prep_rejectlist_update (
  )
{
 #if (GAP_PERIPHERAL_ROLE_SUPPORTED == TRUE)
  BOOLEAN  adv_stop = adapter_is_in_adv_state (GAP_ADAPTER_STATE_ADV_ENABLED) && g_local_adapter.gap_data.adv_use_rejectlist;
 #endif /* GAP_PERIPHERAL_ROLE_SUPPORTED == TRUE */
 #if (GAP_CENTRAL_ROLE_SUPPORTED == TRUE)
  BOOLEAN  scn_stop = adapter_is_in_scn_state (GAP_ADAPTER_STATE_SCN_ENABLED) && g_local_adapter.gap_data.scan_use_rejectlist;
 #endif /* GAP_CENTRAL_ROLE_SUPPORTED == TRUE */

  start_operation (GAP_OPERATION_UPDATING_REJECTLIST);
...
static void
gap_conn_comp_rpa_resolution_cb (
  UINT8    *remote_bd_addr,
  BOOLEAN  local,
  UINT8    bonded_idx,
  UINT8    status
...
static interface_req_elem_t *
create_req_for_rpa_adv_resolution (
  UINT8    *bdaddr,
  BOOLEAN  local
  )
{
  ...
  )
{
  ...
  #if (HOST_PRIVACY)
static interface_req_elem_t *
create_req_for_conn_comp_rpa_resolution (
  UINT8  *remote_bdaddr
  )
{
  interface_req_elem_t      *req = (interface_req_elem_t *)get_interface_req_buffer (API_REQ_OPCODE_RPA_RESOLVE, 0, sizeof (smpm_rpa_resolve_param_t));
  ...
  )
{
  ...
  if (!adapter_is_in_scn_state (GAP_ADAPTER_STATE_SCN_ENABLED)) {
    if ((g_local_adapter.gap_data.prev_adapter_state & GAP_ADAPTER_STATE_SCN_ENABLED) == GAP_ADAPTER_STATE_SCN_ENABLED) {
      gap_start_scan (g_local_adapter.gap_data.scan_req_trans_id, g_local_adapter.gap_data.scan_timeout_msec, g_local_adapter.gap_data.scan_isactive, g_local_adapter.gap_data.scan_is_limited, g_local_adapter.gap_data.scan_use_rejectlist, g_local_adapter.gap_data.scan_is_background, g_local_adapter.gap_data.scan_int, g_local_adapter.gap_data.scan_win, g_local_adapter.gap_data.scan_filter_uuids, FALSE);
    }
  }
...
void
remote_command_timeout_handler (
  osa_timer_arg_t  arg
  )
{
  ...
  switch (device_get_state (connected_device_list_idx)) {
    ...
    case GAP_CONNECTION_STATE_ACL_CONNECTING:
      ...
      ret = hci_ble_cancel_create_ll_conn ();
      if (ret != STATUS_SUCCESS) {
        remote_command_timer_stop (connected_device_list_idx);
        l2cap_connect_cb (device, FALSE);
        pm_connect_cb (device, HCI_EVENT_STATUS_UNKNOWN_CONNECTION_IDENTIFIER);
...
static INT8
gap_enqueue_req_in_pending_queue (
  interface_req_elem_t  *incoming_req
  )
{
  ...
  )
{
  ...
  for (idx = 0; idx < MAX_NUM_ADV_SCANRESP_DATA_STRUCTURES; idx++) {
    if (g_local_adapter.gap_data.scanresp_data[idx]) {
      ...
      if ((type == GAP_ADV_DATA_FLAGS) || (type == GAP_ADV_DATA_APPEARANCE)) {
        // Not allowed in scan response
        continue;
      }
...
static UINT8
add_new_adv_scanresp_data (
  gap_adv_scanresp_data_t  **list,
  UINT8                    length,
  UINT8                    type,
  UINT8                    *data
  )
  ...
  )
{
  ...
  switch (req->req_opcode) {
 ...
 #if 0
 #if (HOST_PRIVACY)
      if (gap_direct_adv_addr_needs_resolution (param->timeout_msec, param->type, param->flags, param->remote_bd_addr, param->addr_type, param->use_rejectlist, param->background)) {
        req_status = STATUS_SUCCESS;
      } else
 #endif /* HOST_PRIVACY */
 #endif
      req_status = gap_start_advertise (param->timeout_msec, param->type, param->flags, param->remote_bd_addr, param->addr_type, param->adv_int_min, param->adv_int_max, param->use_rejectlist, param->background);
      break;
    }
    ...
    case API_REQ_OPCODE_START_SCAN:
    {
      ...
      req_status = gap_start_scan (param->trans_id, param->timeout_msec, param->active, param->limited, param->use_rejectlist, param->background, param->scan_int, param->scan_win, param->filter_uuids, TRUE);
      break;
    }
    ...
    case API_REQ_OPCODE_ADD_TO_REJECTLIST:
    {
      dm_rejectlist_op_param_t  *param = (dm_rejectlist_op_param_t *)(void *)req->parameter;
      req_status = gap_add_to_rejectlist (param->addr_type, param->bdaddr);
      break;
    }
    case API_REQ_OPCODE_REMOVE_FROM_REJECTLIST:
    {
      dm_rejectlist_op_param_t  *param = (dm_rejectlist_op_param_t *)(void *)req->parameter;
      req_status = gap_remove_from_rejectlist (param->addr_type, param->bdaddr);
      break;
    }
    case API_REQ_OPCODE_CLEAR_REJECTLIST:
    {
      req_status = gap_clear_rejectlist ();
      break;
    }
 #if (GAP_CENTRAL_ROLE_SUPPORTED == TRUE || GAP_PERIPHERAL_ROLE_SUPPORTED == TRUE)
    ...
    case API_REQ_OPCODE_GAP_CONN_PARAMS_UPDATE:
    {
      ...
      req_status = pm_conn_param_update_req (
                                             param->trans_id,
                                             param->conn_idx,
                                             param->interval_min,
                                             param->interval_max,
  ...
  )
{
  ...
  if ((type == GAP_ADV_TYPE_CONNECTABLE_DIRECTED_HIGH_DUTY) || (type == GAP_ADV_TYPE_CONNECTABLE_DIRECTED_LOW_DUTY)) {
    ...
    g_local_adapter.gap_data.adv_use_rejectlist = use_rejectlist;
  ...
  )
{
  ...
  adv_params.filter_policy    = use_rejectlist;
  ...
  )
{
  ...
  ret = hci_ble_set_adv_enable (0);      /* Disable adv */
  if (ret != STATUS_SUCCESS) {
  ...
  )
{
  ...
  param->use_rejectlist = g_local_adapter.gap_data.adv_use_rejectlist;
...
UINT8
gap_add_adv_scanresp_data (
  gap_adv_scanresp_data_t  **dlist,
  UINT8                    length,
  UINT8                    type,
  UINT8                    *data
  )
    ...
    switch (type) {
      ...
      case GAP_ADV_DATA_COMPLETE_LOCAL_NAME:
        if (data != (UINT8 *)g_local_adapter.local_name) {
          ...
          if (length < sizeof (g_local_adapter.local_name)) {
            /* fix for JIRA: BT-119005*/
...
{
  UINT8  scan_filter = use_rejectlist;
  INT8   ret         = STATUS_SUCCESS;
  ...
  if (adapter_is_in_scn_state (GAP_ADAPTER_STATE_SCN_ENABLED)) {
    ...
    if (!g_local_adapter.gap_data.scan_timeout_msec) {
      ...
      g_local_adapter.gap_data.scan_use_rejectlist = use_rejectlist;
  ...
  )
{
  ...
  /* Disable scan */
  ret = hci_ble_set_scan_enable (0, 0);
  if (ret != STATUS_SUCCESS) {
  ...
  )
{
 ...
 #if (PARANOID_STACK == TRUE)
  if (g_local_adapter.num_rejectlist_entries >= g_local_adapter.rejectlist_size) {
    return STATUS_ERR_INSUFFICIENT_RESOURCES;
  }
  ...
  ret = hci_ble_add_to_white_list (addr_type, peer_bdaddr);
  ...
  )
{
 ...
 #if (PARANOID_STACK == TRUE)
  if (g_local_adapter.num_rejectlist_entries == 0) {
    return STATUS_ERR_INVALID_PARAM;
  }
  ...
  ret = hci_ble_remove_from_white_list (addr_type, peer_bdaddr);
  ...
  )
{
  ...
  if (!(local_feat & remote_feat & HCI_LE_FEATURE_CONN_PARAM_REQ_PROCEDURE_MASK)) {
    ...
    if (device->local_link_role == LINK_ROLE_CENTRAL) {
      // Do LL connection update request if device is central
      ret = hci_ble_upd_ll_conn_params (device->acl_handle, interval_min, interval_max, conn_latency, sup_timeout, 0x00, 0x00);
    } else {
      // Do L2CAP connection parameter update request if device is peripheral
      ret = l2cap_send_conn_param_update_req (device, interval_min, interval_max, conn_latency, sup_timeout);
    }
  } else {
    ret = hci_ble_upd_ll_conn_params (device->acl_handle, interval_min, interval_max, conn_latency, sup_timeout, 0x00, 0x00);
  }
  ...
  if (ret == STATUS_SUCCESS) {
    ...
    pm_conn_param_update_cb (HCI_EVENT_STATUS_HARDWARE_FAILURE, connected_device_list_idx, 0, 0, 0);
  }
  ...
  )
{
  ...
  ret = hci_ble_read_authpayload_timeout (device->acl_handle);

  if (ret != STATUS_SUCCESS) {
    pm_read_authpayload_timeout_cb (HCI_EVENT_STATUS_HARDWARE_FAILURE, conn_id, 0);
  }
  ...
  )
{
  ...
  if (authpayload_timeout < 0x0001) {
    // Range defined in HCI BT 4.1 spec for hci_ble_write_authpayload_timeout
  ...
  switch (opcode) {
    ...
    case HCI_CMD_LE_SET_ADV_PARAM:
      if (g_local_adapter.gap_data.adv_scanresp_data_changed) {
        data_len = get_formatted_adv_data (adv_data);
        ret      = hci_ble_set_adv_data (data_len, adv_data);
      } else {
        ret = hci_ble_set_adv_enable (1);                /* Enable adv */
      }
    ...
    case HCI_CMD_LE_SET_ADV_DATA:
      ...
      g_local_adapter.gap_data.adv_scanresp_data_changed = FALSE;
      if (!data_len) {
        /* If no scan response data added, mirror adv data */
        data_len = get_formatted_adv_data (scanresp_data);
      }

      ret = hci_ble_set_scan_rsp_data (data_len, scanresp_data);
      if (ret != STATUS_SUCCESS) {
        adapter_enter_adv_state (GAP_ADAPTER_STATE_ADV_DISABLED);
        end_operation (GAP_OPERATION_ADVERTISING);
    ...
    case HCI_CMD_LE_SET_ADV_ENABLE:
      if (adapter_is_in_adv_state (GAP_ADAPTER_STATE_ADV_DISABLING)) {
        ...
        if (g_local_adapter.gap_data.adapter_state & GAP_ADAPTER_STATE_ADV_RESTARTING) {
          ...
          BOOLEAN  use_rejectlist  = g_local_adapter.gap_data.adv_use_rejectlist;
          BOOLEAN  background      = g_local_adapter.gap_data.adv_is_background;
          gap_start_advertise (timeout_msec, type, flags, remote_bd_addr, addr_type, adv_int_min, adv_int_max, use_rejectlist, background);
          end_operation (GAP_OPERATION_ADVERTISING);
        } else {
          end_operation (GAP_OPERATION_ADVERTISING);
          finish_transac_and_process_new_req ();
    ...
    case HCI_CMD_LE_SET_SCAN_PARAM:
      ret = hci_ble_set_scan_enable (1, 1);          /* Enable scan, filter duplicates */
      if (ret != STATUS_SUCCESS) {
        if (osa_is_timer_active (&g_local_adapter.gap_data.scan_timer, GAP_SCAN_TIMER, 0)) {
          osa_stop_timer (&g_local_adapter.gap_data.scan_timer, GAP_SCAN_TIMER, 0);
        }

        osa_delete_timer (&g_local_adapter.gap_data.scan_timer, GAP_SCAN_TIMER);
        adapter_enter_scn_state (GAP_ADAPTER_STATE_SCN_DISABLED);
        if (adapter_is_bgscanning ()) {
          adapter_exit_bgscanning_state ();
          end_operation (GAP_OPERATION_BG_SCANNING);
    ...
    case HCI_CMD_LE_SET_SCAN_ENABLE:
      if (adapter_is_in_scn_state (GAP_ADAPTER_STATE_SCN_DISABLING)) {
        ...
        if (g_local_adapter.gap_data.adapter_state & GAP_ADAPTER_STATE_SCN_RESTARTING) {
          ...
          BOOLEAN  use_rejectlist = g_local_adapter.gap_data.scan_use_rejectlist;
          ...
          gap_start_scan (g_local_adapter.gap_data.scan_req_trans_id, timeout_msec, active, limited, use_rejectlist, background, scan_int, scan_win, filter_uuids, FALSE);
          if (bgscan) {
            end_operation (GAP_OPERATION_BG_SCANNING);
          } else {
            end_operation (GAP_OPERATION_FG_SCANNING);
    ...
    case HCI_CMD_LE_ADD_DEV_TO_WHITE_LIST:
      if (status == HCI_EVENT_STATUS_SUCCESS) {
        g_local_adapter.num_rejectlist_entries++;
      }

      if (get_root_operation () == GAP_OPERATION_UPDATING_REJECTLIST) {
        adapter_restore_prev_state ();
      }
    ...
    case HCI_CMD_LE_CLEAR_WHITE_LIST:
      if (status == HCI_EVENT_STATUS_SUCCESS) {
        g_local_adapter.num_rejectlist_entries = 0;
      }
  ...
  )
{
  ...
  if (g_local_adapter.ble_local_features[HCI_LE_FEATURE_PERIPHERAL_INIT_FEAT_EXCHANGE_OFFSET] & HCI_LE_FEATURE_PERIPHERAL_INIT_FEAT_EXCHANGE_MASK) {
  ...
  )
{
 ...
 #if (GAP_PERIPHERAL_ROLE_SUPPORTED == TRUE)
  else if (status == GAP_CONNECT_ERROR_DIRECTED_ADVERTISING_TIMEOUT) {
  ...
  #if (LE_PING_ENABLE == TRUE)
INT8
gap_handle_authpayload_timeout_exp_evt (
  UINT8   *buffer,
  UINT16  len
  )
{
...
INT8
gap_handle_data_len_change_evt (
  UINT8   *buffer,
  UINT16  len
  )
{
  ...
  logd (
        "conn_handle:0x%x max_tx_octets:%x, max_tx_time:%x, max_rx_octets:%x, max_rx_time:%x",
        conn_handle,
        max_tx_octets,
        max_tx_time,
        max_rx_octets,
...
INT8
gap_handle_conn_param_update_cmpl (
  UINT8   *buffer,
  UINT16  len
  )
{
 ...
 */
INT8
gap_handle_conn_read_remote_feat_cmpl (
  UINT8   *buffer,
  UINT16  len
  )
{
...
INT8
gap_handle_remote_conn_param_req (
  UINT8   *buffer,
  UINT16  len
  )
{
  ...
  if (conn_param_is_valid (interval_min, interval_max, conn_latency, sup_timeout)) {
    ...
    ret = hci_ble_rem_conn_param_req_rep (acl_handle, interval_min, interval_max, conn_latency, sup_timeout, 0x00, 0x00);
    if (ret == STATUS_SUCCESS) {
