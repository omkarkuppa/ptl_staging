...
static INT8
gatt_discover_prim_srv_by_uuid (
  UINT8   conn_id,
  UINT16  start_handle,
  UINT16  end_handle,
  uuid_t  *filter_uuid
...
static void
gatt_discover_chars_cb (
  UINT8     conn_id,
  buffer_t  *buffer
  );

static void
gatt_read_char_using_uuid_cb (
  UINT8     conn_id,
  buffer_t  *buffer
  );
  ...
  )
{
  ...
  if (length_of_each_elem == 0) {
    /* Checking for NULL, to aviod divide-by-zero. JIRA: BT-119002*/
    return;
  }
