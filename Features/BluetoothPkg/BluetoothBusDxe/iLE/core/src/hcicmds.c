  ...
  )
{
  UINT8  *p_buf = (UINT8 *)alloc_queue_element (
                                                HCI_CMD_PREAMBLE_SIZE + UART_HDR_SIZE + HCI_CMD_RESET_PARAM_LEN,
                                                HCI_ALLOC
                                                );
  UINT8  *p = p_buf;
...
INT8
hci_ble_write_host_supported (
  UINT8  le_host_spt,
  UINT8  simul_le_host_spt
  )
{
  UINT8  *p_buf = (UINT8 *)alloc_queue_element (
                                                HCI_CMD_PREAMBLE_SIZE + UART_HDR_SIZE
  ...
  return hci_send_cmd (
                       HCI_CMD_WRITE_LE_HOST_SUPPORTED,
                       p_buf,
                       HCI_CMD_WRITE_LE_HOST_SUPPORTED_PARAM_LEN
                       );
}
  ...
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_READ_LOCAL_SUPPORTED_FEAT,
                       p_buf,
                       HCI_CMD_LE_READ_LOCAL_SUPPORTED_FEAT_PARAM_SIZE
                       );
}
  ...
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_READ_REMOTE_USED_FEAT,
                       p_buf,
                       HCI_CMD_LE_READ_REMOTE_USED_FEAT_PARAM_SIZE
                       );
}
...
INT8
hci_ble_read_authpayload_timeout (
  UINT16  handle
  )
{
  UINT8  *p_buf = (UINT8 *)alloc_queue_element (
  ...
  return hci_send_cmd (
                       HCI_CMD_READ_AUTHPAYLOAD_TIMEOUT,
                       p_buf,
                       HCI_CMD_READ_AUTHPAYLOAD_TIMEOUT_PARAM_SIZE
                       );
}
...
INT8
hci_ble_write_authpayload_timeout (
  UINT16  handle,
  UINT16  authpayload_timeout
  )
{
  UINT8  *p_buf = (UINT8 *)alloc_queue_element (
                                                HCI_CMD_PREAMBLE_SIZE + UART_HDR_SIZE
  ...
  return hci_send_cmd (
                       HCI_CMD_WRITE_AUTHPAYLOAD_TIMEOUT,
                       p_buf,
                       HCI_CMD_WRITE_AUTHPAYLOAD_TIMEOUT_PARAM_SIZE
                       );
}
  ...
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_ADD_DEV_TO_WHITE_LIST,
                       p_buf,
                       HCI_CMD_LE_ADD_DEV_TO_WHITE_LIST_PARAM_SIZE
                       );
}
...
INT8
hci_ble_remove_from_white_list (
  UINT8  addr_type,
  UINT8  *bda
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_REMOVE_DEV_FRM_WHITE_LIST,
                       p_buf,
                       HCI_CMD_LE_REMOVE_DEV_FRM_WHITE_LIST_PARAM_SIZE
                       );
}
...
INT8
hci_ble_write_sug_def_data_len (
  UINT16  tx_octets,
  UINT16  tx_time
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_WRITE_SUG_DEF_DATA_LEN,
                       p_buf,
                       HCI_CMD_LE_WRITE_SUG_DEF_DATA_LEN_PARAM_SIZE
                       );
}
...
INT8
hci_ble_rem_conn_param_req_rep (
  UINT16  handle,
  UINT16  conn_int_min,
  UINT16  conn_int_max,
  UINT16  conn_latency,
  ...
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_REM_CONN_PARAM_REQ_REPLY,
                       p_buf,
                       HCI_CMD_LE_REM_CONN_PARAM_REQ_REPLY_PARAM_SIZE
                       );
}
...
INT8
hci_ble_rem_conn_param_req_rej (
  UINT16  handle,
  UINT8   reason
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_REM_CONN_PARAM_REQ_REJCT,
                       p_buf,
                       HCI_CMD_LE_REM_CONN_PARAM_REQ_REJCT_PARAM_SIZE
                       );
}
  ...
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_LONG_TERM_KEY_REQ_REPLY,
                       p_buf,
                       HCI_CMD_LE_LONG_TERM_KEY_REQ_REPLY_PARAM_SIZE
                       );
}
  ...
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_LONG_TERM_KEY_NEG_REPLY,
                       p_buf,
                       HCI_CMD_LE_LONG_TERM_KEY_NEG_REPLY_PARAM_SIZE
                       );
}
...
INT8
hci_ble_add_device_to_resolving_list (
  UINT8  addr_type,
  UINT8  *bda,
  UINT8  *peer_irk,
  UINT8  *local_irk
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_ADD_DEV_TO_RESOLVING_LIST,
                       p_buf,
                       HCI_CMD_LE_ADD_DEV_TO_RESOLVING_LIST_PARAM_SIZE
                       );
}
...
INT8
hci_ble_remove_device_from_resolving_list (
  UINT8  addr_type,
  UINT8  *bda
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_REMOVE_DEV_FROM_RESOLVING_LIST,
                       p_buf,
                       HCI_CMD_LE_REMOVE_DEV_FROM_RESOLVING_LIST_PARAM_SIZE
                       );
}
...
INT8
hci_ble_read_peer_resolvable_addr (
  UINT8  addr_type,
  UINT8  *bda
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_READ_PEER_RESOLVABLE_ADDR,
                       p_buf,
                       HCI_CMD_LE_READ_PEER_RESOLVABLE_ADDR_PARAM_SIZE
                       );
}
...
INT8
hci_ble_read_local_resolvable_addr (
  UINT8  addr_type,
  UINT8  *bda
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_READ_LOCAL_RESOLVABLE_ADDR,
                       p_buf,
                       HCI_CMD_LE_READ_LOCAL_RESOLVABLE_ADDR_PARAM_SIZE
                       );
}
...
INT8
hci_ble_set_addr_resolution_enable (
  UINT8  enable
  )
{
  UINT8  *p_buf = (UINT8 *)alloc_queue_element (
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_SET_ADDR_RESOLUTION_EN,
                       p_buf,
                       HCI_CMD_LE_SET_ADDR_RESOLUTION_EN_PARAM_SIZE
                       );
}
  ...
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_SET_RESOLVABLE_PRIVATE_ADDR_TOUT,
                       p_buf,
                       HCI_CMD_LE_SET_RESOLVABLE_PRIVATE_ADDR_TOUT_PARAM_SIZE
                       );
}
  ...
  )
{
  ...
  return hci_send_cmd (
                       HCI_CMD_LE_READ_LOCAL_P256_PUBLIC_KEY,
                       p_buf,
                       HCI_CMD_LE_READ_LOCAL_P256_PUBLIC_KEY_PARAM_SIZE
                       );
}
  ...
  )
{
  ...
  /*    UINT8 cmd_buffer[] = {0x66,0x55,0x44,0x33,0x22,0x11,0x00,0x00,0x00,0xF0,0x49 \
                                                          ...
                                                          */
  UINT8  cmd_buffer[] = {
    0x66, 0x55, 0x44, 0x33, 0x22, 0x11, 0x00, 0x00, 0x00, 0xF0, \
