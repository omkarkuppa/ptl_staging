...
static UINT8
check_attr_read_permission (
  UINT8   conn_id,
  UINT16  permission
  );

static UINT8
check_attr_write_permission (
  UINT8   conn_id,
  UINT16  permission
  );
  ...
  )
{
  ...
  logd (
        "set opcode:%d device->gatt_device_data.server_priv_data:%p",
        device->gatt_device_data.server_priv_data->opcode,
        device->gatt_device_data.server_priv_data
        );
  return STATUS_SUCCESS;
}
 ...
 */
static void
flush_saved_multi_read_handle_list (
  connected_device_t  *device
  )
{
...
void
gatt_read_by_group_type_req_cb (
  UINT8     conn_id,
  UINT8     opcode,
  buffer_t  *in_buffer
  )
...
void
gatt_find_by_type_value_req_cb (
  UINT8     conn_id,
  UINT8     opcode,
  buffer_t  *in_buffer
  )
  ...
  for (att_hdl = start_handle; att_hdl <= att_hdl_end; att_hdl++) {
    ...
    if (attribute->attr_type == attribute_type) {
      switch (attribute->attr_type) {
        ...
        {
          if (attribute->uuid.length == 2) {
            /* send the UUID only if it is 2 bytes */
        ...
        case ATTR_TYPE_CHARACTERISTIC_VALUE:
        {
          if (uuidcmp (&attribute->uuid, &attribute_type_uuid) == TRUE) {
            ...
            if (error_code == ATT_SUCCESS) {
              ...
              if (g_local_adapter.connected_device[conn_id]->pending_att_packet.in_buffer == NULL) {
                return;
              }
...
void
gatt_handle_value_confirmation_cb (
  UINT8     conn_id,
  UINT8     opcode,
  buffer_t  *in_buffer
  )
  ...
  )
{
  ...
  if (att_opcode == ATT_OPCODE_READ_REQ) {
    ...
    while (cur_node != NULL) {
      cur_char = (read_multiple_chars_value_t *)cur_node->data;
      if ((cur_char->handle == att_id) && !cur_char->buffer) {
        /* Handles duplicate att_id read scenario */
  ...
  )
{
  ...
  if ((att_idx > 0) && (g_local_adapter.attribute_list[att_idx].attr_type == ATTR_TYPE_CHARACTERISTIC_VALUE)) {
    if (IS_ATTRIBUTE_PROPERTY_NOTIFY (g_local_adapter.attribute_list[att_idx-1].attr_data.char_desc_data.property) == TRUE) {
      ...
      loge (
            "Send Notification failed. Notification not supported, Attribute Handle: %u, property: %u !!!", \
            att_idx-1,
            g_local_adapter.attribute_list[att_idx-1].attr_data.char_desc_data.property
            );
    }
  } else {
  ...
  )
{
  ...
  if ((att_idx > 0) && (g_local_adapter.attribute_list[att_idx].attr_type == ATTR_TYPE_CHARACTERISTIC_VALUE)) {
    if (IS_ATTRIBUTE_PROPERTY_INDICATE (g_local_adapter.attribute_list[att_idx-1].attr_data.char_desc_data.property) == TRUE) {
      ...
      loge (
            "Send Indication failed. Indication not supported, Attribute Handle: %u, property: %u !!!", \
            att_idx-1,
            g_local_adapter.attribute_list[att_idx-1].attr_data.char_desc_data.property
            );
    }
...
INT8
gatt_set_service_change_config (
  UINT8    conn_id,
  BOOLEAN  is_notify
  )
{
...
BOOLEAN
gatt_get_service_change_config (
  UINT8  conn_id
  )
{
