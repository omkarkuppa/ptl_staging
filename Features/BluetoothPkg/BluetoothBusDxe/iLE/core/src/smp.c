/** @file
  ...
  Copyright (c) 2017 - 2021, Intel Mobile Communications GmbH. All rights reserved.<BR>
...
static void
smp_pair_failed_cmd_evt (
  UINT8  *data
  );

  #if (SECURE_CONNECTION)
static void
smp_pairing_pub_key_cmd_evt (
  UINT8  *data
  );

static void
smp_dhkey_check_cmd_evt (
  UINT8  *data
  );
...
static void
smp_numeric_check_value_generate_evt (
  UINT8  *data
  );
...
smp_c1_1 (
  UINT8  *tk,
  UINT8  *mrand,
  UINT8  *smp_pair_req_cmd,
  UINT8  *smp_pair_rsp_cmd,
  UINT8  initiating_dev_type,
...
smp_c1_2 (
  UINT8  *tk,
  UINT8  *p1,
  UINT8  *intiating_dev_add,
  UINT8  *responding_dev_add
  );
...
/*Timer functions*/
static void
smp_cmd_timer_expire_cb (
  osa_timer_arg_t  arg
  );

static int
smp_create_cmd_timer (
  );

static int
smp_destroy_cmd_timer (
  );
...
static void
smp_app_timer_expire_cb (
  osa_timer_arg_t  arg
  );

static int
smp_create_app_timer (
  );
  ...
  )
{
  ...
  if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
    /*
     * We are peripheral. So this authorization grant event is due to a pairing
    ...
    if ((cur_cb->local_auth_req & SMP_SC_Y) && (cur_cb->peer_auth_req & SMP_SC_Y)) {
      ...
      smp_decide_asso_model ();
      /* Since we are the peripheral, we need to wait for central public key*/
    } else
 #endif
    {
      /* Legacy pairing */
    ...
    /* We are central and we are responding to a security request.*/
  ...
  )
{
  ...
  if (cur_cb->state == SMP_STATE_WAIT_NC) {
    if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
      ...
      /* We are central. We should go a head and create the LTK */
  ...
  )
{
  ...
  if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
    ...
    if (cur_cb->state == SMP_STATE_WAIT_CNF_CMD) {
       ...
       * So peripheral will wait for mconfirm
       */
      cur_cb->state = SMP_STATE_WAIT_CNF_CMD;
    }
  } else {
    /* We are central. we will proceed with random number generation */
    cur_cb->state = SMP_STATE_WAIT_CNF_GEN;
    smp_gen_rand ();
  }
}
  ...
  )
{
  ...
  if ((cur_cb->local_auth_req & SMP_SC_Y) && (cur_cb->peer_auth_req & SMP_SC_Y)) {
    ...
    smp_decide_asso_model ();
    if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
      /* Central will have to share his public key now*/
      cur_cb->state = SMP_STATE_WAIT_SC_PUB_KEY;
    } else {
      /* We are the central, we will send the public key and wait for the
       * peer public key
       */
      cur_cb->state = SMP_STATE_WAIT_SC_PUB_KEY;
      smp_send_pub_key_cmd ();
  ...
  )
{
  ...
  /* if we are the peripheral, then we will not receive this command.
  ...
  )
{
  ...
  /* Errata - 15671 Check to ensure Confirm values are different */
  if (memcmp (cur_cb->local_confirm, cur_cb->peer_confirm, SMP_CONFIRM_LEN) == 0) {
    logw ("Same Confirm Value is not allowed");
    ...
    switch (cur_cb->asso_model) {
      ...
      case SMP_IDRI_NC:
        ...
        if (!g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role && (cur_cb->state == SMP_STATE_WAIT_CNF_CMD)) {
          /* Only central will receive a confirm command in these association model*/
      ...
      case SMP_IIRI:
        if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
          if (cur_cb->state == SMP_STATE_WAIT_PASSKEY) {
            /* This might be the case where the peripheral is waiting for the passkey
             * from upper layer
             */
            cur_cb->state = SMP_STATE_WAIT_CNF_CMD;
          } else if (cur_cb->state == SMP_STATE_WAIT_CNF_CMD) {
  ...
  )
{
  ...
  /* Check to ensure Random values are different */
  if (memcmp (cur_cb->local_rand, cur_cb->peer_rand, SMP_RAND_LEN) == 0) {
    logw ("Same Random Value is not allowed");
    cur_cb->status = SMP_UNSPECIFIED_REASON;
    smp_pair_failed_evt ();
    return;
  }
  ...
  if (cur_cb->pairing_method == SMP_LEGACY_PAIRING) {
    ...
    if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
      ...
      switch (cur_cb->asso_model) {
        case SMP_JW:
          /* Send the random number and then wait for dhkey check from central*/
          cur_cb->state = SMP_STATE_WAIT_DHKEY_CHECK_CMD;
          smp_send_rand_cmd ();
          break;
        case SMP_JW_NC:
...
/* This function is called when a central identification command is received on the LE-U link*/
static void
smp_mast_id_cmd_evt (
  UINT8  *data
  )
  ...
  )
{
  ...
  /* Errata - 15668: Local and Remote Public Key shall not be same.
   * i.e. Both X and Y coordinate shall not be same.
   * Errata - 17113: X coordinate shall not be same.
   * Hence checking only X coordinate value satisfies both Erratas.
   * Condition stands still even if remote uses Debug Keys,
   * since Debug Key is not used by Local device.
   */
  if (memcmp (cur_cb->peer_p256_key_x, g_local_adapter.smp_data.local_p256_key_x, SMP_PUB_KEY_COORDINATE_LEN) == 0) {
    logw ("Matching public key should be rejected");
    cur_cb->status = SMP_INVALID_PARAM;
    smp_pair_failed_evt ();
    return;
  }
  ...
  )
{
  ...
  if ((cur_cb->state == SMP_STATE_WAIT_NC) || (cur_cb->state == SMP_STATE_WAIT_NUMERIC_VALUE)) {
    ...
    if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
      /*We are peripheral. We need to generate mackey and LTK and then re-compute
       * the dhkey check sent by the central
       */
      cur_cb->state = SMP_STATE_WAIT_LTK_GEN;
      smp_gen_ltk ();
    } else {
      /* We are the central. We need to recompute the dhkey of peripheral
       * and compare
       */
 ...
 *  comparison or just works, the central will not generate a confirm(commitment).
  ...
  )
{
  ...
  if (cur_cb->state == SMP_STATE_WAIT_RAND_GEN) {
    /*
     * The random number was generated on central side in case of
  ...
  )
{
  ...
  if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
    /* We are the peripheral. We should start distributing the keys*/
    cur_cb->state = SMP_STATE_WAIT_LOCAL_KEY_DIST;
    smp_start_key_dist ();
  } else {
    /* We are the central. We should first receive the keys from the peripheral*/
  ...
  )
{
  ...
  if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
    ...
    if (cur_cb->state == SMP_STATE_WAIT_PEER_KEY_DIST) {
      ...
      /* If we are done with the local key distribution, then wait for the central
    ...
    /* We are central*/
    if (cur_cb->state == SMP_STATE_WAIT_PEER_KEY_DIST) {
      /* If peripheral has done with the key distribution, then start key distribution*/
      cur_cb->state = SMP_STATE_WAIT_LOCAL_KEY_DIST;
      smp_start_key_dist ();
    } else if (cur_cb->state == SMP_STATE_WAIT_LOCAL_KEY_DIST) {
  ...
  )
{
  ...
  if (g_local_adapter.connected_device[g_local_adapter.smp_data.smp_cur_cb.conn_id]->local_link_role) {
    /* We are peripheral. We will send the public key now*/
    smp_send_pub_key_cmd ();
  } else {
  ...
  )
{
  ...
  smp_start_app_timer ();
  smp_mgr_cback (
                 g_local_adapter.connected_device[g_local_adapter.smp_data.smp_cur_cb.conn_id]->id_addr.bdaddr,
                 SMP_NUMERIC_COMPARISON_EVT,
                 (UINT8 *)&numeric_value,
  ...
  )
{
  ...
  if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
    /* We are peripheral
     * We need to re-calculate the dhkey check sent by the central
     */
    cur_cb->state = SMP_STATE_WAIT_PEER_DHKEY_CHECK_GEN;
    iocap[0]      = cur_cb->peer_auth_req;
    iocap[1]      = cur_cb->peer_oob_flag;
  ...
  )
{
  ...
  if (cur_cb->state == SMP_STATE_WAIT_DHKEY_CHECK_GEN) {
    ...
    if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
      /* We are peripheral. Once dhkey is re-calculated by the central, he is expected to
       * initiate the encryption on the link with the LTK generated.
       */
      cur_cb->state = SMP_STATE_WAIT_ENC_CHG;
    } else {
      /* We are central. The peripheral is expected to send his dhkey check value now*/
      cur_cb->state = SMP_STATE_WAIT_DHKEY_CHECK_CMD;
    }
  ...
  )
{
  ...
  switch (cmd_opcode) {
    ...
    case SMP_ENC_INF:
      ...
      break;
    case SMP_CENTRAL_IDF:
      logd ("Central Identification");
      smp_stop_cmd_timer ();
 #if (PARANOID_STACK == TRUE)
      if (data_len != SMP_CENTRAL_IDF_LEN) {
        logd ("Data length not matching with pairing random length");
        break;
      }
  ...
  )
{
  ...
  data_pkt = (buffer_t *)get_l2cap_free_buffer (SMP_CENTRAL_IDF_LEN);
  if (NULL == data_pkt) {
    logd ("L2cap packet unavailable");
    return STATUS_ERR_INSUFFICIENT_RESOURCES;
  }
  ...
  data_pkt->offset = L2CAP_HDR_SIZE + UART_HDR_SIZE + HCI_ACL_PREAMBLE_SIZE;
  data_pkt->length = SMP_CENTRAL_IDF_LEN;
  cmd              = &data_pkt->data[data_pkt->offset];
  UINT8_TO_STREAM (cmd, SMP_CENTRAL_IDF);
  ...
  )
{
  ...
  switch (cur_cb->asso_model) {
    case SMP_JW:
      ...
      if (cur_cb->pairing_method == SMP_LEGACY_PAIRING) {
        if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
          /* We are peripheral. We need to wait for the confirm command from central*/
          cur_cb->state = SMP_STATE_WAIT_CNF_CMD;
        } else {
          /* We are central. we need to
           * 1. Generate a 128 bit random number.
           * 2. Generate the confirm value.
           * 3. Send it the peripheral.
           */
          cur_cb->state = SMP_STATE_WAIT_CNF_GEN;
          smp_gen_rand ();
        }
        ...
        if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
          ...
          /* We are central. We need to wait for confirmation from peripheral*/
          cur_cb->state = SMP_STATE_WAIT_CNF_CMD;
        }
      }
 #endif /* SECURE_CONNECTION */
    ...
    case SMP_IIRD:
      ...
      if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
        ...
        if (hci_ble_rand () != STATUS_SUCCESS) {
          smp_proc_hci_rand (NULL, 0, HCI_EVENT_STATUS_HARDWARE_FAILURE);
        }
      } else {
  ...
  )
{
  ...
  smp_start_cmd_timer ();
  if (hci_ble_start_encryption (g_local_adapter.connected_device[cur_cb->conn_id]->acl_handle, rand, 0, cur_cb->stk) != STATUS_SUCCESS) {
    UINT8  buf = HCI_EVENT_STATUS_HARDWARE_FAILURE;
    smp_enc_chg_cback (&buf, 1);
  }
}
  ...
  )
{
 ...
 #endif
  {
    if (hci_ble_ltk_req_reply (conn_handle, ltk) != STATUS_SUCCESS) {
      UINT8  buf = HCI_EVENT_STATUS_HARDWARE_FAILURE;
      smp_enc_chg_cback (&buf, 1);
    }
  }
  ...
  )
{
  ...
  if (hci_ble_encrypt (g_local_adapter.smp_data.smp_cur_cb.cmac_key, c) != STATUS_SUCCESS) {
    smp_proc_hci_le_encrypt (NULL, 0, HCI_EVENT_STATUS_HARDWARE_FAILURE);
  }
}
  ...
  )
{
  ...
  cal_dhkey (
             g_local_adapter.smp_data.local_p256_private_key,
             cur_cb->peer_p256_key_x,
             cur_cb->peer_p256_key_y,
             cur_cb->dhkey
             );
  ...
  )
{
 ...
 #if (CONTROLLER_ECDH)
  ret = hci_ble_generate_dhkey (
                                g_local_adapter.smp_data.smp_cur_cb.peer_p256_key_x,
                                g_local_adapter.smp_data.smp_cur_cb.peer_p256_key_y,
                                SMP_PUB_KEY_COORDINATE_LEN
                                );
  if (ret != STATUS_SUCCESS) {
    UINT8  buf = HCI_EVENT_STATUS_HARDWARE_FAILURE;
    smp_proc_dhkey (&buf, 1);
    return;
  }
  ...
  )
{
  ...
  if (memcmp (cfm, cur_cb->peer_confirm, SMP_CONFIRM_LEN) == 0) {
    logd ("dhkey matches");
    if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
      /* We are the peripheral. Now we need to generate our local dhkey check and
       * send to the central.
       */
      cur_cb->state = SMP_STATE_WAIT_DHKEY_CHECK_GEN;
      UINT8  iocap[3];
      ...
      /* We are the central. We will start the encryption now*/
      UINT8  rand[8];
      memset (rand, 0, 8);
      cur_cb->state = SMP_STATE_WAIT_ENC_CHG;
      if (hci_ble_start_encryption (g_local_adapter.connected_device[cur_cb->conn_id]->acl_handle, rand, 0, cur_cb->cur_key.peer_ltk) != STATUS_SUCCESS) {
        UINT8  buf = HCI_EVENT_STATUS_HARDWARE_FAILURE;
        smp_enc_chg_cback (&buf, 1);
      }
    }
  ...
  )
{
  ...
  if (status == HCI_EVENT_STATUS_HARDWARE_FAILURE) {
    smp_stop_cmd_timer ();
    smp_stop_app_timer ();
    smp_mgr_cleanup ();
    return;
  ...
  )
{
  logi ("");
  if (status == HCI_EVENT_STATUS_HARDWARE_FAILURE) {
    g_local_adapter.smp_data.smp_cur_cb.status = HCI_EVENT_STATUS_HARDWARE_FAILURE;
    smp_clean_up ();
  }
  ...
  if (memcmp (device->bdaddr, g_local_adapter.smp_data.smp_cur_cb.bdaddr, BD_ADDR_LEN) == 0) {
    ...
    if (cur_cb->pairing_method == SMP_LEGACY_PAIRING) {
      ...
      if (cur_cb->enc_with_ltk == TRUE) {
        ...
        cur_cb->state = SMP_STATE_WAIT_ENC_CHG;
        if (hci_ble_ltk_req_reply (conn_handle, cur_cb->cur_key.local_ltk) != STATUS_SUCCESS) {
          UINT8  buf = HCI_EVENT_STATUS_HARDWARE_FAILURE;
          smp_enc_chg_cback (&buf, 1);
        }
      }
  ...
  )
{
  ...
  switch (cur_cb->enc_stage) {
      ...
      if (g_local_adapter.smp_mgr_data.storage_mgr.storage_cb) {
        if (g_local_adapter.smp_mgr_data.storage_mgr.storage_cb->key_update_cb) {
          g_local_adapter.smp_mgr_data.storage_mgr.storage_cb->key_update_cb (
                                                                              g_local_adapter.smp_mgr_data.storage_mgr.trans_id,
                                                                              dummy_bdaddr,
                                                                              0,
                                                                              LOCAL_IRK,
                                                                              16,
                                                                              g_local_adapter.smp_data.device_irk
                                                                              );
        }

        if (g_local_adapter.smp_mgr_data.storage_mgr.storage_cb->key_request_cb) {
          g_local_adapter.smp_mgr_data.storage_mgr.storage_cb->key_request_cb (
                                                                               g_local_adapter.smp_mgr_data.storage_mgr.trans_id,
                                                                               dummy_bdaddr,
                                                                               0,
                                                                               PEER_IRK
    ...
    case SMP_ENC_CONFIRM_S2:
      ...
      if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
        log (
             "peer bdaddr: to [%02x:%02x:%02x:%02x:%02x:%02x]\n",
             g_local_adapter.connected_device[cur_cb->conn_id]->ota_bdaddr[0],
             g_local_adapter.connected_device[cur_cb->conn_id]->ota_bdaddr[1],
             g_local_adapter.connected_device[cur_cb->conn_id]->ota_bdaddr[2],
             g_local_adapter.connected_device[cur_cb->conn_id]->ota_bdaddr[3],
             g_local_adapter.connected_device[cur_cb->conn_id]->ota_bdaddr[4],
             g_local_adapter.connected_device[cur_cb->conn_id]->ota_bdaddr[5]
             );
        logi ("addrty: %d", g_local_adapter.connected_device[cur_cb->conn_id]->local_addr_type);
        ...
        log (
             "peer bdaadr: to [%02x:%02x:%02x:%02x:%02x:%02x]\n",
             g_local_adapter.connected_device[cur_cb->conn_id]->ota_bdaddr[0],
             g_local_adapter.connected_device[cur_cb->conn_id]->ota_bdaddr[1],
             g_local_adapter.connected_device[cur_cb->conn_id]->ota_bdaddr[2],
             g_local_adapter.connected_device[cur_cb->conn_id]->ota_bdaddr[3],
    ...
    case SMP_ENC_SUB_KEY:
      ...
      cur_cb->cur_block++;
      if (hci_ble_encrypt (cur_cb->cmac_key, p1) != STATUS_SUCCESS) {
        smp_proc_hci_le_encrypt (NULL, 0, HCI_EVENT_STATUS_HARDWARE_FAILURE);
      }
 ...
 #endif
      log (
           "BDADRR Changed to [%02x:%02x:%02x:%02x:%02x:%02x]\n",
           cur_cb->rpa[0],
           cur_cb->rpa[1],
           cur_cb->rpa[2],
           cur_cb->rpa[3],
    ...
    case SMP_ENC_RAND_P256_4:
      ...
      REVERSE_STREAM_TO_ARRAY (pp, data, 8);
      if (gen_public_private_key_pair (
                                       g_local_adapter.smp_data.local_p256_private_key,
                                       g_local_adapter.smp_data.local_p256_private_key,
                                       g_local_adapter.smp_data.local_p256_key_x,
                                       g_local_adapter.smp_data.local_p256_key_y
                                       ) == STATUS_SUCCESS)
      {
  ...
  )
{
  ...
  hex_dump ("KEY:", key, 16);
  if (hci_ble_encrypt (key, plain_text) != STATUS_SUCCESS) {
    smp_proc_hci_le_encrypt (NULL, 0, HCI_EVENT_STATUS_HARDWARE_FAILURE);
  }
}
  ...
  )
{
  ...
  if (hci_ble_read_local_p256_public_key () != STATUS_SUCCESS) {
    UINT8  buf = HCI_EVENT_STATUS_HARDWARE_FAILURE;
    smp_proc_p256_public_key (&buf, 1);
  }
...
{
  return osa_create_timer (
                           &(g_local_adapter.smp_data.smp_cmd_timer_id),
                           g_local_adapter.smp_data.SMP_CMD_TIMER,
                           g_local_adapter.smp_data.SMP_CMD_TIMER_ISR,
                           sizeof (g_local_adapter.smp_data.SMP_CMD_TIMER)/sizeof (osa_timer_static_t),
                           smp_cmd_timer_expire_cb,
                           0
                           );
}
...
{
  return osa_create_timer (
                           &(g_local_adapter.smp_data.smp_app_timer_id),
                           g_local_adapter.smp_data.SMP_APP_TIMER,
                           g_local_adapter.smp_data.SMP_APP_TIMER_ISR,
                           sizeof (g_local_adapter.smp_data.SMP_APP_TIMER)/sizeof (osa_timer_static_t),
                           smp_app_timer_expire_cb,
                           0
                           );
}
  ...
  )
{
  ...
  g_local_adapter.smp_data.smp_cur_cb.operation = SMP_BONDING_OPERATION;
  if (g_local_adapter.connected_device[cur_cb->conn_id]->local_link_role) {
    /* Peripheral ... send security request */
    UINT8  sec_level = 0;
  ...
  )
{
  ...
  if (g_local_adapter.smp_mgr_data.storage_mgr.storage_cb && g_local_adapter.smp_mgr_data.storage_mgr.storage_cb->key_update_cb) {
    ...
    g_local_adapter.smp_mgr_data.storage_mgr.storage_cb->key_update_cb (g_local_adapter.smp_mgr_data.storage_mgr.trans_id, device->id_addr.bdaddr, device->id_addr.type, PEER_COUNTER, 4, sc);
  }
