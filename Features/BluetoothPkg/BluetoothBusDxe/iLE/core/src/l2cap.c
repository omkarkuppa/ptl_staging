...
typedef INT8 (*l2cap_act_t)(
  connected_device_t  *device,
  UINT8               *d
  );
typedef INT8 (*l2cap_coc_act_t)(
  connected_device_t  *device,
  UINT8               ccb_idx,
  UINT8               *d
  );
...
static INT8
l2cap_act_init_connect_param_update (
  connected_device_t  *device,
  UINT8               *d
  );
...
static INT8
l2cap_send_command_conn_param_update_res (
  connected_device_t  *device,
  UINT8               id,
  UINT16              result
  );
...
static INT8
coc_pick_from_queue_to_send_data (
  connected_device_t  *device,
  UINT8               handle
  );

static INT8
coc_segment_and_send_mtu_data (
  connected_device_t  *device,
  UINT8               *buffer,
  UINT16              len,
  UINT16              r_mps,
  ...
  )
{
  ...
  if (l2cap_coc_channel_tbl->l2cap_coc_cb && l2cap_coc_channel_tbl->l2cap_coc_cb->connection_cmpl_cb) {
    l2cap_coc_channel_tbl->l2cap_coc_cb->connection_cmpl_cb (
                                                             l2cap_coc_channel_tbl->owner_trans_id,
                                                             device->idx,
                                                             l2cap_coc_channel_tbl->psm,
                                                             ccb_idx,
                                                             l2cap_coc_channel_tbl->r_mtu,
                                                             MAX_NUMBER_OF_L2CAP_COC_TX_BUFFER,
                                                             result
                                                             );
  }
  ...
  )
{
  ...
  if (data_pkt->pb_flag == L2CAP_PB_FLAG_FIRST_FLUSHABLE_PKT) {
    /* This is the first frame. This packet has the headers */
  ...
  switch (device->data_rx_pkt.cid) {
    ...
    {
 #if (L2CAP_COC == TRUE)
      if (!device || !device->data_rx_pkt.l2cap_data) {
        return STATUS_ERR_INVALID_PARAM;
      }
  ...
  )
{
  ...
  if (id == 0) {
    // TODO: match identifier for reponse pkt and for command pkt check if identifier is not same as last pkt
  ...
  switch (l2cap_command) {
    case L2CAP_COMMAND_CONNECTION_PARAM_UPDATE_REQ:
    {
      ...
      if (device->local_link_role == LINK_ROLE_CENTRAL) {
        ...
        if (buffer_length < sizeof (interval_min) + sizeof (interval_max) + sizeof (peripheral_latency) + sizeof (timeout_mul)) {
          return STATUS_ERR_INVALID_PARAM;
        }

        STREAM_TO_UINT16 (interval_min, buffer);
        STREAM_TO_UINT16 (interval_max, buffer);
        STREAM_TO_UINT16 (peripheral_latency, buffer);
        STREAM_TO_UINT16 (timeout_mul, buffer);
        buffer_length -= (sizeof (interval_min) + sizeof (interval_max) + sizeof (peripheral_latency) + sizeof (timeout_mul));
        if (gap_update_conn_params (device->idx, interval_min, interval_max, peripheral_latency, timeout_mul) == STATUS_SUCCESS) {
          /* Change the state to update continue */
          return l2cap_csm_handler (
                                    L2CAP_EVT_INIT_CONNECT_PARAM_UPDATE,
                                    device,
                                    NULL
                                    );
 ...
 #endif
      {
        /* Peripheral cannot receive this. Reject this */
    ...
    {
      if (length != L2CAP_COC_COMMAND_CONNECT_REQ_LEN) {
        return STATUS_ERR_INVALID_PARAM;
      }
...
static INT8
l2cap_send_command_conn_param_update_req (
  connected_device_t  *device,
  UINT16              interval_min,
  UINT16              interval_max,
  UINT16              conn_latency,
  ...
  )
{
  ...
  if (device->local_link_role == LINK_ROLE_CENTRAL) {
    ...
    return l2cap_send_command_conn_param_update_res (
                                                     device,
                                                     device->l2cap_cur_id,
                                                     result
                                                     );
  ...
  )
{
  ...
  if (device->l2cap_state != L2CAP_STATE_OPEN) {
    if (l2cap_coc_cbs->connection_cmpl_cb) {
      l2cap_coc_cbs->connection_cmpl_cb (
                                         trans_id,
                                         conn_id,
                                         psm,
                                         0xFF,
                                         0xFFFF,
                                         0,
                                         STATUS_ERR_INAVALID_CONNENCTION
                                         );
    }
  ...
  if (ccb_idx == MAX_NUMBER_OF_L2CAP_COC_CHNL) {
    if (l2cap_coc_cbs->connection_cmpl_cb) {
      l2cap_coc_cbs->connection_cmpl_cb (
                                         ...
                                         0xFFFF,
                                         0,
                                         STATUS_ERR_INSUFFICIENT_RESOURCES
                                         );
    }
...
INT8
l2cap_coc_register_cb_for_conn_req (
  trans_id_t             trans_id,
  UINT16                 psm,
  UINT16                 mtu,
  BOOLEAN                auth_req,
  ...
  )
{
  ...
  if ((buffer == NULL) || (buffer_len > l2cap_coc_channel_tbl->r_mtu)) {
    return STATUS_ERR_INVALID_PARAM;
  }
...
void
l2cap_recv_conn_param_update_evt (
  UINT8  *buffer,
  UINT8  len
  )
{
