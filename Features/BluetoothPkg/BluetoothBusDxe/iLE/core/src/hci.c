  ...
  )
{
 ...
 #if (SMP_FEATURE)
      smp_proc_hci_rand (buffer, event_length, status);
 #endif /* SMP_FEATURE */
      break;
    case HCI_CMD_LE_ENCRYPT:
 #if (SMP_FEATURE)
      smp_proc_hci_le_encrypt (buffer, event_length, status);
 #endif /* SMP_FEATURE */
      break;
      // FIXME: Remove this
 ...
 */
static void
hci_num_cmpl_data_pkts_evt_cmpl (
  UINT8   *buffer,
  UINT16  len
  )
{
  ...
  )
{
  ...
  if (g_local_adapter.device_status == STATUS_ERR_DEVICE_ERROR) {
    loge ("BT device is in BAD state!!!!");
  ...
  if (g_local_adapter.hci_data.hci_credit > 0) {
    ...
    if (p_buf) {
      ...
      ret = hci_send_msg (HCI_CMD, p_buf, len + UART_HDR_SIZE + HCI_CMD_PREAMBLE_SIZE);
 ...
 #endif
      } else if (ret == STATUS_ERR_DEVICE_ERROR) {
        g_local_adapter.device_status = STATUS_ERR_DEVICE_ERROR;
        remote_command_timeout_handler (0);
        for (idx = 0; idx < MAX_NUMBER_OF_APP; idx++) {
          if (g_local_adapter.app_list[idx]) {
            iLE_deregister_application (idx);
          }
        }
      }
  ...
  )
{
  ...
  if (device->acl_handle > 0xEFF) {
    /* ACL handlel range 0 - 0xeff */
