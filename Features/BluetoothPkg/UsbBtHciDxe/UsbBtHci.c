  ...
  )
{
  ...
  Status = gBS->SetTimer (
                  UsbBtHciDev->AclEvent,
                  TimerPeriodic,
                  UsbBtHciDev->AclCallbackContext.PollingInterval * 1000 * 10 // Transfer PollingInterval value from unit ns to millisecond, PollingInterval (unit 100ns) * 10 (unit microsecond) * 1000 (unit millisecond)
                  );
  if (EFI_ERROR(Status)) {
