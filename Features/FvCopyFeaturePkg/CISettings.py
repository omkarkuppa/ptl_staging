## @file
# CI configuration for Unit Tests in FreeSpaceSkipFvCopyHostTest module
#
#  @copyright
#  INTEL CONFIDENTIAL
...
class Settings(CiBuildSettingsManager, UpdateSettingsManager, SetupSettingsManager, PrEvalSettingsManager):
    ...
    def GetPackagesSupported(self):
        ''' return iterable of Intel packages supported by this build.
        These should be Intel workspace relative paths '''

        return ("FvCopyFeaturePkg","")
    ...
    def GetWorkspaceRoot(self):
        ''' get WorkspacePath '''
        WorkspaceRootPath = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        WorkspaceRootPath = WorkspaceRootPath[:WorkspaceRootPath.find("Intel")]
        return WorkspaceRootPath

    def GetPackagesPath(self):
        PathList = glob.glob(os.path.join(self.GetWorkspaceRoot (), '**/*.dec'), recursive=True)
        for (Index, Item) in enumerate (PathList):
            PathList[Index] = os.path.dirname (os.path.dirname(Item))
        return set (PathList)
