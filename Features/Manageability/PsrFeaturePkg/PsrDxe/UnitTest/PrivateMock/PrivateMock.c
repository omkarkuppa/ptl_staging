...
{
  check_expected_ptr (Protocol);
  check_expected_ptr (DevicePath);
  check_expected_ptr (Device);

  *Device = mock_ptr_type (EFI_HANDLE*);
  ...
  )
{
  ...
  Time->Year   = (UINT16) tm.tm_year + 1900;
  Time->Month  = (UINT8) tm.tm_mon + 1;
  Time->Day    = (UINT8) tm.tm_mday;
  Time->Hour   = (UINT8) tm.tm_hour;
  Time->Minute = (UINT8) tm.tm_min;
  Time->Second = (UINT8) tm.tm_sec;
  ...
  )
{
  ...
  check_expected_ptr (RootFile);

  *RootFile = mock_ptr_type (EFI_FILE_PROTOCOL*);
...
{
  check_expected_ptr (This);
  check_expected_ptr (NewHandle);
  check_expected_ptr (FileName);
  check_expected (OpenMode);
  check_expected (Attributes);

  *NewHandle = mock_ptr_type (EFI_FILE_PROTOCOL*);
