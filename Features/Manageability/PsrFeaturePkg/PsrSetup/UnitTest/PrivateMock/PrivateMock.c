  ...
  )
{
  ...
  Buffer = AllocateZeroPool (sizeof (PSR_DATA));
  if (Buffer == NULL) {
    return FALSE;
  }

  if (!RandomData (Buffer, sizeof (PSR_DATA), FALSE)) {
    return FALSE;
  }

  ZeroMem (Buffer + EVENT_DATA_TOTAL_COUNT_START_OFFSET, sizeof (UINT32));
  if (!RandomData (Buffer + EVENT_DATA_TOTAL_COUNT_START_OFFSET, sizeof (UINT8), FALSE)) {
    return FALSE;
  }

  if (!RandomData (Buffer + GENESIS_OEM_INFO_OFFSET, MAX_STRING_LENGTH, TRUE)) {
    return FALSE;
  }

  if (!RandomData (Buffer + GENESIS_OEM_MAKE_OFFSET, MAX_STRING_LENGTH, TRUE)) {
    return FALSE;
  }

  if (!RandomData (Buffer + GENESIS_OEM_MODEL_OFFSET, MAX_STRING_LENGTH, TRUE)) {
    return FALSE;
  }

  if (!RandomData (Buffer + GENESIS_MANUFAC_COE_OFFSET, MAX_STRING_LENGTH, TRUE)) {
    return FALSE;
  }

  for (Index = 0; Index < PSR_MAX_EVENT_COUNT; Index++) {
    if (!RandomData (Buffer + EVENT_LOG_START_OFFSET + (SINGLE_EVENT_SIZE * Index) + 8, PSR_MAX_EVENT_DATA_LENGTH, TRUE)) {
      return FALSE;
    }
  }
  ...
  )
{
  ...
  Handle = mock_ptr_type (EFI_HANDLE*);
  ...
  )
{
  ...
  check_expected (PsrDataBlobSize);
  check_expected_ptr (PsrDataBlob);

  CopyMem (PsrDataBlob, mock_ptr_type (UINT8*), PsrDataBlobSize);
...
{
  check_expected_ptr (PackageList);
  check_expected (((MOCK_HII_HANDLE *) PackageList)->Signature);
  check_expected (StringId);
  check_expected_ptr (Language);
  check_expected_ptr (String);
  check_expected_ptr (StringFontInfo);
