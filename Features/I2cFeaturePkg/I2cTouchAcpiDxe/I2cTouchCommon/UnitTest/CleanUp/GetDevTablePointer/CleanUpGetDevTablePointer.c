...
#include "CleanUpGetDevTablePointer.h"

#define CLEAN_UP_FUNCTION_NAME     "CleanUpGetDevTablePointer"
#define CLEAN_UP_FUNCTION_VERSION  "0.1"

/**
  Clean up all variables used in testing GetDevTablePointer function.
...
EFI_STATUS
EFIAPI
CleanGetDevTablePointerEnvironment (
  VOID
  )
{
  FreePool (gBS);
  ...
  )
{
  ...
  Status = CleanGetDevTablePointerEnvironment ();
