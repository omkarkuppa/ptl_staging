...
#include "CleanUpI2cDevPatchEntry.h"

#define CLEAN_UP_FUNCTION_NAME     "CleanUpI2cDevPatchEntry"
#define CLEAN_UP_FUNCTION_VERSION  "0.1"

/**
  Clean up all variables used in testing I2cDevPatchEntry function.
...
EFI_STATUS
EFIAPI
CleanI2cDevPatchEntryEnvironment (
  VOID
  )
{
  FreePool (gBS);
  ...
  )
{
  ...
  Status = CleanI2cDevPatchEntryEnvironment ();
