...
#include "CleanUpPatchI2cDevicesBuffer.h"

#define CLEAN_UP_FUNCTION_NAME     "CleanUpPatchI2cDevicesBuffer"
#define CLEAN_UP_FUNCTION_VERSION  "0.1"

/**
  Clean up all variables used in testing PatchI2cDevicesBuffer function.
...
EFI_STATUS
EFIAPI
CleanPatchI2cDevicesBufferEnvironment (
  VOID
  )
{
  FreePool (gI2cDevicesAcpiTable);
  FreePool (gMockI2cTPTD);
  ...
  )
{
  ...
  Status = CleanPatchI2cDevicesBufferEnvironment ();
