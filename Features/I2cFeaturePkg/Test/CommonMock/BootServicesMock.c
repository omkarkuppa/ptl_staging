  ...
  )
{
  ...
  pProtocolList = mock_ptr_type (PROTOCOLS_LIST **);
  ...
  do {
    ...
    if (Protocol != NULL) {
      if (ProtocolList == NULL) {
        ProtocolList = AllocateZeroPool (sizeof (PROTOCOLS_LIST));
      } else {
        //
        // Reallocate buffer for ProtocolList if more protocols is detected.
        //
        ProtocolList = ReallocatePool (sizeof (PROTOCOLS_LIST), sizeof (PROTOCOLS_LIST) * (Index + 1), ProtocolList);
      }
      ProtocolList[Index].Protocol = Protocol;
      VA_ARG (Args, VOID *);
      Index++;
    }
  } while (Protocol != NULL);
  pCounter = mock_ptr_type (UINT8 *);
  *pCounter = Index;
  *pProtocolList = ProtocolList;
  VA_END (Args);
  return mock_type (EFI_STATUS);
}
/**
  ...
  do {
    ...
    if (Protocol != NULL) {
      if (ProtocolList == NULL) {
        ...
        // Reallocate buffer for ProtocolList if more protocols is detected.
        //
        ProtocolList = ReallocatePool (sizeof (PROTOCOLS_LIST) * Index, sizeof (PROTOCOLS_LIST) * (Index + 1), ProtocolList);
      }
      ProtocolList[Index].Protocol = Protocol;
      VA_ARG (Args, VOID *);
      Index++;
  ...
  )
{
  ...
  *UserHandle = mock_type (EFI_HANDLE);
  if (Interface != NULL) {
