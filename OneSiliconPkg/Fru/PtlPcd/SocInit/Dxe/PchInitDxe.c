...
#include <Library/ThcConfigurePrivateLib.h>
...
THC_RESET_PROTOCOL  mThcResetProtocol = {
  THC_RESET_PROTOCOL_REVISION,
  ...
  )
{
  ...
  //
  // Create events for PCH to do the task before ExitBootServices
  ...
  )
{
  ...
  //
  // Install THC Reset Protocol.
  // Installing gThcResetProtocolGuid should probably take place in the platform code.
  //
  NewHandle = NULL;
  Status = gBS->InstallProtocolInterface (
                  &NewHandle,
                  &gThcResetProtocolGuid,
...
/**
  PCH initialization before ExitBootServices event.
...
/**
  PCH initialization on ExitBootService.
  ...
  )
{
  ...
  NumOfRootPorts        = GetPchMaxPciePortNum();
  ...
  for (PortIndex = 0; PortIndex < NumOfRootPorts; PortIndex++) {
    ...
    if (PciSegmentRead16 (RpBase) != 0xFFFF) {
      ...
      // HSD: 13010408261
      //
      if (mPcieRootPortConfig[PortIndex].PcieRpCommonConfig.HostL0sTxDis) {
        HostL0sTxDisable (RpBase);
      }
    }
  }
  ...
  ThcConfigHob = (THC_CONFIG_HOB *) GET_GUID_HOB_DATA (HobPtr.Guid);
  for (PortIndex = 0; PortIndex < GetPchMaxThcControllersNum (); PortIndex++) {
    ThcBootHandler ((UINT8) PortIndex, &ThcConfigHob->ThcPort[PortIndex]);
  }
