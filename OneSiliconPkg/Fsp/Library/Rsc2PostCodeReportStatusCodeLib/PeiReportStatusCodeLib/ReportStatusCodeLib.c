/** @file
  ...
  If incoming RSC value is Intel RC PostCode value, then get the
  corresponding value from the MapLib and send it via PostCodeLib API.
  ...
  )
{
      ...
      (ReportDebugCodeEnabled () && (((Type) & EFI_STATUS_CODE_TYPE_MASK) == EFI_DEBUG_CODE)))
  {
    ...
    //
    // Check if incoming RSC value is Intel RC PostCode value
    // Get the corresponding value from the MapLib and send it via PostCodeLib API
    //
    if ( (((Type) & EFI_STATUS_CODE_TYPE_MASK) == EFI_PROGRESS_CODE) &&  ((Value & INTEL_RC_STATUS_CODE_MASK) == INTEL_RC_STATUS_CODE)) {
       Status = (*PeiServices)->LocatePpi (PeiServices, &gFspRsc2PostCodeMapServicePpiGuid, 0, NULL, (VOID **)&mFspRsc2PostCodeMapServicePpi);
       if (!EFI_ERROR (Status)) {
        mFspRsc2PostCodeMapServicePpi->PostCodeFromRscMap (Value);
       }
    }
