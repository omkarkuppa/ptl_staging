...
{
  EFI_GUID            *GuidPtr;
  EFI_STATUS          FspReturnStatusResetType;

  GuidPtr = NULL;
  FspReturnStatusResetType = 0;

  DEBUG ((DEBUG_INFO, "[FspResetSystem2] FSP Reset Initiated\n"));
  ...
  )
{
  ...
  if (FspData->FspMode == FSP_IN_API_MODE) { // FSP_MODE_CHECK
    ...
    REPORT_STATUS_CODE (EFI_PROGRESS_CODE, INTEL_RC_STATUS_CODE_FSP_MEM_INIT_API_EXIT); //PostCode (0x87F)
    ...
    if (GetFspApiCallingIndex () == TempRamExitApiIndex) {
      ...
      REPORT_STATUS_CODE (EFI_PROGRESS_CODE, INTEL_RC_STATUS_CODE_FSP_MEM_TEMPRAMEXIT_API_ENTRY); //PostCode (0x800)
      ...
      //FSP Common Module(0x800) | Api Exit(0x7F)
      //
      REPORT_STATUS_CODE (EFI_PROGRESS_CODE, INTEL_RC_STATUS_CODE_FSP_MEM_TEMPRAMEXIT_API_EXIT); //PostCode (0x87F)
      SetFspApiReturnStatus (EFI_SUCCESS);
      Pei2LoaderSwitchStack ();
    }
    ...
    //FSP Common Module(0x800) | Api Entry(0x00)
    REPORT_STATUS_CODE (EFI_PROGRESS_CODE, INTEL_RC_STATUS_CODE_FSP_SILICON_INIT_API_ENTRY); //PostCode (0x800)
    //
  ...
  )
{
  ...
  SI_PREMEM_POLICY_PPI                  *PchPreMemPolicyPpi;
  ...
  //FSP Common Module(0x800) | Api Entry(0x00)
  REPORT_STATUS_CODE (EFI_PROGRESS_CODE, INTEL_RC_STATUS_CODE_FSP_MEM_INIT_API_ENTRY); //PostCode (0x800)
  ...
  if (GetFspGlobalDataPointer()->FspMode == FSP_IN_API_MODE) { // FSP_MODE_CHECK
    ...
    FspmArchConfigPpi->NvsBufferPtr        = (VOID *)(UINTN) FspmUpdDataPtr->FspmArchUpd.NvsBufferPtr;
    ...
    //
    Status = PeiServicesLocatePpi (
             &gPchPreMemDefaultPolicyInitPpiGuid,
             0,
             NULL,
             (VOID **) &PeiPreMemPchDefaultPolicyInitPpi
             );
    if (PeiPreMemPchDefaultPolicyInitPpi != NULL) {
      Status = PeiPreMemPchDefaultPolicyInitPpi->PeiPreMemPolicyInit ();
      if (EFI_ERROR (Status)) {
        ASSERT_EFI_ERROR (Status);
        return Status;
      }
