  ...
  )
{
  ...
  while (Status == EFI_SUCCESS) {
    ...
    BufferSize  = 0;
    SchemaCount = 0;
    if (BdatSchemaListHob->SchemaHobCount >= MAX_SCHEMA_LIST_LENGTH || BdatSchemaListHob->SchemaHobCount <= 0) {
      DEBUG ((DEBUG_INFO, "SchemaHobCount value is invalid, exiting without creating table\n"));
      Status = EFI_UNSUPPORTED;
      break;
    }
    ...
    ///
    MchBar = PciSegmentRead32 (PCI_SEGMENT_LIB_ADDRESS (0, 0, 0, 0, MCHBAR_OFFSET)) & ~BIT0;
