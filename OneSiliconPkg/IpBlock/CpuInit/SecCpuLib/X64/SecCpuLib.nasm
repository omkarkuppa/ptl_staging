...
;
; RDI for calling stack
; args 1:RoutineLabel
; Modified: RDI
;
%macro CALL_RDI 1
  mov     rdi, .ReturnAddress2
  jmp     %1
  .ReturnAddress2:
%endmacro
...
ASM_PFX(SecCarInit):
  mov   r9, rcx                         ; Save FSPT UPD to R9
...
InitMtrrLoop:
  add   ebx, -2
  mov   rsi, MtrrInitTable
  movzx ecx, WORD [(rsi + rbx)]         ; ecx <- address of mtrr to zero
  wrmsr
...
Find_Farthest_Cache_Parameter:
  ...
  ;                          = (RBX[31:22] + 1) * (RBX[21:12] + 1) * (RBX[11:0] + 1) * (RCX + 1)
  ;
  mov     rdi, rbx
  and     rbx, 0FFFh
  inc     rbx
  mov     rax, rbx                          ; rax = (RBX[11:0] + 1)
  mov     rbx, rdi
  and     rbx, ~(0FFC00FFFh)
  shr     rbx, 12
  inc     rbx
  mul     rbx                               ; rax = (RBX[11:0] + 1) * (RBX[21:12] + 1)
  movd    mm3, rax                          ; mm3 = (RBX[11:0] + 1) * (RBX[21:12] + 1)
  mov     rbx, rdi
  shr     rbx, 22
  inc     rbx
  mul     rbx                               ; rax = (RBX[11:0] + 1) * (RBX[21:12] + 1) * (RBX[31:22] + 1)
  movd    mm4, rax                          ; mm4 = WaysPartitionsLines
  mov     rax, rcx                          ;
  inc     rax                               ; rax = (RCX + 1)
  movd    mm2, rax                          ; mm2 = SetCnt
  movd    rbx, mm3
  mul     rbx                               ; rax = (ECX + 1) * (RBX[11:0] + 1) * (RBX[21:12] + 1)
  movd    mm3, rax                          ; mm3 = WayCacheSize = GbSize

  ;
  ; EffSetCnt = previous_power_of_2(SetCnt)
  ;
  movd    rbx, mm2                      ; mm2 = SetCnt
  xor     rcx, rcx
  bsr     rcx, rbx
  mov     rdx, 1
  shl     rdx, cl
  mov     rbx, rdx                      ; rbx = EffSetCnt

  ;
  ;EffCacheSize = (WayCnt) * (PartitionCnt) * (LineSize) * (EffSetCnt)
  ;
  movd    rax, mm4                      ; mm4 = WaysPartitionsLines = (WayCnt) * (PartitionCnt) * (LineSize)
  mul     rbx                           ; rax = EffCacheSize = WaysPartitionsLines*EffSetCnt

  ;
  ; Since there is one code cache region and one data cache region, it requires one guard band.
  ;
  movd    rbx, mm3                      ; mm3 = WayCacheSize = GbSize
  sub     rax, rbx                      ; rax = EffCacheSize - GbSize

Get_NEM_size:
  ;
  ; Code cache size = Total NEM size - DataStack size
  ; If secondary DataStack is enabled, Code cache size = Total NEM size - DataStack size - Secondary DataStack size
  ;
  LOAD_TEMPORARY_RAM_SIZE rcx
  mov     rdx, r9                       ; Get FSPT UPD from R9
  cmp     byte [edx + SecCarInitParamsFsp24.EnableSecondaryDataCache], 1  ; Check if secondary data region is enabled.
  jnz     SecondaryDataStackNotEnabled
  mov     rdx, ASM_PFX(PcdGet32 (PcdSecondaryDataStackSize))
  mov     edx, [rdx]                    ; Secondary DataStack size in EDX
  add     ecx, edx
SecondaryDataStackNotEnabled:
  cmp     ecx, eax
  ja      InvalidParameter
...
Rounded:
  ...
  mov     rcx, r9                                             ; Restore FSPT UPD from R9
  cmp     rcx, 0
  jz      InvalidParameter
...
FoundFsptUpd:
  ...
  mov     rdx, rcx
  mov     rdi, qword [rdx + SecCarInitParamsFsp24.CodeRegionBase]   ; Code region base
  mov     rcx, qword [rdx + SecCarInitParamsFsp24.CodeRegionSize]   ; Code region size

  cmp     rcx,  0
  jz      InvalidParameter
  cmp     rcx, 1000000h                 ; compare bios size. rcx = PcdBiosSize > 16M ? 16M : PcdBiosSize
...
FspBiosBaseAddressAboveFF000000:
  ...
  ;
  ; use FSP24 parameters
  mov     ebx, dword [edx + FSP24_FSPT_CFG_TOP_MEMORY_CACHE_SIZE_OFFSET]   ; TopSwap Cache size

  ; check FSP UPD header revision
  cmp     byte [edx + SecCarInitParamsFsp24.FspUpdHeaderRevision], 2
  jb      InvalidParameter
  cmp     byte [edx + SecCarInitParamsFsp24.FsptArchRevision], 2
  jb      InvalidParameter
...
CodeSizeDone:
  ...
  ; ymm6[192:255] is used to save time-stamp counter value.
  ;
...
CodeRegionMtrrdone:
  ;
  ; Setup MTRR for secondary DataStack cache region.
  ;
  CALL_RDI     ASM_PFX(ProgramSecondaryDataStack)
  ;
  ; Enable the MTRRs by setting the IA32_MTRR_DEF_TYPE MSR E flag.
  ;
  mov     ecx, MTRR_DEF_TYPE            ; Load the MTRR default type index
  rdmsr
  or      rax, MTRR_DEF_TYPE_E          ; Enable variable range MTRRs
  wrmsr
...
BootGuardNemSetup:
  ...
  ;
  xor  eax, eax
  mov  rax, TXT_PUBLIC_BASE
  add  rax, BOOT_GUARD_BOOTSTATUS
  add  rax, 04h
  mov  eax, DWORD [rax]
...
ContinueCacheConfig:
  ...
  ;
  ; Setup MTRR for secondary DataStack cache region for Boot Guard enabled case.
  ;
  CALL_RDI     ASM_PFX(ProgramSecondaryDataStack)
...
FinishedCacheConfig:
  ...
  and     eax, 01h
  jz SkipBtgCnb
  mov     rax, BOOT_GUARD_CNB_SIZE
  add     edi, eax
  mov     ecx, esi
  sub     rcx, rax
  mov     rax, CACHE_TEST_VALUE
...
TestDataStackArea:
  ...
  ;
  mov     rdx, r9                       ; Get FSPT UPD from R9
  ; Check if secondary data region is enabled.
  cmp     byte [edx + SecCarInitParamsFsp24.EnableSecondaryDataCache], 1
  jnz     DataStackTestPass
  mov     rax, ASM_PFX(PcdGet32 (PcdSecondaryDataStackBase))
  mov     edi, [rax]
  mov     rax, ASM_PFX(PcdGet32 (PcdSecondaryDataStackSize))
  mov     ecx, [rax]
  shr     ecx, 3
  mov     rax, CACHE_TEST_VALUE
TestSecondaryDataStack:
  stosq
  cmp     rax, QWORD [edi-8]
  jnz     DataStackTestFail
  loop    TestSecondaryDataStack,ecx
#endif
...
;
; Program secondary DataStck cache region as write-back (WB)
; cacheable memory type using the variable range MTRRs.
;
global ASM_PFX(ProgramSecondaryDataStack)
ASM_PFX(ProgramSecondaryDataStack):
  mov     rdx, r9                       ; Get FSPT UPD from R9
  ; Check if secondary data region is enabled.
  cmp     byte [edx + SecCarInitParamsFsp24.EnableSecondaryDataCache], 1
  jnz     SkipSecondaryDataStackProgramming
  ;
  ; Set the base address of the secondary DataStack
  ;
  CALL_RBP     ASM_PFX(FindFreeMtrr)    ; Load free MTRR index in ECX
  mov     rax, ASM_PFX(PcdGet32 (PcdSecondaryDataStackBase))
  mov     eax, [rax]                    ; Secondary DataStack base adress in EAX
  or      eax, MTRR_MEMORY_TYPE_WB
                                        ; Load the write-back cache value
  xor     edx, edx                      ; clear upper dword
  wrmsr                                 ; the value in MTRR_PHYS_BASE

  ;
  ; Set the mask for secondary DataStack cache range
  ; Compute MTRR mask value:  Mask = NOT (Size - 1)
  ;
  mov  rax, ASM_PFX(PcdGet32 (PcdSecondaryDataStackSize))
  mov  eax, [rax]                       ; Secondary DataStack size in EAX
  dec  eax
  not  eax
  or   eax, MTRR_PHYS_MASK_VALID
                                        ; turn on the Valid flag
  mov  edx, esi                         ; edx <- MTRR_PHYS_MASK_HIGH
  inc  ecx
  wrmsr                                 ; the value in MTRR_PHYS_MASK

SkipSecondaryDataStackProgramming:
  RET_RDI
