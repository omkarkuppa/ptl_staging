  ...
  )
{
  ...
  Status = gBS->CreateEvent (EVT_NOTIFY_WAIT, TPL_NOTIFY, AbsPtrWaitForInput, QuickI2cDev, &((QuickI2cDev->AbsPtrProtocol).WaitForInput));
  ...
  THC_LOCAL_DEBUG (L"QuickI2cInstallAbsPtr Install AbsPtrProtocol  Status: %r\n", Status);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "QuickI2cInstallAbsPtr ERROR - QuickI2c failed to install AbsolutePointerProtocol, Status: %r\n", Status));
    return Status;
  }
