...
/**
  Function to unload Thc Driver when gBS->ExitBootServices() called.
  ...
  )
{
  ...
  ///
  /// Create ExitBootServices for calling the ThcDriverBindingStop procedure
  ///
  DEBUG ((DEBUG_INFO, "ThcCreateExitEvents Create ExitBootServices Event\n"));
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "ThcStartPolling error, cannot create PollingEvent Status %r\n", Status));
    return Status;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "ThcStartPolling error, cannot enable PollingEvent timer, %r\n", Status));
    return Status;
  }
  ...
  )
{
  ...
  if (ThcIsDeviceIdSupported (VendorId, DeviceId, &DeviceIndex)) {
    if (mThcMode[DeviceIndex] == Thc) {
      THC_LOCAL_DEBUG(L"ThcDriverBindingSupported DeviceId: 0x%X DeviceIndex %d\n", DeviceId, DeviceIndex)
      DEBUG ((DEBUG_INFO, "ThcDriverBindingSupported DeviceId: 0x%X DeviceIndex %d\n", DeviceId, DeviceIndex));
      return EFI_SUCCESS;
    }
  }
  ...
  )
{
  ...
  ThcDev->ThcProtocol.HidEnableAfterReset   = HidEnableAfterReset;
  ...
  ThcDev->InputReportTable.Quantity = 0;
  ThcDev->InputReportTable.Report   = NULL;
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "ERROR - Thc failed to Initialize Device Status: %r\n", Status));
    goto THC_ERROR_EXIT;
  }

  Status = ThcReadConfig (ThcDev);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "ERROR - Thc failed to Read Device Configuration Status: %r\n", Status));
    goto THC_ERROR_EXIT;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "ERROR - Thc failed to Set New Thc Frequency Device Status: %r\n", Status));
    goto THC_ERROR_EXIT;
  }

  Status = ThcEnableEndDevice (ThcDev);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "ERROR - Thc failed to Configure Device Status: %r\n", Status));
    goto THC_ERROR_EXIT;
  }

  Status = ThcDmaInitialize (ThcDev);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "ERROR - Thc failed to Initialize DMA Status: %r\n", Status));
    goto THC_ERROR_EXIT;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "ERROR - Thc failed to StartPolling Status: %r\n", Status));
    goto THC_ERROR_EXIT;
  }

  Status = ThcReadHidDescriptor (ThcDev);
  if (EFI_ERROR (Status)) { // Note: Some devices might support Single Finger Report and follow EDS, for such cases HID support is not required
    DEBUG ((DEBUG_INFO, "ThcDriverBindingStart failed to Read the HID descriptor Status: %r\n", Status));
  }

  Status = ThcInstallAbsPtr (ThcDev);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "ERROR - Thc failed to Install AbsPtr Protocol Status: %r\n", Status));
    goto THC_ERROR_EXIT;
  }

  Status = gBS->ConnectController (ThcDev->ControllerHandle, NULL, NULL, TRUE);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "ERROR - Thc failed to ConnectController Status: %r\n", Status));
    goto THC_ERROR_EXIT;
  }
