...
{
  if (QuickSpiDev->ThcProtocol.DeviceStatus != EFI_SUCCESS) {
    DEBUG ((DEBUG_ERROR, "ThcProtocol HidSanityCheck Error QuickSpi Status: %r\n", QuickSpiDev->ThcProtocol.DeviceStatus));
    return EFI_NOT_READY;
  }

  if (QuickSpiDev->HidActive == TRUE) {
    DEBUG ((DEBUG_ERROR, "ThcProtocol HidSanityCheck Error HID transaction is still active: %r\n", EFI_ALREADY_STARTED));
    return EFI_ALREADY_STARTED;
  }

  if (QuickSpiDev->ReadDone == FALSE) {
  ...
  )
{
  ...
  //
  Status = HidSanityCheck (QuickSpiDev, Length);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  Status = QuickSpiDmaWriteTx (QuickSpiDev, OutputHeader, Buffer);
  if (EFI_ERROR (Status)) {
    goto THC_HID_EXIT;
  }
...
THC_HID_EXIT:
  //
  // Restore QuickSpi Hid state to default
  //
  QuickSpiDev->HidDataAvailable = FALSE;
  ...
  )
{
  ...
  QUICK_SPI_OUTPUT_REPORT_HEADER  OutputHeader;

  QuickSpiDev = QUICK_SPI_CONTEXT_FROM_THC_PROTOCOL (This);

  OutputHeader.Fields.ReportType    = QuickSpiOutputReportSetFeature;
  OutputHeader.Fields.ContentLength = Length;
  OutputHeader.Fields.ContentId     = Buffer[0];

  return HidGetSetFeature (QuickSpiDev, OutputHeader, &Buffer[1], Timeout, Length);
}
  ...
  )
{
  ...
  OutputHeader.Fields.ReportType    = QuickSpiOutputReportGetFeature;
  OutputHeader.Fields.ContentLength = Length;
  OutputHeader.Fields.ContentId     = Buffer[0];
  ...
  )
{
  ...
  Status = QuickSpiEnableAfterReset (QuickSpiDev);
  ...
  )
{
  ...
  while (DescriptorLength > 0) {
    ...
    HidUpdateStack (ParseStack, Token, &(QuickSpiDev->InputReportTable) );
  }
  HidExportReport (ParseStack, &(QuickSpiDev->InputReportTable) );
  HidShowReportTable (QuickSpiDev->InputReportTable);
  FreePool(ParseStack);
}
    ...
    if (InputStream[0] == ReportTable.Report[Index].Id) {
      for (Collections = 0; Collections < ReportTable.Report[Index].CollectionCount; Collections++) {
        ...
        if (ReportTable.Report[Index].UsageTouchPad == 1 || ReportTable.Report[Index].UsageMouse== 1) {
          ...
          MinMax->MinX = 0;
          MinMax->MaxX = QUICK_SPI_RELATIVE_COORDINATES_MAX_X;
          MinMax->MinY = 0;
          MinMax->MaxY = QUICK_SPI_RELATIVE_COORDINATES_MAX_Y;
