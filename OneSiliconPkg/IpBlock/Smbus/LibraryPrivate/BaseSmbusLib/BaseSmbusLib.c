  ...
  )
{
  ...
  if ((HostStatus & B_SMBUS_IO_HSTS_IUS) != 0) {
    return RETURN_TIMEOUT;
  } else if ((HostStatus & B_SMBUS_IO_HSTS_HBSY) != 0) {
    //
    // Clear host status register and exit.
    //
    IoWrite8 (IoPortBaseAddress + R_SMBUS_IO_HSTS, V_SMBUS_IO_HSTS_ALL);
    return RETURN_TIMEOUT;
  }
  //
  ...
  )
{
  ...
  //
  IoWrite8 (IoPortBaseAddress + R_SMBUS_IO_HCTL, (UINT8)(HostControl + B_SMBUS_IO_HCTL_START));
  ...
  do {
    ...
    //
    HostStatus = IoRead8 (IoPortBaseAddress + R_SMBUS_IO_HSTS);
  } while ((HostStatus & (B_SMBUS_IO_HSTS_INTR | B_SMBUS_IO_HSTS_DERR | B_SMBUS_IO_HSTS_BERR | B_SMBUS_IO_HSTS_FAIL | B_SMBUS_IO_HSTS_BDS)) == 0);
  ...
  //
  IoWrite8 (IoPortBaseAddress + R_SMBUS_IO_HSTS, B_SMBUS_IO_HSTS_DERR | B_SMBUS_IO_HSTS_BERR | B_SMBUS_IO_HSTS_FAIL);
  //
  ...
  )
{
  ...
  //
  IoWrite8 (IoPortBaseAddress + R_SMBUS_IO_HSTS, V_SMBUS_IO_HSTS_ALL);
  IoWrite8 (IoPortBaseAddress + R_SMBUS_IO_AUXS, B_SMBUS_IO_AUXS_CRCE);
