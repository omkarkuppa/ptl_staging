  ...
  )
{
  ...
  if ((StreamTracerBase == 0) || (StreamTracerSize == 0)) {
    return;
  }
  ...
  Status = GetConfigBlock ((VOID *) SiPreMemPolicyPpi, &gTelemetryPeiPreMemConfigGuid, (VOID *) &TelemetryPreMemConfig);
  if ((Status != EFI_SUCCESS) || (TelemetryPreMemConfig == NULL)) {
    ASSERT_EFI_ERROR (Status);
    return;
  }
  ...
  BuildResourceDescriptorHob (
    ResourceType,              // MemoryType,
    ResourceAttribute,         // MemoryAttribute
    StreamTracerBase,          // MemoryBegin
    StreamTracerSize           // MemoryLength
  );
  ...
  TelemetryPreMemConfig->StreamTracerBase = StreamTracerBase;
}