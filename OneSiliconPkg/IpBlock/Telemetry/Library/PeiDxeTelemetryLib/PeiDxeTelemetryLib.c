  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "BaseAddr = 0x%X\n", BaseAddr));
...
{
  return CpuCrashLogMailboxCommand (V_CPU_CRASHLOG_CMD_REARM, 0);
}
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "CrashLogHeaderAddress = 0x%X\n", CrashLogHeaderAddress));

  ZeroMem (CrashLogHeaderBuff, sizeof (CPU_CRASHLOG_HEADER));
  CrashLogHeaderBuff->CrashlogHeaderData.HeaderData = MmioRead32 (CrashLogHeaderAddress);
  CrashLogHeaderBuff->CrashlogCapability.Guid = MmioRead32 (CrashLogHeaderAddress + 4); // Get next 32 bits data
}
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "CpuCrashLogMainDiscovery CrashLogPointerBuff = 0x%X\n", CrashLogPointerBuff));

  if (!EFI_ERROR (Status)) {
    CrashLogStruct->MainBuffer.Data = CrashLogPointerBuff.Data;
    DEBUG ((DEBUG_INFO, "CrashLogStruct->MainBuffer.Data = 0x%X\n", CrashLogStruct->MainBuffer.Data));
  }

  Status = CpuCrashLogTelemetryDiscovery (CrashLogBaseAddress, &CrashLogPointerBuff);
  DEBUG ((DEBUG_INFO, "CpuCrashLogTelemetryDiscovery CrashLogPointerBuff = 0x%X\n", CrashLogPointerBuff));

  if (!EFI_ERROR (Status)) {
    CrashLogStruct->TelemetryBuffer.Data = CrashLogPointerBuff.Data;
    DEBUG ((DEBUG_INFO, "CrashLogStruct->TelemetryBuffer.Data = 0x%X\n", CrashLogStruct->TelemetryBuffer.Data));
  }

  Status = CpuCrashLogTraceDiscovery (CrashLogBaseAddress, &CrashLogPointerBuff);
  DEBUG ((DEBUG_INFO, "CpuCrashLogTraceDiscovery CrashLogPointerBuff = 0x%X\n", CrashLogPointerBuff));

  if (!EFI_ERROR (Status)) {
    CrashLogStruct->TraceBuffer.Data = CrashLogPointerBuff.Data;
    DEBUG ((DEBUG_INFO, "CrashLogStruct->TraceBuffer.Data = 0x%X\n", CrashLogStruct->TraceBuffer.Data));
  }
