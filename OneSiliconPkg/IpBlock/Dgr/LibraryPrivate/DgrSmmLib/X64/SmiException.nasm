...
global   ASM_PFX(gSmmDgrSmiExceptionCr3)
global   ASM_PFX(mSmmDgrSmiGdtr)
extern   ASM_PFX(mSmmDgrMceHandlerLongMode)
extern   ASM_PFX(gSmmDgrFeatureSmiHandlerIdtr)
...
ASM_PFX(mSmmDgrSmiGdtr):
    DW      GDT_SIZE - 1
    DQ        NullSeg

ASM_PFX(gSmmDgrSmiExceptionCr3) EQU gSmmDgrSmiExceptionCr3Patch - 4
;
...
global ASM_PFX(PMIdtHandlerDgr)
ASM_PFX(PMIdtHandlerDgr):
    jmp    $
    retf

global ASM_PFX(PMIdtHandlerMachineCheckDgr)
ASM_PFX(PMIdtHandlerMachineCheckDgr):
    sub     esp, 8
...
LongMode:                               ; long mode starts here
    lea     rax, [ASM_PFX(gSmmDgrFeatureSmiHandlerIdtr)] ; load long mode IDT table to handle the exception
    ...
    push    rax
    lea     rax, [ASM_PFX(mSmmDgrMceHandlerLongMode)]
