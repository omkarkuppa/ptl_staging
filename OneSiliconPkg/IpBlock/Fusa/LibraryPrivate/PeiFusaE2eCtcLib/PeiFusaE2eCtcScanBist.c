...
{
  UINTN                       *FusaPatternFile;
  UINTN                       FusaStartupFileAddrPtr = FusaStartupFileAddr;
  ...
  FusaPatternFile = (void *)  FusaStartupFileAddrPtr;
  SignaturePtr = (UINT32*)((UINTN)(FusaPatternFile)+ 148);
  ...
  if (0 != CompareMem(SignaturePtr, FUSA_PATTERN_SIGNATURE_START, AsciiStrLen(FUSA_PATTERN_SIGNATURE_START))) {
    DEBUG((DEBUG_INFO, "FspDxCheckScanBist: Fusa Pattern file does not have valid signature.\n"));
    TotalNumberofPatternFiles = 0;
    *NumberOfPatternFiles = TotalNumberofPatternFiles;
    StartFuSaHeader = NULL;
  ...
  TotalNumberofPatternFiles = 1; //hardcode for testing purposes
  ...
  )
{
  ...
  for (int i = 0; i < MAX_PTL_ATOM_CORE_COUNT; i++)
  {
    pFusaTestResult[i].TestNumber = FusaTestNumScan0Bist + i;
  ...
  DEBUG((DEBUG_INFO, "Total Number of Pattern Files are [%d]\n", TotalNumberOfPatternFiles));
  for (int i = 0; i < TotalNumberOfPatternFiles; i++)
  {
      FuSaHeaderPtr = FusaTestpatternHeaderptr[i];
      DEBUG((DEBUG_INFO, "FuSaHeaderPtr 0x%lx\n", (UINT32 *) FuSaHeaderPtr));
      DEBUG((DEBUG_INFO, " HeaderVersion; =%x\n", FuSaHeaderPtr->HeaderVersion));
      DEBUG((DEBUG_INFO, " PatchID; =%x\n", FuSaHeaderPtr->PatchID));
      DEBUG((DEBUG_INFO, " Date; =%x\n", FuSaHeaderPtr->Date));
      DEBUG((DEBUG_INFO, " CpuId; =%x\n", FuSaHeaderPtr->CpuId));
      DEBUG((DEBUG_INFO, " Checksum; =%x\n", FuSaHeaderPtr->Checksum));
      DEBUG((DEBUG_INFO, " LoaderVersion; =%x\n", FuSaHeaderPtr->LoaderVersion));
      ...
      DEBUG((DEBUG_INFO, " FuSaInfoReserve =%x\n", FuSaHeaderPtr->FuSaInfoReserved));
      DEBUG((DEBUG_INFO, " Reserved1; =%x\n", FuSaHeaderPtr->Reserved1));
      DEBUG((DEBUG_INFO, " Reserved2; =%x\n", FuSaHeaderPtr->Reserved2));
      //CpuDeadLoop();
  }

  Local_Fusa_info = FusaInfoListGet();

  //initialize the test result buffer as some test may not run because of the device not being available
  CoreCount = 0;
  for (CoreIndex = 0; CoreIndex < Local_Fusa_info->TotalProcessor ; CoreIndex++)
  {
    if( Local_Fusa_info->Processor[CoreIndex].CoreType == CPUID_CORE_TYPE_INTEL_ATOM )
    {
      ...
      DEBUG ((DEBUG_INFO, "Local_Fusa_info->Processor[%x].CoreType = 0x%x, Fusa_Capabilities =0x%x\n",
          CoreIndex,
      ...
      LibStatus = FusaTestAndReporting (
                    NULL,
                    FusaTestNumScan0Bist + CoreCount,
                    TotalNumberOfPatternFiles,
