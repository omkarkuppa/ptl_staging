...
/**
  ...
  @param[in] Sbdf   device's segment:bus:device:function coordinates
  @param[in] Mrrs   Max read request size, encoded in the same way as in register (0=128b, 1=256b, etc)
**/
STATIC
VOID
SetMrrs (
...
{
  PciSegmentAndThenOr16 (SbdfToBase (Sbdf) + Sbdf.PcieCap + R_PCIE_DCTL_OFFSET, (UINT16) ~B_PCIE_CFG_DCTL_MRRS, Mrrs << N_PCIE_CFG_DCTL_MRRS);
}
...
/**
  ...
  @param[in] Segment,Bus,Device,Function    address of currently visited PCIe device
  @param[in] Mrrs                           Maximum Read Resource Size for endpoint device
**/
STATIC
VOID
MrrsConfiguration (
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "MrrsConfiguration %x:%x:%x\n", Sbdf.Bus, Sbdf.Dev, Sbdf.Func));

  if (HasChildBus (Sbdf, &ChildSbdf)) {
    DevType = GetDeviceType (Sbdf);
    while (FindNextPcieChild (DevType, &ChildSbdf)) {
      MrrsConfiguration (ChildSbdf, Mrrs);
    }
  }
  DevType = GetDeviceType (Sbdf);
  if (DevType == DevTypePcieEndpoint) {
  ...
  )
{
  ...
  if (PciSku == EnumPchPcie) {
    ...
    if (FeatureConfiguration->ConfigureMrrs) {
      MrrsConfiguration (RpSbdf, Mps);
    }
  }
...
)
{
  ...
  DEBUG ((DEBUG_INFO, "SpecCompliantLtrOverride Start \n"));
  ...
  DEBUG ((DEBUG_INFO, "SpecCompliantLtrOverride End \n"));
}
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "ConfigureRpLtrOverride Start \n"));
  ...
  DEBUG ((DEBUG_INFO, "ConfigureRpLtrOverride End\n"));
}
  ...
  )
{
  ...
  LtrCapable = RecursiveIsLtrCapable (RpSbdf);
  if (PcieRpCommonConfig->PcieRpLtrConfig.LtrOverrideSpecCompliant && !LtrCapable) {
    SpecCompliantLtrOverride (RpBase);
  } else {
  ...
  if (FeatureConfiguration->ConfigureDynamicLinkThrottling) {
  ...
  )
{
  ...
  BridgeCleanupList.Count = 0;
  RecursiveBusAssignment (Sbdf, BusMin, BusMax, &BridgeCleanupList);
  ...
  )
{
  ...
  PcieCap = PcieBaseFindCapId(RpBase, EFI_PCI_CAPABILITY_ID_PCIEXP);
  Data16 = PciSegmentRead16 (RpBase + PcieCap + R_PCIE_LCTL_OFFSET);
  L1sEnabled = ((Data16 & B_PCIE_LCTL_ASPM ) == V_PCIE_LCTL_ASPM_L1 || (Data16 & B_PCIE_LCTL_ASPM ) == V_PCIE_LCTL_ASPM_L0S_L1);
  if (L1sEnabled) {
    PciSegmentAndThenOr16 (RpBase + PcieCap + R_PCIE_LCTL_OFFSET, (UINT16)~B_PCIE_LCTL_ASPM, V_PCIE_LCTL_ASPM_L1);
  } else {
    PciSegmentAnd16 (RpBase + PcieCap + R_PCIE_LCTL_OFFSET, (UINT16)~B_PCIE_LCTL_ASPM);
  }
