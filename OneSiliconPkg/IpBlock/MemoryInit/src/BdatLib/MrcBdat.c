...
/**
@brief
  Fill Ppr BDAT with the information provided by the memory initialization code.

  @param[in, out] MrcData - Constant pointer to the Mrc data structure which conatins the BDAT structure to fill.
  @param[out] PprDataBuffer - Ppr data buffer
  @param[in] PprDataBufferSize - Ppr data buffer size
  @param[in] MsgPrint - whether print logs.
**/
/*
static
VOID
FillPprBdatStructure (
  IN OUT MrcParameters *const MrcData,
  OUT   VOID           *PprDataBuffer,
  IN    UINT32          PprDataBufferSize,
  IN    BOOLEAN         MsgPrint
)
{
  const MrcInput      *Inputs;
  ...
  MrcDebug            *Debug;
  MrcDebugMsgLevel    DebugLevel;
  BASE_PPR_RESULT     *PprResults;
  PPR_RESULT_COLUMNS  *PprRow;
  INT32 MaxNumberRows;
  UINT8 Controller;
  UINT8 Channel;
  UINT8 BG;
  ...
  PprResults = (BASE_PPR_RESULT*) PprDataBuffer;
  MrcCall->MrcSetMem ((UINT8*) PprDataBuffer, PprDataBufferSize, 0);
  MrcCall->MrcCopyMem ((UINT8*) (&PprResults->ResultsHeader.MdBlock.MetadataType), (UINT8*) (&gPprResultMetadataGuid), sizeof (PprResults->ResultsHeader.MdBlock.MetadataType));
  PprResults->ResultsHeader.MdBlock.MetadataSize = sizeof (PPR_RESULT_METADATA);
  MrcCall->MrcCopyMem ((UINT8*) (&PprResults->ResultsHeader.RsBlock.ResultType), (UINT8*) (&gPprResultColumnsGuid), sizeof (PprResults->ResultsHeader.RsBlock.ResultType));
  PprResults->ResultsHeader.RsBlock.ResultElementSize = sizeof (PPR_RESULT_COLUMNS);
  MaxNumberRows = PprDataBufferSize - sizeof (RESULTS_DATA_HDR) - sizeof (PPR_RESULT_METADATA);
  MaxNumberRows = MaxNumberRows / sizeof (PPR_RESULT_COLUMNS);

  PprResults->ResultsHeader.RsBlock.ResultCapacity      = MaxNumberRows;
  PprResults->ResultsHeader.RsBlock.ResultElementCount  = MaxNumberRows;
  PprResults->Metadata.TotalRowFailuresDiscovered       = (UINT16)MaxNumberRows;
  PprResults->Metadata.MajorVer                         = 1;
  PprResults->Metadata.MinorVer                         = 1;
  PprResults->Metadata.ResultEleCount                   = (UINT16)MaxNumberRows;
  PprRow = &(PprResults->Rows[0]);

  for (Controller = 0; Controller < MAX_CONTROLLER; Controller++){
    for (Channel = 0; Channel < MAX_CHANNEL; Channel++) {
      for (Index = 0; Index < MAX_FAIL_RANGE; Index++) {
        FailPtr = &Outputs->FailRange[Controller][Channel][Index];
        NumBG = MrcCountBitsEqOne(FailPtr->BankGroupMask);
        BG = 0;
        while(NumBG != 0) {
          if ((FailPtr->BankGroupMask & 1 << BG) == 0) {
            BG++;
            continue;
          }
          PprRow->Controller = Controller;
          PprRow->Channel    = Channel;
          PprRow->Rank       = (UINT8)FailPtr->Addr.Bits.Rank;
          PprRow->BankGroup  = BG;
          PprRow->Bank       = (UINT8)FailPtr->Addr.Bits.BankPair;
          PprRow->Row        = FailPtr->Addr.Bits.Row;
          if ((FailPtr->BankGroupRepairedMask & 1 << BG) == 0) {
            PprRow->PprRowRepairsSuccessful = 0;
            PprResults->Metadata.TotalUncorrectableDevices++;
          } else {
            PprRow->PprRowRepairsSuccessful = 1;
            PprResults->Metadata.TotalPprRowRepairsSuccessful++;
          }
          PprRow->PprRowRepairsAttemptCnt  = FailPtr->BankGroupAttempts[BG];
          PprResults->Metadata.TotalPprRowRepairsAttemptCnt += PprRow->PprRowRepairsAttemptCnt;
          MRC_DEBUG_MSG(Debug, DebugLevel, "MC %d CH %d RK %d BG %d BK %d Row 0x%x: Ppr attempt = %d, Ppr result %s\n", Controller,
                                           Channel, PprRow->Rank, BG, PprRow->Bank, PprRow->Row,
                                           PprRow->PprRowRepairsAttemptCnt, (PprRow->PprRowRepairsSuccessful == 0) ? "fail": "pass");
          NumBG--;
          BG++;
          PprRow++;
        }
  ...
  MRC_DEBUG_MSG(Debug, DebugLevel, "detected: %d, repaired: %d, unrepaired: %d\n", PprResults->Metadata.TotalRowFailuresDiscovered,
              PprResults->Metadata.TotalPprRowRepairsSuccessful, PprResults->Metadata.TotalUncorrectableDevices);
}
*/
  ...
  )
{
  ...
  if (Outputs->PprRunningState != PPR_IS_DONE) {
    // PPR didn't run
    return mrcSuccess;
  }

  ResultsGuid = &gSsaBiosResultPprGuid;
  ...
  // Create a HOB to store the BDAT Schema
  BdatSchemaSize = sizeof (RESULTS_DATA_HDR) + sizeof (PPR_RESULT_METADATA) + (sizeof (PPR_RESULT_COLUMNS) * GetNumberPprFailResult (MrcData));

  MRC_DEBUG_MSG (Debug, MSG_LEVEL_NOTE, "Filling/Saving Ppr BDAT structure of size %d\n", BdatSchemaSize);
  ...
  BdatMemoryHobPointer = BdatMemoryHob->Pointer;
  MRC_DEBUG_MSG (Debug, MSG_LEVEL_NOTE, "Ppr HOB at %ph\n", BdatMemoryHob);
  ...
  )
{
  ...
  // MrcFillPprBdatStructure(MrcData);
