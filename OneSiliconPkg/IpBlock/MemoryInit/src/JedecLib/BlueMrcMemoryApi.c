    ...
    for (Controller = 0; Controller < MAX_CONTROLLER; Controller++) {
      for (Channel = (MAX_CHANNEL - 1); Channel >= 0; Channel--) {
        ...
        if (Channel % 2) {
          ...
          GetSetVal = ValidRankBitMask << 2;
        } else {
          GetSetVal |= ValidRankBitMask;
          OverrideValRankBitMask = (CsOverrideVal == 0) ? 0 : GetSetVal;
          OverrideEnRankBitMask = (CsOverrideEn == FALSE) ? 0 : GetSetVal;
