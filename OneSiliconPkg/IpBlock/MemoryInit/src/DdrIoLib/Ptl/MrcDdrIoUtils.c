  ...
  )
{
  ...
  Outputs = &MrcData->Outputs;
  IsGear4 = Outputs->GearMode == 1 ? TRUE : FALSE;  //TRUE Gear4, FALSE Gear2

  if (!Outputs->IsDdr5) {
    return;
  }

  if (IsGear4 && Outputs->Frequency == f3200 && *tCL < 28) {
    *tCL = 28;     // 28 is the smallest valid CL in DDR5 3200G4.
  }
  ...
  )
{
  ...
  if (FsmStage & MrcRefPi2XOverAlign) {
    ...
    RefPiFsmControl.Bits.MaxVal   = MaxWidth + MinWidth + 8;
