...
{
  MmioOr32 (GbeBar + R_GBE_MEM_CTRL_EXT, B_GBE_MEM_CTRL_EXT_PHY_WOL);
}
  ...
  )
{
  ...
  for (Count = 0; Count < GBE_MAX_LOOP_TIME; ++Count) {
    if (MmioRead32 (GbeBar + R_GBE_MEM_MDIC) & B_GBE_MEM_MDIC_R) {
      return EFI_SUCCESS;
    }
  ...
  )
{
  ...
  MmioOr32 (GbeBar + R_GBE_MEM_EXTCNF_CTRL, B_GBE_MEM_EXTCNF_CTRL_SWFLAG);
  for (Count = 0; Count < GBE_MAX_LOOP_TIME; ++Count) {
    ExtCnfCtrl = MmioRead32 (GbeBar + R_GBE_MEM_EXTCNF_CTRL);
    if (ExtCnfCtrl & B_GBE_MEM_EXTCNF_CTRL_SWFLAG) {
      return EFI_SUCCESS;
    }
    MicroSecondDelay (GBE_ACQUIRE_MDIO_DELAY);
  }
  ...
  )
{
  ...
  ExtcnfCtrl = MmioRead32 (GbeBar + R_GBE_MEM_EXTCNF_CTRL);
  if (ExtcnfCtrl & B_GBE_MEM_EXTCNF_CTRL_SWFLAG) {
    DEBUG ((DEBUG_ERROR, "GbeMdiReleaseMdio: ERROR GBE_MEM_EXTCNF_CTRL_SWFLAG = %X\n", ExtcnfCtrl));
  } else {
    DEBUG ((DEBUG_INFO, "GbeMdiReleaseMdio: GBE_MEM_EXTCNF_CTRL_SWFLAG = %X\n", ExtcnfCtrl));
  }
  MmioAnd32 (GbeBar + R_GBE_MEM_EXTCNF_CTRL, (UINT32) ~B_GBE_MEM_EXTCNF_CTRL_SWFLAG);
  ASSERT ((MmioRead32 (GbeBar + R_GBE_MEM_EXTCNF_CTRL) & B_GBE_MEM_EXTCNF_CTRL_SWFLAG) == 0);
}
  ...
  )
{
  ...
  MmioWrite32 (GbeBar + R_GBE_MEM_MDIC, (~B_PHY_MDI_READY) & (B_PHY_MDI_WRITE | B_PHY_MDI_PHY_ADDRESS_01 | R_PHY_MDI_PHY_REG_SET_PAGE | ((Page * 32) & 0xFFFF)));
...
{
  MmioWrite32 (GbeBar + R_GBE_MEM_MDIC, (~B_PHY_MDI_READY) & (B_PHY_MDI_WRITE | B_PHY_MDI_PHY_ADDRESS_01 | R_PHY_MDI_PHY_REG_SET_ADDRESS | (Register & 0xFFFF)));
  ...
  )
{
  ...
  MmioWrite32 (GbeBar + R_GBE_MEM_MDIC, (~B_PHY_MDI_READY) & (B_PHY_MDI_WRITE | PhyAddress | PhyRegister | (WriteData & 0xFFFF)));
  return GbeMdiWaitReady (GbeBar);
}
  ...
  )
{
  ...
  MmioWrite32 (GbeBar + R_GBE_MEM_MDIC, (~B_PHY_MDI_READY) & (B_PHY_MDI_READ | PhyAddress | PhyRegister));
  Status = GbeMdiWaitReady (GbeBar);
  if (EFI_SUCCESS == Status) {
    *ReadData = (UINT16) MmioRead32 (GbeBar + R_GBE_MEM_MDIC);
  }
  return Status;
}
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "GbeMdiGetLanPhyRevision failed to aquire MDIO semaphore. Status: %r\n", Status));
    return Status;
  }

  Status = GbeMdiSetPage (GbeBar, PHY_MDI_PAGE_769_PORT_CONTROL_REGISTERS);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "GbeMdiGetLanPhyRevision failed to Set Page 769. Status: %r\n", Status));
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "GbeMdiGetLanPhyRevision failed to enable slow MDIO mode. Status: %r\n", Status));
    GbeMdiReleaseMdio (GbeBar);
    return Status;
  }
  ...
  if (*LanPhyRevision == 0x0) {
    if ((MmioRead32 (GbeBar + R_GBE_MEM_CTRL) & (B_GBE_MEM_CTRL_LANPHYPC_OVRIDE | B_GBE_MEM_CTRL_LANPHYPC_VAL))) {
    ...
    //
    MmioOr32 (GbeBar + R_GBE_MEM_CTRL, B_GBE_MEM_CTRL_LANPHYPC_OVRIDE);
    MmioOr32 (GbeBar + R_GBE_MEM_CTRL, B_GBE_MEM_CTRL_LANPHYPC_VAL);
    ...
    LpcdLoop = 101;
    while (LpcdLoop > 0) {
      if (MmioRead32 (GbeBar + R_GBE_MEM_CTRL_EXT) & B_GBE_MEM_CTRL_EXT_LPCD) {
        break;
      } else {
        LpcdLoop--;
    ...
    //
    MmioAnd32 (GbeBar + R_GBE_MEM_CTRL, (UINT32) ~B_GBE_MEM_CTRL_LANPHYPC_VAL);
    MmioAnd32 (GbeBar + R_GBE_MEM_CTRL, (UINT32) ~B_GBE_MEM_CTRL_LANPHYPC_OVRIDE);
  }

phy_exit:
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "GbeMdiGetLanPhyRevision failed to read Revision and Model Number from PHY Identifier 2. Status: %r\n", Status));
    GbeMdiReleaseMdio (GbeBar);
    return Status;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "GbeMdiGetLanPhyRevision failed to disable slow MDIO mode. Status: %r\n", Status));
  }
