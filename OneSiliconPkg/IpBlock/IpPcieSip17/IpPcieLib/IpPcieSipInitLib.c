  ...
  )
{
  ...
  if (pInst->PrivateConfig.EnableHotPlugInController && pInst->PcieRpCommonConfig.HpSci == 0) {
    Slsts.Data = (UINT16) IpWrRegRead (pInst->RegCntxt_Cfg_Sb, SLSTS_PCIE_CFG_REG, IpWrRegFlagSize16Bits);
    PRINT_LEVEL1 ("Slsts.dllsc: %d\n", Slsts.Bits.dllsc);
    if (Slsts.Bits.dllsc) {
      //
      // BIOS has to clear DLLSC before setting HPME to 1
      // DLLSC is RW/1C/V, "write 1 to clear it"
      //
      PRINT_LEVEL1 ("Clearing Slsts.dllsc!\n");
      Slsts.Bits.dllsc = 1;
      IpWrRegWrite (pInst->RegCntxt_Cfg_Sb, SLSTS_PCIE_CFG_REG, Slsts.Data, IpWrRegFlagSize16Bits);
      Slsts.Data = (UINT16) IpWrRegRead (pInst->RegCntxt_Cfg_Sb, SLSTS_PCIE_CFG_REG, IpWrRegFlagSize16Bits);
      PRINT_LEVEL1 ("Slsts.dllsc: %d (Slsts: 0x%04x)\n", Slsts.Bits.dllsc, Slsts.Data);
    }

    Mpc.Data      = (UINT32)IpWrRegRead (pInst->RegCntxt_Cfg_Sb, MPC_PCIE_CFG_REG, IpWrRegFlagSize32Bits);
    Mpc.Bits.hpce = 0;
    Mpc.Bits.hpme = 1;
    IpWrRegWrite (pInst->RegCntxt_Cfg_Sb, MPC_PCIE_CFG_REG, Mpc.Data, IpWrRegFlagSize32Bits);

    Mpc.Data = (UINT32)IpWrRegRead (pInst->RegCntxt_Cfg_Sb, MPC_PCIE_CFG_REG, IpWrRegFlagSize32Bits);
    PRINT_LEVEL1 ("Mpc.hpme: %d Mpc.hpce: %d\n", Mpc.Bits.hpme, Mpc.Bits.hpce);
  }
}