...
#ifdef CONTINUE_SILICON_NOTIFY
  // SNRC (Silicon Notify the PCIE RP Device and Child)
  //
  // This Method Will Notify the PCIE RP and Child Device Under PCIE RP.
  //
  // Arguments: (4)
  //   Arg0: GPE Controller Location
  //   Arg1: GPE Index Number
  //   Arg2: Child Device Depth Location.
  //             0: Not a Child Device,
  //             1: 1st Child Behind the Bridge,
  //             2: 2nd Child Behind the Bridge
  //             3: Deeper Than 2nd Child Behind the Bridge
  //   Arg3: GPE Event Type or Notification Value
  // Return Value:
  //   None
  //
  Method (SNRC, 0x04, Serialized) {
    ADBG (Concatenate (Concatenate ("SNRC: For RP Type =", ToHexString (PRTP)), Concatenate (" , Index =", ToHexString (SLOT))))
    Store (\_SB.PC00.PC2M (_ADR (), PBNU), Local0) // Store PCIE PR PCIE CGF Base Address
    If (LNotEqual (\_SB.ERPS (Arg0, Arg1, Arg2, WAKG, Local0), CONTINUE_SILICON_NOTIFY)) {
      ADBG ("[PM NTFY] SNRC: END as Skipping due to ERPS() return Value")
      Return ()
    }
    If (LEqual (Arg2, 0x00)) { // PCIE RP Device
      ADBG (Concatenate ("Notify the RP with Value =", ToHexString (Arg3)))
      Notify (^, Arg3)
    } Else { // PCIE RP Child Device
      ADBG (Concatenate ("Notify the Child Device with Value =", ToHexString (Arg3)))
      Notify (PXSX, Arg3)
    }
    // Return From Here
    ADBG ("SNRC: END")
