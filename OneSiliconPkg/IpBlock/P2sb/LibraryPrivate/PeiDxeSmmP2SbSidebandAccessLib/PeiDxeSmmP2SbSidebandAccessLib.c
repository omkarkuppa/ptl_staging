  ...
  )
{
  ...
  *Response = SBI_INVALID_RESPONSE;
  ...
  ///
  PciSegmentWrite32 (P2sbBase + R_P2SB_CFG_SBIADDR, (UINT32) (((Pid << N_P2SB_CFG_SBIADDR_DESTID) & B_P2SB_CFG_SBIADDR_DESTID) | (Offset & B_P2SB_CFG_SBIADDR_OFFSET)));
  ///
  ...
  //
  if (IsHfpgaEnvironment () || IsHSLEEnvironment ()) {
    //
    // Simics(and HFPGA) seems to have a problem with FBE set to
...
/**
  ...
  @param[in]  P2sb20bPcrSupported  20 bits mode supported
...
P2SbIsPcrOffsetValid (
  IN UINT32  Offset,
  IN UINTN   Size,
  IN BOOLEAN P2sb20bPcrSupported
  )
{
  if (!P2sb20bPcrSupported) {
  ...
  switch (Private->AccessMethod) {
    ...
    case P2SbMsgAccess:
      ...
      if (IsHfpgaEnvironment ()) {
        //
        // Simics(and by extension HFPGA) doesn't support byte enables correctly and it needs bios
        // to do a msg transaction with FBE being set to 0xF always(hangs otherwise).
        // Because of that limitation we have to perform AndThenOr32 instead of write to
        // avoid modifying unneccessary fields. This might still result in bad behavior if
        // we write 1b to w1c field but currently this seems to be the least intrusive way to
        // handle byte and word access via msg in simics.
        //
...
#ifndef MDEPKG_NDEBUG
  ASSERT (P2SbIsPcrOffsetValid (Offset, 2, Private->P2SbCtrl->P2sb20bPcrSupported));
#endif
  ...
  )
{
  ...
  switch (Private->AccessMethod) {
    case P2SbMmioAccess:
#ifndef MDEPKG_NDEBUG
      ASSERT (P2SbIsPcrOffsetValid (Offset, 4, Private->P2SbCtrl->P2sb20bPcrSupported));
#endif
      ASSERT (Private->P2SbCtrl->Mmio);
