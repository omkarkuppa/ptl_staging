...
Scope (\)
{
  ...
  //  Enter SETUP to enable Acpi Debug.
  //  Different variations of Acpi Debugger can be called from here.
  //  The currently supported version writes strings to a memory buffer ,serial port, trace hub, Kernel Debuuger.
  //
  //  Arguments:
  ...
  {
    If (IADG) {
      If (CondRefOf (ODBG)) { // OEM specific platform callback
        ODBG (Arg0)
      }
      Store (Arg0, Debug) // WinDbg capable message
      If (CondRefOf (SRLD)) {
        If (LEqual (SRLD,1)) {
          SDBG (Arg0)
        }
