  ...
  )
{
  ...
  if (PciCfgBase < GetReservedSerialIoMmio ()) {
    return (UINT64) ((PciSegmentRead32 ((PciCfgBase + PCI_BASE_ADDRESSREG_OFFSET)) & 0xFFFFF000) + LShiftU64 (PciSegmentRead32 ((PciCfgBase + PCI_BASE_ADDRESSREG_OFFSET + 4)), 32));
  }
...
{
  if (LpssI2cFuncNumber (I2cIndex) == 0) {
    if (SerialIoHigherFunctionsEnabled (LpssI2cDevNumber (I2cIndex))) {
      return TRUE;
    }
  }
  return FALSE;
...
{
  if (LpssUartFuncNumber (UartIndex) == 0) {
    if (SerialIoHigherFunctionsEnabled (LpssUartDevNumber (UartIndex))) {
      return TRUE;
    }
  }
  return FALSE;
...
{
  if (LpssSpiFuncNumber (SpiIndex) == 0) {
    if (SerialIoHigherFunctionsEnabled (LpssSpiDevNumber (SpiIndex))) {
      return TRUE;
    }
  }
  return FALSE;
