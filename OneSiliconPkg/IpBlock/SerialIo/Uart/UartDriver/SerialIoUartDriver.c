  ...
  )
{
  ...
  if (DevicePath->Type == ACPI_DEVICE_PATH && DevicePath->SubType == ACPI_EXTENDED_DP) {
    ...
    PciCfgBase = (UINT32) (((SERIAL_IO_UART_ACPI_EXTENDED_DEVICE_PATH*)DevicePath)->PciCfgBase);
  } else {
    return EFI_UNSUPPORTED;
  }
  //
  //
  if ((DeviceId == 0xFFFF) || (PciCfgBase == 0xFFFFFFFF) || (UartIndex == 0xFF) || (UartIndex > 2) || (DeviceId != GetSerialIoUartDeviceId (UartIndex))) {
    return EFI_UNSUPPORTED;
  }
  ...
  )
{
  ...
  SerialIoUartWrite ((UINTN) UartDevice->PciCfgBase, &(UartDevice->UartIndex), Buffer, *BufferSize);
  ...
  )
{
  ...
  Tpl = gBS->RaiseTPL (TPL_NOTIFY);
  *BufferSize = SerialIoUartRead ((UINTN) UartDevice->PciCfgBase, &(UartDevice->UartIndex), Buffer, *BufferSize, FALSE);
  if (*BufferSize == 0) {
    gBS->RestoreTPL (Tpl);
    return EFI_TIMEOUT;
  }
  ...
  )
{
  ...
  UartDevice->PciCfgBase              = PciCfgBase;
