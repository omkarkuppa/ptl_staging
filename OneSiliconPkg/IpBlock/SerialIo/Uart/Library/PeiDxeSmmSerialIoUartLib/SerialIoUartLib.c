  ...
  )
{
  ...
  return SerialIoUartWritePriv (AccessMode, BaseAddress, Buffer, NumberOfBytes);
}
  ...
  )
{
  ...
  if ((BaseAddress & 0xFFFFF000) == 0x0 || (BaseAddress & 0xFFFFF000) == 0xFFFFF000) { // Bit[0:11] isn't the BAR address.
    return;
  }
  ...
  )
{
  ...
  if (MmioRead32 ((UINTN) (MmioBaseAddress + R_SERIAL_IO_UART_MEM_CTR)) != UART_COMPONENT_IDENTIFICATION_CODE) {
    ResetOffset = R_SERIAL_IO_UART_MEM_RESETS/4; // Requires 8 bit offset
  }

  UartHostControllerResetReg.Data32 = (UINT8) MmioRead8 ((UINTN) MmioBaseAddress + ResetOffset);
  ...
  MmioWrite8 ((UINTN) MmioBaseAddress + ResetOffset, (UINT8) UartHostControllerResetReg.Data32);
}
