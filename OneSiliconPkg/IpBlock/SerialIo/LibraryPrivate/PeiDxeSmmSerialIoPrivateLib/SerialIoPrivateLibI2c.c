  ...
  )
{
  ...
  if ((I2cDeviceConfig->Mode == SerialIoI2cDisabled) && (LpssI2cFuncNumber (I2cNumber) == 0x0)) {
    if (SerialIoHigherFunctionsEnabled (LpssI2cDevNumber (I2cNumber))) {
      TurnOff = TRUE;
    }
  }

  if (TurnOff) {
    PciCfgBase = LpssI2cPciCfgBase (I2cNumber);
