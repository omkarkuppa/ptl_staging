...
#define B_SPI_MEM_HSFSC_SAVE_MASK     (B_SPI_MEM_BIOS_HSFSTS_CTL_FDBC | B_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE)
  ...
  )
{
  ...
  PchSpiBar0 = PciSegmentRead32 (SpiInstance->PchSpiBase + PCI_BASE_ADDRESSREG_OFFSET) & B_SPI_CFG_BIOS_SPI_BAR0_MEMBAR;
  if (PciSegmentRead32 (SpiInstance->PchSpiBase + PCI_BASE_ADDRESSREG_OFFSET) & BIT2) {
    PchSpiBar0 |= LShiftU64 ((UINT64) PciSegmentRead32 (SpiInstance->PchSpiBase + PCI_BASE_ADDRESSREG_OFFSET + 4), 32);
  }
  if (PchSpiBar0 == 0) {
  ...
  if ((MmioRead32 (PchSpiBar0 + R_SPI_MEM_BIOS_HSFSTS_CTL) & B_SPI_MEM_BIOS_HSFSTS_CTL_FDV) == 0) {
  ...
  //
  SpiInstance->ReadPermission = MmioRead8 (PchSpiBar0 + R_SPI_MEM_BIOS_FRACC) & B_SPI_MEM_BIOS_FRACC_BRRA;
  ...
  //
  SpiInstance->WritePermission = (UINT8) ((MmioRead16 (PchSpiBar0 + R_SPI_MEM_BIOS_FRACC) &
                                           B_SPI_MEM_BIOS_FRACC_BRWA) >> N_SPI_MEM_BIOS_FRACC_BRWA);
  DEBUG ((DEBUG_INFO, "Flash Region write Permission : %0x\n", SpiInstance->WritePermission));

  SpiInstance->SfdpVscc0Value = MmioRead32 (PchSpiBar0 + R_SPI_MEM_BIOS_SFDP0_VSCC0);
  DEBUG ((DEBUG_INFO, "Component 0 SFDP VSCC value : %0x\n", SpiInstance->SfdpVscc0Value));
  SpiInstance->SfdpVscc1Value = MmioRead32 (PchSpiBar0 + R_SPI_MEM_BIOS_SFDP1_VSCC1);
  ...
  //
  MmioAndThenOr32 (
    PchSpiBar0 + R_SPI_MEM_BIOS_FDOC,
    (UINT32) (~(B_SPI_MEM_BIOS_FDOC_FDSS | B_SPI_MEM_BIOS_FDOC_FDSI)),
    (UINT32) (V_SPI_MEM_BIOS_FDOC_FDSS_FSDM | R_FLASH_FDBAR_FLASH_MAP0)
    );

  //
  // Copy Zero based Number Of Components
  //
  SpiInstance->NumberOfComponents = (UINT8) ((MmioRead16 (PchSpiBar0 + R_SPI_MEM_BIOS_FDOD) & B_FLASH_FDBAR_NC) >> N_FLASH_FDBAR_NC);
  DEBUG ((DEBUG_INFO, "Component Number : %0x\n", SpiInstance->NumberOfComponents + 1));

  MmioAndThenOr32 (
    PchSpiBar0 + R_SPI_MEM_BIOS_FDOC,
    (UINT32) (~(B_SPI_MEM_BIOS_FDOC_FDSS | B_SPI_MEM_BIOS_FDOC_FDSI)),
    (UINT32) (V_SPI_MEM_BIOS_FDOC_FDSS_COMP | R_FLASH_FCBA_FLCOMP)
    );

  //
  // Copy Component 0 Density
  //
  Data32 = MmioRead32 (PchSpiBar0 + R_SPI_MEM_BIOS_FDOD);
  ...
  MmioAndThenOr32 (
    (PchSpiBar0 + R_SPI_MEM_BIOS_FDOC),
    (UINT32) (~(B_SPI_MEM_BIOS_FDOC_FDSS | B_SPI_MEM_BIOS_FDOC_FDSI)),
    (UINT32) (V_SPI_MEM_BIOS_FDOC_FDSS_FSDM | R_FLASH_FDBAR_FLASH_MAP1)
    );
  //
  ...
  MmioAndThenOr32 (
    (PchSpiBar0 + R_SPI_MEM_BIOS_FDOC),
    (UINT32) (~(B_SPI_MEM_BIOS_FDOC_FDSS | B_SPI_MEM_BIOS_FDOC_FDSI)),
    (UINT32) (V_SPI_MEM_BIOS_FDOC_FDSS_FSDM | R_FLASH_FDBAR_FLASH_MAP2)
    );
  //
  ...
  //
  // Check Linked List Support enable bit first
  //
  MmioAndThenOr32 (
    PchSpiBar0 + R_SPI_MEM_BIOS_FDOC,
    (UINT32) (~(B_SPI_MEM_BIOS_FDOC_FDSS | B_SPI_MEM_BIOS_FDOC_FDSI)),
    (UINT32) (V_SPI_MEM_BIOS_FDOC_FDSS_COMP | R_FLASH_FCBA_FLCOMP2)
  );
  Flcomp2 = MmioRead32(PchSpiBar0 + R_SPI_MEM_BIOS_FDOD);

  //
  // Using linked list for CPU softstraps
  //
  if ((Flcomp2 & B_FLASH_FCBA_FLCOMP2_SSLLSE) != 0) {
    SpiInstance->CpuStrapBaseAddr = (UINT16) ((Data32 & B_FLASH_FDBAR_NTDSOCSBA) >> N_FLASH_FDBAR_NTDSOCSBA) << 4;
    if (SpiInstance->CpuStrapBaseAddr != 0) {
      Status = SpiProtocolFlashRead (
        &SpiInstance->SpiProtocol,
        FlashRegionDescriptor,
        SpiInstance->CpuStrapBaseAddr + 2,
        2,
        (UINT8 *) &Buffer
        );
      if (EFI_ERROR (Status)) {
        DEBUG ((DEBUG_ERROR, "%a - cannot read SoC SS length, Status = %r\n", __FUNCTION__, Status));
        return Status;
      }

      SpiInstance->CpuStrapSize = Buffer;
    }
  } else {
    // Using data from FLMAP2
  ...
  )
{
  ...
  for (WaitTicks = 0; WaitTicks < WaitCount; WaitTicks++) {
    Data32 = MmioRead32 (PchSpiBar0 + R_SPI_MEM_BIOS_HSFSTS_CTL);
    if ((Data32 & B_SPI_MEM_BIOS_HSFSTS_CTL_H_SCIP) == 0) {
      MmioWrite8 (PchSpiBar0 + R_SPI_MEM_BIOS_HSFSTS_CTL, B_SPI_MEM_BIOS_HSFSTS_CTL_FCERR | B_SPI_MEM_BIOS_HSFSTS_CTL_FDONE);
      if (((Data32 & B_SPI_MEM_BIOS_HSFSTS_CTL_FCERR) != 0) && (ErrorCheck == TRUE)) {
        return FALSE;
      } else {
        return TRUE;
      }
  ...
  )
{
  ...
  if (SetFcErrorBit) {
    for (Index = 0; Index < 64; Index += sizeof (UINT32)) {
      MmioWrite32 (PchSpiBar0 + R_SPI_MEM_BIOS_FDATA0 + Index, 0xFFFFFFFF);
    }
  }
  ...
  //
  MmioWrite32 (
    (PchSpiBar0 + R_SPI_MEM_BIOS_FADDR),
    (UINT32) (HardwareSpiAddr & B_SPI_MEM_BIOS_FADDR_FLA)
    );
  //
  ...
  MmioAndThenOr32 (
    PchSpiBar0 + R_SPI_MEM_BIOS_HSFSTS_CTL,
    (UINT32) (~(B_SPI_MEM_BIOS_HSFSTS_CTL_FDBC | B_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE)),
    (UINT32) (((DataCount << N_SPI_MEM_BIOS_HSFSTS_CTL_FDBC) & B_SPI_MEM_BIOS_HSFSTS_CTL_FDBC) |
              (V_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE_READ << N_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE)             |
              B_SPI_MEM_BIOS_HSFSTS_CTL_FGO)
    );
  ...
  if (SetFcErrorBit) {
    ...
    //
    for (Index = 0; Index < 64; Index += sizeof (UINT32)) {
      MmioWrite32 (PchSpiBar0 + R_SPI_MEM_BIOS_FDATA0 + Index, 0x0);
    }
  }
  ...
  )
{
  ...
  IoWrite32 ((UINTN) ABase + R_ACPI_IO_SMI_EN, SmiEnSave & (UINT32) (~B_ACPI_IO_SMI_EN_GBL_SMI_EN));
  BiosCtlSave = PciSegmentRead8 (SpiBaseAddress + R_SPI_CFG_BIOS_SPI_BC) & B_SPI_CFG_BIOS_SPI_BC_SRC;
  ...
  if ((FlashCycleType == FlashCycleWrite) ||
    ...
    PciSegmentAndThenOr8 (
      ...
      (UINT8) (~B_SPI_CFG_BIOS_SPI_BC_SRC),
      (UINT8) (V_SPI_CFG_BIOS_SPI_BC_SRCC_PREF_DIS_CACHE_DIS <<  N_SPI_CFG_BIOS_SPI_BC_SRC)
      );
  }
  ...
  if (IsSpiControllerSaveRestoreEnabled ()) {
    ...
    HsfscSave       = MmioRead32 (PchSpiBar0 + R_SPI_MEM_BIOS_HSFSTS_CTL);
    HsfscFdoneSave  = ((HsfscSave & B_SPI_MEM_BIOS_HSFSTS_CTL_FDONE) != 0) ? TRUE : FALSE;
    HsfscFcerrSave  = ((HsfscSave & B_SPI_MEM_BIOS_HSFSTS_CTL_FCERR) != 0) ? TRUE : FALSE;
    HsfscSave      &= B_SPI_MEM_HSFSC_SAVE_MASK;
    FaddrSave       = MmioRead32 (PchSpiBar0 + R_SPI_MEM_BIOS_FADDR);
    for (Index = 0; Index < 64; Index += sizeof (UINT32)) {
      FdataSave[Index >> 2] = MmioRead32 (PchSpiBar0 + R_SPI_MEM_BIOS_FDATA0 + Index);
    }
    RestoreState = TRUE;
  }
  ...
  if (FlashCycleType == FlashCycleWriteStatus) {
    HsfstsCtl = MmioRead32 (PchSpiBar0 + R_SPI_MEM_BIOS_HSFSTS_CTL);
    if ((HsfstsCtl & B_SPI_MEM_BIOS_HSFSTS_CTL_WRSDIS) != 0) {
  ...
  switch (FlashCycleType) {
    case FlashCycleRead:
      FlashCycle = (UINT32) (V_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE_READ << N_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE);
      break;
    case FlashCycleWrite:
      FlashCycle = (UINT32) (V_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE_WRITE << N_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE);
      break;
    ...
    case FlashCycleReadSfdp:
      FlashCycle = (UINT32) (V_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE_READ_SFDP << N_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE);
      break;
    case FlashCycleReadJedecId:
      FlashCycle = (UINT32) (V_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE_READ_JEDEC_ID << N_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE);
      break;
    case FlashCycleWriteStatus:
      FlashCycle = (UINT32) (V_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE_WRITE_STATUS << N_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE);
      break;
    case FlashCycleReadStatus:
      FlashCycle = (UINT32) (V_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE_READ_STATUS << N_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE);
      break;
    default:
      //
  ...
  do {
    ...
    if (FlashCycleType == FlashCycleErase) {
      if (((ByteCount / SIZE_64KB) != 0) &&
        ...
        if ((SpiInstance->NumberOfComponents == 0) ||
          ...
          //
          if ((SpiInstance->SfdpVscc0Value & B_SPI_MEM_BIOS_SFDP0_VSCC0_EO_64K_VALID) != 0) {
          ...
          //
          if ((SpiInstance->SfdpVscc1Value & B_SPI_MEM_BIOS_SFDP0_VSCC0_EO_64K_VALID) != 0) {
            SpiDataCount = SIZE_64KB;
          } else {
            SpiDataCount = SIZE_4KB;
          }
      ...
      if (SpiDataCount == SIZE_4KB) {
        FlashCycle = (UINT32) (V_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE_4K_ERASE << N_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE);
      } else {
        FlashCycle = (UINT32) (V_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE_64K_ERASE << N_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE);
      }
    }
    //
    ...
    if ((FlashCycleType == FlashCycleWrite) || (FlashCycleType == FlashCycleWriteStatus)) {
      if ((SpiDataCount & 0x07) != 0) {
        ...
        for (Index = 0; Index < SpiDataCount; Index++) {
          MmioWrite8 (PchSpiBar0 + R_SPI_MEM_BIOS_FDATA0 + Index, Buffer[Index]);
        }
      } else {
        //
        ...
        for (Index = 0; Index < SpiDataCount; Index += sizeof (UINT32)) {
          MmioWrite32 (PchSpiBar0 + R_SPI_MEM_BIOS_FDATA0 + Index, *(UINT32 *) (Buffer + Index));
        }
      }
    }
    ...
    MmioAndThenOr32 (
      PchSpiBar0 + R_SPI_MEM_BIOS_HSFSTS_CTL,
      (UINT32) (~(B_SPI_MEM_BIOS_HSFSTS_CTL_FDBC | B_SPI_MEM_BIOS_HSFSTS_CTL_FCYCLE)),
      (UINT32) ((((SpiDataCount - 1) << N_SPI_MEM_BIOS_HSFSTS_CTL_FDBC) & B_SPI_MEM_BIOS_HSFSTS_CTL_FDBC) | FlashCycle | B_SPI_MEM_BIOS_HSFSTS_CTL_FGO)
      );
    //
    ...
    if ((FlashCycleType == FlashCycleRead) ||
      ...
      if ((SpiDataCount & 0x07) != 0) {
        ...
        //
        for (Index = 0; Index < SpiDataCount; Index++) {
          Buffer[Index] = MmioRead8 (PchSpiBar0 + R_SPI_MEM_BIOS_FDATA0 + Index);
        }
      } else {
        //
        ...
        //
        for (Index = 0; Index < SpiDataCount; Index += sizeof (UINT32)) {
          *(UINT32 *) (Buffer + Index) = MmioRead32 (PchSpiBar0 + R_SPI_MEM_BIOS_FDATA0 + Index);
        }
      }
    }
  ...
  if (RestoreState) {
    ...
    MmioAndThenOr32 (
      ...
      (UINT32)  ~(B_SPI_MEM_HSFSC_SAVE_MASK |
                  B_SPI_MEM_BIOS_HSFSTS_CTL_H_SAF_CE    |   // This bit clears when set to 1, ensure 0 is written
                  B_SPI_MEM_BIOS_HSFSTS_CTL_H_SAF_LE    |   // This bit clears when set to 1, ensure 0 is written
                  B_SPI_MEM_BIOS_HSFSTS_CTL_H_SAF_DLE   |   // This bit clears when set to 1, ensure 0 is written
                  B_SPI_MEM_BIOS_HSFSTS_CTL_H_SAF_ERROR |   // This bit clears when set to 1, ensure 0 is written
                  B_SPI_MEM_BIOS_HSFSTS_CTL_H_AEL       |   // This bit clears when set to 1, ensure 0 is written
                  B_SPI_MEM_BIOS_HSFSTS_CTL_FCERR     |   // This bit clears when set to 1, ensure 0 is written
                  B_SPI_MEM_BIOS_HSFSTS_CTL_FDONE),       // This bit clears when set to 1, ensure 0 is written
      HsfscSave
      );
    MmioWrite32 (PchSpiBar0 + R_SPI_MEM_BIOS_FADDR, FaddrSave);
    for (Index = 0; Index < 64; Index += sizeof (UINT32)) {
      MmioWrite32 (PchSpiBar0 + R_SPI_MEM_BIOS_FDATA0 + Index, FdataSave[Index >> 2]);
    }
  }
  ...
  )
{
  ...
  if (SpiIsSafModeActive ()) {
    DEBUG ((DEBUG_WARN, "Unallowed call to %a while SAF Mode is active.\n", __FUNCTION__));
  ...
  )
{
  ...
  if (((MmioRead32 (PchSpiBar0 + R_SPI_MEM_BIOS_BFPREG) & B_SPI_MEM_BIOS_BFPREG_SBRS) != 0) &&
      (FlashRegionType == FlashRegionBios)) {
    FlashRegionType = FlashRegionSecondaryBios;
  }

  if (FlashRegionType < FlashRegion10Gbe_B) {
    ReadValue = MmioRead32 (PchSpiBar0 + (R_SPI_MEM_BIOS_FREG0 + (S_SPI_MEM_BIOS_FREGX * ((UINT32) FlashRegionType))));
  } else {
    ReadValue = MmioRead32 (PchSpiBar0 + (R_SPI_MEM_BIOS_FREG12 + (S_SPI_MEM_BIOS_FREGX * ((UINT32) (FlashRegionType - FlashRegion10Gbe_B)))));
  }
  ...
  //
  if (ReadValue == B_SPI_MEM_BIOS_FREG0_RB) {
    return EFI_DEVICE_ERROR;
  }
  *BaseAddress = ((ReadValue & B_SPI_MEM_BIOS_FREG0_RB) >> N_SPI_MEM_BIOS_FREG0_RB) <<
    N_SPI_MEM_FREGX_BASE_REPR;
  //
  // Region limit address Bits[11:0] are assumed to be FFFh
  //
  *RegionSize = ((((ReadValue & B_SPI_MEM_BIOS_FREG0_RL) >> N_SPI_MEM_BIOS_FREG0_RL) + 1) <<
