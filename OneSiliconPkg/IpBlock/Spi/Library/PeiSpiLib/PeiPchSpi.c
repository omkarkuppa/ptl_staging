  ...
  )
{
  ...
  SpiMmio = PCH_SPI_BASE_ADDRESS;
  PchSpiBase = SpiPciCfgBase ();
  PciSegmentAnd8 (PchSpiBase + PCI_COMMAND_OFFSET, (UINT8) ~EFI_PCI_COMMAND_MEMORY_SPACE);
  PciSegmentWrite32 (PchSpiBase + PCI_BASE_ADDRESSREG_OFFSET, (UINT32) SpiMmio);
  if (PciSegmentRead32 (PchSpiBase + PCI_BASE_ADDRESSREG_OFFSET) & BIT2) {
    PciSegmentWrite32 (PchSpiBase + PCI_BASE_ADDRESSREG_OFFSET + 4, (UINT32) RShiftU64 (SpiMmio, 32));
  }
