...
//
GLOBAL_REMOVE_IF_UNREFERENCED UINT64                mSpiResvMmioAddr;
//
...
//
GLOBAL_REMOVE_IF_UNREFERENCED UINT64                mSpiSavedMmioAddr;
//
  ...
  )
{
  ...
  if ((SpiBar0 != mSpiResvMmioAddr) || ((PciCmdReg & EFI_PCI_COMMAND_MEMORY_SPACE) == 0)) {
    ...
    PciSegmentWrite32 (SpiInstance->PchSpiBase + PCI_BASE_ADDRESSREG_OFFSET, (UINT32) mSpiResvMmioAddr);
    if (PciSegmentRead32 (SpiInstance->PchSpiBase + PCI_BASE_ADDRESSREG_OFFSET) & BIT2) {
      PciSegmentWrite32 (SpiInstance->PchSpiBase + PCI_BASE_ADDRESSREG_OFFSET + 4, (UINT32) RShiftU64 (mSpiResvMmioAddr, 32));
    }
  ...
  )
{
  ...
  if (mPchSpiBar0RefCount <= 1) {
    ...
    if (mSpiSavedMmioAddr != 0) {
      ...
      //
      PciSegmentAnd8 (SpiInstance->PchSpiBase + PCI_COMMAND_OFFSET, (UINT8)~EFI_PCI_COMMAND_MEMORY_SPACE);
      PciSegmentWrite32 (SpiInstance->PchSpiBase + PCI_BASE_ADDRESSREG_OFFSET, (UINT32) mSpiSavedMmioAddr);
      if (PciSegmentRead32 (SpiInstance->PchSpiBase + PCI_BASE_ADDRESSREG_OFFSET) & BIT2) {
        PciSegmentWrite32 (SpiInstance->PchSpiBase + PCI_BASE_ADDRESSREG_OFFSET + 4, (UINT32) RShiftU64 (mSpiSavedMmioAddr, 32));
      }
  ...
  )
{
  ...
  //
  PciSegmentOr8 (
    SpiBaseAddress + R_SPI_CFG_BIOS_SPI_BC + 1,
    (B_SPI_CFG_BIOS_SPI_BC_SPI_SYNC_SS >> 8)
    );
  ///
  ...
  )
{
  ...
  ///
  if (((PciSegmentRead8 (SpiBaseAddress + R_SPI_CFG_BIOS_SPI_BC)) & B_SPI_CFG_BIOS_SPI_BC_EISS) == B_SPI_CFG_BIOS_SPI_BC_EISS) {
    PchClearInSmmSts ();
  }
}
