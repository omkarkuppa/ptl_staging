  ...
  *Ptr64  = sizeof (BIOS_OS_DATA_REGION);
  ...
  if (BiosOsDataRegion->Version >= 6) { // From CBNT, version = 6
    ...
    if ((AsmReadMsr64 (MSR_BOOT_GUARD_SACM_INFO) & BIT34) == 0) {
      BiosOsDataRegion->Flags = LShiftU64 ((UINT64)BIT1, 32);
    } else {
      BiosOsDataRegion->Flags = LShiftU64 ((UINT64)BIT2, 32);
    }
  }
  ...
  )
{
  ...
  if (Operation == TRUE) {
    ...
    IoWrite32 (
      ...
      (UINT32) (SavedSmiControl & (UINT32)(~B_ACPI_IO_SMI_EN_GBL_SMI_EN))
      );
