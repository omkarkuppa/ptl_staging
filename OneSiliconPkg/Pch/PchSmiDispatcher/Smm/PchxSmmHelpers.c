  ...
  )
{
  ...
  //
  SmiEn |= B_ACPI_IO_SMI_EN_GBL_SMI_EN;
  ...
  )
{
  ...
  if (!SciEn) {
    ...
    Gpe0Sts &= (UINT32)~(B_ACPI_IO_GPE0_STS_127_96_WADT_STS);
  ...
  DevActSts     = IoRead16 ((UINTN) mAcpiBaseAddr + R_ACPI_IO_DEVTRAP_STS);
  Tco1Sts       = IoRead16 ((UINTN) mTcoBaseAddr + R_TCO_IO_TSTS1);
  ...
  DevActSts |=
  (
    B_ACPI_IO_DEVTRAP_STS_D12_TRP_STS |
    B_ACPI_IO_DEVTRAP_STS_D9_TRP_STS |
    B_ACPI_IO_DEVTRAP_STS_D8_TRP_STS |
    B_ACPI_IO_DEVTRAP_STS_D7_TRP_STS |
    B_ACPI_IO_DEVTRAP_STS_D6_TRP_STS
    );
  Tco1Sts |= mTco1StsClear;
  ...
  if (EFI_ERROR (Status)) {
    ...
    for (Index = 0; Index < GpioServicesCnt; Index++) {
      GpioPad = GPIOV2_PAD_NONE;
      while (GpioServices[Index]->GetNextPad (GpioServices[Index], GpioPad, &GpioPad) == EFI_SUCCESS) {
        GpioServices[Index]->ClearSmiSts (GpioServices[Index], GpioPad);
      }
    }
  ...
  IoWrite16 ((UINTN) mTcoBaseAddr + R_TCO_IO_TSTS1, Tco1Sts);

  //
  // We do not want to write 1 to clear INTRD_DET bit.
  //
  IoWrite16 ((UINTN) mTcoBaseAddr + R_TCO_IO_TSTS2, (UINT16) ~B_TCO_IO_TSTS2_INTRD_DET);

  IoWrite32 ((UINTN) mAcpiBaseAddr + R_ACPI_IO_SMI_STS, SmiSts);

  IoWrite16 ((UINTN) mAcpiBaseAddr + R_ACPI_IO_DEVTRAP_STS, DevActSts);

  //
  // Try to clear the EOS bit.
  //
  ...
  )
{
  ...
  switch (BitDesc->Reg.Type) {
    ...
    case PCR_ADDR_TYPE:
      ...
      switch (BitDesc->SizeInBytes) {
        case 1:
          PchPcrAndThenOr8  ((P2SB_PID) BitDesc->Reg.Data.Pcr.Fields.Pid, (UINT16) BitDesc->Reg.Data.Pcr.Fields.Offset, (UINT8)  AndVal, (UINT8)  OrVal);
          break;

        case 2:
          PchPcrAndThenOr16 ((P2SB_PID) BitDesc->Reg.Data.Pcr.Fields.Pid, (UINT16) BitDesc->Reg.Data.Pcr.Fields.Offset, (UINT16) AndVal, (UINT16) OrVal);
          break;

        case 4:
          PchPcrAndThenOr32 ((P2SB_PID) BitDesc->Reg.Data.Pcr.Fields.Pid, (UINT16) BitDesc->Reg.Data.Pcr.Fields.Offset, (UINT32) AndVal, (UINT32) OrVal);
          break;
