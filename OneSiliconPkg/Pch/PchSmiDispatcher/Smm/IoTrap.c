  ...
  )
{
  ...
  //
  PchPcrRead32 (PID_PSTH_DEPRECATED, R_PSTH_PCR_IOTRP1_1 + TrapHandlerNum * 8);
  PchPcrWrite32 (PID_PSTH_DEPRECATED, R_PSTH_PCR_IOTRP1_1 + TrapHandlerNum * 8, Value);
  PchPcrRead32 (PID_PSTH_DEPRECATED, R_PSTH_PCR_IOTRP1_1 + TrapHandlerNum * 8);
  ...
  if (SaveToBootscript) {
    ...
    PCH_PCR_BOOT_SCRIPT_READ (S3BootScriptWidthUint32, PID_PSTH_DEPRECATED, R_PSTH_PCR_IOTRP1_1 + TrapHandlerNum * 8, &BitMask, &BitValue);
    PCH_PCR_BOOT_SCRIPT_WRITE (S3BootScriptWidthUint32, PID_PSTH_DEPRECATED, R_PSTH_PCR_IOTRP1_1 + TrapHandlerNum * 8, 1, &Value);
    PCH_PCR_BOOT_SCRIPT_READ (S3BootScriptWidthUint32, PID_PSTH_DEPRECATED, R_PSTH_PCR_IOTRP1_1 + TrapHandlerNum * 8, &BitMask, &BitValue);
  }
  SocSetIoTrapLowDword (TrapHandlerNum, Value, SaveToBootscript);
}
...
{
  PchPcrWrite32 (PID_PSTH_DEPRECATED, R_PSTH_PCR_IOTRP1_1 + TrapHandlerNum * 8 + 4, Value);
  if (SaveToBootscript) {
    PCH_PCR_BOOT_SCRIPT_WRITE (S3BootScriptWidthUint32, PID_PSTH_DEPRECATED, R_PSTH_PCR_IOTRP1_1 + TrapHandlerNum * 8 + 4, 1, &Value);
  }
  SocSetIoTrapHighDword (TrapHandlerNum, Value, SaveToBootscript);
}
...
{
  PchPcrWrite32 (PID_PSTH_DEPRECATED, R_PSTH_PCR_TRPSTS, (UINT32)(1 << TrapHandlerNum));
}
...
{
  return (UINT16) (IoTrapRegLowDword & B_PSTH_PCR_IOTRP_TRPXADDR);
}
...
/**
  The helper function for IoTrap callback dispatcher
  ...
  )
{
  ...
  if (!IsListEmpty (&(mIoTrapData.Entry[TrapHandlerNum].CallbackDataBase))) {
    ...
    //
    // Unhide P2SB
    // Calculate the EPMASKx Register + Bit based on the PSTH PID
    // Store the data in the cooresponding EPMASK register
    // Clear the PSTH P2SB endpoint mask bit
    //
    P2sbBase = P2sbPciCfgBase ();

    // B_P2SB_CFG_P2SBC_HIDE is on second 8 bit part of R_P2SB_CFG_P2SBC register
    // AndOr32 is not possible due PCI config space is hidden
    PciSegmentWrite8 (P2sbBase + R_P2SB_CFG_P2SBC + 1, 0);

    RegisterOff = (PID_PSTH_DEPRECATED / 32) * sizeof (UINT32);
    BitNo = PID_PSTH_DEPRECATED  % 32;
    MaskData = PciSegmentRead32 (P2sbBase + R_P2SB_CFG_EPMASK0 + RegisterOff);
    PciSegmentAnd32 ((UINTN) (P2sbBase + R_P2SB_CFG_EPMASK0 + RegisterOff), ~((UINT32) 1 << BitNo));

    Data32 = PchPcrRead32 (PID_PSTH_DEPRECATED, R_PSTH_PCR_TRPCYC1);
    WriteData = PchPcrRead32 (PID_PSTH_DEPRECATED, R_PSTH_PCR_TRPWRDATA1);

    BaseAddress           = (UINT16) (Data32 & B_PSTH_PCR_TRPCYC1_TRPADDR);
    ActiveHighByteEnable  = (UINT8)((Data32 & B_PSTH_PCR_TRPCYC1_TRPBE) >> 16);
    ReadCycle             = (BOOLEAN) ((Data32 & B_PSTH_PCR_TRPCYC1_TRPRWR) == B_PSTH_PCR_TRPCYC1_TRPRWR);
    //
    ...
    //
    // Restore the EPMASKx register to it's previous value
    // Hide P2SB
    //
    PciSegmentWrite32 (P2sbBase + R_P2SB_CFG_EPMASK0 + RegisterOff, MaskData);
    PciSegmentOr32 (P2sbBase + R_P2SB_CFG_P2SBC, B_P2SB_CFG_P2SBC_HIDE);
  } // end of if else block
}
  ...
  )
{
  ...
  for (TrapHandlerNum = 0; TrapHandlerNum < IO_TRAP_HANDLER_NUM; TrapHandlerNum++) {
    ...
    //
    IoTrapRegLowDword = PchPcrRead32 (PID_PSTH_DEPRECATED, R_PSTH_PCR_IOTRP1_1 + TrapHandlerNum * 8);
    ...
    if (AddressFromLowDword (IoTrapRegLowDword) == 0) {
      ...
      if (BaseAddress == 0) {
        ...
        *Address   = (UINT16) BaseAddress;
        ...
        BaseAddress &= B_PSTH_PCR_IOTRP_TRPXADDR;
        UsedLength = Length;
      }
      ...
      if (UsedLength < 4) {
        ...
        //
        IoTrapRegHighDword  = (((1 << UsedLength) - 1) << ((*Address & 0x3) + (N_PSTH_PCR_IOTRP_TRPXBEM - 32))) |
          (UINT32) (Type << N_PSTH_PCR_IOTRP_TRPXRW);
      } else {
        //
        // Fill in the ByteEnable, ByteEnableMask, and Type of Io Trap register
        //
        IoTrapRegHighDword  = ((ByteEnableMask & 0xF) << (N_PSTH_PCR_IOTRP_TRPXBEM - 32)) |
          ((ByteEnable & 0xF) << (N_PSTH_PCR_IOTRP_TRPXBE - 32)) |
          (UINT32) (Type << N_PSTH_PCR_IOTRP_TRPXRW);
      }
      ...
      //
      //  Assign an address from common pool if the caller's address is 0
      //
      if (*Address == 0) {
        //
      ...
      //
      IoTrapRegHighDword = PchPcrRead32 (PID_PSTH_DEPRECATED, R_PSTH_PCR_IOTRP1_1 + TrapHandlerNum * 8 + 4);
      if ((IoTrapRegHighDword & B_PSTH_PCR_IOTRP_TRPXRWM) == 0 &&
          (UINT32) ((IoTrapRegHighDword & B_PSTH_PCR_IOTRP_TRPXRW) >> N_PSTH_PCR_IOTRP_TRPXRW) !=
          (UINT32) Type) {
        IoTrapRegHighDword = ((IoTrapRegHighDword | B_PSTH_PCR_IOTRP_TRPXRWM) & ~B_PSTH_PCR_IOTRP_TRPXRW);
        SetIoTrapHighDword (TrapHandlerNum, IoTrapRegHighDword, TRUE);
      }
    }
    break;
      ...
      if ((UINTN) (BaseAddress + UsedLength) == ((UINTN) (RecordToDelete->Context.Address) + RecordToDelete->Context.Length)) {
        ...
        if (BaseAddress == RecordToDelete->Context.Address) {
          ...
          if (LengthIndex < sizeof (mLengthTable) / sizeof (UINT16)) {
            ...
            //
            IoTrapRegLowDword = (UINT32) ((((UsedLength - 1) &~(BIT1 + BIT0)) << 16) | BaseAddress | B_PSTH_PCR_IOTRP_TRPXEN);
          }
          SetIoTrapLowDword (TrapHandlerNum, IoTrapRegLowDword, TRUE);
        }
      }
  ...
  )
{
  ...
  for (TrapHandlerNum = 0; TrapHandlerNum < IO_TRAP_HANDLER_NUM; TrapHandlerNum++) {
    ...
    if (DisableIoTrap) {
      ...
      //
      if ((IoTrapRegLowDword & B_PSTH_PCR_IOTRP_TRPXEN) == 0) {
        return EFI_ACCESS_DENIED;
      }
      //
      // Clear IoTrap register SMI enable bit
      //
      IoTrapRegLowDword &= (~B_PSTH_PCR_IOTRP_TRPXEN);
