...
/**
  Lock SMI Registration if the EndOfDxe event has been triggered.

  When MM Runtime Update is in progress, temporarily allow to register and unregister SMIs;
  After MM Runtime Update finishes, lock it again.

**/
BOOLEAN
PchSmiReigsterIsLocked (
  VOID
  )
{
  if (mReadyToLock && PlatformPchSmiRegisterIsLocked ()) {
    return TRUE;
  }
