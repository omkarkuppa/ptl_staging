  ...
  )
{
  ...
  PEI_SI_DEFAULT_POLICY_INIT_PPI    *PeiPchDefaultPolicyInitPpi;
  ...
  PeiPchDefaultPolicyInitPpi = NULL;
  ...
  Status = PeiServicesLocatePpi (
             ...
             0,
             NULL,
             (VOID **) &PeiPchDefaultPolicyInitPpi
             );
  if (PeiPchDefaultPolicyInitPpi != NULL) {
    Status = PeiPchDefaultPolicyInitPpi->PeiPolicyInit ();
    ASSERT_EFI_ERROR (Status);
    if (Status == EFI_SUCCESS) {
      Status = PeiServicesLocatePpi (
                 &gPchPolicyPpiGuid,
