...
EFI_RSC_HANDLER_PROTOCOL  *mTraceHubRscHandlerProtocol       = NULL;
BOOLEAN                   mTraceHubRegistered                = FALSE;
  ...
  )
{
  ...
  EFI_GUID   DrvGui = {0};
  if (((Value & EFI_STATUS_CODE_CLASS_MASK)|(Value & EFI_STATUS_CODE_SUBCLASS_MASK)) == EFI_SOFTWARE_DXE_CORE) {
  ...
  //
#if FixedPcdGetBool (PcdTraceHubCatalogEnable) == 0
  ...
  if ((CodeType & EFI_STATUS_CODE_TYPE_MASK) == EFI_PROGRESS_CODE &&
    ...
    if (mTraceHubRegistered) {
      mTraceHubRscHandlerProtocol->Unregister (TraceHubStatusCodeReportWorker);
    }
  }
...
)
{
  ...
  Status = gBS->LocateProtocol (
                  ...
                  NULL,
                  (VOID **) &mTraceHubRscHandlerProtocol
                  );
  ASSERT_EFI_ERROR (Status);

  mTraceHubRscHandlerProtocol->Register (TraceHubStatusCodeReportWorker, TPL_HIGH_LEVEL);
  ASSERT_EFI_ERROR (Status);
  mTraceHubRegistered = TRUE;
}

/**
  Constructor function of RuntimeDxeTraceHubStatusCodeHandlerLib.
...
EFI_STATUS
EFIAPI
RuntimeDxeTraceHubStatusCodeHandlerLibConstructor (
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    RegisterTraceHubBootTimeHandlers (NULL, NULL);
  } else {
