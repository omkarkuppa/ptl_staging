/** @file
  Post Code Library instance that writes post code values to I/O port 0x80, I2C
  and TraceTool.
  @copyright
  INTEL CONFIDENTIAL
...
GLOBAL_REMOVE_IF_UNREFERENCED UINT8 mI2cPostCodeSupported   = 0xFF;
GLOBAL_REMOVE_IF_UNREFERENCED UINT8 mSmbusPostCodeSupported = 0xFF;
GLOBAL_REMOVE_IF_UNREFERENCED UINT8 mSmbusPostCodeAddress   = 0;
GLOBAL_REMOVE_IF_UNREFERENCED UINT8 mSmbusPostCodeCommand   = 0;
...
{
  if (mI2cPostCodeSupported == 0xFF) {
    mI2cPostCodeSupported = PcdGet8(PcdI2cPostCode);
  }
  if (mI2cPostCodeSupported) {
    // Postcode display using I2C Interface
    DisplayI2cPostCode (Value);
  } else if (mSmbusPostCodeSupported == 0x1) {
    // Postcode display using Smbus Interface
    DisplaySmbusPostCode (Value, mSmbusPostCodeAddress, mSmbusPostCodeCommand);
  } else {
...
/**
  The constructor function for the PostCode Lib.
...
EFI_STATUS
EFIAPI
BasePostCodeLibConstructor (
  IN EFI_HANDLE        ImageHandle,
  IN EFI_SYSTEM_TABLE  *SystemTable
  )
{
  if (mSmbusPostCodeSupported == 0xFF) {
    DEBUG ((DEBUG_INFO, "Entry SMM BasePostCodeLibConstructor...\n"));
    mSmbusPostCodeSupported = PcdGet8(PcdSmbusPostCode);
    DEBUG ((DEBUG_INFO, "SMM Smbus Postcode status: %x\n", mSmbusPostCodeSupported));
    mSmbusPostCodeAddress   = PcdGet8(PcdSmbusPostCodeAddress);
    mSmbusPostCodeCommand   = PcdGet8(PcdSmbusPostCodeCommand);
  }
